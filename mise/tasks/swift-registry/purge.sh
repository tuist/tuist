#!/usr/bin/env bash
#MISE description="Purge a Swift package (or a specific version) from the swift-registry service"
...
#USAGE flag "--namespace <namespace>" help="Kubernetes namespace (defaults to swift-registry)"
#USAGE flag "--context <context>" help="kubectl context (defaults to current)"

set -euo pipefail

readonly DEFAULT_NAMESPACE="${TUIST_SWIFT_REGISTRY_NAMESPACE:-swift-registry}"
...
log()  { printf '[swift-registry:purge] %s\n' "$*"; }
warn() { printf '[swift-registry:purge] WARNING: %s\n' "$*" >&2; }
fail() { printf '[swift-registry:purge] ERROR: %s\n' "$*" >&2; exit 1; }

print_rclone_setup_instructions() {
    cat >&2 <<EOF
[swift-registry:purge] ERROR: rclone remote "${RCLONE_REMOTE}" is not configured.

Set it up with the registry Tigris credentials from 1Password (vault: tuist-k8s-<env>, item: SWIFT_REGISTRY):
...
purge_local_caches() {
    ...
    if [[ "${#pods[@]}" -eq 0 ]]; then
        warn "No running swift-registry pods found in ${namespace}; skipping local PVC purge"
        return 0
    fi
    ...
    for pod in "${pods[@]}"; do
        ...
        if kubectl "${context_flag[@]}" -n "$namespace" exec -c swift-registry "$pod" -- rm -rf "$local_path"; then
...
normalize_version() {
    ...
    if [[ "$hyphen_count" == "1" ]]; then
        base="${version%%-*}"
        prerelease="${version#*-}"
        # Matches SwiftRegistry.Registry.KeyNormalizer behaviour: preserve
...
main() {
    ...
    if [[ -z "$version" ]]; then
        log "About to purge ENTIRE package ${normalized_scope}/${normalized_name} from swift-registry"
        log "This will delete all versions from S3 and all swift-registry pod caches in namespace ${namespace}"
        read -r -p '[swift-registry:purge] Are you sure? (y/N) ' confirm
        if [[ "$confirm" != [yY] ]]; then
            log "Aborted by user"
            exit 0
        fi
