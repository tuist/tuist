#!/usr/bin/env bash
#MISE description="Trigger a registry sync for a Swift package version on the swift-registry service"
...
log()  { printf '[swift-registry:sync] %s\n' "$*"; }
fail() { printf '[swift-registry:sync] ERROR: %s\n' "$*" >&2; exit 1; }
...
main() {
    ...
    log "Selecting a swift-registry pod in namespace \"${namespace}\""
    ...
    if [[ -z "$pod" ]]; then
        fail "No running swift-registry pod found in namespace ${namespace} matching ${APP_SELECTOR}"
    fi
...
EOF
    ...
    kubectl "${context_flag[@]}" -n "$namespace" exec -c swift-registry "$pod" -- \
        /app/bin/swift_registry rpc "$elixir_code"
