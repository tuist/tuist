#!/usr/bin/env bash
#MISE description="Compute the next Kura version for a release channel (canary | rc-new | rc | promote)"
set -euo pipefail

# Resolve the next Kura version for one of the release channels and emit it,
# with the SHA to build and the changelog base, to GITHUB_OUTPUT (and stdout).
# Run by the release workflows as
# `mise run kura:release:channel-version -- <channel>`:
#
#   canary            kura-release.yml   next X.Y.0-canary.N off main
#   rc-new            kura-rc.yml        cut next minor: X.Y.0-rc.1 (+ branch to create)
#   rc      <branch>  kura-rc.yml        iterate X.Y.0-rc.(N+1) on a release branch
#   promote <branch>  kura-promote.yml   promote X.Y.0 stable from a release branch
#   promote           kura-promote.yml   promote the newest cut line not yet stable
#                                        (should_publish=false when there is none)
#
# Cloned from mise/tasks/cli/release/channel-version.sh; the only differences
# are the git-tag prefix (kura@) and the branch namespace prefix. See the CLI
...
# Kura release branches carry a component prefix so they don't collide with
# CLI or server release branches at the same minor.
line_from_branch() {
  [[ "$1" =~ ^releases/kura-([0-9]+\.[0-9]+)\.x$ ]] ||
    die "Branch must be named releases/kura-<major>.<minor>.x (got '${1:-<empty>}')."
