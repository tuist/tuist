...
A `Scheme` will help to define a collection of targets to `Build, Run, Test, Profile, Analyze and Archive`:
> Note: Tuist will auto-generate Scheme for each target if they are not defined. 
- **name: String:** Name of Scheme
- **shared: Bool:** Scheme is shared or not 
- **buildAction: BuildAction?:** Scheme Build options
    - **targets: [String]:** A list of targets will be built, which are defined in Project
...
- **testAction: TestAction?:** Scheme Test options
    - **targets: [String]:** A list of targets will be tested, which are defined in Project
...
- **runAction: RunAction?:** Run or Launch action
    ...
    - **executable: String?:** Executable or target will be launch
    - **arguments: [Arguments?](#arguments):** Commandline arguments passed on launch and Environment variables
...
- **target: String?:** Name of a build or test target that will provide build settings.
...
- **launch: [String: Bool]:** Commandline arguments
