...
let workspace = Workspace(name: "CustomWorkspace",
                          ...
                          schemes: [
                            WorkspaceScheme(name: "Workspace-App",
                                            shared: true,
...
A `WorkspaceScheme` defines a collection of targets to `Build, Run, Test, Profile, Analyze and Archive`. Note that the `WorkspaceScheme` is very similar to `Scheme` that are defined for projects. The main difference is where targets are referenced instead of using the type 'String' the type `TargetReference` is used. An example of using Build Actions in Workspace Schemes in below.
...
<TargetReference
  path="Project to the target's project."
