---
{
  "title": "Examples",
  "titleTemplate": ":title · References · Tuist",
  "description": "This page documents how to migrate the Tuist CLI from the version 3 to version 4."
}
---
# الأمثلة

ستجد في هذا القسم أمثلة على <LocalizedLink href="/guides/features/projects"> المشاريع التي تم إنشاؤها </LocalizedLink> والتي نستخدمها كـ [تركيبات]
(https://github.com/tuist/tuist/tree/main/cli/Fixtures) لتشغيل اختبارات القبول
مقابلها. قد تجدها مفيدة عند إنشاء مشروع جديد تم إنشاؤه، أو ترحيل مشروع Xcode
موجود ليتم إنشاؤه بواسطة Tuist.
