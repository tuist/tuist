...
2. **قم بتشغيل 'tuist auth login'**: في سير عمل CI الخاص بك، قم بتشغيل `tuist
   auth login` قبل أي أوامر تتطلب مصادقة. ستقوم CLI تلقائيًا باكتشاف بيئة CI
...
| `--تنتهي صلاحيته` | اختياري. متى يجب أن تنتهي صلاحية الرمز المميز. استخدم التنسيق مثل `30d` (أيام)، أو `6m` (أشهر)، أو `1y` (سنوات). إذا لم يتم تحديد ذلك، فلن تنتهي صلاحية الرمز المميز أبدًا. |
