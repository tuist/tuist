...
سيأخذك الأمر عبر عملية مصادقة على شبكة الإنترنت. بمجرد المصادقة، سيخزن CLI رمز
تحديث طويل الأمد ورمز وصول قصير الأمد تحت `~/.config/tuist/credentials`. يمثل كل
...
يتم منح رموز OIDC مجموعة نطاق `ci` ، والتي توفر الوصول إلى جميع المشاريع المتصلة
بالمستودع. راجع [مجموعات النطاق](#scope-groups) للحصول على تفاصيل حول ما يشمله
نطاق `ci`.
...
- الرموز قصيرة العمر ومحددة النطاق لتشغيل سير العمل الفردي
...
| `--المشاريع` | قم بتحديد الرمز المميز لمعالجات مشاريع محددة. يتمتع الرمز المميز بحق الوصول إلى جميع المشاريع إذا لم يتم تحديده.                                                                  |
...
(ذاكرة التخزين المؤقت، والمعاينات، والحزم، والاختبارات، وعمليات البناء، وعمليات
التشغيل). قم بتخزين الرمز المميز الذي تم إنشاؤه كسر في بيئة CI الخاصة بك وقم
...
من المتوقع أن يتم تعريف رموز الحساب كمتغيرات بيئية `TUIST_TOKEN`:
...
- الرموز المحددة زمنياً التي تنتهي صلاحيتها تلقائياً
<!-- -->
:::
