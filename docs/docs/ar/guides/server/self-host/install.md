...
من Tuist متاح فقط للمؤسسات التي تستخدم خطة المؤسسة. إذا كنت مهتمًا بهذا الإصدار،
يُرجى التواصل مع [contact@tuist.dev] (mailto:contact@tuist.dev).
<!-- -->
:::
...
<!-- -->
إذا كان هدفك في المقام الأول هو إحضار دلو خاص بك لتخزين الثنائيات وتقليل زمن
انتقال ذاكرة التخزين المؤقت، فقد لا تحتاج إلى استضافة الخادم بالكامل ذاتيًا.
يمكنك استضافة عقد ذاكرة التخزين المؤقت ذاتيًا وتوصيلها بخادم Tuist المستضاف أو
خادمك المستضاف ذاتيًا.

راجع <LocalizedLink href="/guides/cache/self-host"> دليل الاستضافة الذاتية
لذاكرة التخزين المؤقت</LocalizedLink>.
<!-- -->
:::
...
| `tuist_license_clicense_certificate_base64` | **بديل استثنائي لـ 'TUIST_LICENSE'**. شهادة عامة مشفرة بترميز Base64 للتحقق من صحة الترخيص دون اتصال بالإنترنت في البيئات الموصولة بالهواء حيث يتعذر على الخادم الاتصال بالخدمات الخارجية. تستخدم فقط عندما يتعذر استخدام `TUIST_LICENSE` | نعم*  |         | `LS0tLS1CRUdJTiBDRDRVJUSUSUZJZJQ0FURS0tLS0t...` |
...
| `tuist_secret_secret_key_password`           | الفلفل لتوليد كلمات مرور مجزأة                                                                                                                                                                   | لا يوجد | `$TUIST_secret_secret_key_base`      |                                                                              |                                                                                                                                    |
...
| `معالجات_عمليات_المستخدم_المستخدم_المعالج`   | قائمة مفصولة بفاصلة من مقابض المستخدمين الذين لديهم حق الوصول إلى عناوين URL للعمليات                                                                                                            | لا يوجد |                                      | `مستخدم1،مستخدم2`                                                            |                                                                                                                                    |
...
إنشاء بيانات اعتماد جديدة من نوع معرّف عميل OAuth. عند إنشاء بيانات الاعتماد،
حدد "تطبيق الويب" كنوع التطبيق، وقم بتسميته `Tuist` ، وقم بتعيين URI إعادة
التوجيه إلى `{base_url} / Users/users/auth/google/callback` حيث `base_url` هو
عنوان URL الذي تعمل عليه الخدمة المستضافة. بمجرد إنشاء التطبيق، انسخ معرف العميل
والسر وقم بتعيينهما كمتغيري بيئة `GOOGLE_CLIENT_ID` و `GOOGLE_CLIENT_SECRET` على
التوالي.
...
(https://oauth.net/2/). سيتعين عليك [إنشاء تطبيق]
...
| `tuist_s3_s3_pool_max_idle_idtime`                                    | الحد الأقصى لوقت الخمول (بالمللي ثانية) للاتصالات في التجمع. استخدم `اللانهاية` لإبقاء الاتصالات حية إلى أجل غير مسمى                           | لا يوجد | `اللانهاية`            | `60000`                                                   |
...
| `tuist_s3_virtual_virtual_host`                                       | ما إذا كان ينبغي إنشاء عنوان URL مع اسم المجموعة كمجال فرعي (مضيف افتراضي)                                                                      | لا يوجد | `كاذبة`                | `1`                                                       |
...
| `المجال_البريدي_التويست`              | المجال الذي سيتم إرسال رسائل البريد الإلكتروني منه                                                                                               | نعم*    |                                                                                | `mg.tuist.io`                 |
...
فكر في إنشائه ديناميكيًا ضمن خط أنابيب النشر المستمر (CD). فيما يلي مثال مرجعي
لاستخدامك:
...
`fly deploy - محلي فقط`. لا يسمح Fly.io بسحب صور Docker الخاصة، ولهذا السبب
