...
تقوم الفرق بذلك عن طريق إنشاء تطبيقاتهم وتوقيعها ونشرها على منصات مثل
[TestFlight](https://developer.apple.com/testflight/) من Apple. ومع ذلك، قد تكون
هذه العملية مرهقة وبطيئة، خاصةً إذا كنت تبحث فقط عن تعليقات سريعة من زميل أو
صديق.

لتسهيل هذه العملية، يوفر Tuist طريقة لإنشاء معاينات لتطبيقاتك ومشاركتها مع أي
شخص.
...
سيُنشئ الأمر رابطًا يمكنك مشاركته مع أي شخص لتشغيل التطبيق – سواء على جهاز
محاكاة أو جهاز حقيقي. كل ما سيحتاجون إليه هو تشغيل الأمر أدناه:
...
رابط المعاينة. الروابط المؤدية إلى معاينات `.ipa` هي بشكل افتراضي _خاصة_ ، مما
يعني أن المستلم يحتاج إلى المصادقة باستخدام حساب Tuist الخاص به لتنزيل التطبيق.
يمكنك تغيير هذا إلى عام في إعدادات المشروع إذا كنت ترغب في مشاركة التطبيق مع أي
شخص.
...
<!-- -->
تأكد من أن `CFBundleVersion` (إصدار البناء) فريد من نوعه من خلال الاستفادة من
رقم تشغيل CI الذي يعرضه معظم مزودي CI. على سبيل المثال، في GitHub Actions، يمكنك
تعيين `CFBundleVersion` إلى المتغير <code v-pre>${{ github.run_number }}</code>.

سيفشل تحميل معاينة بنفس الملف الثنائي (build) ونفس `CFBundleVersion`.
<!-- -->
:::
...
المسارات بشكل مؤجل — ما عليك سوى تحديد اسم المسار عند المشاركة، وسيتم إنشاؤه
تلقائيًا إذا لم يكن موجودًا.

لمشاركة معاينة لمسار معين، استخدم الخيار `--track`:
...
- **تنظيم المعاينات**: قم بتجميع المعاينات حسب الغرض (على سبيل المثال، `beta` ،
  `nightly` ، `internal`)
- **التحديثات داخل التطبيق**: يستخدم Tuist SDK المسارات لتحديد التحديثات التي
  يجب إخطار المستخدمين بها
- **تصفية**: اعثر على المعاينات وأدِرها بسهولة حسب المسار في لوحة تحكم Tuist

::: warning PREVIEWS' VISIBILITY
<!-- -->
لا يمكن الوصول إلى المعاينات إلا للأشخاص الذين لديهم حق الوصول إلى المؤسسة التي
ينتمي إليها المشروع. نخطط لإضافة دعم لانتهاء صلاحية الروابط.
<!-- -->
:::

## تطبيق Tuist لنظام macOS {#tuist-macos-app}
...
لتسهيل تشغيل Tuist Previews، قمنا بتطوير تطبيق Tuist لشريط قوائم macOS. بدلاً من
...
:::: متطلبات التحذير
<!-- -->
يجب أن يكون Xcode مثبتًا محليًا وأن يكون نظام التشغيل macOS 14 أو أحدث.
<!-- -->
:::
...
على غرار تطبيق macOS، تعمل تطبيقات Tuist لنظام iOS على تبسيط الوصول إلى معايناتك
وتشغيلها.
...
إنشاء تطبيق محليًا يضيف عوائق غير ضرورية، مما يؤدي غالبًا إلى تخطي المطورين
لاختبار الوظائف على أجهزتهم تمامًا. لكن *ماذا لو احتوى كل طلب سحب على رابط إلى
النسخة التي ستشغل التطبيق تلقائيًا على جهاز قمت باختياره في تطبيق Tuist macOS؟*

بمجرد ربط مشروع Tuist الخاص بك بمنصة Git مثل [GitHub](https://github.com)، أضف
...
يتحقق SDK من وجود تحديثات ضمن نفس مسار المعاينة **** . عندما تشارك معاينة مع
مسار محدد باستخدام `--track` ، سيبحث SDK عن تحديثات على هذا المسار. إذا لم يتم
تحديد أي مسار، يتم استخدام فرع git كمسار — لذا فإن المعاينة التي تم إنشاؤها من
الفرع الرئيسي `` ستقوم فقط بالإخطار بوجود معاينات أحدث تم إنشاؤها أيضًا من الفرع
الرئيسي `` .
...
### تابع التحديثات {#sdk-monitor-updates}
...
`monitorPreviewUpdates` تُرجع مهمة `` يمكن إلغاؤها:
...
README` الخاص بـ `تشير إلى أحدث معاينة Tuist:

[![معاينة
Tuist](https://tuist.dev/Dimillian/IcySky/previews/latest/badge.svg)](https://tuist.dev/Dimillian/IcySky/previews/latest)

لإضافة الشارة إلى ملف README الخاص بـ `` ، استخدم الترميز التالي واستبدل أسماء
الحساب والمشروع بأسماء حسابك ومشروعك:
```
[![Tuist Preview](https://tuist.dev/{account-handle}/{project-handle}/previews/latest/badge.svg)](https://tuist.dev/{account-handle}/{project-handle}/previews/latest)
```

إذا كان مشروعك يحتوي على عدة تطبيقات بمعرفات حزم مختلفة، يمكنك تحديد معاينة
التطبيق التي تريد الارتباط بها عن طريق إضافة معلمة الاستعلام `bundle-id`:
```
...
يُعد إخراج JSON مفيدًا لإنشاء عمليات أتمتة مخصصة، مثل نشر رسالة Slack باستخدام
مزود CI الخاص بك. يحتوي JSON على عنوان URL `` مع رابط المعاينة الكامل ومفتاح
