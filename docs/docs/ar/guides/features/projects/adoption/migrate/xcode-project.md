...
مشروعًا جديدًا باستخدام Tuist</LocalizedLink>، وفي هذه الحالة سيتم تكوين كل شيء
تلقائيًا، فسيتعين عليك تعريف مشاريع Xcode الخاصة بك باستخدام عناصر Tuist
الأساسية. يعتمد مدى صعوبة هذه العملية على مدى تعقيد مشاريعك.

كما تعلمون على الأرجح، يمكن أن تصبح مشاريع Xcode فوضوية ومعقدة بمرور الوقت:
...
علاوة على ذلك، تعد الترحيل اليدوي ممارسة ممتازة لتنظيف مشاريعك وتبسيطها. لن يكون
المطورون في مشروعك هم وحدهم من يشكرونك على ذلك، بل سيشكرك أيضًا Xcode، الذي
سيصبح أسرع في معالجتها وفهرستها. بمجرد اعتماد Tuist بالكامل، سيضمن لك أن
المشاريع محددة بشكل متسق وأنها تظل بسيطة.

بهدف تسهيل هذه المهمة، نقدم لك بعض الإرشادات بناءً على التعليقات التي تلقيناها
من المستخدمين.

## إنشاء هيكل المشروع {#create-project-scaffold}

أولاً، قم بإنشاء هيكل أساسي لمشروعك باستخدام ملفات Tuist التالية:
...
البيان الذي ستحدد فيه التبعيات الخاصة بك. ملف `Tuist.swift` هو المكان الذي يمكنك
فيه تحديد إعدادات Tuist الخاصة بمشروعك.

::: tip PROJECT NAME WITH -TUIST SUFFIX
<!-- -->
لمنع حدوث تعارضات مع مشروع Xcode الحالي، نوصي بإضافة اللاحقة `-Tuist` إلى اسم
...
لضمان صحة ترحيل كل تغيير، نوصي بتوسيع نطاق التكامل المستمر الخاص بك لإنشاء
واختبار المشروع الذي أنشأه Tuist من ملف البيان الخاص بك:
...
## استخرج إعدادات بناء المشروع إلى ملفات ` و .xcconfig و` {#extract-the-project-build-settings-into-xcconfig-files}

استخرج إعدادات البناء من المشروع إلى ملف .xcconfig` في ` لجعل المشروع أكثر بساطة
...
ثم قم بتحديث ملف `Project.swift` ليشير إلى ملف `.xcconfig` الذي أنشأته للتو:
...
على إعدادات البناء مباشرةً في ملفات `.xcconfig`:
...
استخرج جميع تبعيات مشروعك إلى ملف Tuist/Package.swift` في مجلد `:
...
::: tip PRODUCT TYPES
<!-- -->
يمكنك تجاوز نوع المنتج لحزمة معينة عن طريق إضافته إلى قاموس
productTypes` في `ضمن بنية PackageSettings` في `. بشكل افتراضي، يفترض Tuist أن
جميع الحزم هي أطر عمل ثابتة.
<!-- -->
:::
...
التالي لإدراج أهداف المشروع، مرتبة حسب عدد التبعيات:

```bash
tuist migration list-targets -p Project.xcodeproj
```

ابدأ في ترحيل الأهداف من أعلى القائمة، لأنها الأكثر اعتمادًا عليها.


## ترحيل الأهداف {#migrate-targets}

قم بترحيل الأهداف واحدًا تلو الآخر. نوصي بإجراء طلب سحب لكل هدف لضمان مراجعة
التغييرات واختبارها قبل دمجها.

### استخرج إعدادات البناء المستهدفة إلى ملفات ` و .xcconfig و` {#extract-the-target-build-settings-into-xcconfig-files}

كما فعلت مع إعدادات إنشاء المشروع، قم باستخراج إعدادات إنشاء الهدف إلى ملف
`.xcconfig` لجعل الهدف أكثر بساطة وأسهل في الترحيل. يمكنك استخدام الأمر التالي
...
::: info TEST TARGETS
<!-- -->
إذا كان الهدف يحتوي على هدف اختبار مرتبط به، فيجب عليك تعريفه في ملف
...
كرر هذه الخطوات حتى يتم ترحيل جميع الأهداف بالكامل. بمجرد الانتهاء، نوصي بتحديث
خطوط أنابيب CI و CD الخاصة بك لإنشاء المشروع واختباره باستخدام `tuist generate`
متبوعًا بـ `xcodebuild build` و `tuist test`.

## استكشاف الأخطاء وإصلاحها {#استكشاف الأخطاء وإصلاحها}

### أخطاء تجميع بسبب ملفات مفقودة. {#compilation-errors-due-to-missing-files}

إذا لم تكن الملفات المرتبطة بأهداف مشروع Xcode الخاص بك موجودة بالكامل في دليل
نظام الملفات الذي يمثل الهدف، فقد ينتهي بك الأمر بمشروع لا يمكن ترجمته. تأكد من
أن قائمة الملفات بعد إنشاء المشروع باستخدام Tuist تتطابق مع قائمة الملفات في
مشروع Xcode، واغتنم الفرصة لمواءمة بنية الملفات مع بنية الهدف.
