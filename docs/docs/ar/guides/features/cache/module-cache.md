...
التطبيق، قم بإنشاء مشروع مع المصادر باستخدام `- ملف تعريف ذاكرة التخزين المؤقت
لا شيء`.
<!-- -->
:::
...
<!-- -->
تم إهمال العلامة `--لا-لا-ذاكرة تخزين مؤقت ثنائية`. استخدم `- ملف تعريف ذاكرة
التخزين المؤقت لا شيء` بدلاً من ذلك. لا تزال العلامة المهملة تعمل من أجل التوافق
...
1. `-لا يوجد ملف تعريف ذاكرة التخزين المؤقت`
...
المؤقت الثنائي، يجب الإعلان عن التبعيات بشكل صريح؛ وإلا سترى على الأرجح أخطاء في
التحويل البرمجي عندما يتعذر العثور على الرموز. لتصحيح هذا الأمر، نوصي باستخدام
الأمر
<LocalizedLink href="/guides/features/projects/inspect/implicit-dependencies">`tuist
فحص التبعيات - فقط الضمني`</LocalizedLink> وإعداده في CI لمنع حدوث انحدارات في
الربط الضمني.

### ذاكرة التخزين المؤقت للوحدات القديمة {#legacy-module-cache}

في تويست `4.128.0` ، جعلنا البنية التحتية الجديدة لذاكرة التخزين المؤقت للوحدة
النمطية هي الافتراضية. إذا واجهت مشكلات مع هذا الإصدار الجديد، يمكنك العودة إلى
سلوك ذاكرة التخزين المؤقت القديمة عن طريق تعيين متغير البيئة
`TUIST_LEGACY_MODULE_CACHE`.

تُعد ذاكرة التخزين المؤقت للوحدة النمطية القديمة هذه احتياطية مؤقتة وستتم
إزالتها من جانب الخادم في تحديث مستقبلي. خطط للترحيل بعيدًا عنه.
