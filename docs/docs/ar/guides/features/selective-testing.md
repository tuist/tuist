...
<LocalizedLink href="/guides/features/projects">مشروعك الذي تم
...
للوحدة النمطية</LocalizedLink>، وعند النجاح، يحفظ التجزئات لتحديد ما الذي تغير
في عمليات التشغيل المستقبلية. في عمليات التشغيل المستقبلية، يستخدم الأمر `tuist
test` التجزئات بشكل شفاف لتصفية الاختبارات وتشغيل الاختبارات التي تغيرت فقط منذ
آخر عملية تشغيل ناجحة للاختبار.

`يتكامل اختبار tuist` مباشرة مع
<LocalizedLink href="/guides/features/cache/module-cache">ذاكرة التخزين المؤقتة
للوحدة النمطية</LocalizedLink> لاستخدام أكبر عدد ممكن من الملفات الثنائية من
التخزين المحلي أو البعيد لتحسين وقت الإنشاء عند تشغيل مجموعة الاختبارات الخاصة
بك. يمكن أن يؤدي الجمع بين الاختبار الانتقائي وتخزين الوحدات النمطية مؤقتًا إلى
