...
由於 Xcode 與 XcodeProj 的設計，維護依賴關係圖可能是一項繁瑣且容易出錯的工作。以下是一些您可能遇到的問題範例：
...
- 目標的傳遞動態依賴項需複製至任何屬於以下建置設定的目錄：`LD_RUNPATH_SEARCH_PATHS`
  若未複製，目標在執行時將無法找到這些依賴項。當圖結構較小時，此設定易於理解與配置；但隨著圖結構擴展，此問題將日益顯著。
- 當目標連結靜態的
  [XCFramework](https://developer.apple.com/documentation/xcode/creating-a-multi-platform-binary-framework-bundle)
  時，需為目標新增建置階段，以便 Xcode 處理套件並提取符合當前平台與架構的正確二進位檔。此建置階段不會自動新增，且容易遺漏。

以上僅為部分範例，多年來我們遇見的特殊情況遠不止於此。試想若需工程團隊維護依賴關係圖並確保其有效性，甚至更糟的是，這些複雜規則由封閉式建置系統在編譯階段強制執行，而您既無法控制也無法自訂。聽起來很熟悉嗎？這正是蘋果在
Xcode 與 XcodeProj 中採用的作法，而 Swift Package Manager 亦沿襲了此模式。
...
<!-- -->
Tuist 在生成專案時會驗證圖結構，確保不存在迴路且所有依賴關係皆有效。此機制使任何團隊皆能參與依賴圖的演進，無須擔憂破壞結構完整性。
<!-- -->
:::
...
Swift Packages 是我們推薦的專案依賴項宣告方式。您可透過 Xcode 的預設整合機制，或採用 Tuist 的 XcodeProj
整合方案進行整合。
...
XcodeProj 的目標將 Swift Packages 整合至專案中。 藉此不僅能強化整合控制力，更能相容於
<LocalizedLink href="/guides/features/cache">快取</LocalizedLink>與
<LocalizedLink href="/guides/features/test/selective-testing">選擇性測試執行</LocalizedLink>等工作流程。

XcodeProj 的整合功能可能需要較長時間才能支援新的 Swift Package 功能或處理更多套件設定。然而，Swift Packages 與
XcodeProj 目標之間的對應邏輯屬開源性質，可由社群貢獻開發。這與 Xcode 的預設整合機制形成對比——後者採封閉式開發，由 Apple 負責維護。
...
<!-- -->
`透過編譯器指令封裝的 PackageSettings`
實例，可讓您設定套件整合方式。例如上文範例中，此設定用於覆寫套件預設的產品類型。通常情況下，您無需使用此設定。
<!-- -->
:::

> [!重要] 自訂建置設定 若您的專案使用自訂建置設定（非標準設定：`Debug` 及`Release` ），必須透過`baseSettings`
...
如您所見，我們採用類似[CocoaPods](https://cocoapods.org)的作法，將依賴項解析設為獨立指令。此設計讓使用者能自主決定解析與更新依賴項的時機，並可直接開啟專案的Xcode進行編譯。
我們認為，隨著專案規模擴大，Apple 與 Swift Package Manager 的整合所提供的開發者體驗，在此領域會隨時間逐漸惡化。
...
<!-- -->
為保持方案清單簡潔，Swift Package 專案不會自動建立**方案（例如** ）。您可透過 Xcode 介面手動建立這些方案。
<!-- -->
:::

#### Xcode 的預設整合{#xcodes-default-integration}

若欲使用 Xcode 的預設整合機制，可在建立專案時傳遞以下清單：`packages`
...
若要生成完整保留建置工具外掛的 Xcode 專案，必須在專案清單的 ``` 區塊中宣告 `packages` 陣列 `` `，並於目標的依賴項中加入類型 ```
的 `.plugin` 外掛 `` `。
...
目錄中的依賴關係，接著使用`.framework` 或`.xcframework`
...
<!-- -->
若您透過以下指令建置與測試專案：`xcodebuild build` ` tuist test` 則需在建置或測試前執行以下指令，確保 Carthage
解析的依賴項已存在：`carthage update`
<!-- -->
:::
...
在專案圖譜中切換靜態與動態連結的挑戰在於，此操作在 Xcode
中並非易事——因任何變更皆會對整個圖譜產生連鎖效應（例如：函式庫不可包含資源，靜態框架無需嵌入）。 Apple曾嘗試透過編譯時解決方案處理此問題，例如Swift
Package
...
請注意，Tuist
<LocalizedLink href="/guides/features/projects/cost-of-convenience">不會因成本考量</LocalizedLink>而預設透過隱含設定提供便利性。這意味著我們仰賴您自行設定連結類型及某些必要之額外建置設定（例如連結器旗標
[`-ObjC`
...
在某些情境下，將連結完全設定為靜態或動態皆不可行或不妥當。以下列舉部分可能需要混合使用靜態與動態連結的情境（非完整清單）：

- **具備擴充功能的應用程式：**
  由於應用程式及其擴充功能需共享程式碼，您可能需要將這些目標設為動態。否則將導致應用程式與擴充功能重複包含相同程式碼，造成二進位檔體積膨脹。
- **預編譯外部依賴項：** 有時您會獲得預編譯的二進位檔，其類型可能是靜態或動態。靜態二進位檔可封裝於動態框架或函式庫中，以實現動態連結。

當修改圖結構時，Tuist
會進行分析，若偵測到「靜態副作用」將顯示警告。此警告旨在協助您識別因靜態連結目標所引發的問題——該目標透過動態目標間接依賴於靜態目標。此類副作用通常會導致二進位檔體積增大，最嚴重時甚至可能引發執行階段崩潰。

## 疑難排解{#troubleshooting}

### Objective-C 依賴項{#objectivec-dependencies}

整合 Objective-C 依賴項時，可能需要在使用目標中加入特定標記，以避免執行階段崩潰，詳見 [Apple 技術問答
...
` 使用 Objective-C 依賴項（或內部 Objective-C 目標）的消費者，應在消費目標中設定 ``` 並為 `OTHER_LDFLAGS` 賦值
`` ` 或 ```，以在需要時應用 ``` 旗標 `-ObjC` `` ` 或 ` ` 旗標 `-force_load`。
...
``` 許多 Google 函式庫採用 Objective-C 編寫。因此，任何使用這些函式庫的目標需在其建置設定中加入 ``` 的
`OTHER_LDFLAGS` 選項：`-ObjC``` `此設定可透過 ``` 的 `.xcconfig` 檔案設定，或於 Tuist
清單文件中手動指定於目標設定中。範例如下：
...
某些 Google 函式庫依賴於`FBLPromises` 這個函式庫，而該函式庫又依賴於 Google
的另一函式庫。您可能會遇到提及`FBLPromises` 的當機狀況，其內容類似如下：
...
如[此處](https://github.com/pointfreeco/swift-composable-architecture/discussions/1657#discussioncomment-4119184)及[疑難排解章節](#troubleshooting)所述，靜態連結套件時（Tuist預設連結類型），需將`的OTHER_LDFLAGS設定為`
，並將` 設定為`$(inherited) -ObjC。另可將套件的產品類型覆寫為動態連結。 靜態連結時，測試與應用程式目標通常運作正常，但 SwiftUI
預覽功能會失效。此問題可透過動態連結解決。下例中同時將
...
以下設定將動態連結所有元素——因此應用程式 + 測試目標與 SwiftUI 預覽功能皆可正常運作。
...
當動態框架或函式庫透過以下方式依賴靜態框架時：`import StaticSwiftModule`
，其符號會被包含在動態框架或函式庫的`.swiftmodule`
中，可能<LocalizedLink href="https://forums.swift.org/t/compiling-a-dynamic-framework-with-a-statically-linked-library-creates-dependencies-in-swiftmodule-file/22708/1">導致編譯失敗</LocalizedLink>。為避免此情況，需使用以下方式導入靜態依賴項：<LocalizedLink href="https://github.com/swiftlang/swift-evolution/blob/main/proposals/0409-access-level-on-imports.md">`internal
...
::: info
<!-- -->
Swift 6 引入了輸入函式庫的存取層級機制。若您使用的是舊版
