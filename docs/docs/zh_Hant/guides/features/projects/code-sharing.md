...
- 這能簡化**的維護工作** ，因為變更只需在單一處進行，所有專案便會自動獲得更新。
...
<!-- -->
許多組織青睞 Tuist，因為他們在專案描述輔助工具中看見平台團隊制定自身規範、建立專屬專案描述語言的契機。舉例而言，基於 YAML 的專案生成器必須開發專屬的
YAML 模板解決方案，否則將迫使組織依賴其工具進行建置。
<!-- -->
:::

## 專案說明輔助工具{#project-description-helpers}

專案描述輔助程式是 Swift 檔案，編譯後會成為模組`ProjectDescriptionHelpers`
，供清單檔案匯入。此模組透過彙整`Tuist/ProjectDescriptionHelpers` 目錄內所有檔案進行編譯。

您可在檔案頂端添加 import 陳述式，將其匯入您的清單檔案：
...
- `Project.swift`
- ` `Package.swift` (僅在啟用編譯器標記 #TUIST` 時生效)
- `工作區.swift`

## 範例{#example}

以下程式片段展示如何擴展`專案` 的模型以新增靜態建構函式，以及如何從`專案的 Project.swift` 檔案中使用這些函式：
