...
能自動偵測不穩定測試，並協助您長期追蹤其變化。

![不穩定測試頁面](/images/guides/features/test-insights/flaky-tests-page.png)
...
當您使用 Xcode 的重試功能執行測試（透過`-retry-tests-on-failure` 或`-test-iterations` 執行）時，Tuist
會分析每次嘗試的結果。若某項測試在部分嘗試中失敗而在其他嘗試中通過，則標記為不穩定測試。

例如，若某項測試首次執行失敗但重試時通過，Tuist 會將其記錄為不穩定測試。
...
即使不進行測試重試，Tuist 也能透過比較同一提交在不同 CI 執行中的結果來偵測不穩定測試。若某項測試在一次 CI
執行中通過，但在同一提交的另一次執行中失敗，則兩次執行皆會標記為不穩定。

此功能特別適用於偵測不穩定測試：這些測試失敗的頻率不足以被重試機制捕捉，卻仍會導致持續性CI失敗。
