...
此指令將引導您完成網頁式驗證流程。驗證成功後，CLI 會將長效刷新令牌與短效存取令牌儲存至`~/.config/tuist/credentials`
。該目錄中的每個檔案代表您驗證的網域，預設應為`tuist.dev.json` 。此目錄儲存的資訊屬敏感資料，請務必妥善保管**** 。

CLI 在向伺服器發送請求時會自動查詢憑證。若存取權杖已過期，CLI 將使用更新權杖取得新的存取權杖。

## OIDC 通證{#oidc-tokens}

對於支援 OpenID Connect (OIDC) 的持續整合環境，Tuist 可自動進行驗證，無需您管理長期存效的機密金鑰。在支援的 CI
...
OIDC 憑證授予`ci` 範圍群組，該群組提供存取儲存庫所連結之所有專案的權限。有關`ci` 範圍群組包含的內容詳情，請參閱
[範圍群組](#scope-groups)。

::: tip SECURITY BENEFITS
<!-- -->
OIDC 驗證比長期存效憑證更安全，原因如下：
- 無需輪替或管理的機密
- 代幣具有短暫生命週期，且僅限於單次工作流程執行範圍內生效
- 驗證機制與您的儲存庫身分綁定
<!-- -->
:::
...
對於不支援 OIDC 的 CI 環境，或需要精細控制權限時，可使用帳戶憑證。帳戶憑證允許您精確指定憑證可存取的範圍與專案。
...
| `--到期時間` | 可選。指定代幣失效時間。格式如下：`30d` （天數），`6m` （月數），或`1y` （年數）。若未指定，代幣永久有效。 |
...
對於不支援 OIDC 的 CI 環境，可透過`ci` 建立具有 scope group 的帳戶憑證，用於驗證 CI 工作流程：
