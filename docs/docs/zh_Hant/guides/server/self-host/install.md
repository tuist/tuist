...
我們強烈建議建立持續部署管道，每日自動部署 Tuist 最新版本。此舉可確保您隨時享有最新功能、改進項目及安全性更新。
...
<!-- -->
若您的主要目標是建立專屬儲存二進位檔的儲存桶並降低快取延遲，則未必需要自行架設完整伺服器。您可自行架設快取節點，並將其連接至託管的 Tuist
伺服器或您自行架設的伺服器。
...
| `TUIST_S3_CONNECT_TIMEOUT`                            | 與儲存供應商建立連線的超時設定（單位：毫秒）                                             | 不   | `3000`   | `3000`                                                        |
