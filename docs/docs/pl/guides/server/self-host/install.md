...
które potrzebują większej kontroli nad swoją infrastrukturą. Ta wersja pozwala
na hostowanie Tuist na własnej infrastrukturze, zapewniając bezpieczeństwo i
prywatność danych.

::: warning LICENSE REQUIRED
<!-- -->
Samodzielne hostowanie Tuist wymaga prawnie ważnej płatnej licencji. Wersja
lokalna Tuist jest dostępna wyłącznie dla organizacji korzystających z planu
Enterprise. Jeśli jesteś zainteresowany tą wersją, skontaktuj się z nami pod
adresem [contact@tuist.dev](mailto:contact@tuist.dev).
<!-- -->
:::

## Kadencja zwalniania {#release-cadence}

W miarę pojawiania się nowych zmian gotowych do wydania w gałęzi głównej, na
bieżąco udostępniamy nowe wersje Tuist. Stosujemy [wersjonowanie
semantyczne](https://semver.org/), aby zapewnić przewidywalność wersjonowania i
kompatybilność.

Komponent ten służy do sygnalizowania istotnych zmian w serwerze Tuist, które
będą wymagały koordynacji z użytkownikami lokalnymi. Nie należy oczekiwać, że
będziemy z niego korzystać, a w razie potrzeby zapewniamy, że będziemy
współpracować z Państwem, aby przejście przebiegło płynnie.

## Wdrażanie ciągłe {#continuous-deployment}
...
Oto przykładowy przepływ pracy GitHub Actions, który codziennie sprawdza
dostępność nowych wersji i je wdraża:
...
W tej sekcji opisano wymagania dotyczące hostowania serwera Tuist w Twojej
infrastrukturze.
...
używanym do przechowywania danych szeregów czasowych i wyszukiwania. Jednak
**TimescaleDB jest przestarzałe** i wkrótce przestanie być wymaganym elementem,
bo przenosimy całą funkcjonalność szeregów czasowych do ClickHouse. Na razie
upewnij się, że Twoja instancja PostgreSQL ma zainstalowane i włączone
TimescaleDB.
<!-- -->
:::
...
Aby uruchomić to rozwiązanie, Twoja infrastruktura musi obsługiwać obrazy
Docker. Pamiętaj, że większość dostawców infrastruktury obsługuje tę
technologię, ponieważ stała się ona standardowym kontenerem do dystrybucji i
uruchamiania oprogramowania w środowiskach produkcyjnych.

### Baza danych Postgres {#postgres-database}

Oprócz uruchomienia obrazów Docker, będziesz potrzebować [bazy danych
Postgres](https://www.postgresql.org/) z [rozszerzeniem
TimescaleDB](https://www.timescale.com/) do przechowywania danych relacyjnych i
szeregów czasowych. Większość dostawców infrastruktury uwzględnia bazy danych
Postgres w swojej ofercie (np. [AWS](https://aws.amazon.com/rds/postgresql/) i
[Google Cloud](https://cloud.google.com/sql/docs/postgres)).

**Wymagane rozszerzenie TimescaleDB:** Tuist wymaga rozszerzenia TimescaleDB do
wydajnego przechowywania danych szeregów czasowych i wykonywania zapytań.
Rozszerzenie to jest używane do zdarzeń poleceń, analiz i innych funkcji
opartych na czasie. Przed uruchomieniem Tuist upewnij się, że na instancji
PostgreSQL zainstalowano i włączono TimescaleDB.
...
wyszukiwania dużych ilości danych analitycznych. ClickHouse jest **wymaganym**
dla funkcji takich jak budowanie wniosków i będzie główną bazą danych szeregów
czasowych w miarę wycofywania TimescaleDB. Możesz wybrać, czy chcesz
samodzielnie hostować ClickHouse, czy skorzystać z ich usługi hostingowej.
...
<!-- -->
Jeśli Twoim głównym celem jest przede wszystkim zapewnienie własnej przestrzeni
do przechowywania plików binarnych i zmniejszenie opóźnienia pamięci podręcznej,
samodzielne hostowanie całego serwera może nie być konieczne. Możesz
samodzielnie hostować węzły pamięci podręcznej i połączyć je z hostowanym
serwerem Tuist lub własnym serwerem.

Zobacz <LocalizedLink href="/guides/cache/self-host">przewodnik po samodzielnym
hostingu pamięci podręcznej</LocalizedLink>.
<!-- -->
:::

## Konfiguracja {#configuration}

Konfiguracja usługi odbywa się w czasie wykonywania za pomocą zmiennych
środowiskowych. Biorąc pod uwagę wrażliwy charakter tych zmiennych, zalecamy ich
szyfrowanie i przechowywanie w bezpiecznych rozwiązaniach do zarządzania
hasłami. Zapewniamy, że Tuist obchodzi się z tymi zmiennymi z najwyższą
starannością, dbając o to, by nigdy nie pojawiały się one w logach.

::: info LAUNCH CHECKS
<!-- -->
Niezbędne zmienne są sprawdzane podczas uruchamiania. Jeśli którejś z nich
brakuje, uruchomienie zakończy się niepowodzeniem, a komunikat o błędzie będzie
zawierał szczegółowe informacje o brakujących zmiennych.
<!-- -->
:::

### Konfiguracja licencji {#license-configuration}

Jako użytkownik lokalny otrzymasz klucz licencyjny, który musisz udostępnić jako
zmienną środowiskową. Klucz ten służy do weryfikacji licencji i zapewnienia, że
usługa działa zgodnie z warunkami umowy.

| Zmienna środowiskowa               | Opis                                                                                                                                                                                                                                                                              | Wymagane | Domyślnie | Przykłady                                 |
| ---------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | --------- | ----------------------------------------- |
| `TUIST_LICENSE`                    | Licencja udzielona po podpisaniu umowy o gwarantowanym poziomie usług                                                                                                                                                                                                             | Tak*     |           | `******`                                  |
| `TUIST_LICENSE_CERTIFICATE_BASE64` | **Wyjątkowa alternatywa dla `TUIST_LICENSE`**. Certyfikat publiczny zakodowany w Base64 do offline'owej weryfikacji licencji w środowiskach typu air-gapped, gdzie serwer nie może łączyć się z usługami zewnętrznymi. Używaj tylko wtedy, gdy `TUIST_LICENSE` nie może być użyty | Tak*     |           | `LS0tLS1CRUdJTiBDRVJUSUZJQ0FURS0tLS0t...` |
...
korzystania z poleceń Tuist, które komunikują się z serwerem, jeśli licencja
wygaśnie w ciągu mniej niż 30 dni. Jeśli chcesz odnowić swoją licencję,
skontaktuj się z nami pod adresem [contact@tuist.dev](mailto:contact@tuist.dev).
<!-- -->
:::
...
| `TUIST_SECRET_KEY_ENCRYPTION`         | 32-bajtowy klucz do szyfrowania danych wrażliwych algorytmem AES-GCM                                                                                                                                                                      | Nie      | `$TUIST_SECRET_KEY_BASE`           |                                                                                 |                                                                                                                                    |
| `TUIST_USE_IPV6`                      | Gdy `1`, aplikacja zostaje skonfigurowana do korzystania z adresów IPv6                                                                                                                                                                   | Nie      | `0`                                | `1`                                                                             |                                                                                                                                    |
| `TUIST_LOG_LEVEL`                     | Poziom logowania, którego należy używać w aplikacji                                                                                                                                                                                       | Nie      | `info`                             | [Poziomy logowania](https://hexdocs.pm/logger/1.12.3/Logger.html#module-levels) |                                                                                                                                    |
| `TUIST_GITHUB_APP_NAME`               | Wersja URL nazwy Twojej aplikacji GitHub                                                                                                                                                                                                  | Nie      |                                    | `my-app`                                                                        |                                                                                                                                    |
| `TUIST_GITHUB_APP_PRIVATE_KEY_BASE64` | Klucz prywatny zakodowany w base64, używany przez aplikację GitHub do odblokowania dodatkowych funkcji, takich jak automatyczne publikowanie komentarzy do PR                                                                             | Nie      | `LS0tLS1CRUdJTiBSU0EgUFJJVkFUR...` |                                                                                 |                                                                                                                                    |
| `TUIST_GITHUB_APP_PRIVATE_KEY`        | Klucz prywatny używany przez aplikację GitHub do odblokowania dodatkowych funkcji, takich jak automatyczne publikowanie komentarzy do PR. **Zalecamy używanie wersji zakodowanej w base64, aby uniknąć problemów ze znakami specjalnymi** | Nie      | `-----BEGIN RSA...`                |                                                                                 |                                                                                                                                    |
| `TUIST_OPS_USER_HANDLES`              | Lista nazw użytkowników z dostępem do adresów URL operacji, rozdzielona przecinkami                                                                                                                                                       | Nie      |                                    | `user1,user2`                                                                   |                                                                                                                                    |
...
| Zmienna środowiskowa                 | Opis                                                                                                                                                                                                                             | Wymagane | Domyślnie | Przykłady                                                              |
| ------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | --------- | ---------------------------------------------------------------------- |
| `DATABASE_URL`                       | Adres URL umożliwiający dostęp do bazy danych Postgres. Należy pamiętać, że adres URL powinien zawierać dane uwierzytelniające                                                                                                   | Tak      |           | `postgres://username:password@cloud.us-east-2.aws.test.com/production` |
| `TUIST_CLICKHOUSE_URL`               | Adres URL umożliwiający dostęp do bazy danych ClickHouse. Należy pamiętać, że adres URL powinien zawierać dane uwierzytelniające                                                                                                 | Nie      |           | `http://username:password@cloud.us-east-2.aws.test.com/production`     |
| `TUIST_USE_SSL_FOR_DATABASE`         | Gdy wartość jest prawdziwa, używa [SSL](https://en.wikipedia.org/wiki/Transport_Layer_Security) do połączenia z bazą danych                                                                                                      | Nie      | `1`       | `1`                                                                    |
...
| `TUIST_DATABASE_QUEUE_INTERVAL`      | Czas progowy (w milisekundach) w kolejce, który pula wykorzystuje do określenia, czy powinna zacząć odrzucać nowe połączenia [(Więcej informacji)](https://hexdocs.pm/db_connection/DBConnection.html#start_link/2-queue-config) | Nie      | `1000`    | `1000`                                                                 |
| `TUIST_CLICKHOUSE_FLUSH_INTERVAL_MS` | Odstęp czasu w milisekundach między opróżnieniami bufora ClickHouse                                                                                                                                                              | Nie      | `5000`    | `5000`                                                                 |
...
| `TUIST_CLICKHOUSE_BUFFER_POOL_SIZE`  | Liczba procesów buforujących ClickHouse do uruchomienia                                                                                                                                                                          | Nie      | `5`       | `5`                                                                    |
...
są obecne w środowisku serwera. **Brakujące zmienne** spowodują, że Tuist
pominie tego dostawcę.

#### GitHub {#github}

Zalecamy uwierzytelnianie za pomocą [aplikacji
GitHub](https://docs.github.com/en/apps/creating-github-apps/about-creating-github-apps/about-creating-github-apps),
ale można również użyć [aplikacji
OAuth](https://docs.github.com/en/apps/oauth-apps/building-oauth-apps/creating-an-oauth-app).
Należy pamiętać o uwzględnieniu wszystkich niezbędnych zmiennych środowiskowych
określonych przez GitHub w środowisku serwera. Brakujące zmienne spowodują, że
Tuist pominie uwierzytelnianie GitHub. Aby poprawnie skonfigurować aplikację
GitHub:
...
utworzyć nowe poświadczenie typu identyfikator klienta OAuth. Podczas tworzenia
poświadczenia wybierz „Aplikacja internetowa” jako typ aplikacji, nazwij je
`Tuist` i ustaw adres URI przekierowania na
`{base_url}/users/auth/google/callback`, gdzie `base_url` to adres URL, pod
którym działa Twoja usługa hostowana. Po utworzeniu aplikacji skopiuj
identyfikator klienta i sekret, a następnie ustaw je jako zmienne środowiskowe
odpowiednio `GOOGLE_CLIENT_ID` oraz `GOOGLE_CLIENT_SECRET`.

::: info CONSENT SCREEN SCOPES
<!-- -->
Być może konieczne będzie utworzenie ekranu zgody. W takim przypadku należy
dodać `userinfo.email` oraz `openid` zakresy i oznaczyć aplikację jako
wewnętrzną.
<!-- -->
:::
...
aplikacji Okta konieczne będzie ustawienie następujących zmiennych
środowiskowych:

| Zmienna środowiskowa         | Opis                                                                                                                | Wymagane | Domyślnie | Przykłady |
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------- | -------- | --------- | --------- |
| `TUIST_OKTA_1_CLIENT_ID`     | Identyfikator klienta służący do uwierzytelniania w Okta. Numer ten powinien być identyfikatorem Twojej organizacji | Tak      |           |           |
| `TUIST_OKTA_1_CLIENT_SECRET` | Sekret klienta do uwierzytelniania w Okta                                                                           | Tak      |           |           |

Liczbę `1` należy zastąpić identyfikatorem Twojej organizacji. Zazwyczaj będzie
to 1, ale sprawdź to w swojej bazie danych.
...
#### Pamięci zgodne ze standardem S3 {#s3compliant-storages}

Do przechowywania artefaktów można używać dowolnego dostawcy pamięci masowej
zgodnego ze standardem S3. Do uwierzytelnienia i skonfigurowania integracji z
dostawcą pamięci masowej wymagane są następujące zmienne środowiskowe:

| Zmienna środowiskowa                                     | Opis                                                                                                                                                                                                    | Wymagane | Domyślnie                        | Przykłady                                                           |
| -------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | -------------------------------- | ------------------------------------------------------------------- |
| `TUIST_S3_ACCESS_KEY_ID` lub `AWS_ACCESS_KEY_ID`         | Identyfikator klucza dostępu służący do uwierzytelniania u dostawcy pamięci masowej                                                                                                                     | Tak      |                                  | `AKIAIOSFOD`                                                        |
| `TUIST_S3_SECRET_ACCESS_KEY` lub `AWS_SECRET_ACCESS_KEY` | Tajny klucz dostępu do uwierzytelniania u dostawcy pamięci masowej                                                                                                                                      | Tak      |                                  | `wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY`                          |
| `TUIST_S3_REGION` lub `AWS_REGION`                       | Region, w którym znajduje się zbiornik                                                                                                                                                                  | Nie      | `auto`                           | `us-west-2`                                                         |
...
| `TUIST_S3_CA_CERT_PEM`                                   | Certyfikat CA zakodowany w formacie PEM do weryfikacji połączeń HTTPS w usłudze S3. Przydatny w środowiskach typu air-gapped z certyfikatami z podpisem własnym lub wewnętrznymi urzędami certyfikacji. | Nie      | Pakiet CA systemu                | `-----POCZĄTEK CERTYFIKATU-----\n...\n-----KONIEC CERTYFIKATU-----` |
...
| `TUIST_S3_POOL_MAX_IDLE_TIME`                            | Maksymalny czas bezczynności (w milisekundach) dla połączeń w puli. Użyj `infinity`, aby połączenia pozostawały aktywne przez czas nieokreślony                                                         | Nie      | `nieskończoność`                 | `60000`                                                             |
...
| `TUIST_S3_PROTOCOL`                                      | Protokół, którego należy używać podczas łączenia się z dostawcą pamięci masowej (`http1` lub `http2`)                                                                                                   | Nie      | `http1`                          | `http1`                                                             |
| `TUIST_S3_VIRTUAL_HOST`                                  | Czy adres URL powinien być skonstruowany z nazwą segmentu jako subdomeną (wirtualnym hostem)                                                                                                            | Nie      | `false`                          | `1`                                                                 |
...
Tuist użyje tej metody, korzystając ze standardowych zmiennych środowiskowych
AWS.
<!-- -->
:::

#### Google Cloud Storage {#google-cloud-storage}
W przypadku Google Cloud Storage postępuj zgodnie z [tą
dokumentacją](https://cloud.google.com/storage/docs/authentication/managing-hmackeys),
aby uzyskać parę `AWS_ACCESS_KEY_ID` oraz `AWS_SECRET_ACCESS_KEY`.
`AWS_ENDPOINT` należy ustawić na `https://storage.googleapis.com`. Pozostałe
zmienne środowiskowe są takie same jak w przypadku każdej innej pamięci zgodnej
ze standardem S3.

### Konfiguracja poczty elektronicznej {#email-configuration}
...
| `TUIST_SKIP_EMAIL_CONFIRMATION`  | Pomiń potwierdzenie e-mailowe przy rejestracji nowych użytkowników. Po włączeniu tej opcji użytkownicy są automatycznie potwierdzani i mogą zalogować się natychmiast po rejestracji | Nie      | `true`, jeśli adres e-mail nie jest skonfigurowany, `false`, jeśli adres e-mail jest skonfigurowany | `true`, `false`, `1`, `0` |

\* Zmienne konfiguracyjne poczty e-mail są wymagane tylko wtedy, gdy chcesz
wysyłać wiadomości e-mail. Jeśli nie zostaną skonfigurowane, potwierdzenie
e-mailowe zostanie automatycznie pominięte

::: info SMTP SUPPORT
<!-- -->
Ogólna obsługa protokołu SMTP nie jest obecnie dostępna. Jeśli potrzebujesz
...
`TUIST_SKIP_EMAIL_CONFIRMATION=true`. Aby wymagać potwierdzenia e-mailowego, gdy
poczta e-mail jest skonfigurowana, ustaw `TUIST_SKIP_EMAIL_CONFIRMATION=false`.
<!-- -->
:::
...
utworzyłeś aplikację GitHub OAuth. W sekcji `Uprawnienia i zdarzenia`'s
`Uprawnienia do repozytorium` musisz dodatkowo ustawić uprawnienie `Pull
requests` na `Odczyt i zapis`.

Oprócz `TUIST_GITHUB_APP_CLIENT_ID` oraz `TUIST_GITHUB_APP_CLIENT_SECRET`,
potrzebne będą następujące zmienne środowiskowe:
...
Udostępniamy kompleksową konfigurację Docker Compose, która zawiera wszystkie
...
- MinIO dla pamięci masowej zgodnej ze standardem S3
- Redis do trwałego przechowywania danych klucz-wartość (KV) między wdrożeniami
  (opcjonalnie)
- pgweb do administrowania bazami danych

::: danger LICENSE REQUIRED
<!-- -->
Prawidłowa zmienna środowiskowa `TUIST_LICENSE` jest wymagana prawnie do
uruchomienia serwera Tuist, w tym lokalnych instancji deweloperskich. Jeśli
potrzebujesz licencji, skontaktuj się z
...
- Konsola MinIO: http://localhost:9003 (dane logowania: `tuist` /
  `tuist_dev_password`)
- API MinIO: http://localhost:9002
- pgweb (interfejs użytkownika PostgreSQL): http://localhost:8081
...
Zresetuj wszystko (usunie wszystkie dane):
```bash
...
dostawcy usług w chmurze oraz podejścia Twojej organizacji do ciągłego
wdrażania. Ponieważ większość rozwiązań i narzędzi chmurowych, takich jak
...
::: warning
<!-- -->
`Jeśli Twój potok wdrażania musi sprawdzić, czy serwer działa, możesz wysłać
żądanie HTTP typu GET` z parametrem `na adres `/ready` i sprawdzić, czy w
odpowiedzi znajduje się kod statusu 200`.
<!-- -->
:::
...
konfiguracyjny fly.toml` z opcją `. Rozważ wygenerowanie go dynamicznie w ramach
potoku ciągłego wdrażania (CD). Poniżej znajduje się przykładowy plik, z którego
możesz skorzystać:
...
--local-only`, musisz uruchomić `fly deploy --local-only`. Fly.io nie pozwala na
pobieranie prywatnych obrazów Docker, dlatego musimy użyć flagi `--local-only`.


## Metryki Prometeusza {#prometheus-metrics}

Tuist udostępnia metryki Prometheusa pod adresem `/metrics`, aby pomóc Ci
monitorować Twoją instancję hostowaną samodzielnie. Metryki te obejmują:

### Wskaźniki klienta HTTP Finch {#finch-metrics}
...
#### Wskaźniki kolejki puli połączeń
...
- `tuist_prom_ex_finch_queue_idle_time_milliseconds` - Czas, przez jaki
  połączenie pozostawało w stanie bezczynności przed użyciem (histogram)
  - Etykiety: `finch_name`, `scheme`, `host`, `port`, `pool`
...
#### Wskaźniki połączeń
- `tuist_prom_ex_finch_connect_duration_milliseconds` - Czas potrzebny na
  nawiązanie połączenia (histogram)
...
#### Wyślij dane statystyczne
- `tuist_prom_ex_finch_send_duration_milliseconds` - Czas wysyłania żądania
  (histogram)
  - Etykiety: `finch_name`, `method`, `scheme`, `host`, `port`, `error`
  - Przedziały czasowe: 1 ms, 5 ms, 10 ms, 25 ms, 50 ms, 100 ms, 250 ms, 500 ms,
    1 s
- `tuist_prom_ex_finch_send_idle_time_milliseconds` - Czas, przez jaki
  połączenie pozostawało w stanie bezczynności przed wysłaniem (histogram)
  - Etykiety: `finch_name`, `method`, `scheme`, `host`, `port`, `error`
  - Przedziały: 1 ms, 5 ms, 10 ms, 25 ms, 50 ms, 100 ms, 250 ms, 500 ms

Wszystkie metryki histogramu udostępniają warianty `_bucket`, `_sum` oraz
`_count` do szczegółowej analizy.

### Inne wskaźniki

Oprócz wskaźników Finch, Tuist udostępnia wskaźniki dla:
- Wydajność maszyny wirtualnej BEAM
- Niestandardowe wskaźniki logiki biznesowej (magazyn, konta, projekty itp.)
- Wydajność bazy danych (w przypadku korzystania z infrastruktury hostowanej
  przez Tuist)

## Operacje {#operations}

Tuist udostępnia zestaw narzędzi pod adresem `/ops/`, których można używać do
zarządzania instancją.

::: warning Authorization
<!-- -->
Dostęp do punktów końcowych `/ops/` mają wyłącznie osoby, których identyfikatory
są wymienione w zmiennej środowiskowej `TUIST_OPS_USER_HANDLES`.
<!-- -->
:::
...
- **Pulpit nawigacyjny (`/ops/dashboard`):** Możesz wyświetlić pulpit
  ...
  (np. zużycie pamięci, uruchomione procesy, liczba żądań). Ten pulpit
  nawigacyjny może być bardzo przydatny, aby zrozumieć, czy używany sprzęt jest
  wystarczający do obsługi obciążenia.
