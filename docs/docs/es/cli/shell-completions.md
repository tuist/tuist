...
<!-- -->
Una instalación global es aquella que está disponible en la variable de entorno
$PATH `` de tu shell. Esto significa que puedes ejecutar `tuist` desde cualquier
directorio de tu terminal. Este es el método de instalación predeterminado para
Homebrew.
<!-- -->
:::
...
autocompletado. Primero, añade estas líneas a `~/.zshrc`:
...
autocompletado al nuevo directorio, de nuevo en un archivo llamado `_tuist`.
...
solo tienes que copiar tu nuevo script de autocompletado en el archivo
...
Sin bash-completion, tendrás que cargar el script de autocompletado
directamente. Cópialo en un directorio como `~/.bash_completions/` y, a
continuación, añade la siguiente línea a `~/.bash_profile` o `~/.bashrc`:
...
de autocompletado en `~/.config/fish/completions/tuist.fish`:
