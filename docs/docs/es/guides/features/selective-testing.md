...
A medida que tu proyecto crece, también lo hace el número de pruebas. Durante
mucho tiempo, ejecutar todas las pruebas en cada PR o envío a `main` lleva
decenas de segundos. Pero esta solución no se adapta a los miles de pruebas que
tu equipo podría tener.

En cada ejecución de pruebas en la CI, lo más probable es que vuelvas a ejecutar
todas las pruebas, independientemente de los cambios. Las pruebas selectivas de
Tuist te ayudan a acelerar drásticamente la ejecución de las propias pruebas, ya
que solo se ejecutan aquellas que han cambiado desde la última ejecución
satisfactoria, basándose en nuestro
<LocalizedLink href="/guides/features/projects/hashing">algoritmo de
hash</LocalizedLink>.
...
<!-- -->
Debido a la imposibilidad de detectar las dependencias en el código entre
pruebas y fuentes, la granularidad máxima de las pruebas selectivas se encuentra
en el nivel de destino. Por lo tanto, recomendamos mantener los destinos
pequeños y específicos para maximizar los beneficios de las pruebas selectivas.
<!-- -->
:::
...
se ejecuta de una sola vez, lo que las hace incompatibles con las ejecuciones
selectivas de pruebas; esto significa que los datos de cobertura podrían no
...
significativa en este contexto y, si es así, tengan la seguridad de que ya
