...
# Caché de autoalojamiento {#self-host-cache}

El servicio de caché de Tuist se puede alojar de forma autónoma para
proporcionar una caché binaria privada a tu equipo. Esto resulta especialmente
útil para organizaciones con artefactos de gran tamaño y compilaciones
frecuentes, en las que situar la caché más cerca de tu infraestructura de CI
reduce la latencia y mejora la eficiencia de la caché. Al minimizar la distancia
entre tus agentes de compilación y la caché, te aseguras de que la sobrecarga de
la red no anule las ventajas de velocidad que ofrece el almacenamiento en caché.

::: info
<!-- -->
El autoalojamiento de nodos de caché requiere un plan Enterprise de **** .

Puedes conectar nodos de caché autohospedados al servidor Tuist alojado
(`https://tuist.dev`) o a un servidor Tuist autohospedado. El autohospedaje del
propio servidor Tuist requiere una licencia de servidor independiente. Consulta
la <LocalizedLink href="/guides/server/self-host/install">guía de autohospedaje
del servidor</LocalizedLink>.
<!-- -->
:::
...
- Depósito de almacenamiento compatible con S3
- Una instancia del servidor Tuist en ejecución (alojada o autoalojada)

## Despliegue {#deployment}

El servicio de caché se distribuye como una imagen de Docker en
[ghcr.io/tuist/cache](https://ghcr.io/tuist/cache). Proporcionamos archivos de
configuración de referencia en el [directorio
cache](https://github.com/tuist/tuist/tree/main/cache).

::: consejo
<!-- -->
Proporcionamos una configuración de Docker Compose porque es una base de
referencia práctica para la evaluación y las implementaciones pequeñas. Puedes
utilizarla como referencia y adaptarla a tu modelo de implementación preferido
(Kubernetes, Docker sin más, etc.).
<!-- -->
:::
...
::: consejo
<!-- -->
El servicio está desarrollado con Elixir/Phoenix, por lo que algunas variables
utilizan el prefijo `PHX_`. Puedes tratarlas como una configuración estándar del
servicio.
<!-- -->
:::
...
| `PUBLIC_HOST`                     | Sí        |                           | Nombre de host público o dirección IP de tu servicio de caché. Se utiliza para generar URL absolutas.                              |
| `SERVER_URL`                      | Sí        |                           | URL de tu servidor Tuist para la autenticación. El valor predeterminado es `https://tuist.dev`                                     |
| `DATA_DIR`                        | Sí        |                           | Directorio donde se almacenan los artefactos de CAS en el disco. La configuración de Docker Compose proporcionada utiliza `/data`. |
| `S3_BUCKET`                       | Sí        |                           | Nombre del bucket de S3.                                                                                                           |
| `S3_HOST`                         | Sí        |                           | Nombre de host del punto final de S3.                                                                                              |
| `S3_ACCESS_KEY_ID`                | Sí        |                           | Clave de acceso S3.                                                                                                                |
| `S3_SECRET_ACCESS_KEY`            | Sí        |                           | Clave secreta de S3.                                                                                                               |
...
| `PHX_SOCKET_LINK`                 | No        | `/run/cache/current.sock` | Ruta del enlace simbólico que utiliza Nginx para conectarse al servicio.                                                           |
...
1. Accede a la página de configuración de **de tu organización:**
...
Una vez configurado, la CLI de Tuist utilizará tu caché autohospedada.
...
- `GET /up` — Devuelve 200 cuando todo está bien
...
El servicio ejecuta migraciones de la base de datos automáticamente al
iniciarse.

## Solución de problemas {#troubleshooting}

### No se está utilizando la caché {#troubleshooting-caching}

Si esperas que se utilice el almacenamiento en caché pero observas fallos de
caché constantes (por ejemplo, la CLI sube repetidamente los mismos artefactos o
las descargas nunca se producen), sigue estos pasos:

1. Comprueba que el punto final de caché personalizado esté correctamente
   configurado en los ajustes de tu organización.
...
### Discrepancia en la ruta del socket {#troubleshooting-socket}

Si ves errores de conexión rechazada:

- Asegúrate de que `PHX_SOCKET_LINK` apunte a la ruta del socket configurada en
  nginx.conf (por defecto: `/run/cache/current.sock`)
- Comprueba que `PHX_SOCKET_PATH` y `PHX_SOCKET_LINK` estén configurados
  correctamente en docker-compose.yml
- Comprueba que el volumen `cache_socket` esté montado en ambos contenedores
