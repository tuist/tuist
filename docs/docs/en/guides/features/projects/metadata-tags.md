...
Tuist uses the `tuist:` prefix for system-managed tags. These tags are automatically applied by Tuist and can be used in cache profiles to target specific types of generated content.

### Available system tags

| Tag | Description |
| --- | --- |
| `tuist:synthesized` | Applied to synthesized bundle targets created for resources in targets that don't natively support them (e.g., static libraries) |

### Using system tags with cache profiles

You can use system tags in cache profiles to include or exclude synthesized targets:
