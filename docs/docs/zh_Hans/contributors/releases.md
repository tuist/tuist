...
| `文档`  | 文件更改     | No release     | `文档：更新安装指南`                    |
| `风格`  | 代码样式更改   | No release     | `style：用 swiftformat 格式化代码`    |
| `重构`  | 代码重构     | No release     | `重构（服务器）：简化认证逻辑`               |
| `敷衍`  | 性能改进     | 补丁版本提升         | `perf(cli): 优化依赖关系解析`          |
| `测试`  | 测试增加/更改  | No release     | `测试：为缓存添加单元测试`                 |
| `苦差事` | 维护任务     | No release     | `苦差事：更新依赖关系`                   |
| `ci`  | CI/CD 变化 | No release     | `CI：为发布添加工作流程`                 |
