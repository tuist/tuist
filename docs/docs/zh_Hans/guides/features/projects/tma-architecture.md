...
应用程序，以实现可扩展性、优化构建和测试周期，并确保团队的良好实践。其核心理念是通过构建独立的功能来构建应用程序，这些功能通过简洁明了的应用程序接口相互连接。
...
开发人员应该能够**，独立于主应用程序，快速构建、测试和尝试** 自己的功能，同时确保 Xcode 的 UI 预览、代码自动补全和调试等功能能够可靠地工作。
...
当一个模块依赖于另一个模块时，它会声明对其接口目标的依赖。这样做有两个好处。它可以防止一个模块的实现与另一个模块的实现耦合，还可以加快简洁构建，因为它们只需编译我们功能的实现，以及直接和传递依赖关系的接口。这种方法的灵感来自
