...
<LocalizedLink href="/guides/features/inspect/implicit-dependencies">
命令</LocalizedLink>。您可以使用该命令在 CI 中验证所有依赖关系都是显式的。
<!-- -->
:::
...
Apple
在某一时刻决定，与其将图模型演化成更易于管理的东西，不如在构建时添加一个解决隐式依赖关系的选项，这样会更有意义。这又是一个值得商榷的设计选择，因为最终可能会导致较慢的构建时间或不可预测的构建。例如，在本地编译时，可能会因为派生数据中的某些状态而通过，派生数据就像一个
