...
:::
> 
> 即使在这种情况下，Xcode 中一个未注明的功能也允许您执行它。`` 在相关`project.pbxproj`
...
如果您使用的是带有自定义方案的生成项目，您可以为构建洞察设置后置操作：
...
当`xcodebuild` 在没有`-resultBundlePath` 的情况下构建项目时，不会生成所需的活动日志和结果捆绑文件。`tuist inspect
build` 后操作需要这些文件来分析您的构建。
