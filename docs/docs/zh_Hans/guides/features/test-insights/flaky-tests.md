...
不稳定测试是指使用相同代码多次运行时产生不同结果（通过或失败）的测试。它们会削弱对测试套件的信任，并浪费开发者时间调查虚假失败。Tuist可自动检测不稳定测试，并帮助您长期追踪其状态。
...
例如，若测试首次失败但重试通过，Tuist 将记录为不稳定测试。
...
这对于捕捉不稳定的测试特别有用——这类测试失败的频率不足以被重试机制捕获，却仍会导致持续集成环境间歇性失败。

## 管理不稳定的测试{#managing-flaky-tests}

### 自动清除

Tuist会自动清除连续14天未出现异常的测试项的"不稳定"标记。此机制确保已修复的测试不会永久保留为不稳定状态。
...
通过在 Slack 集成中设置
<LocalizedLink href="/guides/integrations/slack#flaky-test-alerts">不稳定测试警报</LocalizedLink>，当测试出现不稳定时立即收到通知。
