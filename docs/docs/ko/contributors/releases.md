...
| `문서`    | 문서 변경 사항     | No release       | `문서: 업데이트 설치 가이드`             |
| `스타일`   | 코드 스타일 변경    | No release       | `스타일: 스위프트포맷을 사용한 포맷 코드`      |
| `리팩터링`  | 코드 리팩토링      | No release       | `refactor(server): 인증 로직 간소화` |
| `perf`  | 성능 개선        | 패치 버전 범프         | `perf(cli): 종속성 해결 최적화`       |
| `테스트`   | 테스트 추가/변경    | No release       | `테스트: 캐시에 대한 단위 테스트 추가`       |
| `chore` | 유지 관리 작업     | No release       | `잡일: 종속성 업데이트`                |
| `ci`    | CI/CD 변경 사항  | No release       | `CI: 릴리스용 워크플로 추가`            |
