...
To run tests selectively, use the `tuist test` command. The command hashes your project the same way it does for [warming the cache](/ko/guides/develop/build/cache#cache-warming), and on success, it persists the hashes on to determine what has changed in future runs.
