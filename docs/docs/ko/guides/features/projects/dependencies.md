...
Target은 동일하거나 다른 프로젝트의 다른 Target 및 바이너리에 종속될 수 있습니다. `Target` 을 인스턴스화할 때 다음 옵션 중
하나를 사용하여 `dependencies` 인수를 전달할 수 있습니다:

- `Target`: 동일한 프로젝트 내에서 Target과의 종속성을 선언합니다.
- `Project`: 다른 프로젝트의 Target에 대한 종속성을 선언합니다.
- `Framework`: 바이너리 프레임워크에 대한 종속성을 선언합니다.
- `Library`: 바이너리 라이브러리에 대한 종속성을 선언합니다.
- `XCFramework`: 바이너리 XCFramework에 대한 종속성을 선언합니다.
- `SDK`: 시스템 SDK에 대한 종속성을 선언합니다.
- `XCTest`: XCTest에 대한 종속성을 선언합니다.

::: info 의존성 조건
<!-- -->
모든 의존성 유형은 플랫폼에 따라 종속성을 조건부로 연결하기 위해 `condition` 옵션을 허용합니다. 기본적으로 Target에서 지원하는
모든 플랫폼에 대한 의존성을 연결합니다.
<!-- -->
:::

## 외부 의존성 {#external-dependencies}

Tuist는 또한 프로젝트에서 외부 종속성을 선언할 수 있게 합니다.
...
XcodeProj의 Target을 사용하여 프로젝트에 Swift 패키지를 통합할 수 있는 XcodeProj 기반 통합을 제공합니다. 덕분에
통합을 더 잘 제어할 수 있을 뿐만 아니라
...
실행</LocalizedLink>과 같은 작업과 호환되도록 만들 수 있습니다.

XcodeProj의 통합은 새로운 Swift 패키지 기능을 지원하거나 더 많은 패키지 설정을 처리하는 데 더 많은 시간이 걸릴 가능성이
높습니다. 그러나 Swift 패키지와 XcodeProj 대상 간의 연결 로직은 오픈 소스이며 커뮤니티에서 기여할 수 있습니다. 이는 비공개
소스로 Apple에서 유지 관리하는 Xcode의 기본 통합과는 상반됩니다.

외부 종속성을 추가하려면 `Tuist/` 또는 프로젝트의 최상위에서 `Package.swift`를 만들어야 합니다.
...
<!-- -->
컴파일러 지시어로 감싸진 `PackageSettings` 인스턴스를 사용하면 패키지가 통합되는 방식을 설정할 수 있습니다. 예를 들어, 위의
예에서는 패키지에 사용되는 기본 제품 유형을 재정의하는 데 사용됩니다. 기본적으로 이 옵션은 필요하지 않습니다.
<!-- -->
:::

> [!IMPORTANT] 사용자 정의 빌드 구성 프로젝트에서 사용자 정의 환경설정(표준 `Debug` 및 `Release` 이외)을 사용하는
> 경우 `baseSettings` 를 사용하여 `PackageSettings`에서 지정해야 합니다. 외부 종속성은 프로젝트의 환경설정을 알아야
> 올바르게 빌드할 수 있습니다. 예제:
> 
> ```swift
> #if TUIST
...
`Package.swift` 파일은 외부 의존성을 선언하는 인터페이스일 뿐 다른 것은 없습니다. 패키지에서 Target이나 Product를
정의하지 않는 이유 입니다. 의존성을 정의한 후에는 다음 명령을 실행하여 의존성을 확인하고 `Tuist/Dependencies` 디렉터리로
가져올 수 있습니다:
...
이미 눈치채셨겠지만, 저희는 의존성 확인이 그 자체 명령어인 [CocoaPods](https://cocoapods.org)'와 유사한 접근
방식을 취하고 있습니다. 이를 통해 사용자는 의존성 확인 및 업데이트 시기를 제어할 수 있으며, 프로젝트에서 Xcode를 열고 컴파일할 준비를
할 수 있습니다. 이 부분은 프로젝트가 성장함에 따라 Apple과 Swift 패키지 관리자의 통합이 제공하는 개발자 경험이 시간이 지남에 따라
저하되는 부분이라고 생각합니다.

그런 다음 프로젝트 대상에서 `TargetDependency.external` 의존성 유형을 사용하여 해당 의존성을 참조할 수 있습니다:
...
::: info 외부 패키지를 위해 자동 생성된 Scheme가 없는 경우
<!-- -->
**스키마** 목록을 정돈된 상태로 유지하기 위해 스키마를 Swift 패키지 프로젝트에 대해 자동으로 만들지 않습니다. Xcode UI에서
만드세요.
<!-- -->
:::
...
Xcode의 기본 통합 메커니즘을 사용하려면 프로젝트를 인스턴스화할 때 `packages` 목록을 전달하면 됩니다:
...
<!-- -->
프로젝트 의존성에 Tuist의 [XcodeProj 기반 통합](#tuist-s-xcodeproj-based-integration)을 사용하는
경우에도 SPM 빌드 도구 플러그인은 [Xcode의 기본 통합](#xcode-s-default-integration) 메커니즘을 사용하여
선언해야 합니다.
<!-- -->
:::

SPM 빌드 도구 플러그인의 실제 적용 사례는 Xcode의 "빌드 도구 플러그인 실행" 빌드 단계에서 코드 Lint를 수행하는 것입니다. 패키지
...
빌드 툴 플러그인을 그대로 유지한 채 Xcode 프로젝트를 생성하려면 프로젝트 매니페스트의 `packages` 배열에 패키지를 선언한 다음,
Target의 의존에 `.plugin` 형식의 패키지를 포함시켜야 합니다.
...
를 출력하므로 `carthage update` 를 실행하여 `Carthage/Build` 디렉터리에 의존성을 출력한 다음 `.framework`
또는 `.xcframework` Target 종속성 유형을 사용하여 Target에 종속성을 선언할 수 있습니다. 이를 프로젝트를 생성하기 전에
실행할 수 있는 스크립트로 래핑할 수 있습니다.
...
::: warning 빌드와 테스트
<!-- -->
`xcodebuild build` 및 `tuist test`로 통해 프로젝트를 빌드하고 테스트하는 경우, 마찬가지로 빌드 또는 테스트 전에
`carthage update` 명령을 실행하여 Carthage에서 가져온 의존성이 나타나는지 확인해야 합니다.
<!-- -->
:::
...
프로젝트를 생성한 다음 `pod install` 을 실행하여 프로젝트와 Pods 의존성이 포함된 Workspace를 생성하여 종속성을 통합할 수
...
::: warning
<!-- -->
CocoaPods 의존성은 프로젝트 생성 직후에 `xcodebuild`를 실행하는 `build` 또는 `test` 같은 명령과 호환되지
않습니다. 또한 식별 로직이 Pods 의존성을 고려하지 않기 때문에 바이너리 캐싱 및 선택적 테스트와도 호환되지 않습니다.
<!-- -->
:::

## 정적 또는 동적 {#static-or-dynamic}

프레임워크와 라이브러리는 정적으로 또는 동적으로 연결할 수 있으며, **앱 크기 및 부팅 시간과 같은 측면에 중대한 영향을 미치는
선택**입니다. 그 중요성에도 불구하고 많은 고려 없이 결정하는 경우가 종종 발생합니다.

**일반적인 경험에 의하면** 여러분은 빠른 부팅 시간을 달성하기 위해 릴리스 빌드에서 가능한 한 많은 것을 정적으로 링크하고, 빠른 반복
시간을 달성하기 위해 디버그 빌드에서 가능한 한 많은 것을 동적으로 링크하는 것을 원한다는 것입니다.

프로젝트 그래프에서 정적 링크와 동적 링크 사이를 변경할 때의 문제는 변경 사항이 전체 그래프에 연쇄적으로 영향을 미치기 때문에(예:
라이브러리에 리소스를 포함할 수 없고, 정적 프레임워크는 내장될 필요가 없는 경우) Xcode에서 간단하지 않다는 것입니다. Apple은 정적
...
같은 컴파일 시점의 솔루션으로 이 문제를 해결하려고 시도했습니다. 그러나 이는 컴파일 그래프에 새로운 동적 변수를 추가하여 비일관적인 새로운
...
결합하면 호출 시 연결 유형을 전달하고 설정 파일의 값을 사용하여 Target의 Product Type을 설정할 수 있습니다.
...
Tuist <LocalizedLink href="/guides/features/projects/cost-of-convenience">는
비용</LocalizedLink> 때문에 암시적 설정을 통한 편의성을 기본 값으로 제공하지 않는다는 점에 유의하세요. 즉, 최종 바이너리가
...
::: tip 예제: THE COMPOSABLE ARCHITECTURE
<!-- -->
많은 프로젝트에서 통합하는 Swift 패키지는 [The Composable
Architecture](https://github.com/pointfreeco/swift-composable-architecture)입니다.
자세한 내용은 [이 섹션](#the-composable-architecture)에서 확인하세요.
<!-- -->
:::
...
혼합해야 할 수 있는 모든 경우를 다룬 시나리오가 아닙니다:

- **확장 프로그램을 가진 앱:** 앱과 확장 프로그램은 코드를 공유해야 하므로 이러한 대상을 동적으로 만들어야 할 수 있습니다. 그렇지
  않으면 앱과 확장 프로그램 모두에 동일한 코드가 중복되어 바이너리 크기가 증가하게 됩니다.
- **미리 컴파일된 외부 종속성:** 정적이거나 동적인 사전 컴파일된 바이너리가 제공되는 경우가 있습니다. 정적 바이너리는 동적 프레임워크나
  라이브러리로 묶어서 동적으로 연결할 수 있습니다.

그래프를 변경할 때 Tuist는 그래프를 분석하여 '정적 사이드 이펙트'을 감지하면 경고를 표시합니다. 이 경고는 동적 타깃을 통해 정적 타깃에
일시적으로 의존하는 타깃을 정적으로 연결할 때 발생할 수 있는 문제를 식별하는 데 도움을 주기 위한 것입니다. 이러한 사이드 이펙트는 종종
바이너리 크기 증가 또는 최악의 경우 실행 중 비정상 종료로 나타납니다.

## 문제 해결 {#troubleshooting}

### Objective-C 의존성 {#objectivec-dependencies}

Objective-C 의존성을 통합할 때 [Apple 기술 Q&A
QA1490](https://developer.apple.com/library/archive/qa/qa1490/_index.html)에 설명된
대로 런타임 충돌을 방지하기 위해 소비 Target에 특정 플래그를 포함해야 할 수 있습니다.

빌드 시스템과 Tuist는 플래그가 필요한지 여부를 유추할 방법이 없고, 플래그에는 잠재적으로 바람직하지 않은 부작용이 있을 수 있기 때문에
Tuist는 이러한 플래그를 자동으로 적용하지 않으며, Swift 패키지 관리자는 `-ObjC`가 대부분의 패키지는 필요한 경우에 기본 링크
설정의 일부로 포함할 수 없는 ` .unsafeFlag`를 통해 포함될 것이라고 생각하기 때문입니다.

Objective-C 의존성(또는 내부 Objective-C Target)의 사용자는 필요한 경우 `-ObjC` 또는 `-force_load`
플래그를 적용하고, 대상 Target에 `OTHER_LDFLAGS` 를 설정해야 합니다.

### Firebase 및 기타 Google 라이브러리 {#firebase-other-google-libraries}

Google의 오픈 소스 라이브러리는 강력하지만 빌드 방식에서 비표준 아키텍처와 기술을 사용하는 경우가 많기 때문에 Tuist 내에서 통합하기
어려울 수 있습니다.

다음은 Firebase와 Google의 다른 Apple 플랫폼 라이브러리들을 통합하는데 필요한 몇 가지 팁입니다:

#### `OTHER_LDFLAGS에 `-ObjC` 이 추가되었는지 확인하세요 {#ensure-objc-is-added-to-other_ldflags}

Google의 많은 라이브러리는 Objective-C로 작성되었습니다. 따라서 이를 사용하는 모든 Target은 `OTHER_LDFLAGS`
빌드 설정에 `-ObjC` 태그를 포함해야 합니다. 이는 `.xcconfig` 파일에서 설정하거나 Tuist 매니페스트 내의 Target
설정에서 수동으로 지정할 수 있습니다. 예시:
...
자세한 내용은 위의 [Objective-C 의존성](#objective-c-dependencies) 섹션을 참조하세요.

#### `FBLPromises` 의 Product Type을 동적 프레임워크로 설정하세요. {#set-the-product-type-for-fblpromises-to-dynamic-framework}

특정 Google 라이브러리는 `FBLPromises`와 또 다른 Google 라이브러리에 의존합니다. 다음과 같이 `FBLPromises` 를
언급하는 크래시가 발생할 수 있습니다:

```
NSInvalidArgumentException. Reason: -[FBLPromise HTTPBody]: unrecognized selector sent to instance 0x600000cb2640.
```

`Package.swift` 파일에서 `FBLPromises` 의 Product Type을 `.framework`로 명시적으로 설정하면 문제가
해결됩니다:
...
[여기](https://github.com/pointfreeco/swift-composable-architecture/discussions/1657#discussioncomment-4119184)와
[문제 해결 섹션](#troubleshooting)에 설명된 대로 패키지를 정적으로 링크할 때 Tuist의 기본 링크 유형인
`OTHER_LDFLAGS` 빌드 설정을 `$(상속된) -ObjC` 로 설정해야 합니다. 또는 패키지의 제품 유형을 동적으로 재정의할 수
있습니다. 정적으로 연결할 때 테스트 및 앱 Target은 일반적으로 문제 없이 작동하지만 SwiftUI 미리보기가 손상됩니다. 이 문제는 모든
것을 동적으로 연결하여 해결할 수 있습니다. 아래 예시에서
[공유하기](https://github.com/pointfreeco/swift-sharing)도 의존성으로 추가되었는데, 이는 TCA와 함께
자주 사용되며 자체 [Configuration
함정](https://github.com/pointfreeco/swift-sharing/issues/150#issuecomment-2797107032)이
있기 때문입니다.

다음 설정은 모든 것을 동적으로 연결하므로 앱 + 테스트 대상 및 SwiftUI 미리보기가 작동합니다.

::: tip 정적 또는 동적
<!-- -->
동적 링크가 항상 권장되는 것은 아닙니다. 자세한 내용은 [정적 또는 동적](#static-or-dynamic) 섹션을 참조하세요. 이
예제에서는 단순화를 위해 모든 종속성을 조건 없이 동적으로 연결했습니다.
<!-- -->
:::
...
::: warning
<!-- -->
`import Sharing` 대신 `import SwiftSharing` 을 입력해야 합니다.
<!-- -->
:::

### 전파될 수 있는 정적 의존성이 `.swiftmodule을 통해 누출되는 경우` {#transitive-static-dependencies-leaking-through-swiftmodule}
...
import`</LocalizedLink>를 사용하여 정적 의존성을 가져와야 합니다:
