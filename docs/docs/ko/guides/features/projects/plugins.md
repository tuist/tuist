...
- 플러그인은 타사 Swift 패키지에 대한 의존성을 가질 수 없습니다
...
- 모든 Tuist 릴리스와 함께 배포되는 `ProjectAutomation.xcframework` 를 계속 사용하면 로직에서 `let
  graph = try Tuist.graph()` 를 통해 프로젝트 그래프에 접근할 수 있습니다. 이 명령어는 시스템 프로세스를 사용하여
...
- ` 작업 분배를 위해 GitHub 릴리즈에 다음을 포함하는 것을 권장합니다: - `(arm64 지원) -` (x86_64 지원)
  ...
  [Tuist's](https://github.com/asdf-community/asdf-tuist)를 참고하세요.
...
있게 할 예정입니다. 이를 여러분만의 Tuist를 구축하는 것으로 생각하십시오.
<!-- -->
:::
...
<!-- -->
아시다시피, 플러그인에 대한 버전 해결 기능을 제공하지 않습니다. 재현성을 보장하기 위해 Git 태그나 SHA를 사용하는 것을 권장합니다.
<!-- -->
:::
