...
프로젝트의 규모와 복잡성이 커짐에 따라 전체 코드베이스를 한 번에 다루는 것은 비효율적일 수 있습니다. Tuist는 타깃을 논리적인 그룹으로
구성하고 개발 과정에서 프로젝트의 특정 부분에 집중할 수 있는 방법으로 타깃 관리( **) 메타데이터 태그** 를 제공합니다.

## 메타데이터 태그란 무엇인가요? {#what-are-metadata-tags}

메타데이터 태그는 프로젝트의 대상에 부착할 수 있는 문자열 레이블입니다. 이 태그는 다음과 같은 기능을 수행하는 마커 역할을 합니다:

- **관련 대상 그룹화** - 동일한 기능, 팀 또는 아키텍처 계층에 속하는 대상에 태그 지정
- **작업 공간에 집중하기** - 특정 태그가 지정된 대상만 포함하는 프로젝트 생성하기
- **워크플로우 최적화** - 코드베이스의 관련 없는 부분을 로드하지 않고 특정 기능 작업 수행
- **** - 캐싱 시 소스로 유지할 대상 그룹 선택
...
타깃에 태그를 지정하면 ` `` 명령어를 사용하여 특정 타깃만 포함된 집중 프로젝트를 생성할 수 있습니다: `` `

### 태그별 집중

`태그 사용:` 접두사를 사용하여 특정 태그와 일치하는 모든 타깃을 가진 프로젝트를 생성합니다:
...
1. **포함된 대상** - 검색어와 일치하는 대상이 생성된 프로젝트에 포함됩니다
2. **의존성** - 집중 대상의 모든 의존성이 자동으로 포함됩니다
3. **테스트 대상** - 집중 대상에 대한 테스트 대상이 포함됨
4. **** 제외 - 작업 공간에서 다른 모든 대상은 제외됩니다
...
- **** 의 집중도 향상 - 관련 없는 코드로 인한 주의 분산 방지
