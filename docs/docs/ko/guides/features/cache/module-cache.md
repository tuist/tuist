...
소스를 사용할 때, Xcode의 빌드 시스템은 Derived Data를 통해 명시적으로 선언되지 않은 종속성을 해결할 수 있습니다. 그러나
바이너리 캐시에 의존하는 경우 종속성을 명시적으로 선언해야 하며, 그렇지 않으면 Symbol을 찾을 수 없을 때 컴파일 오류가 발생할 수
있습니다. 이를 디버깅하기 위해
