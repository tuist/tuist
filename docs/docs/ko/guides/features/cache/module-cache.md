...
Tuist의 모듈 캐시는 모듈을 바이너리(`.xcframework`)로 캐시하고 여러 환경에서 공유하여 빌드 시간을 최적화할 수 있는 강력한
방법을 제공합니다. 이 기능은 반복 컴파일의 필요성을 줄이고 개발 프로세스의 속도를 높이게 위해 이전에 생성된 바이너리를 활용할 수 있게
해줍니다.
...
*"warming"*으로 알려진 이 작업은 로컬에서 사용하거나 Tuist를 통해 팀원 및 CI 환경과 공유할 수 있는 바이너리를 생성합니다.
...
기본적으로 Tuist 명령은 프로젝트 생성이 필요한 경우, 의존성을 캐시에서 사용할 수 있는 경우 해당 바이너리로 자동 대체합니다. 또한 집중할
Target 목록을 지정하는 경우, 의존하는 Target도 사용 가능한 캐시된 바이너리로 대체합니다. 다른 접근 방식을 선호하는 분들을 위해,
특정 플래그를 사용하여 이 동작을 완전히 해제 할 수 있는 옵션이 있습니다:
...
::: warning
<!-- -->
바이너리 캐싱은 시뮬레이터나 기기에서 앱을 실행하거나 테스트를 실행하는 등의 개발 업무를 위해 설계된 기능입니다. Release 빌드용이
...
캐시를 워밍업하기 위해 **메인 브랜치의 모든 커밋에서 실행**하는 CI 작업을 수행하는 것이 좋습니다. 이렇게 하면 캐시에 항상 `메인` 의
변경 사항에 대한 바이너리가 포함되므로, 로컬 및 CI 브랜치가 이를 기반으로 점진적으로 빌드할 수 있습니다.

::: tip CACHE 워밍업의 바이너리 사용
<!-- -->
`tuist cache` 명령은 바이너리 캐시를 사용하여 워밍업 속도를 높입니다.
<!-- -->
:::
...
2. `tuist generate`를 실행합니다.
3. Tuist는 `main` 에서 최신 바이너리를 가져와서 프로젝트를 생성합니다.

### 개발자가 변경 사항을 푸시합니다 {#a-developer-pushes-changes-upstream}

1. CI 파이프라인은 `xcodebuild build` 또는 `tuist test` 를 실행하여 프로젝트를 빌드하거나 테스트합니다.
2. 워크플로는 `main` 에서 최신 바이너리를 가져와서 프로젝트를 생성합니다.
...
기본적으로 Tuist는 동시성 제한 없이 캐시를 다운로드 및 업로드하여 처리량을 최대화합니다. `
...
### 내 Target에 대한 바이너리를 사용하지 않습니다. {#it-doesnt-use-binaries-for-my-targets}

환경과 실행 모두에서
<LocalizedLink href="/guides/features/projects/hashing#debugging">해시가 결정된
상태</LocalizedLink>인지 확인하세요. 프로젝트에 절대 경로 같은, Environment에 대한 참조가 있는 경우 이런 문제가 발생할
수 있습니다. `diff` 명령을 사용하여 연속 두번 `tuist generate` 실행 또는 여러 Environment, 실행 사이에 생성된
프로젝트를 비교할 수 있습니다.

또한 Target이
<LocalizedLink href="/guides/features/cache/generated-project#supported-products">캐시할
수 없는 Target</LocalizedLink>에 직접 또는 간접적으로 의존하지 않는지 확인하세요.

### 누락된 Symbol {#missing-symbols}

소스를 사용할 때, Xcode의 빌드 시스템은 Derived Data를 통해 명시적으로 선언되지 않은 의존성을 가져올 수 있습니다. 그러나
바이너리 캐시에 의존하는 경우 의존성을 명시적으로 선언해야 하며, 그렇지 않으면 Symbol을 찾을 수 없을 때 컴파일 오류가 발생할 수
있습니다. 이를 디버깅하기 위해
<LocalizedLink href="/guides/features/projects/inspect/implicit-dependencies">`tuist
inspect implicit-imports`</LocalizedLink> 명령을 사용하고 CI에서 설정하여 암시적 연결의 회귀를 방지하는 것을
권장합니다.

### 과거 모듈 캐시 {#legacy-module-cache}

Tuist`4.128.0` 에서는 모듈 캐시를 위한 새로운 인프라를 기본 값으로 설정했습니다. 이 새 버전에서 문제가 발생하는 경우
`TUIST_LEGACY_MODULE_CACHE` 환경 변수를 설정하여 이전 캐시 방식으로 되돌릴 수 있습니다.

이전 모듈 캐시는 일시적인 대체 수단이며 향후 업데이트에서 서버 측에서 제거될 예정입니다. 마이그레이션을 준비하세요.
