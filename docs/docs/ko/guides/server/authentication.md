...
사용하여 요청을 인증해야 합니다. CLI는 사용자, 계정 또는 OIDC 토큰을 사용한 인증을 지원합니다.
...
유효성 리프레시 토큰과 단기 유효성 액세스 토큰을 저장합니다. 해당 디렉토리 내 각 파일은 인증 대상 도메인을 나타내며, 기본값은
...
범위가 포함하는 항목에 대한 자세한 내용은 [범위 그룹](#scope-groups)을 참조하세요.
...
| `--projects` | 토큰을 특정 프로젝트 핸들만 사용하도록 제한하십시오. 지정하지 않으면 토큰은 모든 프로젝트에 접근할 수 있습니다.                                          |
...
비밀로 저장하고, 환경 변수 ` `` 또는 `TUIST_TOKEN`(` )으로 설정하세요.
