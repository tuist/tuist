...
**TimescaleDB는 더 이상 권장되지 않으며** 모든 시계열 기능을 ClickHouse로 이전함에 따라 가까운 시일 내에 필수 의존성에서
...
<!-- -->
라이선스에는 만료일이 있습니다. 라이선스 만료일까지 30일 미만인 경우, 서버와 상호작용하는 Tuist 명령어를 사용할 때 사용자에게 경고가
