...
ClickHouse는 인사이트 구축과 같은 기능에 **** 필요하며, 향후 타임스케일DB를 단계적으로 폐지할 때 기본 시계열 데이터베이스가 될
...
| `tuist_s3_connect_timeout`                              | 스토리지 공급자에 대한 연결을 설정하는 데 걸리는 시간 제한(밀리초)                                          | 아니요 | `3000`     | `3000`                                                        |
