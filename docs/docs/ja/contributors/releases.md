...
| `諸注意`   | ドキュメントの変更    | No release           | `ドキュメント：アップデート・インストール・ガイド`      |
| `スタイル`  | コードスタイルの変更   | No release           | `スタイル：swiftformatでコードをフォーマットする` |
| `手戻り`   | コードのリファクタリング | No release           | `refactor(server)：認証ロジックの簡素化`   |
| `完璧`    | パフォーマンス向上    | パッチのバージョンアップ         | `perf(cli): 依存関係の解決を最適化する`      |
| `テスト`   | テストの追加／変更    | No release           | `test: キャッシュのユニットテストを追加`        |
| `雑用`    | メンテナンス・タスク   | No release           | `chore: 依存関係の更新`                |
| `ci`    | CI/CDの変更     | No release           | `ci: リリースのワークフローを追加`            |
