...
OpenID
Connect（OIDC）をサポートするCI環境では、Tuistは長期有効なシークレットを管理する必要なく自動認証が可能です。サポート対象のCI環境で実行時、CLIはOIDCトークンプロバイダーを自動検知し、CI提供トークンをTuistアクセストークンと交換します。
...
OIDCトークンには`ci` のスコープグループが付与され、リポジトリに接続された全プロジェクトへのアクセス権を提供します。`ci`
のスコープに含まれる内容の詳細については[Scope groups](#scope-groups)を参照してください。
