...
::: info
<!-- -->
このページでは、Tuistキャッシュサービスのアーキテクチャに関する技術的な概要を説明します。主に、**のセルフホスティングユーザー**
および**のコントリビューター**
で、サービスの内部動作を理解する必要がある方を対象としています。キャッシュを利用するだけの一般ユーザーは、このページを読む必要はありません。
<!-- -->
:::

Tuistキャッシュサービスは、ビルド成果物向けのコンテンツアドレス指定型ストレージ（CAS）と、キャッシュメタデータ向けのキーバリューストアを提供するスタンドアロンサービスです。

## 概要{#overview}

このサービスは、2層のストレージアーキテクチャを採用しています：

- **ローカルディスク**: 低レイテンシのキャッシュヒットのためのプライマリストレージ
...
- **ダウンロード**: このキャッシュサービスは認証を検証した後、`X-Accel-Redirect`
  ヘッダーを返します。Nginxはファイルをディスクから直接配信するか、S3からプロキシ配信します。
- **アップロード**: Nginxはリクエストをキャッシュサービスにプロキシし、キャッシュサービスはデータをディスクにストリーミングします。

### コンテンツアドレス指定型ストレージ{#cas}

アーティファクトは、シャード化されたディレクトリ構造でローカルディスクに保存されます：

- **パス**:`{account}/{project}/cas/{shard1}/{shard2}/{artifact_id}`
- **** のシャーディング：アーティファクトIDの最初の4文字を使用して2階層のシャーディングを作成します（例： ABCD1234 →
  AB/CD/ABCD1234 ）`` ``
...
| `/api/cache/module/:id`       | HEAD | モジュールアーティファクトが存在するか確認する |
| `/api/cache/module/:id`       | GET  | モジュール成果物をダウンロード         |
| `/api/cache/module/start`     | POST | マルチパートアップロードを開始         |
| `/api/cache/module/part`      | POST | アップロード部分                |
| `/api/cache/module/complete`  | POST | 複数ファイルのアップロードを完了する      |
