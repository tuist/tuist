...
- **のメンテナンスを容易にします。** 変更は一箇所で適用でき、すべてのプロジェクトが自動的に変更を受け取ります。
...
Tuistでは、**プロジェクト記述ヘルパー** の概念により、マニフェストファイル間でコードを再利用できます。
...
プロジェクト記述ヘルパーはSwiftファイルであり、コンパイルされてモジュール（`ProjectDescriptionHelpers`
）となり、マニフェストファイルがインポートできます。このモジュールは、`Tuist/ProjectDescriptionHelpers`
