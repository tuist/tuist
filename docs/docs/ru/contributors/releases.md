...
| `docs`      | Изменения в документации          | Не выпускать                       | `Документация: руководство по установке обновлений` |
| `стиль`     | Изменения в стиле кода            | Не выпускать                       | `стиль: форматирование кода с помощью swiftformat`  |
| `рефактор`  | Рефакторинг кода                  | Не выпускать                       | `refactor(server): упростить логику авторизации`    |
| `perf`      | Улучшение производительности      | Увеличение версии патча            | `perf(cli): оптимизация разрешения зависимостей`    |
| `тест`      | Дополнения/изменения в испытаниях | Не выпускать                       | `test: добавить модульные тесты для кэша`           |
| `работа`    | Задачи технического обслуживания  | Не выпускать                       | `работа: обновление зависимостей`                   |
| `ci`        | Изменения CI/CD                   | Не выпускать                       | `ci: добавить рабочий процесс для релизов`          |
...
### Делайте:
- Используйте настоящее время: "добавить функцию", а не "добавить функцию".
...
### Не надо:
- Используйте расплывчатые сообщения вроде "исправить ошибку" или "обновить
  код".
...
- **Ручные релизы**: релизы приложений для iOS требуют ручной отправки в App
  Store.
- **Задержки с рецензированием**: Каждый релиз должен пройти через процесс
  рассмотрения Apple, который может занять 1-7 дней
- **Пакетные изменения**: В каждом выпуске iOS обычно объединяются несколько
  изменений.
