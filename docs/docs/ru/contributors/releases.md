...
| `fix`      | Исправление ошибок                | Обновление версии патча (x.y.Z)    | `fix(app): resolve crash when opening projects` |
| `docs`     | Изменения в документации          | Нет релиза                         | `docs: update installation guide`               |
| `style`    | Изменения в стиле кода            | Нет релиза                         | `style: format code with swiftformat`           |
| `refactor` | Рефакторинг кода                  | Нет релиза                         | `refactor(server): simplify auth logic`         |
| `perf`     | Улучшение производительности      | Увеличение версии патча            | `perf(cli): optimize dependency resolution`     |
| `test`     | Дополнения/изменения в испытаниях | Нет релиза                         | `test: add unit tests for cache`                |
| `chore`    | Задачи технического обслуживания  | Нет релиза                         | `chore: update dependencies`                    |
| `ci`       | Изменения CI/CD                   | Нет релиза                         | `ci: add workflow for releases`                 |
