...
манифеста `Plugin.swift`, в котором объявляется имя плагина, и каталогом
`ProjectDescriptionHelpers`, содержащим файлы Swift-помощников.
...
Задачи — это `$PATH` — открытые исполняемые файлы, которые можно вызвать с
помощью команды `tuist`, если они соответствуют соглашению об именовании
`tuist-<task-name>`. В более ранних версиях Tuist предоставлял некоторые слабые
соглашения и инструменты в рамках `tuist plugin` для `build`, `run`, `test` и
...
<!-- -->
При использовании плагина помощников описания проекта, имя модуля, содержащего
помощников, является именем плагина.
```swift
import ProjectDescription
