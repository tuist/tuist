...
Приемочное тестирование и ручной контроль качества могут восполнить эти пробелы,
но они требуют больших ресурсов и плохо масштабируются.

Агент QA от Tuist решает эту задачу, имитируя поведение реального пользователя.
...
этапах разработки, позволяя избежать излишних затрат и бремени обслуживания,
связанных с традиционными приемочными и QA-тестами.

## Предпосылки {#prerequisites}

Чтобы начать использовать Tuist QA, вам необходимо:
- Настройте загрузку
  <LocalizedLink href="/guides/features/previews">превью</LocalizedLink> из
  вашего рабочего процесса PR CI, которые агент затем сможет использовать для
  тестирования
- <LocalizedLink href="/guides/integrations/gitforge/github">Интегрируйте</LocalizedLink>
  с GitHub, чтобы вы могли запускать агент прямо из вашего PR

## Использование {#usage}

В настоящее время Tuist QA запускается непосредственно из PR. Как только у вас
появится превью, связанное с вашим PR, вы можете запустить агент QA, оставив
комментарий `/qa test I want to test feature A` в PR:
...
наблюдать за ходом работы агента QA и любыми обнаруженными им проблемами. Как
только агент завершит работу, он опубликует сводку результатов обратно в PR:

![Сводка теста QA](/images/guides/features/qa/qa-test-summary.png)

В рамках отчета на панели инструментов, на который ведет ссылка в комментарии
PR, вы получите список проблем и временную шкалу, чтобы вы могли проверить, как
именно возникла проблема:

![Хронология QA](/images/guides/features/qa/qa-timeline.png)

Вы можете увидеть все циклы QA, которые мы проводим для нашего
<LocalizedLink href="/guides/features/previews#tuist-ios-app">приложения для
iOS</LocalizedLink>, на нашей общедоступной панели инструментов:
https://tuist.dev/tuist/tuist/qa

::: info
<!-- -->
Агент QA работает автономно и после запуска его нельзя прервать с помощью
дополнительных подсказок. Мы предоставляем подробные журналы на протяжении всего
выполнения, чтобы помочь вам понять, как агент взаимодействовал с вашим
приложением. Эти журналы полезны для итерации контекста вашего приложения и
тестирования запросов, чтобы лучше направлять поведение агента. Если у вас есть
...
мог правильно в нем ориентироваться. У нас есть три типа контекста приложения:
- Описание приложения
- Учетные данные
- Группы аргументов запуска

Все эти настройки можно изменить в панели управления вашего проекта (`Settings`
> `QA`).

#### Описание приложения {#app-description}

Описание приложения служит для предоставления дополнительного контекста о том,
что делает ваше приложение и как оно работает. Это поле для длинного текста,
которое передается как часть запроса при запуске агента. Пример:
...
можете предоставить ему учетные данные. Агент введет эти учетные данные, если
определит, что ему необходимо войти в систему.
...
систему, тратя ваши токены и минуты работы, вы можете вместо этого указать здесь
свои учетные данные. Если агент распознает, что ему следует запустить сессию с
авторизацией, он будет использовать группу аргументов запуска с учетными данными
