...
<!-- -->
Флаг `--no-binary-cache` устарел. Вместо него используйте `--cache-profile
none`. Устаревший флаг все еще работает для обратной совместимости.
<!-- -->
:::
...
### Кэш-память устаревших модулей {#legacy-module-cache}

В Tuist `4.128.0` мы сделали нашу новую инфраструктуру для кэша модулей по
умолчанию. Если у вас возникнут проблемы с этой новой версией, вы можете
вернуться к устаревшему поведению кэша, установив переменную окружения
`TUIST_LEGACY_MODULE_CACHE`.

Этот кэш устаревших модулей является временным запасным вариантом и будет удален
на стороне сервера в одном из будущих обновлений. Планируйте отказаться от него.
