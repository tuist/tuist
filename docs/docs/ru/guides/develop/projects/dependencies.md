...
Thanks to that, we can not only give you more control over the integration but also make it compatible with workflows like [caching](/ru/guides/develop/build/cache) and [smart test runs](/ru/guides/develop/test/smart-runner).
...
Luckily, Tuist conceptually compresses the complexity associated with changing between static and dynamic and synthesizes [bundle accessors](/ru/guides/develop/projects/synthesized-files#bundle-accessors) that are standard across linking types. In combination with [dynamic configurations via environment variables](/ru/guides/develop/projects/dynamic-configuration), you can pass the linking type at invocation time, and use the value in your manifests to set the product type of your targets.
