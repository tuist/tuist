...
Мы предлагаем версию сервера Tuist для самостоятельного хостинга организациям,
которым требуется больший контроль над своей инфраструктурой. Эта версия
позволяет разместить Tuist на вашей собственной инфраструктуре, гарантируя
...
## Частота выпусков {#release-cadence}

Мы постоянно выпускаем новые версии Tuist по мере появления новых изменений,
готовых к выпуску, в основном репозитории. Мы следуем [семантическому
...
ожидать, что мы будем его использовать, но в случае необходимости будьте
уверены, что мы будем работать с вами, чтобы обеспечить плавный переход.
...
новых версий и развертывает их:
...
## Требования к среде выполнения {#runtime-requirements}
...
| TimescaleDB | 2.16.1             | Требуемое расширение PostgreSQL (устарело) |
| ClickHouse  | 25                 | Требуется для аналитики                    |

::: warning TIMESCALEDB DEPRECATION
<!-- -->
В настоящее время TimescaleDB является обязательным расширением PostgreSQL для
...
из списка обязательных зависимостей, поскольку мы переносим всю функциональность
...
### Запуск образов, виртуализированных с помощью Docker {#running-dockervirtualized-images}
...
Обратите внимание, что большинство поставщиков инфраструктуры поддерживают эту
технологию, поскольку она стала стандартным контейнером для распространения и
запуска программного обеспечения в производственных средах.
...
используется для событий команд, аналитики и других функций, основанных на
...
Tuist использует [ClickHouse](https://clickhouse.com/) для хранения и запросов
больших объемов аналитических данных. ClickHouse является обязательным
компонентом **** для таких функций, как build insights, и станет основной базой
данных временных рядов по мере того, как мы будем постепенно отказываться от
TimescaleDB. Вы можете выбрать, хостить ли ClickHouse самостоятельно или
использовать их хостинговый сервис.

::: info MIGRATIONS
<!-- -->
Точка входа образа Docker автоматически запускает все ожидающие миграции схем
...
<!-- -->
Если ваша цель в первую очередь состоит в том, чтобы создать собственное
хранилище для бинарных файлов и сократить задержку кэша, вам, возможно, не
понадобится самостоятельно размещать весь сервер. Вы можете самостоятельно
разместить узлы кэша и подключить их к размещенному серверу Tuist или к вашему
...
Учитывая конфиденциальный характер этих переменных, мы рекомендуем шифровать их
и хранить в надежных решениях для управления паролями. Будьте уверены, Tuist
...
отсутствует, запуск завершится сбоем, и в сообщении об ошибке будет указано,
какие именно переменные отсутствуют.
<!-- -->
:::

### Настройка лицензии {#license-configuration}

Как локальный пользователь, вы получите лицензионный ключ, который необходимо
указать в качестве переменной среды. Этот ключ используется для проверки
лицензии и обеспечения того, что сервис работает в соответствии с условиями
соглашения.
...
| `TUIST_LICENSE_CERTIFICATE_BASE64` | **Исключительная альтернатива `TUIST_LICENSE`**. Общедоступный сертификат с кодировкой Base64 для автономной проверки лицензии в средах с физическим разрывом, где сервер не может связываться с внешними службами. Используйте только в том случае, если `TUIST_LICENSE` не может быть использован | Да*         |              | `LS0tLS1CRUdJTiBDRVJUSUZJQ0FURS0tLS0t...` |

\* Необходимо указать либо `TUIST_LICENSE`, либо
...
лицензии, обратитесь по адресу [contact@tuist.dev](mailto:contact@tuist.dev).
<!-- -->
:::

### Настройка базовой среды {#base-environment-configuration}

| Переменная среды                      | Описание                                                                                                                                                                                                                                                     | Обязательно | По умолчанию                       | Пример                                                                       |                                                                                                                                    |
| ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------- | ---------------------------------- | ---------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| `TUIST_APP_URL`                       | Базовый URL для доступа к экземпляру из Интернета                                                                                                                                                                                                            | Да          |                                    | https://tuist.dev                                                            |                                                                                                                                    |
...
| `TUIST_SECRET_KEY_PASSWORD`           | Pepper для генерации хешированных паролей                                                                                                                                                                                                                    | Нет         | `$TUIST_SECRET_KEY_BASE`           |                                                                              |                                                                                                                                    |
...
| `TUIST_GITHUB_APP_PRIVATE_KEY_BASE64` | Частный ключ с кодировкой base64, используемый приложением GitHub для разблокировки дополнительных функций, таких как публикация автоматических комментариев к PR                                                                                            | Нет         | `LS0tLS1CRUdJTiBSU0EgUFJJVkFUR...` |                                                                              |                                                                                                                                    |
| `TUIST_GITHUB_APP_PRIVATE_KEY`        | Закрытый ключ, используемый приложением GitHub для разблокировки дополнительных функций, таких как публикация автоматических комментариев к PR. **Мы рекомендуем использовать версию с кодировкой base64, чтобы избежать проблем со специальными символами** | Нет         | `-----BEGIN RSA...`                |                                                                              |                                                                                                                                    |
...
| Переменная среды                     | Описание                                                                                                                                                                                                                                    | Обязательно | По умолчанию | Пример                                                                 |
| ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- | ------------ | ---------------------------------------------------------------------- |
| `DATABASE_URL`                       | URL-адрес для доступа к базе данных Postgres. Обратите внимание, что URL-адрес должен содержать данные для аутентификации                                                                                                                   | Да          |              | `postgres://username:password@cloud.us-east-2.aws.test.com/production` |
| `TUIST_CLICKHOUSE_URL`               | URL-адрес для доступа к базе данных ClickHouse. Обратите внимание, что URL-адрес должен содержать данные для аутентификации                                                                                                                 | Нет         |              | `http://username:password@cloud.us-east-2.aws.test.com/production`     |
| `TUIST_USE_SSL_FOR_DATABASE`         | Если значение true, для подключения к базе данных используется [SSL](https://en.wikipedia.org/wiki/Transport_Layer_Security)                                                                                                                | Нет         | `1`          | `1`                                                                    |
| `TUIST_DATABASE_POOL_SIZE`           | Количество соединений, которые необходимо держать открытыми в пуле соединений                                                                                                                                                               | Нет         | `10`         | `10`                                                                   |
| `TUIST_DATABASE_QUEUE_TARGET`        | Интервал (в миллисекундах) для проверки, не превысили ли все соединения, взятые из пула, интервал очереди [(Дополнительная информация)](https://hexdocs.pm/db_connection/DBConnection.html#start_link/2-queue-config)                       | Нет         | `300`        | `300`                                                                  |
| `TUIST_DATABASE_QUEUE_INTERVAL`      | Пороговое время (в миллисекундах) в очереди, которое пул использует для определения необходимости отбрасывания новых соединений [(Дополнительная информация)](https://hexdocs.pm/db_connection/DBConnection.html#start_link/2-queue-config) | Нет         | `1000`       | `1000`                                                                 |
| `TUIST_CLICKHOUSE_FLUSH_INTERVAL_MS` | Интервал времени в миллисекундах между сбросами буфера ClickHouse                                                                                                                                                                           | Нет         | `5000`       | `5000`                                                                 |
...
Мы обеспечиваем аутентификацию через [поставщиков идентификации
...
поставщика присутствуют в среде сервера. **Отсутствие переменных** приведет к
...
Убедитесь, что в среде сервера указаны все необходимые переменные среды,
указанные GitHub. Отсутствие переменных приведет к тому, что Tuist пропустит
аутентификацию GitHub. Чтобы правильно настроить приложение GitHub:
- В общих настройках приложения GitHub:
    - Скопируйте идентификатор клиента `` и установите его в качестве
    ...
    - Укажите URL-адрес обратного вызова `` в виде
      `http://YOUR_APP_URL/users/auth/github/callback`. `YOUR_APP_URL` также
      может быть IP-адресом вашего сервера.
- Требуются следующие разрешения:
  - Репозитории:
    - Запросы на слияние: чтение и запись
  - Учетные записи:
    - Адреса электронной почты: Только для чтения

В разделе « `» (Права доступа и события)` « `» (Права доступа к учетной записи)`
установите для права « `» (Адрес электронной почты)` значение « `Read-only»
(Только для чтения)`.

Затем вам нужно будет задать следующие переменные среды в среде, где работает
сервер Tuist:
...
создать новые учетные данные типа «Идентификатор клиента OAuth». При создании
учетных данных выберите «Веб-приложение» в качестве типа приложения, назовите
его `Tuist` и установите URI перенаправления на
`{base_url}/users/auth/google/callback`, где `base_url` — это URL, по которому
работает ваш хостируемый сервис. После создания приложения скопируйте
идентификатор клиента и секретный ключ и задайте их в качестве переменных среды
`GOOGLE_CLIENT_ID` и `GOOGLE_CLIENT_SECRET` соответственно.

::: info CONSENT SCREEN SCOPES
<!-- -->
Возможно, вам понадобится создать экран согласия. При этом обязательно добавьте
`userinfo.email` и `openid` в список областей доступа (scopes) и пометьте
приложение как внутреннее.
<!-- -->
:::
...
| Переменная среды             | Описание                                                                                                           | Обязательно | По умолчанию | Пример |
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------ | ----------- | ------------ | ------ |
| `TUIST_OKTA_1_CLIENT_ID`     | Идентификатор клиента для аутентификации в Okta. Это число должно соответствовать идентификатору вашей организации | Да          |              |        |
| `TUIST_OKTA_1_CLIENT_SECRET` | Секретный ключ клиента для аутентификации в Okta                                                                   | Да          |              |        |

Число `1` необходимо заменить на ID вашей организации. Обычно это будет 1, но
проверьте в своей базе данных.

### Настройка среды хранения {#storage-environment-configuration}

Tuist требует хранилища для размещения артефактов, загруженных через API. Для
эффективной работы Tuist необходимо настроить одно из поддерживаемых решений для
хранения **** .
...
| Переменная среды                                         | Описание                                                                                                                                                                | Обязательно | По умолчанию                       | Пример                                                        |
| -------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- | ---------------------------------- | ------------------------------------------------------------- |
| `TUIST_S3_ACCESS_KEY_ID` или `AWS_ACCESS_KEY_ID`         | Идентификатор клавиши доступа для аутентификации у поставщика хранилища                                                                                                 | Да          |                                    | `AKIAIOSFOD`                                                  |
...
| `TUIST_S3_REGION` или `AWS_REGION`                       | Регион, в котором находится корзина                                                                                                                                     | Нет         | `auto`                             | `us-west-2`                                                   |
...
| `TUIST_S3_CA_CERT_PEM`                                   | Сертификат CA с кодировкой PEM для проверки HTTPS-соединений S3. Полезно для изолированных сред с самоподписанными сертификатами или внутренними центрами сертификации. | Нет         | Пакет системных CA                 | `-----BEGIN CERTIFICATE-----\n...\n-----END CERTIFICATE-----` |
| `TUIST_S3_CONNECT_TIMEOUT`                               | Таймаут (в миллисекундах) для установления соединения с поставщиком хранилища                                                                                           | Нет         | `3000`                             | `3000`                                                        |
| `TUIST_S3_RECEIVE_TIMEOUT`                               | Таймаут (в миллисекундах) для получения данных от поставщика хранилища                                                                                                  | Нет         | `5000`                             | `5000`                                                        |
| `TUIST_S3_POOL_TIMEOUT`                                  | Время ожидания (в миллисекундах) для пула подключений к поставщику хранилища. Используйте `infinity` для отключения времени ожидания                                    | Нет         | `5000`                             | `5000`                                                        |
...
| `TUIST_S3_POOL_COUNT`                                    | Количество пулов соединений, которые будут использоваться                                                                                                               | Нет         | Количество системных планировщиков | `4`                                                           |
...
| `TUIST_S3_VIRTUAL_HOST`                                  | Следует ли формировать URL-адрес с именем корзины в качестве субдомена (виртуального хоста)                                                                             | Нет         | `false`                            | `1`                                                           |
...
`AWS_ENDPOINT` следует установить на `https://storage.googleapis.com`. Остальные
...
Tuist требует функции электронной почты для аутентификации пользователей и
отправки уведомлений о транзакциях (например, сброс пароля, уведомления об
учетной записи). В настоящее время **поддерживается только Mailgun** в качестве
поставщика услуг электронной почты.

| Переменная среды                 | Описание                                                                                                                                                                                              | Обязательно | По умолчанию                                                                           | Пример                    |
| -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- | -------------------------------------------------------------------------------------- | ------------------------- |
| `TUIST_MAILGUN_API_KEY`          | API-ключ для аутентификации в Mailgun                                                                                                                                                                 | Да*         |                                                                                        | `key-1234567890abcdef`    |
...
| `TUIST_SKIP_EMAIL_CONFIRMATION`  | Пропустить подтверждение по электронной почте при регистрации новых пользователей. Если эта опция включена, пользователи автоматически подтверждаются и могут войти в систему сразу после регистрации | Нет         | `true`, если электронная почта не настроена, `false`, если электронная почта настроена | `true`, `false`, `1`, `0` |
...
по электронной почте при ее настройке, установите
...
с платформами Git</LocalizedLink>, чтобы предоставлять дополнительные функции,
такие как автоматическая публикация комментариев в ваших пул-реквестах.
...
только вы не создавали приложение OAuth GitHub. В разделе `Permissions and
events`'s `Repository permissions` вам необходимо дополнительно установить `Pull
requests` permission to `Read and write`.

Помимо `TUIST_GITHUB_APP_CLIENT_ID` и `TUIST_GITHUB_APP_CLIENT_SECRET`, вам
понадобятся следующие переменные среды:

| Переменная среды               | Описание                      | Обязательно | По умолчанию | Пример                               |
| ------------------------------ | ----------------------------- | ----------- | ------------ | ------------------------------------ |
| `TUIST_GITHUB_APP_PRIVATE_KEY` | Личный ключ приложения GitHub | Да          |              | `-----BEGIN RSA PRIVATE KEY-----...` |

## Тестирование локально {#testing-locally}

Мы предоставляем полную конфигурацию Docker Compose, включающую все необходимые
зависимости для тестирования сервера Tuist на вашем локальном компьютере перед
развертыванием в вашей инфраструктуре:

- PostgreSQL 15 с расширением TimescaleDB 2.16 (устарело)
- ClickHouse 25 для аналитики
- ClickHouse Keeper для координации
- MinIO для хранилища, совместимого с S3
- Redis для постоянного хранения ключ-значений при развертывании (опционально)
...
4. Перейдите на сервер по адресу http://localhost:8080
...
Официальный образ Tuist для Docker доступен по адресу:
```
ghcr.io/tuist/tuist
```

### Загрузка образа Docker {#pulling-the-docker-image}
...
::: warning
<!-- -->
` `Если вашему конвейеру развертывания необходимо проверить, что сервер
работает, вы можете отправить HTTP-запрос GET` на `/ready` и проверить наличие
кода статуса 200` в ответе.
<!-- -->
:::
...
конфигурации fly.toml` с указанием `. Рассмотрите возможность его динамического
создания в рамках вашего конвейера непрерывного развертывания (CD). Ниже
приведен пример для использования:
...
приложение. При последующих развертываниях вместо `fly launch --local-only` вам
нужно будет запустить `fly deploy --local-only`. Fly.io не позволяет загружать
частные образы Docker, поэтому нам нужно использовать флаг `--local-only`.


## Показатели Prometheus {#prometheus-metrics}

Tuist предоставляет метрики Prometheus по адресу `/metrics`, чтобы помочь вам
отслеживать работу вашего самостоятельно развернутого экземпляра. Эти метрики
включают:

### Показатели HTTP-клиента Finch {#finch-metrics}

Tuist использует [Finch](https://github.com/sneako/finch) в качестве
HTTP-клиента и предоставляет подробные метрики по HTTP-запросам:
...
- `tuist_prom_ex_finch_request_duration_milliseconds` - Продолжительность
  ...
  - Метки: `finch_name`, `method`, `scheme`, `host`, `port`, `status`
  - Интервалы: 10 мс, 50 мс, 100 мс, 250 мс, 500 мс, 1 с, 2,5 с, 5 с, 10 с
...
#### Показатели очереди пула соединений
- `tuist_prom_ex_finch_queue_duration_milliseconds` - Время ожидания в очереди
  ...
  - Интервалы: 1 мс, 5 мс, 10 мс, 25 мс, 50 мс, 100 мс, 250 мс, 500 мс, 1 с
- `tuist_prom_ex_finch_queue_idle_time_milliseconds` - Время, в течение которого
  ...
  - Метки: `finch_name`, `scheme`, `host`, `port`, `pool`
  - Интервалы: 10 мс, 50 мс, 100 мс, 250 мс, 500 мс, 1 с, 5 с, 10 с
- `tuist_prom_ex_finch_queue_exception_count_total` - Общее количество
  исключений очереди Finch (счетчик)
...
- `tuist_prom_ex_finch_connect_duration_milliseconds` - Время, затраченное на
  установку соединения (гистограмма)
  - Метки: `finch_name`, `scheme`, `host`, `port`, `error`
  - Интервалы: 10 мс, 50 мс, 100 мс, 250 мс, 500 мс, 1 с, 2,5 с, 5 с
...
- `tuist_prom_ex_finch_send_idle_time_milliseconds` - Время простоя соединения
  ...
  - Метки: `finch_name`, `method`, `scheme`, `host`, `port`, `error`
  - Интервалы: 1 мс, 5 мс, 10 мс, 25 мс, 50 мс, 100 мс, 250 мс, 500 мс
...
Помимо метрик Finch, Tuist предоставляет метрики для:
...
Tuist предоставляет набор утилит по адресу `/ops/`, которые можно использовать
для управления вашим экземпляром.

::: warning Authorization
<!-- -->
Только пользователи, у которых дескрипторы указаны в переменной среды
...
- **Ошибки (`/ops/errors`):** Вы можете просматривать непредвиденные ошибки,
  ...
  так, и мы можем попросить вас поделиться этой информацией с нами, если у вас
  возникнут проблемы.
- **Панель инструментов (`/ops/dashboard`):** Вы можете просмотреть панель
  инструментов, которая предоставляет информацию о производительности и
  работоспособности приложения (например, потребление памяти, запущенные
  процессы, количество запросов). Эта панель инструментов может быть весьма
  полезна для понимания того, достаточно ли используемого вами оборудования для
  обработки нагрузки.
