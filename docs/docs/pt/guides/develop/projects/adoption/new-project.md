...
The command will initialize the project in the current directory. You can then <LocalizedLink href="/guides/develop/projects/editing">edit the project</LocalizedLink> running `tuist edit`, and Xcode will open a project where you can edit the project. 生成されるファイルの1つは `Project.swift` で、プロジェクトの定義が含まれています。 If you are familiar with the Swift Package Manager, think of it as the `Package.swift` but with the lingo of Xcode projects.
