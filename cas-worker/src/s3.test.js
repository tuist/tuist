...
import { createS3Client, getS3Key, checkS3ObjectExists } from "./s3.js";
...
describe("S3 Module", () => {
  describe("createS3Client", () => {
    ...
    it("should throw error when TUIST_S3_REGION is missing", () => {
      ...
      expect(() => createS3Client(env)).toThrow(
        "Missing required environment variable: TUIST_S3_REGION",
      );
    });
    ...
    it("should throw error when TUIST_S3_ENDPOINT is missing", () => {
      ...
      expect(() => createS3Client(env)).toThrow(
        "Missing required environment variable: TUIST_S3_ENDPOINT",
      );
    });
    ...
    it("should throw error when TUIST_S3_ACCESS_KEY_ID is missing", () => {
      ...
      expect(() => createS3Client(env)).toThrow(
        "Missing required environment variable: TUIST_S3_ACCESS_KEY_ID",
      );
    });
    ...
    it("should throw error when TUIST_S3_SECRET_ACCESS_KEY is missing", () => {
      ...
      expect(() => createS3Client(env)).toThrow(
        "Missing required environment variable: TUIST_S3_SECRET_ACCESS_KEY",
      );
    });
  });
