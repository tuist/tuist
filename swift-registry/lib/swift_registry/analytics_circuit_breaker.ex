defmodule SwiftRegistry.AnalyticsCircuitBreaker do
  @moduledoc """
    ...
    if previous_state != :blown and :fuse.ask(name, @fuse_mode) == :blown do
      Logger.warning(
        "Opening analytics circuit for #{label} after repeated failures; " <>
          "dropping events for #{SwiftRegistry.Config.analytics_cooldown_ms()} ms. Last failure: #{reason}"
      )
    end
  ...
  defp fuse_options do
    threshold = max(SwiftRegistry.Config.analytics_failure_threshold(), 1)
    cooldown_ms = max(SwiftRegistry.Config.analytics_cooldown_ms(), 1)
