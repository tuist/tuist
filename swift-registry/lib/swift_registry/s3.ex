defmodule SwiftRegistry.S3 do
  ...
  def exists?(key, opts \\ []) when is_binary(key) do
    ...
    case Cachex.get(@exists_cache, cache_key) do
      ...
      {:ok, cached} ->
        :telemetry.execute([:swift_registry, :s3, :head], %{duration: 0}, %{result: :cache_hit})
        cached
    end
  end
  ...
  """
  def upload(key) do
    local_path = SwiftRegistry.Disk.artifact_path(key)
  ...
  defp handle_download_result(key, local_path, tmp_path, dl_duration, {:ok, :done}) do
    case publish_download(tmp_path, local_path) do
      :ok ->
        :telemetry.execute([:swift_registry, :s3, :download], %{duration: dl_duration}, %{result: :ok})
        {:ok, :hit}

      {:error, reason} ->
        :telemetry.execute([:swift_registry, :s3, :download], %{duration: dl_duration}, %{result: :error})
  ...
  defp handle_download_result(key, _local_path, tmp_path, dl_duration, {:error, {:http_error, 429, _}}) do
    ...
    :telemetry.execute([:swift_registry, :s3, :download], %{duration: dl_duration}, %{result: :rate_limited})
  ...
  defp publish_download(tmp_path, local_path) do
    case SwiftRegistry.Disk.move_file(tmp_path, local_path) do
      :ok ->
        :ok
  ...
  def delete_all_with_prefix(prefix, opts \\ []) do
    ...
    case result do
      {:ok, count} ->
        :telemetry.execute([:swift_registry, :s3, :delete], %{duration: duration, count: count}, %{result: :ok})
        Logger.info("Successfully deleted #{count} objects from S3 with prefix: #{prefix}")
        {:ok, count}

      {:error, reason} = error ->
        :telemetry.execute([:swift_registry, :s3, :delete], %{duration: duration, count: 0}, %{result: :error})
