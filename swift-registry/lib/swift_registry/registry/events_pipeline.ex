defmodule SwiftRegistry.Registry.EventsPipeline do
  @moduledoc """
  ...
  def start_link(_opts) do
    batch_size = Application.get_env(:swift_registry, :events_batch_size, 100)
    batch_timeout = Application.get_env(:swift_registry, :events_batch_timeout, 5_000)
  ...
  def async_push(event) do
    if SwiftRegistry.AnalyticsCircuitBreaker.accept_event?(webhook_url()) do
  ...
  defp send_batch(events) do
    ...
    SwiftRegistry.WebhookClient.signed_post(webhook_url(), body, "registry download events")
  end

  defp webhook_url do
    "#{SwiftRegistry.Config.server_url()}/webhooks/registry"
  end
end
