defmodule SwiftRegistry.Config do
  @moduledoc false
  ...
  def analytics_failure_threshold do
    Application.get_env(:swift_registry, :analytics_failure_threshold, 3)
  end

  def analytics_cooldown_ms do
    Application.get_env(:swift_registry, :analytics_cooldown_ms, 60_000)
  end

  def analytics_receive_timeout_ms do
    Application.get_env(:swift_registry, :analytics_receive_timeout_ms, 2_000)
  end

  def analytics_pool_timeout_ms do
    Application.get_env(:swift_registry, :analytics_pool_timeout_ms, 1_000)
  end

  def api_key do
    case Application.get_env(:swift_registry, :api_key) do
  ...
  def oban_web_credentials do
    Application.get_env(:swift_registry, :oban_web_basic_auth, [])
  end

  def registry_bucket, do: Application.get_env(:swift_registry, :s3)[:registry_bucket]

  def registry_github_token do
    case Application.get_env(:swift_registry, :registry_github_token) do
  ...
  def registry_sync_enabled? do
    Application.get_env(:swift_registry, :registry_sync_enabled, true)
  end

  def s3_protocols do
    case Application.get_env(:swift_registry, :s3)[:protocols] do
      protocols when is_list(protocols) and protocols != [] -> protocols
      _ -> [:http2, :http1]
    end
  end

  def s3_ca_cert_pem(s3_config \\ Application.get_env(:swift_registry, :s3, [])) do
  ...
  def server_ca_cert_pem(value \\ Application.get_env(:swift_registry, :server_ca_cert_pem)) do
    case value do
      pem when is_binary(pem) and pem != "" -> pem
      _ -> nil
    end
  ...
  def server_url, do: Application.get_env(:swift_registry, :server_url)

  @default_orphan_scan_max_dirs 50
  def orphan_scan_max_dirs,
    do: Application.get_env(:swift_registry, :orphan_scan_max_dirs, @default_orphan_scan_max_dirs)

  def repo_busy_timeout_ms(repo \\ SwiftRegistry.Repo) do
    Application.get_env(:swift_registry, repo)[:busy_timeout] || 30_000
  end
