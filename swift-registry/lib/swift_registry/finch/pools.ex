defmodule SwiftRegistry.Finch.Pools do
  @moduledoc false

  def config do
    server_url = SwiftRegistry.Config.server_url()
    s3_protocols = SwiftRegistry.Config.s3_protocols()
      ...
      server_url => [
        conn_opts: [
          ...
          transport_opts:
            [
              verify: :verify_peer
            ] ++ TuistCommon.FinchPools.ca_cert_opts(SwiftRegistry.Config.server_ca_cert_pem())
        ],
        size: 32,
        count: 8,
        protocols: [:http2, :http1],
    ...
    case SwiftRegistry.Config.s3_config() do
      {:ok, s3_config} ->
        s3_url =
        ...
        {_s3_url, s3_pool_opts} =
          TuistCommon.FinchPools.s3_pool(
            ...
            ca_cert_pem: SwiftRegistry.Config.s3_ca_cert_pem()
          )
  ...
  defp virtual_host_s3_endpoints(s3_url) do
    if SwiftRegistry.Config.s3_virtual_host() do
      [SwiftRegistry.Config.registry_bucket()]
