defmodule SwiftRegistryWeb.RegistryControllerTest do
  use SwiftRegistryWeb.ConnCase
  use Mimic

  alias SwiftRegistry.CacheArtifacts
  alias SwiftRegistry.Registry
  ...
  setup do
    stub(SwiftRegistry.Config, :registry_github_token, fn -> "test-token" end)
    stub(SwiftRegistry.Config, :registry_bucket, fn -> "test-bucket" end)
    stub(SwiftRegistry.Config, :registry_enabled?, fn -> true end)
    stub(SwiftRegistry.Config, :analytics_enabled?, fn -> false end)
  ...
  describe "GET /api/registry/swift/:scope/:name (list_releases)" do
    ...
    test "returns root-relative release urls on the standalone service root", %{conn: conn} do
      scope = "apple"
      name = "swift-argument-parser"
