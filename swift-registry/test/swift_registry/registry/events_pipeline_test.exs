defmodule SwiftRegistry.Registry.EventsPipelineTest do
  use ExUnit.Case, async: true
  use Mimic

  import SwiftRegistry.AnalyticsCircuitBreakerTestHelpers, only: [setup_analytics_circuit_breaker: 1]

  alias SwiftRegistry.Registry.EventsPipeline
  ...
  setup do
    stub(SwiftRegistry.Config, :server_url, fn -> @server_url end)
    :ok
  end
  ...
  describe "handle_batch/4" do
    test "skips sending events when API key is not configured" do
      stub(SwiftRegistry.Config, :api_key, fn -> nil end)
    ...
    test "computes correct HMAC-SHA256 signature" do
      ...
      stub(SwiftRegistry.Config, :api_key, fn -> secret end)
