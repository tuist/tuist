...
The old `/api/registry/swift` prefix is still routed for compatibility, but `https://swift-registry.tuist.dev` serves the registry API at the root.
...
- `PUBLIC_HOST` - Public host, for example `swift-registry.tuist.dev`.
...
- `TUIST_SWIFT_REGISTRY_API_KEY` - API key for server webhook calls. Falls back to `TUIST_CACHE_API_KEY`.

Managed deployments use `infra/helm/swift-registry`. The Helm chart mounts PVCs for `/data` and `/storage`, and syncs runtime secrets from 1Password through External Secrets Operator.
