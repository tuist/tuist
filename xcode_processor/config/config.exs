import Config

config :xcode_processor, XcodeProcessorWeb.Endpoint,
  url: [host: "localhost"],
  adapter: Bandit.PhoenixAdapter,
  render_errors: [
    formats: [json: XcodeProcessorWeb.ErrorJSON],
    layout: false
  ],
  pubsub_server: XcodeProcessor.PubSub

config :xcode_processor,
  env: config_env(),
  namespace: XcodeProcessor

config :logger, :console,
  format: "$time $metadata[$level] $message\n",
  metadata: [:request_id]

config :phoenix, :json_library, Jason

config :tuist_common, finch_name: XcodeProcessor.Finch

config :sentry,
  client: TuistCommon.SentryHTTPClient,
  enable_source_code_context: true,
  root_source_code_paths: [File.cwd!()],
  before_send: {TuistCommon.SentryEventFilter, :before_send}

import_config "#{config_env()}.exs"
