defmodule TuistJitWeb.PolicyController do
  @moduledoc """
  ...
  def evaluate(conn, %{"subject" => subject, "env" => env}) when is_binary(subject) and env in @valid_envs do
    case resolve(subject, env) do
      ...
      {:deny, reason} ->
        Logger.info("tuist_jit policy: deny subject=#{subject} env=#{env} reason=#{reason}")
  ...
  defp resolve(subject, env) do
    case TailscaleClient.user_role(subject) do
      ...
      {:error, reason} ->
        Logger.warning("tuist_jit policy: tailnet lookup failed for #{subject}: #{inspect(reason)}")
