defmodule TuistCloudWeb.UserLoginLive do
  use TuistCloudWeb, :live_view
  alias TuistCloud.Environment

  def mount(_params, _session, socket) do
    email = live_flash(socket.assigns.flash, :email)
    form = to_form(%{"email" => email}, as: "user")

    {
      :ok,
      socket
      |> assign(:form, form)
      |> assign(:smtp_configured?, Environment.smtp_configured?())
      |> assign(:github_configured, Environment.github_configured?())
      |> assign(:google_configured, Environment.google_oauth_configured?())
      |> assign(:okta_configured, Environment.okta_configured?()),
      temporary_assigns: [form: form]
    }
  end

  def render(assigns) do
    ~H"""
    <link phx-track-static rel="stylesheet" href={~p"/v2/css/auth.css"} />

    <.stack class="auth-page" gap="4xl">
      <.decorative_background class="auth-page__background" />
      <.stack class="auth-header" gap="3xl">
        <img
          class="auth-header__logo"
          src="/v2/images/tuist_logo_32x32@2x.png"
          alt="Tuist Cloud Icon"
        />
        <.stack gap="lg">
          <h5 class="auth-header__title font--semibold color--text-primary">
            <%= gettext("Log in to your account") %>
          </h5>
          <p class="text--medium color--text-tertiary">
            <%= gettext("Welcome back! Please enter your details.") %>
          </p>
        </.stack>
      </.stack>

      <.simple_form
        for={@form}
        id="login_form"
        action={~p"/v2/users/log_in"}
        phx-update="ignore"
        class="auth-form"
      >
        <.stack gap="3xl">
          <%= if @smtp_configured? do %>
            <.stack gap="2xl">
              <.input
                field={@form[:email]}
                type="email"
                label={gettext("Email")}
                placeholder={gettext("Enter your email")}
                required
              />
              <.input
                field={@form[:password]}
                type="password"
                label={gettext("Password")}
                placeholder={gettext("Enter your password")}
                required
              />
            </.stack>
            <.stack direction="horizontal">
              <.input
                field={@form[:remember_me]}
                type="checkbox"
                label={gettext("Keep me logged in")}
              />
              <a href={~p"/v2/users/reset_password"} class="text--small font--semibold">
                <%= gettext("Forgot your password?") %>
              </a>
            </.stack>
          <% end %>
          <.stack gap="xl">
            <%= if @smtp_configured? do %>
              <.button type="submit" variant="primary" class="auth-form__primary-action">
                <%= gettext("Sign in") %>
              </.button>
            <% end %>
            <%= if @github_configured do %>
              <.social_button>
                <a href={~p"/v2/users/auth/github"}>
                  <%= gettext("Sign in with GitHub") %>
                </a>
                <:icon><.github_logo /></:icon>
              </.social_button>
            <% end %>
            <%= if @google_configured do %>
              <.social_button>
                <a href={~p"/v2/users/auth/google"}>
                  <%= gettext("Sign in with Google") %>
                </a>
                <:icon><.google_logo /></:icon>
              </.social_button>
            <% end %>
            <%= if @okta_configured do %>
              <.social_button>
                <a href={~p"/v2/users/auth/okta"}>
                  <%= gettext("Sign in with Okta") %>
                </a>
                <:icon><.okta_logo /></:icon>
              </.social_button>
            <% end %>
          </.stack>
        </.stack>
      </.simple_form>

      <%= if @smtp_configured? do %>
        <.stack direction="horizontal" gap="xs">
          <span class="text--small font--regular color--text-tertiary">
            <%= gettext("Don't have an account?") %>
          </span>
          <a href={~p"/v2/users/register"} class="text--small font--semibold">
            <%= gettext("Sign up") %>
          </a>
        </.stack>
      <% end %>

      <.flash_group flash={@flash} />
    </.stack>
    """
  end
end
