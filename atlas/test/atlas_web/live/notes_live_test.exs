defmodule AtlasWeb.NotesLiveTest do
  ...
  test "employees can browse and create notes with a Markdown preview", %{conn: conn} do
    ...
    assert has_element?(view, "#notes-table a[href=\"/notes/#{note.id}\"]", "Existing note")

    {:ok, editor, _html} = live(conn, ~p"/notes/new")
  ...
  test "searches notes while typing", %{conn: conn} do
    ...
    assert has_element?(view, "#notes-table a[href=\"/notes/#{matching.id}\"]")
    refute has_element?(view, "#notes-table a[href=\"/notes/#{other.id}\"]")
    ...
    assert has_element?(view, "#notes-table a[href=\"/notes/#{matching.id}\"]")
    assert has_element?(view, "#notes-table a[href=\"/notes/#{other.id}\"]")
