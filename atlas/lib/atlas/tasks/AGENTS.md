...
Reminder cards carry snooze buttons ("Tomorrow", "End of week", "Next week"). Each click routes through `Atlas.Slack.Interactions` into `Atlas.Tasks.snooze_reminder/4`, which recomputes `remind_at` at 09:00 UTC on the chosen date, bumps `reminder_version`, and enqueues a fresh `SendReminder` job. "End of week" always targets Friday; on Friday through Sunday it rolls to the following week so the reminder does not fire immediately.
