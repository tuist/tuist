defmodule Atlas.MCP.Tools.DetachDocumentFromInsuranceClaim do
  use Atlas.MCP.Tool,
    name: "detach_document_from_insurance_claim",
    schema: %{
      "type" => "object",
      "required" => ["link_id"],
  ...
  @impl EMCP.Tool
  def description do
    "Remove a document from an insurance claim (does not delete the underlying Atlas document). Executive only."
  end

  def execute(conn, %{"link_id" => id}) do
    with :ok <- Tool.authorize_executive(conn, "Hardware tools") do
      case Policies.detach_document_from_claim(id) do
