defmodule Atlas.MCP.Tools.AddEmailAudienceSubscriber do
  @moduledoc false

  use Atlas.MCP.Tool,
    name: "add_email_audience_subscriber",
    schema: %{
      "type" => "object",
  ...
  @impl EMCP.Tool
  def description, do: "Add an existing email subscriber to an Atlas audience."
  ...
  def execute(conn, args) do
    ...
    with audience when not is_nil(audience) <- audience,
         ...
         {:ok, _membership} <- GTM.add_email_audience_subscriber(audience, subscriber, Tool.current_user(conn)) do
      ...
      {:error, reason} -> {:error, "Could not add subscriber: #{inspect(reason)}"}
    end
  end
end
