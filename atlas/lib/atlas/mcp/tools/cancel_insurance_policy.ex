defmodule Atlas.MCP.Tools.CancelInsurancePolicy do
  use Atlas.MCP.Tool,
  ...
  @impl EMCP.Tool
  def description do
    "Cancel an insurance policy mid-term (distinct from natural expiration) and close open members on the given date. Executive only."
  end
  ...
  def execute(conn, %{"policy_id" => id} = args) do
    with :ok <- Tool.authorize_executive(conn, "Hardware tools"),
      ...
      case Policies.cancel(policy, opts) do
        {:ok, updated} -> {:ok, Serializer.policy(updated)}
        {:error, changeset} -> {:error, "Could not cancel policy: #{Tool.format_changeset_errors(changeset)}"}
      end
    else
