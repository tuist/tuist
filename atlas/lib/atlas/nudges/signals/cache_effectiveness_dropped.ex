defmodule Atlas.Nudges.Signals.CacheEffectivenessDropped do
  @moduledoc """
  Fires when a paying customer's 7-day binary-cache hit rate has dropped
  below an absolute floor AND fallen by at least `@baseline_drop` versus
  their 28-day baseline. The pitch is a short conversation about what
  might have invalidated the fingerprint (new build setting, target
  added without caching, cold runner, etc.).

  Reads through `Atlas.Nudges.Analytics`, which returns a `%Reading{}`
  with a `stage` guard so we cleanly suppress on missing history,
  insufficient sample, staleness, or refresh failure.
  """
  ...
  @signal_name "cache_effectiveness_dropped"
  @short_window 7
  @baseline_window 28
  @absolute_ceiling 0.60
  @baseline_drop 0.15
  ...
  def evaluate(%Account{} = account) do
    current = Analytics.cache_effectiveness(account, window: @short_window)
    baseline = Analytics.cache_effectiveness(account, window: @baseline_window)
  ...
  defp build_proposal(%Account{} = account, episode, current, baseline, evidence) do
    ...
    %Proposal{
      ...
      title: "#{account_label}: cache hit rate #{current_pct} (was #{baseline_pct})",
      rationale:
        "Binary cache hit rate over the last #{@short_window} days is #{current_pct}, " <>
          "down from a #{@baseline_window}-day baseline of #{baseline_pct}. Common causes: " <>
          "a new build setting that invalidates the fingerprint, a target added without a " <>
          "cacheable configuration, or a runner without the cache warmed.",
      evidence: evidence,
      draft_subject: "Cache hit rate on #{account_label} dropped this week",
  ...
  defp draft_body(account_label, current_pct, baseline_pct) do
    ...
    Wanted to flag a change on the #{account_label} cache: the last week's hit rate is #{current_pct}, \
    down from a four-week baseline of #{baseline_pct}. That level of drop usually points at a build \
    setting that changed the fingerprint, a target added without a cacheable configuration, or a \
    runner that isn't warmed.

    Happy to jump on a short call and dig into what shifted, if that would help.
