...
- Replication delivery is never paused for memory pressure, at any tier. Because a full outbox rejects cache writes, pausing the drain would not defer work — it would reject writes and leave the node permanently divergent from its peers. The drain loop is serial and node-wide and each delivery streams its body in 4 KiB chunks without a transient reservation, so what pausing could reclaim is tens of KiB and does not scale with peer count or backlog depth. The usage (metering) outbox has no such coupling and still pauses under critical pressure.
