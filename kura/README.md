...
| `KURA_OUTBOX_MAX_INFLIGHT` | How many outbox deliveries the drain keeps in flight at once. The drain awaits each delivery before dispatching the next, so this is what separates queue throughput from the round trip to a peer: at `1` a node delivers one message per RTT however much bandwidth it has been given, and every artifact enqueues one message per peer. Raise it on a write-primary whose peers are in another region; each slot holds one in-flight body. | Yes | `8` |
