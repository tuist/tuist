...
| `KURA_REPLICATION_PULL` | Flips the node to pull replication: it advertises `pulling` in `/_internal/status`, reads from every peer that also pulls (its same-region siblings' arrival feed; remote gateways' ascending listings while it holds its region's gateway role) and keeps pushing through the outbox to every peer that does not. The control plane's account flag (`kura_replication_pull`, carried in the mesh heartbeat and peers-sync responses) can also switch it on for enrolled nodes; this value only ever adds to it. | Yes | `false` |
