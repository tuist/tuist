...
| `KURA_OUTBOX_MAX_INFLIGHT` | How many outbox deliveries the drain dispatches before it waits for one to finish. This is what separates queue throughput from the round trip to a peer: throughput is roughly this many messages per RTT, and at `1` the drain is serial and delivers one message per RTT however much bandwidth it has been given. Every artifact enqueues one message per peer, so the artifact rate is this divided by the peer count. Raise it on a write-primary whose peers are in another region; each slot holds one in-flight body. | Yes | `8` |
