...
| `KURA_OUTBOX_STALE_TARGET_GRACE_MS` | How long a replication target must be continuously absent from the node's current peer set before its queued outbox messages are dropped (a departed peer rejoins through a recovery re-enrollment, which re-bootstraps the full dataset, so dropped deltas are recovered). `0` drops on first sight of a missing target. Applies to control-plane-managed targets only: peers known solely through discovery (in-cluster siblings, cross-region pods) are platform-managed like the static seeds and use a fixed 24h grace, since their absence usually means a network flap — which nothing re-bootstraps — rather than departure. | Yes | `900000` |
