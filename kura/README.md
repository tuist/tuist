...
- Kura samples the container's control-group memory every 200 milliseconds and publishes both the complete charge and its heap, file, kernel, shared-memory, dirty-file, writeback, and inactive-file breakdown. Pressure and admission subtract inactive file-backed pages only after excluding shared-memory, dirty, and writeback bytes. Because statistics and the total charge cannot be read atomically, admission caps that discount to half of the gap between the hard watermark and the runtime limit. This simultaneously enforces the discounted working-set watermark and keeps the sampled raw charge plus reservations below the midpoint between the hard watermark and runtime limit, retaining the other half for unmodelled growth. The hard working-set watermark is 85% of the enforced limit; the complete charge may rise above that while the kernel still holds reclaimable pages. Sampling only publishes atomic pressure state; a separate task performs lock-taking cache trims so cleanup latency cannot blind the sensor. Concurrent response materialization, memory-mapped serving, foreground uploads, multipart assembly, and bootstrap transfers reserve predicted transient bytes against live headroom before starting. A memory-mapped response reserves its complete page-aligned mapping span for the response lifetime, then releases it immediately because the residency gate proves those pages were already present in the sampled charge. A foreground upload reserves up to a 32 MiB source-plus-destination working set. Objects larger than 16 MiB, smaller uploads that had to queue for headroom, and overlapping foreground uploads synchronize and release completed staging and append-only segment ranges every 8 MiB. Kura closes the synchronized writer before advising the kernel to release a range, then reopens it in append mode, so cache reclamation cannot invalidate later buffered writes. Accelerated artifact reads keep the operating system's hot file cache during normal operation and release each completed, page-aligned transfer chunk while memory is constrained or critical, allowing the node to recover without sacrificing the normal hot-read path. The reservation remains held through persistence. A bounded upload or waiting request keeps its already bounded reservation charged until an observation begun after completion incorporates the real footprint, so fast completions cannot reuse stale headroom. Linux startup requires a complete container accounting sample, including the enforced limit, before Kura serves traffic. A missing sample or a runtime budget larger than that limit fails startup instead of falling back to process resident memory. Later failed or incompatible samples preserve the last safe state, and a five-second freshness watchdog replaces a node whose sensor stops advancing. Admission times out after 30 seconds with `503 Service Unavailable` or gRPC `RESOURCE_EXHAUSTED`. REAPI ByteStream keeps its existing 64 MiB decode limit. A request-body scanner reads every five-byte gRPC envelope header and non-blockingly grows the reservation to twice the largest message actually observed before Tonic can allocate its retained wire buffer and decoded byte vector. Once the first resource name reveals the blob size, Kura adds only its bounded disk working set. Excess growth returns retryable `RESOURCE_EXHAUSTED` without waiting behind a shared HTTP/2 connection window, while small chunks retain high concurrency even for a large artifact. Large decoded messages use the same 8 MiB file-cache windows. Stored multipart parts release their clean file cache immediately and use the same bounded window during final assembly. Bootstrap pauses when the soft watermark is crossed and applies equivalent bounded-range reclamation while copying a large artifact. Temporary upload, assembly, and bootstrap files are owned by cancellation-safe cleanup guards, so aborted futures cannot strand disk usage; cancellation cleanup runs on Tokio's blocking pool instead of a runtime worker. The allocator reclaims unused pages on one background thread with a four-second decay, so a quiet node returns memory after a burst without relying on a later request to trigger maintenance.
- Normal artifact and ByteStream response readers share a process-wide weighted byte pool. File-backed HTTP responses reserve four buffers sized from 8 KiB to 512 KiB according to the actual response size. ByteStream applies the same four-buffer bound after rounding for Tonic's 8 KiB encoding allocation, and inline responses also reserve the complete value. Materialized Remote Execution responses reserve both their source payload and encoded transport copy. Admission happens before a file is opened. A single guard follows the response through Tonic encoding and every Hyper-owned byte buffer, so a stalled or cancelled client cannot release its memory slot early. Once the final userspace buffer drops, the weighted slot becomes immediately reusable while the transient charge remains through one memory observation to incorporate allocator, encryption, and kernel socket copies. Small responses therefore retain high throughput without weakening the large-response bound. Active bytes and queued readers are bounded. A public read never fails for want of pool capacity: a request that cannot reserve its full buffers promptly is served from the 8 KiB chunk floor instead, so a busy node degrades throughput rather than returning an error to clients that treat a 5xx read as fatal. Degraded streams hold a slot counted at the 512 KiB per-stream transport send buffer rather than the 8 KiB they reserve, so a flood of slow readers cannot grow the aggregate beyond the pool budget as the file-descriptor pool allows more concurrency. That slot wait is bounded, because a stalled client holds its slot for as long as it stalls; a read that outlasts the wait is still served without a slot and is counted separately, so the overflow is visible rather than silent. How long admission waits depends on what the caller does when it fails: an HTTP read degrades and still serves the object, so it waits only briefly, while a ByteStream read surfaces a retryable error and therefore waits far longer before giving up. Bootstrap reads never queue, cannot barge ahead of public waiters, and share a background sub-budget that leaves at least half of the pool for public traffic whenever the pool is large enough to serve the maximum inline bootstrap object.
...
- `KURA_MEMORY_SOFT_LIMIT_BYTES` is `70%` of detected memory, rounded down to MiB boundaries.
