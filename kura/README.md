...
| `KURA_PEERS` | Static seed peer list. Immutable for the process lifetime, so it should carry only platform-stable peers (enrollment seeds it with the managed regions' public peer gateways); volatile self-hosted membership flows through the mesh heartbeat instead. | Yes | `KURA_NODE_URL` |
...
| `KURA_OUTBOX_STALE_TARGET_GRACE_MS` | How long a replication target must be continuously absent from the node's current peer set before its queued outbox messages are dropped (a departed peer rejoins through a recovery re-enrollment, which re-bootstraps the full dataset, so dropped deltas are recovered). `0` drops on first sight of a missing target. Applies to control-plane-managed targets only: peers known solely through discovery (in-cluster siblings, cross-region pods) are treated like the static seeds and never pruned within a process lifetime, since their absence usually means a network flap — which nothing re-bootstraps — rather than departure; a genuinely removed pod is never rediscovered after the observer's next restart, so its small frozen backlog cleans up via the normal grace at the next deploy. | Yes | `900000` |
