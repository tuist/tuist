...
| `KURA_COMBINED_PORT` | Optional port that co-hosts the public HTTP cache API and the REAPI gRPC service on one listener, dispatching by request path. Lets a single client-facing endpoint speak both protocols. Serves TLS (reusing the `KURA_PUBLIC_TLS_*` cert, ALPN-negotiated) when public TLS is configured, else plaintext h2c. When unset, only `KURA_PORT`/`KURA_GRPC_PORT` serve traffic. | Yes | disabled |
