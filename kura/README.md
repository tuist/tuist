...
| `KURA_OUTBOX_MAX_DEPTH_PER_PEER` | Replication outbox messages one peer may hold. Every cache write enqueues one outbox message per replication target and is refused with retryable backpressure once any of its targets is at this depth, so one slow peer fills its own share without taking the room meant for the others. Per-target depth is exported as `kura_outbox_target_messages`, the share as `kura_outbox_peer_capacity`, and the node-wide room (share times peer count, at least one share) as `kura_outbox_capacity` and `outbox_capacity` on the rollout status endpoint. Each message is roughly half a KiB of RocksDB on disk. | Yes | `100000` |
| `KURA_OUTBOX_MAX_DEPTH` | A fixed node-wide outbox total enforced on top of the per-peer share when set. | Yes | unset |
