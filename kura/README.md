...
| `KURA_OUTBOX_MAX_DEPTH_PER_PEER` | Replication outbox capacity per replication target. Every cache write enqueues one outbox message per peer, so the cap is this times the current peer count (at least one share, at most `500000`) and follows the mesh as peers join and leave; the effective value is exported as `kura_outbox_capacity` and as `outbox_capacity` on the rollout status endpoint. Messages are reserved atomically by the store, and cache writes receive retryable backpressure at the cap. Each message is roughly half a KiB of RocksDB on disk. | Yes | `50000` |
| `KURA_OUTBOX_MAX_DEPTH` | A fixed replication outbox cap. When set it replaces the per-peer scaling entirely. | Yes | unset |
...
- Cache writes are rejected with retryable transport-specific backpressure when memory pressure reaches `Critical`, when the outbox is at capacity (`KURA_OUTBOX_MAX_DEPTH_PER_PEER` times the peer count, or the fixed `KURA_OUTBOX_MAX_DEPTH`), when the file-descriptor pool is exhausted, or when the data volume has insufficient free space for a new segment.
