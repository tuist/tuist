...
| `KURA_OUTBOX_STALE_TARGET_GRACE_MS` | How long a replication target must be continuously absent from the node's current peer set before its queued outbox messages are dropped (a rejoining peer re-bootstraps the full dataset, so dropped deltas are recovered). `0` drops on first sight of a missing target. | Yes | `900000` |
