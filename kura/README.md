...
| `KURA_SYNC_PEER_SERVING_MAX_INFLIGHT` | Bodies requests this node serves in flight across every peer identity, the bound the per-peer count cannot give when many peers converge on one gateway. Refusals answer the same `503 peer_busy` shape and count as `rejected_node_busy` on `kura_backfill_bodies_peer_requests_total`. | Yes | `8` |
