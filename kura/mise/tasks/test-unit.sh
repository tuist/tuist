#!/usr/bin/env bash
#MISE description="Run the kura unit tests via Bazel (bazel test //:kura_lib_test; extra flags pass through, e.g. mise run test-unit -- --test_output=all). Fall back to cargo test only if Bazel is unavailable."
set -euo pipefail
