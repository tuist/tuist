...
- Observability and analytics: `src/metrics.rs`, `src/telemetry.rs`, `src/request_observability.rs`, `src/analytics.rs`
  - Upload staging (`src/utils.rs`) keeps incoming-body failures separate from storage I/O. Classify typed disconnects as 499 and malformed bodies as 400; unknown body failures and storage faults remain 5xx. Preserve the cause in rate-limited completion logs through response extensions. Protocol and cleanup coverage lives in `src/http/upload_tests.rs` and `spec/e2e/upload_errors_spec.sh`.
