...
- Runtime configuration and limits: `src/config.rs`, `src/constants.rs`
  - ByteStream response admission uses `src/memory/mod.rs` and `src/memory/pools.rs`: preserve FIFO order, cancellation cleanup, the bounded deadline, and lifetime-bound byte permits. Waiting requests do not own stream buffers, so size their bounded queue independently of stream chunk size. `spec/e2e/bytestream_admission_spec.sh` and `test/e2e/bytestream-admission/` exercise paced local readers and collect resource samples.
