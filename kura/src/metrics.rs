...
pub struct Metrics {
    ...
    memory_accounted_bytes: Gauge,
    memory_reclaim_credit_bytes: Gauge,
    memory_accounting_samples: Counter,
    memory_raw_guard_dominant_samples: Counter,
    memory_soft_limit_bytes: Gauge,
    memory_hard_limit_bytes: Gauge,
    memory_transient_reserved_bytes: Gauge,
    memory_deferred_release_bytes: Gauge,
...
impl Metrics {
    pub fn new(region: String, tenant_id: String) -> Self {
        ...
        let memory_accounted_bytes = Gauge::default();
        let memory_reclaim_credit_bytes = Gauge::default();
        let memory_accounting_samples = Counter::default();
        let memory_raw_guard_dominant_samples = Counter::default();
        ...
        let memory_deferred_release_bytes = Gauge::default();
        ...
        registry.register(
            "kura_memory_accounted_bytes",
            "Bounded memory accounting value used for pressure and admission",
            memory_accounted_bytes.clone(),
        );
        registry.register(
            "kura_memory_reclaim_credit_bytes",
            "Maximum raw container memory discount allowed by the admission guard",
            memory_reclaim_credit_bytes.clone(),
        );
        registry.register(
            "kura_memory_accounting_samples_total",
            "Successful container memory accounting samples",
            memory_accounting_samples.clone(),
        );
        registry.register(
            "kura_memory_raw_guard_dominant_samples_total",
            "Container memory samples where the raw-charge guard exceeded the discounted working set",
            memory_raw_guard_dominant_samples.clone(),
        );
        registry.register(
        ...
        registry.register(
            "kura_memory_deferred_release_bytes",
            "Completed foreground reservation bytes held until an observation begun after completion",
            memory_deferred_release_bytes.clone(),
        );
        registry.register(
        ...
        let metrics = Self {
            ...
            memory_reclaim_credit_bytes,
            memory_accounting_samples,
            memory_raw_guard_dominant_samples,
            memory_soft_limit_bytes,
            memory_hard_limit_bytes,
            memory_transient_reserved_bytes,
            memory_deferred_release_bytes,
    ...
    pub fn update_memory_deferred_release_bytes(&self, bytes: u64) {
        self.memory_deferred_release_bytes.set(bytes as i64);
    }
    ...
    pub fn update_memory_accounting(
        &self,
        accounted_bytes: u64,
        reclaim_credit_bytes: u64,
        raw_guard_dominant: bool,
    ) {
        self.memory_accounted_bytes.set(accounted_bytes as i64);
        self.memory_reclaim_credit_bytes
            .set(reclaim_credit_bytes as i64);
        self.memory_accounting_samples.inc();
        if raw_guard_dominant {
            self.memory_raw_guard_dominant_samples.inc();
        }
    }
...
mod tests {
    ...
    fn render_includes_recorded_metrics() {
        ...
        assert!(rendered.contains("kura_memory_accounted_bytes"));
        assert!(rendered.contains("kura_memory_reclaim_credit_bytes"));
        assert!(rendered.contains("kura_memory_accounting_samples_total"));
        assert!(rendered.contains("kura_memory_raw_guard_dominant_samples_total"));
        ...
        assert!(rendered.contains("kura_memory_deferred_release_bytes"));
