...
"#,
    ...
    mod tuist_hook {
        ...
        fn module_download_ctx(status_code: u16) -> ExtensionContext {
            ExtensionContext {
                transport: "http".into(),
                route: "/api/cache/module/{id}".into(),
                method: "GET".into(),
                ...
                namespace_id: Some("ios".into()),
                producer: Some("module".into()),
                artifact_key: Some("builds/hash-1/Framework.zip".into()),
                artifact_hash: None,
                headers: BTreeMap::new(),
                query: BTreeMap::from([("hash".into(), "hash-1".into())]),
                status_code: Some(status_code),
            }
        }

        #[tokio::test]
        async fn signs_successful_module_downloads_for_legacy_tuist_clients() {
            let engine = test_engine(&script(), |_| unsafe {
                std::env::set_var(
                    "KURA_EXTENSION_SIGNER_TUIST_LICENSE_ALGORITHM",
                    "hmac-sha256",
                );
                std::env::set_var(
                    "KURA_EXTENSION_SIGNER_TUIST_LICENSE_SECRET",
                    BASE64.encode(b"super-secret"),
                );
            })
            .await;
            ...
            let headers = engine
                .response_headers(&module_download_ctx(200), None)
                .await;
        ...
        #[tokio::test]
        async fn omits_legacy_signature_when_signer_is_not_configured() {
            let engine = test_engine(&script(), |_| {}).await;
        ...
        #[tokio::test]
        async fn omits_legacy_signature_for_failed_module_downloads() {
            let engine = test_engine(&script(), |_| unsafe {
                std::env::set_var(
                    "KURA_EXTENSION_SIGNER_TUIST_LICENSE_ALGORITHM",
                    "hmac-sha256",
            ...
            let headers = engine
                .response_headers(&module_download_ctx(404), None)
                .await;
