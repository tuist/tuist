...
path = '/api/cache/cas/upload-fault?tenant_id=default&namespace_id=' + uuid.uuid4().hex
...
def counters():
    ...
    result = {}
    for line in lines:
        if line.startswith('kura_http_requests_total_total{') and 'route="/api/cache/cas/{id}"' in line:
