...
#
# Chicken-and-egg: the Kura we run as the cache is the published server image
# (KURA_REMOTE_IMAGE, default ghcr.io/tuist/kura:latest), NOT a Bazel-built artifact — so
# the cache exists before we build the thing it caches. Override the tag to pin a version.
#
...
#   KURA_REMOTE_IMAGE   server image to run as the cache (default ghcr.io/tuist/kura:latest)
