...
- Node geographic attribution: each pod resolves its own country and subdivision at startup (`src/node_location.rs`) from a single egress-IP probe, with `KURA_NODE_COUNTRY` / `KURA_NODE_SUBDIVISION` overrides and a deployment-region prefix fallback for country only. The result stamps `geo.country.iso_code` and `geo.region.iso_code` on the OTel Resource (next to the unchanged `kura.region` cloud deployment region) so every span carries them. Combined with the request-side `geo.country.iso_code` / `geo.region.iso_code`, traces have both endpoints needed to compute geographic distance.
