...
- Public plaintext HTTP/1 artifact downloads can use the same-port Linux accelerator. The public listener first peeks and parses request headers with `httparse` without consuming bytes. Only known artifact GET routes that match the local tenant, pass extension access checks, and resolve to a file-backed local artifact are consumed by the accelerator. Everything else, including HTTPS, HTTP/2, non-GET requests, inline artifacts, cold misses, unsupported routes, saturated accelerator capacity, and non-Linux builds, falls through to the normal Axum/Hyper path. Accelerated transfers are bounded by `KURA_ACCELERATED_FILE_SERVING_MAX_CONCURRENT` and use `splice` by default, with `sendfile` available as a runtime mode. Accelerated responses are framed with `content-length` and keep the connection alive, so a client can pipeline further requests on the same socket; the accelerator only forces `connection: close` when the client requests it or sends an unconsumed request body, and an idle reused connection is dropped after a bounded keep-alive timeout. A follow-up request that is not accelerable is handed back to the Axum/Hyper path mid-connection without consuming its bytes. Normal traffic keeps the kernel file cache warm; constrained and critical transfers advise the kernel to release their completed ranges so read bursts cannot strand a node above its recovery watermark.
- The normal Axum/Hyper path still serves public HTTPS and fallback HTTP traffic. It prefers mmap-backed `Bytes` chunks for file-backed artifacts only when a bounded memory budget is available **and the region is already resident in the page cache** (checked with `mincore`), so hot fallback responses avoid copying artifact bytes into heap buffers. Hot mmap responses are yielded in 1 MiB chunks to keep Hyper body overhead low. Cold or budget-constrained reads fall back to the streaming reader path, which isolates blocking reads with `spawn_blocking` and sizes each read between 8 KiB and 512 KiB according to the remaining response. REAPI ByteStream reads stay **streaming**, so they do not materialize whole artifacts in memory. Both transports share one process-wide weighted byte pool, reserve their reader, encoding, and transport buffers against the transient ledger, and include the complete value size for an inline artifact. Admission happens before opening the reader. Active streams and the public waiter queue are bounded. A public request that cannot reserve its full buffers falls back to the 8 KiB chunk floor rather than receiving an error, because clients outside our control (older CLIs, Gradle build-cache clients, Metro) treat a 5xx on a read as a hard failure instead of a retryable miss. A degraded stream still charges the transient ledger, and it holds one of a bounded set of degraded slots sized by dividing the pool budget by the 512 KiB per-stream transport send buffer — the real cost a slow client can impose, rather than the 8 KiB the stream reserves — so degraded concurrency stays inside the pool budget instead of scaling with the file-descriptor pool. Waiting for a slot is deliberately bounded rather than indefinite: a stalled client holds its slot until it drains, so blocking forever would let a few dead connections wedge every later read. A read that outlasts the wait is served without a slot, keeping the never-fail guarantee while recording the overflow. Admission patience is chosen by the caller rather than shared: a degradable HTTP read waits only a fraction of a second, because its fallback already serves the object and waiting longer would just delay bytes it could be sending, whereas a ByteStream read waits seconds because its fallback is a retryable error. The memory-mapped fast path never waits at all, since it is only an optimization over the streaming reader and queueing for it would spend the admission budget twice before the read degraded. Internal bootstrap reads use a non-queuing background share of the same pool and cannot barge ahead of public waiters.
- REAPI surfaces that must build whole responses in memory (`BatchReadBlobs`, `GetActionResult` inline expansions, action-cache proto loads) use two gates:
  ...
  2. a **shared concurrent materialization pool** across the node that reserves both the source payload and its encoded transport copy, with byte permits attached to the encoded response buffers so slow clients cannot leave fully materialized responses unaccounted

The pressure sensor reads the container's control-group memory every 200 milliseconds and publishes atomic state from a bounded accounting value. It brackets the statistics read with total-charge reads, uses the larger total, and subtracts inactive file-backed pages only after excluding shared-memory, dirty, and writeback bytes that cannot be reclaimed immediately. Since those files are not an atomic snapshot, the accounting value is the larger of that estimated working set and the raw charge minus a reclaim credit capped at half of the runtime-to-hard-watermark gap. This preserves half the original gap even if the entire reclaimable estimate is stale, while still allowing clean cache to contribute bounded headroom. The complete charge and detailed heap/file/kernel breakdown remain visible as metrics. The hard working-set watermark is 85% of the enforced container limit, but the complete charge can rise above it while reclaimable file pages remain. A separate one-second actuator performs lock-taking cache cleanup. This separation keeps slow cache locks or extension cleanup from delaying the next pressure observation. A transient byte ledger combines the latest accounting value with predicted bytes for admitted work: foreground materialization, response-stream buffers, memory-mapped responses, and disk-backed uploads may reserve up to the hard watermark, while background snapshot and bootstrap work may reserve only below the soft watermark. The response-stream pool is the smaller of 64 MiB, half the soft-to-hard gap, and half the hard-to-runtime gap. Its compatibility floor fits one maximum-size inline ByteStream response; the live transient ledger remains the final admission bound. File-backed HTTP streams reserve four buffers sized from 8 KiB to 512 KiB according to the response size. ByteStream rounds its four-buffer reservation to Tonic's 8 KiB encoding allocation, and inline artifacts add their full value size. Materialized Remote Execution responses reserve twice their content size for the source message and encoded transport copy. Memory-mapped responses reserve the complete page-aligned mapping span while the body is alive and release immediately when it drops because the residency gate proves those pages were already present in the sampled container charge. Other bounded responses attach one shared guard to the response metadata, the encoded body, and every outgoing Hyper byte buffer. The weighted slot therefore stays occupied for an arbitrarily slow client and releases only when the final userspace buffer drops. The transient reservation then remains until an observation begun afterward incorporates allocator, encryption, and kernel socket copies. Bootstrap reads use a non-queuing background sub-budget and decline whenever a public stream is waiting. On Linux, the controller requires a complete post-store-open baseline and validates its runtime budget against the enforced control-group limit before listeners start. It never substitutes the smaller process resident set. When a bounded upload or waiting request completes, its reservation stays charged until a memory observation that began after the completion incorporates the real footprint. Those deferred bytes are a subset of the already hard-bounded transient ledger, so the handoff cannot grow beyond the hard watermark or reuse stale headroom. A failed or incompatible recurring sample preserves the last safe state, and a watchdog terminates the process if the sensor stops producing valid observations for five seconds so Kubernetes replaces the unhealthy node. The allocator runs one background page-reclamation thread with a four-second dirty and muzzy-page decay; startup fails if that configuration is not active, preventing unused allocator pages from holding an idle node in critical pressure after a burst.
