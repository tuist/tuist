...
- Client geographic attribution sourced from the `X-Forwarded-For` / `X-Real-IP` headers and resolved against the DB-IP Lite City MMDB vendored into the container image at `/opt/geoip/dbip-city-lite.mmdb` (`src/geoip.rs`). Country (ISO 3166-1) lands on both the `kura_http_requests_total` metric and `http.request` spans; subdivision (ISO 3166-2) lands on spans only, deliberately kept off metrics to bound label cardinality. On by default; soft-fails when the database file is absent. A background task refreshes the in-memory copy every `KURA_GEOIP_REFRESH_INTERVAL_SECS` seconds (default `86400`, `0` disables) with download/decompress sizes capped (128 MiB / 256 MiB) to stay within Kura's resource discipline.
- Node geographic attribution: each pod resolves its own country and subdivision at startup (`src/node_location.rs`) from a single egress-IP probe, with `KURA_NODE_COUNTRY` / `KURA_NODE_SUBDIVISION` overrides and a region-prefix fallback for country only. The result stamps `kura.country` and `kura.subdivision` on the OTel Resource so every span carries them. Combined with the request-side `client.country` / `client.subdivision`, traces have both endpoints needed to compute geographic distance.
