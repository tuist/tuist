...
Independent of draining, the co-hosted listener's hyper path recycles every connection after `CONNECTION_MAX_AGE` (300s): the server sends GOAWAY and allows in-flight streams `CONNECTION_MAX_AGE_GRACE` (900s) to finish before severing. This mirrors the dedicated tonic gRPC server it replaced — without recycling, a long-lived Bazel channel would pin to a demoted-but-alive NodePort primary indefinitely after failover. The `axum_server`-served listeners (the TLS port, and the plaintext port when acceleration is disabled) get the drain GOAWAY through the server handle's graceful shutdown but do not age connections.
