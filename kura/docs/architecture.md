...
- Optional co-hosted endpoint: `KURA_COMBINED_PORT` starts an additive plaintext listener that serves the public HTTP cache API and the h2c REAPI gRPC service on one port, dispatching by request path (gRPC service paths route to REAPI via `reapi::routes`, everything else to the HTTP router). It lets a single client-facing URL speak both protocols — e.g. when dispatch hands a runner a single cache endpoint and the client derives its gRPC target from that same host:port — without the client needing the separate `KURA_GRPC_PORT`. The dedicated `KURA_PORT`/`KURA_GRPC_PORT` listeners are unaffected; when unset the combined listener is not started. It has three modes, in precedence order: (1) **TLS** — when `public_tls` is configured it terminates TLS with the public cert and serves both protocols over one TLS port, ALPN-negotiated (`h2` for gRPC, `http/1.1` for HTTP); TLS is incompatible with the sendfile accelerator, so this uses the plain hyper path like the HTTPS listener. (2) **accelerated plaintext** — otherwise it runs through the **same accelerated server** as `KURA_PORT`, so HTTP/1 artifact GETs get the sendfile/splice fast path with no performance loss versus the dedicated HTTP port, while gRPC (h2c) and other non-accelerable requests fall through to hyper. (3) **plain plaintext** — when acceleration is disabled, the merged router over the axum builder. All modes use the fixed REAPI-sized HTTP/2 windows so co-hosted uploads are not throttled. (Enabling adaptive flow control there would override those fixed windows and roughly halve single-stream REAPI upload throughput under WAN latency, so the combined listener does not.)
