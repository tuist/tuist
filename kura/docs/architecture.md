...
- The normal Axum/Hyper path still serves public HTTPS and fallback HTTP traffic. It prefers mmap-backed `Bytes` chunks for file-backed artifacts only when a bounded memory budget is available **and the region is already resident in the page cache** (checked with `mincore`), so hot fallback responses avoid copying artifact bytes into heap buffers. Hot mmap responses are yielded in 1 MiB chunks to keep Hyper body overhead low. Cold or budget-constrained reads fall back to the streaming reader path, which isolates blocking reads with `spawn_blocking` and sizes each read between 8 KiB and 512 KiB according to the remaining response. REAPI ByteStream reads stay **streaming**, so they do not materialize whole artifacts in memory. Both transports share one process-wide weighted byte pool, reserve their reader, encoding, and transport buffers against the transient ledger, and include the complete value size for an inline artifact. Admission happens before opening the reader. Active streams and the public waiter queue are bounded. A public request that cannot reserve its full buffers first falls back to the 8 KiB chunk floor. A degraded stream must hold one of a bounded set of degraded slots sized by dividing the pool budget by the 512 KiB per-stream transport send buffer, which is the real cost a slow client can impose rather than the 8 KiB reader chunk. Its transient reservation charges at least that same transport cost and any larger inline value. The degraded queue and slot wait are also bounded. If either is exhausted, or if live memory headroom cannot cover the response, Kura returns a retryable unavailable response instead of opening an unaccounted stream. Admission patience is chosen by the caller rather than shared: a degradable HTTP read waits only a fraction of a second before trying the smaller pool, whereas a ByteStream read waits seconds because its fallback is immediately a retryable error. The memory-mapped fast path never waits at all, since it is only an optimization over the streaming reader and queueing for it would spend the admission budget twice before the read degraded. Internal bootstrap reads use a non-queuing background share of the same pool and cannot barge ahead of public waiters.
