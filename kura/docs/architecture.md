...
The server also supports zstd wire compression for REAPI ByteStream transfers: a client that negotiates it through `GetCapabilities` (advertisement gated by `KURA_REAPI_COMPRESSION_ENABLED`) can use `compressed-blobs` ByteStream Read/Write resources. Compression and decompression happen on the fly per request — a compressed `ByteStream.Read` streams the stored blob through a zstd encoder, a compressed `Write` decompresses into the upload temp file through a size-bounded sink (capped at the declared uncompressed size as a decompression-bomb guard, so memory stays bounded instead of materializing the full output first). The Batch RPCs are not offered compression. Blobs are always stored uncompressed and keyed by their uncompressed digest, so compression never touches the on-disk segment or replication wire format and the accept/serve paths stay safe under a half-rolled fleet (the flag gates only advertisement).
