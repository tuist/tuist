...
You can also edit tuist with tuist by running `swift run tuist generate` to generate the tuist project with the current changes. If you have not built `ProjectDescription` and `ProjectAutomation` before, run `swift build --product ProjectDescription` and `swift build --product ProjectAutomation` before generating the project. Once the project is generated, you can open it by `open Tuist.xcworkspace` (or using Finder) - _note_ that `xed .` will open the package, and not the project generated by tuist.
To edit the project's manifests, you'll have to run `swift run tuist edit -o`. The `-o` flag is necessary to only edit manifests in the current directory.
If you want to use tuist version you have installed in the system, you might want to simply run `tuist xyz` and drop the `swift run` in all the commands.
