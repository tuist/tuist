...
Allows you to define how targets are grouped into schemes.

| Case                            | Description                                                                                      |
| ------------------------------- | ------------------------------------------------------------------------------------------------ |
| `.singleScheme`                 | A single scheme including all targets.                                                           |
| `.byNameSuffix(build:test:run)` | Targets are grouped depending on their name, dropping the configured suffix (see example below). |
| `.notGrouped`                   | A scheme for each target.                                                                        |

For example, using `.byNameSuffix(build: [], test: ["Tests", "UITests"], run: "Demo")` and having targets: "A", "ATests", "AUITests", "ADemo", "B", then Tuist will generate:

- A scheme `A` having:
  - `A`, `ATests`, `AUITests`, `ADemo` in build action
  - `ATests`, `AUITests` in build action
  - `ADemo` in run action
- A scheme `B` having `B` in build and run action
