...
| `crane`             | `/opt/homebrew/bin/crane`         | Homebrew core, installed by `installBuilderTooling`. Used by the image-build workflows to write GHCR credentials into `~/.docker/config.json` for `tart push` (`crane auth login`), to serve the on-builder local OCI registry that backs the `tart push → skopeo copy` workaround (`crane registry serve --disk`), and by `release.yml`'s runner-image leg to resolve a pushed tag to its immutable digest (`crane digest`). |
| `skopeo`            | `/opt/homebrew/bin/skopeo`        | Homebrew core, installed by `installBuilderTooling`. Used by `macos-xcode-image.yml`, `runner-image.yml`, and `release.yml`'s `runner-image-build` matrix to copy multi-GB OCI artifacts from the on-builder local registry up to ghcr.io with the mature retry semantics `tart push` lacks (cirruslabs/tart#963). |
