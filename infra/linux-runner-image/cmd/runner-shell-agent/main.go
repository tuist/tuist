...
func lookupShellUser(name string) (shellUser, error) {
	...
	if found.Name == "" {
		return shellUser{}, fmt.Errorf("shell user %q missing", name)
	}

	found.Groups = supplementaryGroups(found.Name, found.GID)
	return found, nil
}

func supplementaryGroups(name string, primaryGID uint32) []uint32 {
