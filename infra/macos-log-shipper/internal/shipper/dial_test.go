...
// The production failure this guards: tailscaled on a Mac mini installs no
// resolver into macOS, so no MagicDNS name resolves through the OS at all.
// Every push failed with "no such host" for 2224 consecutive attempts while
// the daemon reported running with correct arguments.
