...
// host Kura over the vmnet bridge. The share carries only this endpoint, not a
// copy of the cache — the host Kura serves the account's data directly.
...
// bridge, and replicates with the account's EM peer over the PN. Because these
// processes are persistent, they survive across one-shot job VMs, which is what
// lets idle freshen/push work.
//
...
type KuraProcess interface {
	...
	// Exited reports whether the process has terminated (crashed, OOM-killed, or
	// Stopped). A host Kura that Exited is dead and must be restarted — Ready
	// alone cannot tell "still bootstrapping" from "died", so without this the
	// manager would hand back a corpse forever.
	Exited() bool
...
func (m *HostKuraManager) Ensure(ctx context.Context, accountID string) (port int, ready bool, err error) {
	...
	if existing, ok := m.procs[accountID]; ok {
		if !existing.proc.Exited() {
			existing.lastUsed = m.now()
			return existing.spec.Port, existing.proc.Ready(ctx), nil
		}
		// The process died (crash/OOM). It never self-heals because Ready stays
		// false forever, so respawn it on the SAME reserved port block rather
		// than returning the corpse.
		return m.restartLocked(ctx, existing)
	}
...
// restartLocked replaces a died process with a fresh one on the SAME reserved
// port block (the block stays owned by the account across the restart, so the
// VM-facing endpoint the reconciler wrote does not move). Caller holds m.mu.
func (m *HostKuraManager) restartLocked(ctx context.Context, mk *managedKura) (int, bool, error) {
	_ = mk.proc.Stop() // reap the corpse

	start := m.Start
	if start == nil {
		start = startHostKuraProcess
	}
	proc, err := start(ctx, m.KuraBinary, mk.spec)
	if err != nil {
		// Could not respawn: forget the account and free its block so a later
		// Ensure starts clean instead of looping on a corpse.
		delete(m.procs, mk.spec.AccountID)
		for off := 0; off < portsPerNode; off++ {
			delete(m.ports, mk.spec.Port+off)
		}
		return 0, false, fmt.Errorf("restart host kura for account %s: %w", mk.spec.AccountID, err)
	}
	mk.proc = proc
	mk.lastUsed = m.now()
	return mk.spec.Port, proc.Ready(ctx), nil
}
