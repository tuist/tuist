...
func (r *Reconciler) convergeMaster(vmName, statusDir, volumeName, account string) {
	...
	// Every path out of here records why. Convergence is the ONLY way a host
	// that holds no master for an account can obtain one: a cold job promotes
	// with base generation 0, which the server rejects whenever a HEAD already
	// exists, so the branch is discarded and the host stays cold. That makes a
	// silently-skipped convergence the difference between an account recovering
	// and its HEAD being frozen indefinitely, and until now every one of these
	// returns was silent.
	outcome := convergeOutcomeError
	defer func() { RecordVolumeConvergeOutcome(outcome) }()
	...
	switch {
	case head == nil:
		// The guest never staged the file within the wait. Distinct from the
		// cases below because the fix is the wait or the guest, not the server.
		outcome = convergeOutcomeHeadAbsent
		return
	case head.Generation <= 0:
		// The account has no HEAD yet: nothing published to converge toward.
		outcome = convergeOutcomeNoHead
		return
	case head.DownloadURL == "":
		// A HEAD exists but no URL came with it, which is what an untrusted
		// (fork) dispatch looks like: the server withholds the volume HEAD.
		outcome = convergeOutcomeNoDownloadURL
		return
	}
	...
	if local, err := r.Volumes.MasterGeneration(account, volumeName); err == nil && local >= head.Generation {
		outcome = convergeOutcomeAlreadyCurrent
		return
	}
	...
	if err := downloadMasterImage(head.DownloadURL, image); err != nil {
		outcome = convergeOutcomeDownloadFailed
	...
	if head.Digest != "" {
		if got, err := r.Volumes.ImageDigest(image); err != nil || got != head.Digest {
			outcome = convergeOutcomeDigestMismatch
	...
	if err != nil {
		outcome = convergeOutcomeInstallFailed
	...
	if !installed {
		...
		// the download was in flight; the generation gate declined the swap.
		outcome = convergeOutcomeSuperseded
		return
	}
	outcome = convergeOutcomeConverged
