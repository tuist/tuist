...
func startHostKuraProcess(ctx context.Context, binary string, spec KuraSpec) (KuraProcess, error) {
	...
	p := &execKuraProcess{
		cmd:      cmd,
		readyURL: "http://127.0.0.1:" + strconv.Itoa(httpPort) + "/ready",
		done:     make(chan struct{}),
	}
	// One goroutine owns cmd.Wait so the child is reaped exactly once. Its exit
	// closes done, which is how Exited() sees death and Stop() avoids a second
	// Wait.
	go func() {
		_ = cmd.Wait()
		close(p.done)
...
type execKuraProcess struct {
	cmd      *exec.Cmd
	readyURL string
	done     chan struct{} // closed by the reaper goroutine when cmd exits
}
...
func (p *execKuraProcess) Exited() bool {
	select {
	case <-p.done:
		return true
	default:
...
func (p *execKuraProcess) Stop() error {
	...
	// Signal, then wait for the reaper goroutine (which owns cmd.Wait) so the
