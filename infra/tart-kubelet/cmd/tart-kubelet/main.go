...
func main() {
	...
	flag.IntVar(&cacheVolumeCASGiB, "cache-volume-cas-gib", envIntOr("TART_KUBELET_CACHE_VOLUME_CAS_GIB", 0),
		"The Xcode compilation cache (CAS) is FOLDED into the cache image (a store dir beside the binary cache). "+
			"This is the CAS's allowance within that shared image: it sets the CAS's share of "+
			"--cache-volume-cap-gib, and the binary cache gets the rest minus a reserve (max(2 GiB, 20%)) that is "+
			"the room a job has before anything prunes, so the two pruners never over-commit the one image. The "+
			"guest is given all of it as COMPILATION_CACHE_LIMIT_SIZE, which already covers both generations a "+
			"store keeps. Persisted across VMs, riding the binary "+
