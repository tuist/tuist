...
Everything after first touch: the VLANs, management interface, services and port roles that apply to a rack's switches as a set. `mise run rack:fleet render|diff|apply|backup|drift` renders each switch's whole configuration from `sites/<site>.json`, compares it against the live device over SSH, and applies it one switch at a time in an order the site definition fixes by blast radius (ToR B, then ToR A which carries the WAN, then the management switch alone). `apply` refuses a switch whose predecessors are not already at the render, so the ordering is a precondition rather than a runbook line. Startup configs are committed, because the single management switch per rack is recovered with a cold spare plus a restorable config. See [`rack-switch-fleet/AGENTS.md`](rack-switch-fleet/AGENTS.md) for the three hardware checks the design rests on, what the render deliberately does not own, and the SSH session-table trap that wedges these units.
