...
- **Kura runtime (hosted)** pins the newest `kura@X.Y.0-canary.N` tag contained in the deployed commit (falling back to the newest stable for commits that predate canaries); a `kura/` change triggers the cascade like a server change. The `kura_runtime_image_tag` input overrides it for one deploy. Self-hosted Kura (the `kura` chart's `appVersion`, GHCR `:latest`, GitHub "Latest") still moves only on `kura-promote.yml`.
