...
Also carries the cluster's Cilium host policy, `fleet-node-ports-no-world` (`fleetNodePortsNetworkPolicy.enabled`). Bare-metal fleet nodes have a routable InternalIP and no host firewall, so it denies `world` the kubelet (10250/tcp), VXLAN (8472/udp) and cilium-health (4240/tcp) ports; 80/443 stay open because the regional gateways are hostNetwork ingress-nginx on the box's public IP. It selects nodes by `node.cluster.x-k8s.io/instance-type` existing rather than by a pool list, which is exactly the set cluster-api-provider-tuist's Linux self-join bootstraps, and admits peers as Cilium's `remote-node` identity rather than an address list. Enforced only where Cilium runs `enable-host-firewall: true`. Verify a rollout with `mise -C infra run k8s:verify-fleet-node-ports <context>` (`--baseline` first). The failure mode is a node that keeps reporting `Ready` with a dead pod network, so neither node nor pod readiness is used as evidence. The external port matrix proves `world` is denied by comparing every node against every port; a probe Job on a selected node proves everything else still works by sending real packets to a pod IP on another node, to a peer's 4240, and to the apiserver ClusterIP. It exits 0 when every check ran and passed, 1 on a failure, 2 when discovery or the baseline is missing, and 3 when a check could not run, so unproven never reads as verified. The probe Job needs create permission in its namespace, which the read-only kubectl tier lacks; the two corroborating Grafana queries need `PROM_URL`, `GRAFANA_USER` and `GRAFANA_TOKEN`.
