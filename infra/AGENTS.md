...
- **The ACL is not applied by CI.** A change to `acls.json` only takes effect once someone pastes it into the admin console's Access controls page. A staging kubectl 403 right after a deploy usually means that step is outstanding. This is an unclosed gap rather than a platform limit — Tailscale exposes the policy file at `GET`/`POST /api/v2/tailnet/<tailnet>/acl` and ships the `tailscale/gitops-acl-action` GitHub Action (`test` on PRs, `apply` on merge); it needs an OAuth client with the `policy_file` scope, which we do not have yet. The client tuist-ops uses is `users:read` only and cannot write the policy.
