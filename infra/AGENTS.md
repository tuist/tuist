...
- `.github/workflows/server-production-deployment.yml` — the monorepo release pipeline (push-on-main): releases the server + fleet/runtime images and, at its tail, runs the production deploy cascade (build → canary → acceptance tests → production, with hotfix fast-path). One serialized lane, no cross-workflow dispatch. Manual re-promotes/rollbacks of a pinned SHA go through `server-deployment.yml`'s own `workflow_dispatch`.
  - **`refs/deployments/server-production` is pipeline state, not a branch or a tag.** It points at the last commit the cascade shipped to production, and the path filter that decides whether to deploy diffs against it rather than against the push that triggered the run. That indirection is load-bearing: the workflow queues rather than cancels, GitHub keeps only one pending run per group, and a third merge evicts the pending one with zero jobs created — so a per-push diff silently drops whatever was in the evicted push (it did, on 2026-09-08, for a CAPI CRD change that then wedged the sa-west MachineDeployment). Deleting the ref is safe but costs a redundant cascade: the gate deploys when it cannot resolve it. Manual re-promotes and rollbacks through `server-deployment.yml` deliberately do not move it.
