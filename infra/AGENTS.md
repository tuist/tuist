...
`internal/podagent/volume*.go` owns the **per-account cache volumes** (spec #76), materialized after dispatch: a VM boots with an empty per-VM branch share, and once the server stamps the account label the host APFS-clonefiles that account's master into it. A master is a sparse disk image (`<account>/<volume>/master.sparseimage` + a `master.digest` sidecar), not a directory tree — virtio-fs cannot carry a macOS cache faithfully (no xattrs on symlinks, which versioned framework bundles and the CLI's artifact signatures both need), so exactly one file crosses the share and the filesystem inside it is real APFS. Consequences worth knowing before editing: the host cannot see into an image, so `MasterDigest` reads the sidecar and the guest reports the digest it computed while mounted; `ImageDigest` is the only place the host looks in, via a read-only attach with a deferred detach (a leaked attach pins the file against LRU eviction); and `Materialize` must leave an image at the branch on **every** path (an empty one on failure), because the guest cannot attach what isn't there and a missing image kills the job rather than costing it warmth. The guest attaches with `-owners off`, which is why no host-side chmod of the cache tree is needed. Off unless `--runner-cache-volume-gib` > 0.

### `registry-router/` — Cloudflare Worker for the Tuist registry path
Routes `tuist.dev/api/registry/*` to the health-checked standalone registry frontend at `registry.tuist.dev`. Regional cache nodes remain automatic fallback origins during the cutover soak period. The ingress hostname is an origin, not a separately advertised registry endpoint.
