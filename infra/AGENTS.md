...
- `mgmt/preview-mgmt-rbac.yaml` — narrow SA + Role on the mgmt cluster used by the preview-deploy workflow to scale the preview MachineDeployment.
...
A macOS Pod's phase and Ready condition are synthesized from "the VM process is alive and has an IP" — there are no container probes here — so nothing in Kubernetes can tell a warm runner from one whose guest died, and warm standby is deliberately unbounded. `publishRunnerHeartbeat` closes that for runner Pods specifically: `dispatch-poll.sh` beats its state into the per-VM status share every poll (`polling` while warm, `claimed` once it takes a job, with the file's mtime as the beat), and the host republishes it as the `tuist.dev/runner-heartbeat-state` / `tuist.dev/runner-heartbeat-at` Pod annotations for the runners-controller's `isWarmCapacity`. Deliberately annotations and not the Ready condition: readiness gates rollouts and drains fleet-wide, and a false negative there would stall both. The timestamp is throttled to one write a minute per Pod (a state change always publishes immediately), and once published the annotations are never withdrawn — clearing them when the guest stops beating would turn the one state worth reporting back into "no signal", which is the reading that fails open. Absence means the host cannot speak for the guest (no status share, or a pre-heartbeat runner image), never that it is dead.

`internal/podagent/volume*.go` owns the **cache volumes** (spec #76), one master per (account, volume), materialized after dispatch. A job's volume is its repository's, `repo-<first 16 hex of sha256(lowercased repository)>`, or the account-wide `tuist-cache` for a job with no repository. A VM boots with an empty per-VM branch share, and once the server stamps the account and `tuist.dev/runner-cache-volume` labels (one patch) the host APFS-clonefiles that volume's master into it. A repository volume with no master on the host is seeded from the account's `tuist-cache` master at base generation 0 (`materialize_total{result="seeded"}`), so its promote can only establish the volume's first HEAD; the seed source is not touched for LRU, so old account masters age out. Admission never evicts the master being cloned. A malformed volume label is treated like an untrusted job (empty image, never promotes). The host advertises `tuist.dev/cache-volumes-per-repository` on its Node, and the server stamps a repository volume only on Pods whose Node carries it; a host on an older tart-kubelet keeps every job on `tuist-cache`. Stamping is also gated server-side by the `runner_cache_volumes_per_repository` flag in canary and production (`FeatureFlags.runner_cache_volumes_per_repository_enabled?/0`): replicas of an older server resolve every promote and upload URL under `tuist-cache`, so stamping while a rollout is in flight could publish a HEAD whose object sits under the other volume's prefix. Reads of the label are never gated, so flipping the flag back off leaves Pods already stamped consistent. A master is a sparse disk image (`<account>/<volume>/master.sparseimage` + a `master.generation` sidecar), not a directory tree — virtio-fs cannot carry a macOS cache faithfully (no xattrs on symlinks, which versioned framework bundles and the CLI's artifact signatures both need), so exactly one file crosses the share and the filesystem inside it is real APFS. Reconciliation is **fast-forward last-writer-wins**: promote and converge both `InstallMaster` — a whole-image CoW replace gated so the local master only ever moves to a NEWER generation. The generation is the version marker both compare against, and the compare-and-swap that prevents a stale writer from clobbering a newer master lives in the server's HEAD bump (`VolumeHeads.bump_head/5`), not on the host; the host only relays a base generation up (staged for the guest at materialize) and installs the branch on an accepted fast-forward (the guest writes the accepted generation back to the status share). Consequences worth knowing before editing: `MasterGeneration` reads the sidecar and is honored only while the master image exists; `ImageDigest` is the only place the host looks inside an image, via a read-only attach with a deferred detach (a leaked attach pins the file against LRU eviction), used solely to verify a downloaded HEAD matches its advertised digest before adopting it; and `Materialize` must leave an image at the branch on **every** path (an empty one on failure), because the guest cannot attach what isn't there and a missing image kills the job rather than costing it warmth. `InstallMaster` grows every master it installs to `CapGiB` (`growImage`, which reads `hdiutil resize -limits` first because `hdiutil resize -size` silently shrinks a larger image), so a master from before the cap was raised reaches the new cap at the account's next promote or convergence. Growing takes an attach and about a second, which is why it happens there and not in `Materialize`, which a starting job waits on; a master that cannot be grown is installed at the size it has. The guest attaches with `-owners off`, which is why no host-side chmod of the cache tree is needed. Off unless `--runner-cache-volume-gib` > 0. **Convergence is the only way a host holding no master for a volume can obtain one** — a cold job promotes with base generation 0, which the server rejects whenever a HEAD already exists, and `Finalize` then discards the branch — so every path out of `convergeMaster` logs why it declined. That also makes a HEAD whose stored object does not reproduce its advertised digest self-sustaining: every host declines it, so none holds that generation, and no promote can ever build on it — a cold host promotes with base 0 and a host left at an older generation promotes with a base the HEAD has moved past, so the fast-forward rejects both, and the account is frozen fleet-wide until the HEAD is retired. Two things keep it from happening and one lets it heal: the guest measures the digest it publishes on the DETACHED image (a read-only re-attach, so the digest describes the bytes it uploads rather than a live mount a straggler can still write past); `convergeMaster` distinguishes being unable to MEASURE the downloaded image (a local fault, declines quietly) from measuring a DIFFERENT one (proof about the object, reproducible on every host); and only in that second case it stages the disproved digest as `volume-head-unverifiable` in the status share, which the guest relays with its promote so `VolumeHeads.bump_head/6` lets that promote retire the lineage at `generation + 1` once it has lost the ordinary rule — from either base, since a cold host and a stale-warm one are wedged identically and the poisoned generation is typically held by nobody. The digest check itself is never softened — it is what stops a corrupt master propagating fleet-wide. At VM create the host stages the budget the binary cache and the compilation cache share inside the image (`cache-budget-bytes`, `cacheImageBudget`: the cap less a reserve of max(2 GiB, 20% of the cap) that is the room a job grows into), which the guest divides between the two by what each holds; beside it stays the fixed split (`cache-max-bytes`, and the `cas-enabled` figure, from `cacheImageSplit`) that runner images older than that division apply, because tart-kubelet and the runner image release and roll out independently. `--cache-volume-cas-gib` therefore only turns the folded compilation cache on and sizes that fixed split. The guest stages each division back in `cache-limits`, which `readCacheLimits` turns into `tart_kubelet_cache_volume_cache_bytes` and `tart_kubelet_cache_volume_cache_limit_bytes` (labelled by cache and by end of job) at Finalize: the per-cache sizes are the only ones the fleet measures, and they are what the division's rule and floors are retuned from, since the runner log they are also written to is re-emitted only as a bounded tail. The host also stages its own Node name (`node-name`, written at VM create beside the budget) for the guest to relay on promote, so `runner_volume_heads.node_name` records which host published each generation — attribution only, read by nothing in the fast-forward, but the first thing you want when a lineage has to be retired and the host still holding that master has to be found. The three that used to share one silent `return` are split apart because they have nothing in common: the guest never staged the HEAD (the guest, or the wait), the account has published no HEAD yet (nothing to adopt), and the HEAD arrived without a download URL (the server withholding it from an untrusted job). The host also **advertises which masters it holds** to the server: `CacheMasterNodeLabels` scans the same directories eviction does and publishes one Node label per resident master, `tuist.dev/cache-master-<account_id>` for a `tuist-cache` master and `tuist.dev/cache-master-<account_id>.<volume>` for a repository's, through the node maintainer's `DynamicLabels` hook (the same hook that advertises golden base VMs), and the server's dispatch reads them to hand a polling runner a job whose cache is already there. Master directories are named after the account id and volume the server stamps on the Pod, so the labels need no translation. This is deliberately a report of what is on disk rather than something the server models: an admission decline, a watermark eviction, or a reprovisioned host all change residency without the server issuing a dispatch, and only the host can see that.
...
Console-driven bring-up for the rack's TP-Link switches: hostname, fixed management address, SSH, config saved. Run as `mise run rack:prep-switch <switch>` with a USB-C cable to the switch's console port, because a factory switch sits on 192.168.0.1 with SSH off and collides with the gateway of whatever network it is unboxed on. The inventory is the site definition in [`rack-switch-fleet/sites/`](rack-switch-fleet/sites/ber1.json); `mise run rack:ztp <device> --interface <iface>` serves DHCP and TFTP on an isolated segment so a switch provisions itself through Auto Install, refusing to run on the interface carrying the default route. `mise run rack:prep-test` covers everything reachable without a cable, and runs in CI; see [`rack-switch-prep/AGENTS.md`](rack-switch-prep/AGENTS.md) for the console baud rate, the CLI's input quirks, and the firmware-line trap that bricks these units.

### `rack-switch-fleet/` — config as code for BER1 rack switches
Everything after first touch: the VLANs, management interface, services and port roles that apply to a rack's switches as a set. `mise run rack:fleet render|diff|apply|backup|drift` (shell and ssh, tested with `mise run rack:fleet-test`) renders each switch's whole configuration from `sites/<site>.json`, compares it against the live device over SSH, and applies it one switch at a time in an order the site definition fixes by blast radius (ToR B, then ToR A which carries the WAN, then the management switch alone). `apply` refuses a switch whose predecessors are not already at the render, so the ordering is a precondition rather than a runbook line. `sites/<site>.json` also carries the rack's port map as the join between `nodes` and switch ports, so a port's config follows from the role of whatever is plugged into it and racking a machine is a data edit; `mise run rack:fleet ports` prints it. Nodes cover appliances (KVMs, PDU, ATS card) as well as machines, and `node_models.json` carries the NIC facts that let the render refuse a management link on a socket without out-of-band (AMT rides the MS-01's i226-LM only). Each switch also renders as a `RackSwitch` (`tuist.dev/v1alpha1`, CRD in `helm/tuist/crds/`) so its drift, reachability and remaining SSH connection budget are visible beside the RackHosts behind it; spec comes from the same site definition and status is pushed by `mise run rack:fleet publish`. Nothing polls the switches over SSH: a switch accepts about eight to ten connections per boot even spaced ten minutes apart, so any observation loop would wedge it. The controller path is the Omada controller, whose Open API writes everything rendered on 6.3 and needs no SSH. `mise run rack:fleet sessions <device> [tid]` lists the switch's terminal lines and frees a leaked one, which is the recovery for a wedged SSH daemon that does not involve power. Startup configs are committed, because the single management switch per rack is recovered with a cold spare plus a restorable config. Zero touch ran end to end on 2026-09-22: `rack:ztp` serves the rendered config with the login and the fleet key download, Auto Install fetches, saves and reboots it, and `rack:fleet save` seals the verbatim save; whether a factory-fresh unit starts Auto Install by itself is still open. `apply` is a confirmed commit: it arms a no-save reboot, applies, verifies and only then cancels and saves. The TFTP config export was confirmed to be text on 2026-09-21, so `mise run rack:fleet replace <device> [--reboot]` pushes a switch's whole configuration and verifies it; it merges the switch's current unmanaged lines in first, because the render omits the admin login and pushing it bare would delete the account used to log in. It ran end to end on both ToRs. See [`rack-switch-fleet/AGENTS.md`](rack-switch-fleet/AGENTS.md) for the three hardware checks the design rests on, what the render deliberately does not own, and the SSH session-table trap that wedges these units. The rack belongs to the staging cluster while it is at home (production once it moves): its `RackSwitch` objects are applied to `tuist-staging` on merge, `rack:ztp --via` serves zero-touch provisioning from the edge node, the edge node is a staging node (a `RackLinuxMachine`, see `rack-nodes/`) whose rack-edge pod routes the switches into the tailnet and serves their DHCP, and `rack:omada` adopts them into the Omada controller in staging.

### `rack-nodes/`: install sticks for the rack's Linux machines
`mise run rack:write-install-usb <disk> --host <node>` writes an Ubuntu stick that installs a rack x86 machine (edge, services; storage once its layout exists) with no keystrokes and joins it to the tailnet on first boot with a single-use tagged key minted from the `TAILSCALE_RACK_NODES` OAuth client. The host is declared in `rackLinuxFleet.hosts` of the tuist chart values, rendered as a `RackLinuxHost`, and the CAPI provider's `RackLinuxMachine` joins it to the cluster over the tailnet and keeps it converged. See [`rack-nodes/AGENTS.md`](rack-nodes/AGENTS.md).

### `rack-switch-controller/`: the RackSwitch reconciler, through the Omada controller
Go controller that adopts each `RackSwitch` with `managedBy: controller` into the site's Omada controller and writes its configuration through the Open API: the management address, hostname, port descriptions and spanning-tree mode, and behind flags that default off, VLANs, LAGs, port spanning tree and site services. It writes only when a switch's revision or generation moves or it has just been adopted, one switch at a time in the site's apply order, and between changes it reads every ten minutes and reports drift without writing over it. It never opens SSH. The RackSwitch CRD is generated from its Go types (`mise run rack-switch-controller:generate`). Deployed beside the Omada controller by `.github/workflows/omada-deployment.yml` from [`helm/rack-switch-controller/`](helm/rack-switch-controller/values.yaml), never onto a node behind the switches it reconciles. See [`rack-switch-controller/AGENTS.md`](rack-switch-controller/AGENTS.md).

### `helm/rack-edge/`: the rack edge node's DaemonSet
One pod on each rack's edge node, which registers with a `tuist.dev/rack-edge=<site>` label and taint (`rackLinuxFleet.roles.edge`): it installs the path from the switches' management addresses into the tailnet (addresses, host routes, an nft table translating into tailscale0 with an MSS clamp) and serves their DHCP with the Omada controller in option 138, which is what makes a factory switch zero touch. Host networking with NET_ADMIN, not privileged; the node carries `cilium.io/no-schedule=true` because its own networks sit inside the pod CIDR, and the Cilium agent stays off it. The files it runs are rendered from the site definition by `rack:fleet render` into `sites/<site>/`, checked in CI. Its image is `rack-switch-fleet/edge/Dockerfile`. Deployed beside the Omada controller by `.github/workflows/omada-deployment.yml`. See [`rack-switch-fleet/AGENTS.md`](rack-switch-fleet/AGENTS.md), "The edge node".
...
Two doors, one identity. **Canary and production** go through Pomerium fronting `https://kube-<env>.tuist.dev`, carrying the team's Google Workspace identity. **Staging** goes straight over the tailnet to `https://tuist-k8s-staging.taild6d7bb.ts.net` — Tailscale's in-process API server proxy on the staging operator, running in auth mode — with no browser step and no `pomerium-cli`; being on the tailnet is the authentication, and the impersonated groups come from the `tailscale.com/cap/kubernetes` grants in [`tailscale/acls.json`](tailscale/acls.json) rather than from tuist-ops. If a staging `kubectl` fails to resolve its host, the answer is `tailscale status`, not a login. Everything below about tiers applies to both doors — they land on the same groups and the same ClusterRoles.

From an agent running on a teammate's laptop, `kubectl --context tuist-k8s-<env>` works for any read operation: `get pods`, `logs`, `describe`, `get configmap`, `get events`, `get nodes`, the cluster-scoped infra CRs (`scalewayelasticmetalmachines`, `machinedeployments`, `kurainstances`, `customresourcedefinitions`), and RBAC objects (`get clusterrole`, `get rolebinding`) for diagnosing authorization. The `pomerium-cli` exec credential plugin handles the bearer; the teammate's existing browser-cached session is used silently. Each request then flows through the `kube-impersonator` sidecar in the Pomerium pod, which calls tuist-ops's `/api/v1/policy` over the tailnet; the policy returns `Impersonate-User: <email>` + `Impersonate-Group: tuist-eng` (or `tuist-admins` for Owner/Admin tailnet roles). RBAC binds those groups to `view`, which **excludes `Secret`s** — deliberate, so `MASTER_KEY`, `DATABASE_URL`, and ESO-synced secrets stay out of agent context. The upstream `view` role also omits cluster-scoped and custom resources, so the `tuist-view-infra-read` aggregated ClusterRole (in [`helm/pomerium/templates/access-tiers.yaml`](helm/pomerium/templates/access-tiers.yaml)) extends it with `get`/`list`/`watch` on `nodes`, `persistentvolumes`, CAPI machines (`cluster.x-k8s.io`, `infrastructure.cluster.x-k8s.io`), Kura CRs (`kura.tuist.dev`), CRDs, and RBAC objects (`rbac.authorization.k8s.io`). It aggregates into `view` via a label (no extra binding) and adds no `Secret` rule, so the exclusion above is unaffected. RBAC read is the one deliberate departure from upstream `view` (which hides the authz graph as anti-recon); accepted here because containment is on mutation, not reads, and it lets agents diagnose a misbound/`NotFound` RBAC object during a deploy.

For anything an agent typically needs (looking at a failing pod, tailing logs, sanity-checking a deploy), this is the right path. Use it freely. The one-time local kubeconfig setup (staging's tailnet entry, plus `pomerium-cli` and the two gateway contexts for canary and production) is in [`k8s/onboarding.md` → Engineer read access](k8s/onboarding.md#engineer-read-access-pomerium-kubeconfig).

### Writes: staging is open, canary and production are not

**Staging is writable with no elevation.** tuist-ops's policy response adds `tuist-staging-write` to every engineering identity's impersonation headers on `kube-staging.tuist.dev` — no request, no second human, no TTL. `kubectl --context tuist-k8s-staging apply/delete/scale/patch` succeeds, as do the capabilities bound straight to the write group (`tuist-fleet-unwedge`, `tuist-fleet-scale`, `tuist-volume-decommission`, `tuist-cnpg-decommission`) and the `edit`-aggregated live-debug verbs (`kubectl debug`, `nodes/proxy`). This is deliberate: staging serves no customers, and gating a pod restart there behind a second human cost more than the containment it bought. It is not a licence to skip the usual care — staging is what acceptance tests and Slack-requested previews run against, so scope the change and say what you did. `/elevate staging` is rejected by the bot: there is nothing left to grant.

**On canary and production, every mutating operation** (`delete pod`, `apply -f`, `scale deployment`, `patch`, `create` of any kind) **returns `403 Forbidden` by default.** This is the agent-containment property the design protects — an agent with a teammate's Pomerium session cannot wreck the envs that serve customers.

The bot's `/elevate <env> [duration] <intent>` Slack flow is the path to write access. **It is not an agent-driven path.** `TuistOps.JIT.Policy` currently allows Owner/Admin and Member tailnet roles alike to self-approve canary and production, but that policy assumes the click comes from a human at a keyboard. An agent that drives both `/elevate` and the Approve click via the same workstation's Slack token has defeated the entire design. Don't do it.

Concrete rules for agents:

- **Do not invoke `/elevate` autonomously.** If you think a canary or production write is needed, surface that to the human ("this would require elevation; can you run `/elevate ...` and grant the access?") and let them drive both the request and the approval.
...
Staging writes produce **no** elevation record — there is no elevation behind them. The Pomerium access log below is their only trail, and it is per-request and complete, so a staging mutation is still attributable to an email and a timestamp. Every canary/production elevation produces records in three independent stores:
...
- Tailnet ACL: [`tailscale/acls.json`](tailscale/acls.json). Static; humans edit it through code review. View-tier only for canary and production — their `edit` tier is a per-request elevation the gateway resolves, which a static ACL cannot express. For **staging** the ACL is the tier decision: its `tailscale.com/cap/kubernetes` grants hand out `tuist-staging-write` alongside the view group. The `tuist-admins` / `tuist-eng` / `tuist-<env>-write` strings are *Kubernetes-side* impersonation labels, bound to ClusterRoles by `infra/helm/pomerium/templates/access-tiers.yaml` — that file now serves both doors, so it outlives any future Pomerium teardown in staging.
- **The ACL is not applied by CI, on purpose.** A change to `acls.json` only takes effect once someone pastes it into the admin console's Access controls page. A staging kubectl 403 right after a deploy usually means that step is outstanding. Tailscale does have both an API and an official GitOps action for this; we deliberately don't use them, because the `policy_file` scope is whole-file and a CI identity holding it could grant itself `tuist-production-write` — the thing the JIT flow exists to gate. The reasoning and the conditions for revisiting are in the header of [`tailscale/acls.json`](tailscale/acls.json). Don't "fix" this by wiring up the action.
