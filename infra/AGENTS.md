...
### `egress-qdisc-init/` — per-pod egress ceiling init container
Tiny alpine+iproute2 init container image the kura-controller renders into KuraInstance pods that carry the `kubernetes.io/egress-bandwidth` annotation. It installs the tenant's egress ceiling as a root qdisc inside the pod netns, closing the node-local read-path bypass of Cilium's NIC-level pacing (tuist/tuist#12363). See `egress-qdisc-init/AGENTS.md`.
