...
Go kubelet-shaped agent that registers a Mac mini as a Kubernetes Node and maps Pods to Tart VMs. Having no per-container CRI to source them from, it synthesizes the Pod's `containerStatuses` itself: running while the VM serves, and terminated (exit code, signal, `finishedAt`) from the `tart run` process when it dies. Both are load-bearing rather than cosmetic — the running ones drive the `kubectl` READY column, and the terminated ones are the only post-mortem a macOS runner leaves behind, read by the runners-controller's reap-time fingerprint and its billing "stopped at". Interactive runner VNC is host-controlled here: runner VMs start with Tart's host-owned experimental VNC enabled, generated VNC credentials stay in host-control state, and a relay opens only while either a legacy host-local `vnc-control-dir/requests/<namespace>_<pod>` file exists or the server stamps its session annotation on the runner Pod.
