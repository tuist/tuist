...
- **The ACL is not applied by CI.** Tailscale has no public API for policy sync, so a change to `acls.json` only takes effect once someone pastes it into the admin console's Access controls page. A staging kubectl 403 right after a deploy usually means that step is outstanding.
