...
# Build the Tart VM image that bundles macOS + a specific Xcode + the
# dev tools customer iOS/macOS workflows depend on. This is our
# in-house replacement for `ghcr.io/cirruslabs/macos-tahoe-xcode:N` —
# the goal is to be on a new Xcode the day Apple ships it instead of
# waiting for Cirrus's catalog to catch up.
#
# This image is the *base layer*. Two downstream images inherit from
# it and add the Tuist-specific runtime:
...
# the Xcode-install tax on every CI rebuild; it just re-clones this
# image and lays the runner agent on top in ~2 min.
#
...
# `xcodes download <version>` against developer.apple.com using that
# session and passes the resulting .xip path here.
...
variable "output_image" {
  type        = string
  description = "Output image name. The push tag uses xcode_version's major-minor (e.g. `:26-4`)."
...
variable "xcode_version" {
  type        = string
  description = "Xcode version installed from the .xip (e.g. \"26.4.1\"). Names /Applications/Xcode_<version>.app and drives the major-minor alias."
}
...
# -downloadAllPlatforms` pulls ~15 GB of simulator runtimes. 140 GB
# matches what Cirrus provisions for their equivalent image and
# leaves headroom for the downstream Layer 2 builds + customer
...
locals {
  ...
  # Used for the second /Applications/Xcode_<…>.app alias so repos
  # pinning either `.xcode-version=26.4` or `.xcode-version=26.4.1`
  # resolve to the same bundle (same nicety GitHub-hosted images
  # expose — see images/macos/macos-26-arm64-Readme.md#xcode).
...
build {
  ...
  # Install xcodes + aria2 (xcodes' parallel-download backend, even
  # though we install from a local .xip here — xcodes still imports
  # aria2 for signature verification). macos-tahoe-base wires brew's
  # shellenv into /Users/admin/.zprofile, so `source` here is what
  # makes `brew` reachable on the next line.
  provisioner "shell" {
    inline = [
      "set -euo pipefail",
      "source ~/.zprofile",
  ...
  # Stage the .xip inside the VM. SSH transfer of a ~10 GB blob over
  # Tart's loopback runs in a few minutes on the bare-metal builder.
  provisioner "file" {
    source      = var.xcode_xip_path
    destination = "/Users/admin/Downloads/Xcode.xip"
  }

  # Install Xcode from the staged .xip. `--experimental-unxip` swaps
  # the single-threaded `xip` extractor for a parallel one — the
  # slow phase of any Xcode install. `--select` switches the active
  # developer dir so /usr/bin/xcrun resolves against the new bundle.
  # `--empty-trash` reclaims the .xip from the trash bin (xcodes
  # moves the source .xip there on success, which without this flag
  # blows past the disk budget on the next install).
  #
  ...
  # Major-minor alias so repos pinning either `.xcode-version=26.4`
  # or `.xcode-version=26.4.1` find the same bundle. The conditional
  # skips the symlink if xcode_version is already in major-minor form
  # (no patch), which would create a self-referential link.
  provisioner "shell" {
    inline = [
      "set -euo pipefail",
