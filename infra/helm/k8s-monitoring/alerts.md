...
- `upload_memory`, `memory_pressure_write`: the transient memory budget, which
  ...
  fires first.
  - `upload_memory` borrows the elastic ceiling headroom before it queues and
    keeps retrying it while queued, so on current versions it means an upload
    waited 30 seconds with the floor and that headroom both spent, or with
    pressure above normal closing the borrow.
    `kura_memory_actions_total{action="foreground_upload_admission_wait_borrowed"}`
    counts queued uploads the headroom admitted. Read `kura_memory_elastic_transient_reserved_bytes` against
