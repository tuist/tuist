...
- `k8s-monitoring/kura-rollout-alert-rules/` holds the two Kura rollout rules as
  gcx manifests. They are pushed with `gcx resources push -p` on that directory only,
  never applied on merge, and are read-only in the Grafana UI (provenance `api`).
