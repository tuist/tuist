...
// TuistClusterSpec is mostly empty — the Mac minis join
...
// TuistClusterStatus is similarly minimal.
type TuistClusterStatus struct {
...
// +kubebuilder:resource:path=tuistclusters,scope=Namespaced,categories=cluster-api,shortName=tcl
// +kubebuilder:printcolumn:name="Endpoint",type=string,JSONPath=".spec.controlPlaneEndpoint.host"
// +kubebuilder:printcolumn:name="Ready",type=boolean,JSONPath=".status.ready"

// TuistCluster is the cluster-level stub the CAPI Cluster
// resource references via `infrastructureRef`.
type TuistCluster struct {
	metav1.TypeMeta   `json:",inline"`
	metav1.ObjectMeta `json:"metadata,omitempty"`

	Spec   TuistClusterSpec   `json:"spec,omitempty"`
	Status TuistClusterStatus `json:"status,omitempty"`
}

// +kubebuilder:object:root=true

// TuistClusterList is a list of TuistCluster.
type TuistClusterList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []TuistCluster `json:"items"`
}

func init() {
	SchemeBuilder.Register(&TuistCluster{}, &TuistClusterList{})
}

func (c *TuistCluster) GetConditions() clusterv1.Conditions {
	return c.Status.Conditions
}

func (c *TuistCluster) SetConditions(conditions clusterv1.Conditions) {
