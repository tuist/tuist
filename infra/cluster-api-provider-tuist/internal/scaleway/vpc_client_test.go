...
func TestEnsurePN_FindsExisting(t *testing.T) {
	f := &fakeVPC{list: &vpc.ListPrivateNetworksResponse{PrivateNetworks: []*vpc.PrivateNetwork{{ID: "pn-1", Name: "tuist-runner-cache"}}}}
	id, err := newVPC(f).EnsurePrivateNetworkByName(context.Background(), "fr-par", "tuist-runner-cache", "172.16.0.0/22")
	if err != nil {
...
func TestEnsurePN_CreatesWhenMissing(t *testing.T) {
	f := &fakeVPC{
		list:    &vpc.ListPrivateNetworksResponse{},
		created: &vpc.PrivateNetwork{ID: "pn-new", Name: "tuist-runner-cache"},
	}
	id, err := newVPC(f).EnsurePrivateNetworkByName(context.Background(), "fr-par", "tuist-runner-cache", "172.16.0.0/22")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	...
	if f.createCalls != 1 || f.lastCreate.Name != "tuist-runner-cache" || f.lastCreate.ProjectID != "proj" {
...
func TestEnsurePN_ExactMatchOnly(t *testing.T) {
	...
	f := &fakeVPC{
		list:    &vpc.ListPrivateNetworksResponse{PrivateNetworks: []*vpc.PrivateNetwork{{ID: "pn-old", Name: "tuist-runner-cache-old"}}},
		created: &vpc.PrivateNetwork{ID: "pn-new", Name: "tuist-runner-cache"},
	}
	id, err := newVPC(f).EnsurePrivateNetworkByName(context.Background(), "fr-par", "tuist-runner-cache", "172.16.0.0/22")
	if err != nil {
...
func TestEnsurePN_AmbiguousErrors(t *testing.T) {
	f := &fakeVPC{list: &vpc.ListPrivateNetworksResponse{PrivateNetworks: []*vpc.PrivateNetwork{
		{ID: "pn-1", Name: "tuist-runner-cache"}, {ID: "pn-2", Name: "tuist-runner-cache"},
	}}}
	if _, err := newVPC(f).EnsurePrivateNetworkByName(context.Background(), "fr-par", "tuist-runner-cache", "172.16.0.0/22"); err == nil {
...
func TestEnsurePN_Caches(t *testing.T) {
	...
	c := newVPC(f)
	for i := 0; i < 3; i++ {
		if _, err := c.EnsurePrivateNetworkByName(context.Background(), "fr-par", "tuist-runner-cache", "172.16.0.0/22"); err != nil {
