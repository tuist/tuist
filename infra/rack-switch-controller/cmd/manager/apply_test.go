...
func TestReadRackSwitchNeedsAMAC(t *testing.T) {
	committed, err := os.ReadFile(filepath.Join("..", "..", "..", "rack-switch-fleet", "k8s", "ber1", "ber1-tor-b.yaml"))
	if err != nil {
		t.Fatal(err)
	}
	var kept []string
	...
	path := filepath.Join(t.TempDir(), "ber1-tor-b.yaml")
	if err := os.WriteFile(path, []byte(strings.Join(kept, "\n")), 0o600); err != nil {
		t.Fatal(err)
	}
	_, err = readRackSwitch(path)
