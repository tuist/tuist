...
- **Snapshot pre-materialization**: when a snapshot lands, the proxy queues materialization of every graph it describes on a dedicated small pool (demand-priority jobs never queue behind it) — bulk content warming with no local state. This replaced the deleted read-ahead/keylog mechanism, which pre-resolved the PREVIOUS build's key list from a machine-local log: that was memoization of a prior local build faking a remote warm pull, useless on a cold machine, and the serial-demand problem it mitigated (measured 702s warm vs an 88s floor at ~60ms RTT) is now solved structurally by the snapshot plus non-blocking resolve.
