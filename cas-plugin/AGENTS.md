...
- **Runner regime caveat**: with the local CAS wiped between builds (ephemeral runner VMs without cache volumes), warm-build time on the runner VMs is gated by a ~240s single-core build-service process (scanning + ingest). Attribution (2026-07-11): it is NOT remote transport — a probe with the proxy killed and read-ahead off measured 248.1s ± 9.7 vs 240.2s ± 2.5 with the full remote path, and the stock Apple dylib under the same methodology lands in the same band (233-258s), all WORSE than no-cache (193.6s). The same work runs at ~3.2-3.6 cores with a kept CAS on the same VM (75.5s ≈ the 77-83s floor) and on a dev Mac with a wiped CAS, so the serialization is specific to plugin-mode caching against a cold store on those VMs — root cause under investigation (mid-build samples + CPU-topology dumps wired into the runners-benchmark). Until it is fixed, a persistent `CompilationCache.noindex` (per-account runner cache volumes) is what holds runners at the floor; historically the same design measured near-floor on a wiped CAS (113.6s vs 105.5s), so the wall is environmental, not architectural.
