...
- **Read-ahead** (proxy): the proxy records every demanded action key per local CAS path, in demand order, to `<state dir>/cas-keylogs/`, and at the start of the next build (first resolve; re-armed after a wipe or prune) replays the whole set through `resolve` on a worker pool (`TUIST_CAS_READAHEAD`, default 64, `0` disables). The demand path is serial — the build system and compilers ask one key at a time — so on a real-RTT link a warm build otherwise degenerates into a chain of round trips (measured 702s warm vs an 88s local floor at ~60ms RTT; with read-ahead 103-119s vs the daemon mode's 193-211s on the same interleaved runs). Two invariants make it work: `resolved` map entries survive `invalidate` (each cached Hit is presence-verified on disk before it is served, so pruned values self-heal per key — the build system prunes several times per build, and clearing the map on each prune threw the wavefront's work away), and the keylog preserves demand order so the wavefront stays ahead of the build.
...
- **Runner regime caveat**: with the local CAS wiped between builds (ephemeral runner VMs without cache volumes), warm-build time is dominated by re-scanning and re-ingesting the input closure inside the build service (~236s of scanner CPU on an 8-core VM for the CLI fixture), not by remote transport. The lever there is a persistent `CompilationCache.noindex` (per-account runner cache volumes), with the remote as backstop; read-ahead on a truly ephemeral machine also needs the keylog distributed through the remote (follow-up).
