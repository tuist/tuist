...
//!
//! The broker is not bound to a single project: it multiplexes REAPI clients
//! per `account/project` instance, which each connection declares (or which
//! the broker recalls from the persisted registry for an Xcode ⌘B build).
//!
//! Environment:
...
//!   machine: the broker fetches one via the CLI (see TUIST_CAS_TUIST_BIN).
...
//! - TUIST_CAS_BROKER_REGISTRY: path to persist the cas_path->instance map
...
// long-lived brokers stay ahead of expiry without shelling out every tick.
...
fn main() {
    let socket_path = std::env::var("TUIST_CAS_BROKER_SOCKET")
        ...
        .unwrap_or_else(tuist_cas_plugin::default_broker_socket);
    ...
    let registry_path = std::env::var("TUIST_CAS_BROKER_REGISTRY")
