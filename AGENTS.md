...
- `server/native/xcresult_nif/` - Swift NIF for xcresult parsing (macOS-only — uses `xcresulttool`). Ships in the macOS server release that the xcresult processor Tart VM (`infra/xcresult-processor-image/`) downloads at boot; the VM runs as a k8s `Deployment` scheduled onto a Mac mini node via tart-cri.
