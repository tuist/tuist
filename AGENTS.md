...
- `infra/cache-gateway/` - Go service that terminates the GitHub Actions cache v2 protocol (Twirp coordination + Azure Block Blob transfer) inside a runner fleet and translates it to S3 against a co-located SeaweedFS cluster. One instance per fleet, fronted by `runner-cache-proxy`. See `infra/cache-gateway/AGENTS.md`.
- `infra/runner-cache-proxy/` - Go host-side interception proxy (Mac mini / Hetzner node, never the guest). pf/nftables DNAT redirects guest `:443`; it SNI-routes the GitHub Actions cache plane to the local `cache-gateway` with a host-side token swap and blind-splices everything else, failing open to GitHub. See `infra/runner-cache-proxy/AGENTS.md`.
