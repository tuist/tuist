...
- `infra/sandboxd/` - Per-node Firecracker microVM sandbox daemon for coding-agent sessions (Claude Managed Agents `self_hosted` environments): golden template snapshots, pause to NVMe when idle, resume on the next turn. Protocol between server, daemon and guest lives in its AGENTS.md. Runners stay on Kata. See `infra/sandboxd/AGENTS.md`.
- `infra/sandbox-image/` - Guest template image for sandboxd: kernel + Ubuntu rootfs with `sbx-init` (PID 1), `sbx-agent` (vsock exec/files/time) and `sbx-worker` (Anthropic SDK environment worker). See `infra/sandbox-image/AGENTS.md`.
