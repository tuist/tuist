...
The library `tuist test` and `tuist xcodebuild test` inject into test hosts to record which coverage counters each test moves, and by how much: the raw material of per-test coverage evidence (`TuistKit.TestCoverageEvidenceService`, `XCResultParser.TestCoverageEvidence`).
...
- `CoverageAttributionTrait.swift` - the Swift Testing trait users copy into a test target. Swift Testing has no hook an injected library can join, so the trait calls `tuist_coverage_scope_begin/end` (resolved with `dlsym`; a no-op when the observer is not injected). It names the scope the way the result bundle names the test, the display name when the test declares one, so the evidence keys on the same test case as the run's results. Not compiled into the CLI.
...
- Output, under `<dir>/<pid>/`: `images.tsv`, `<n>.data` and `<n>.names` (raw `__llvm_prf_data` / `__llvm_prf_names`), and `records.bin`, whose layout is documented at `write_record()` and read by `TuistKit.CoverageObserverOutput`. Change both together, and bump `kRecordVersion` when a record gains a field.
- The deltas are what turns counters into lines: `TuistKit.CoverageMapping` decodes the image's `__llvm_covfun`/`__llvm_covmap` and evaluates each region's counter expression with them, and `TestCoverageEvidenceService` trusts a file's lines only when the same decoding of the run's profile matches `llvm-cov`'s report of it.
