...
final class GraphMapperFactoryTests: TuistUnitTestCase {
    ...
    final class CacheGraphMapperFactoryTests: TuistUnitTestCase {
        ...
        func test_generation_respects_disabled_autogenerated_workspace_schemes() async throws {
            // Given
            let includedTargets: Set<TargetQuery> = Set([.named("MyTarget")])
            let got = subject.generation(
                config: .test(project: .testGeneratedProject()),
            ...
            let mapper = try XCTUnwrap(
                XCTAssertContainsElementOfType(got, AutogeneratedWorkspaceSchemeGraphMapper.self)
            )
            ...
            // When
            let (mappedGraph, _, _) = try await mapper.map(
                graph: graph,
                environment: MapperEnvironment()
            )

            // Then
            XCTAssertEmpty(mappedGraph.workspace.schemes)
        }
        ...
        func test_automation_regenerates_workspace_scheme_after_cache_replacement_when_source_targets_are_kept() {
            // Given
            let config = Tuist.test(
                project: .generated(
                    .test(cacheOptions: .test(keepSourceTargets: true))
                )
            )
            ...
            // Then
            XCTAssertEqual(
                got.filter { $0 is AutogeneratedWorkspaceSchemeGraphMapper }.count,
                2
            )
            XCTAssertContainsElementOfType(
                got,
                AutogeneratedWorkspaceSchemeGraphMapper.self,
                after: TargetsToCacheBinariesGraphMapper.self
            )
        }
