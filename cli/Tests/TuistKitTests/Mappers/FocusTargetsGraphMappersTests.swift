...
final class FocusTargetsGraphMappersTests: TuistUnitTestCase {
    ...
    /// flagged `isImplicit` with `target: nil`. Their environmentBuildable is resolved at
    ...
    func test_map_does_not_preserve_off_shard_target_via_autogenerated_insight_post_action() async throws {
        try await withMockedEnvironment {
            ...
            // The insight post-action is implicit and has no specific target — that's what stops
            ...
            XCTAssertTrue(initialPostAction.isImplicit)
