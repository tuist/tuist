...
final class FocusTargetsGraphMappersTests: TuistUnitTestCase {
    ...
    /// whose `environmentBuildable` is set to the alphabetically first test/build target so
    /// that `tuist inspect test|build` can read `BUILD_DIR`/`DERIVED_DATA_DIR`/`CONFIGURATION`.
    /// In a shard whose included targets exclude that alphabetically first target, focus must:
    ///
    ///   1. Tag the off-shard target prunable (so it does not leak into the shard's build).
    ///   2. Rewrite the post-action target to a surviving on-shard target so the inspect
    ///      script still receives target build settings.
    func test_map_rewrites_autogenerated_insight_post_action_targets_for_off_shard_runs() async throws {
        try await withMockedEnvironment {
            ...
            // Sanity check: autogen pinned the alphabetically first target as the post-action target.
            let initialPostActionTarget = graph.workspace.schemes
                .first(where: { $0.name == "X-Workspace" })?
                .testAction?
                .postActions
                .first?
                .target
            XCTAssertEqual(initialPostActionTarget?.name, "ATests")
            ...
            // ATests must have been tagged prunable. Pre-fix, the post-action target ref kept
            // ATests in `filteredTargets` via the execution-action preservation pass.
            ...
            // The post-action target must have been rewritten to BTests (the surviving testable
            // target) so that the inspect script still receives target build settings.
            let rewrittenPostActionTarget = graph.workspace.schemes
                .first(where: { $0.name == "X-Workspace" })?
                .testAction?
                .postActions
                .first?
                .target
            XCTAssertEqual(rewrittenPostActionTarget?.name, "BTests")

            // Step 3: TreeShakePrunedTargetsGraphMapper — drop pruned targets and scheme refs.
            let treeShakeResult = try TreeShakePrunedTargetsGraphMapper().map(graph: graph, environment: environment)
            graph = treeShakeResult.0

            let schemeTargets = graph.workspace.schemes
                .first(where: { $0.name == "X-Workspace" })?
                .testAction?
                .targets
                .map(\.target.name)
            XCTAssertEqual(schemeTargets, ["BTests"])
        }
    }
}
