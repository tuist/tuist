...
final class FocusTargetsGraphMappersTests: TuistUnitTestCase {
    ...
    /// flagged `isAutogenerated` with `target: nil`. Their environmentBuildable is resolved at
    /// scheme-generation time against the parent scheme's surviving targets, so no off-shard
    /// target ever appears in the post-action — and therefore `FocusTargetsGraphMappers`'
    /// execution-action target preservation pass never adds it back to the kept set.
    func test_map_does_not_preserve_off_shard_target_via_autogenerated_insight_post_action() async throws {
        try await withMockedEnvironment {
            ...
            // The insight post-action is autogenerated and has no specific target — that's what stops
            // the leak. Resolution to a surviving target happens later in scheme generation.
            ...
            XCTAssertTrue(initialPostAction.isAutogenerated)
            XCTAssertNil(initialPostAction.target)
            ...
            // ATests must have been tagged prunable. Pre-fix, the post-action's `target` pointed
            // at ATests and the execution-action preservation pass added it back to the kept set.
