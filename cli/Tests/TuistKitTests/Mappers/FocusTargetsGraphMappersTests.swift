...
final class FocusTargetsGraphMappersTests: TuistUnitTestCase {
    ...
    /// Regression for the sharded-test leak: when AutogeneratedWorkspaceSchemeGraphMapper
    /// emits "Push test/build insights" post-actions, the post-action target must not pin
    /// the alphabetically first test/build target. Otherwise FocusTargetsGraphMappers
    /// preserves that target via its execution-action target preservation pass, and the
    /// off-shard test target leaks into every shard's workspace scheme.
    func test_map_does_not_preserve_off_shard_test_target_via_autogenerated_insight_post_actions() async throws {
        try await withMockedEnvironment {
            ...
            // Sanity check: FocusTargetsGraphMappers must tag ATests prunable. Pre-fix, the
            // "Push test insights" post-action's `target` would point at ATests and the
            // execution-action preservation pass would keep ATests, so this assertion fails.
