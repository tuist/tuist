...
final class FocusTargetsGraphMappersTests: TuistUnitTestCase {
    ...
    /// with `target: nil`. Their `<EnvironmentBuildable>` is resolved later, at scheme-generation
    /// time, against the parent scheme's surviving targets — so no off-shard target ever appears
    /// in the post-action, and `FocusTargetsGraphMappers`' execution-action target preservation
    /// pass (which compacts nil targets out) never adds it back to the kept set.
    func test_map_does_not_preserve_off_shard_target_via_autogenerated_insight_post_action() async throws {
        try await withMockedEnvironment {
            ...
            // The insight post-action carries no target — that's what stops the leak.
