...
struct MCPSetupVSCodeCommandServiceTests {
    ...
    @Test(.inTemporaryDirectory, .withMockedEnvironment()) func run_createsLocalConfiguration_whenNoFlagsProvided() async throws {
        try await withMockedDependencies {
            ...
            // Then
            let configPath = mockedEnvironment.currentWorkingDirectory.appending(components: [".vscode", "settings.json"])
    ...
    @Test(.inTemporaryDirectory, .withMockedEnvironment()) func run_createsGlobalConfiguration_whenGlobalFlagProvided() async throws {
        try await withMockedDependencies {
            ...
            // Then
            #if os(macOS)
            let vscodeConfigDir = mockedEnvironment.homeDirectory.appending(components: ["Library", "Application Support", "Code", "User"])
            #elseif os(Linux)
            let vscodeConfigDir = mockedEnvironment.homeDirectory.appending(components: [".config", "Code", "User"])
            #else
            let vscodeConfigDir = mockedEnvironment.homeDirectory.appending(components: ["AppData", "Roaming", "Code", "User"])
            #endif
    ...
    @Test(.inTemporaryDirectory, .withMockedEnvironment()) func run_mergesWithExistingConfiguration() async throws {
        try await withMockedDependencies {
            ...
            let vscodeDir = mockedEnvironment.currentWorkingDirectory.appending(component: ".vscode")
