...
struct GenerationsListCommandServiceTests {
    private let listGenerationsService = MockListGenerationsServicing()
    private let serverEnvironmentService = MockServerEnvironmentServicing()
    private let configLoader = MockConfigLoading()
    private let subject: GenerationsListCommandService

    init() {
        subject = GenerationsListCommandService(
            listGenerationsService: listGenerationsService,
    ...
    @Test(.withMockedEnvironment()) func run_when_full_handle_is_missing() async throws {
        ...
        await #expect(throws: GenerationsListCommandServiceError.missingFullHandle, performing: {
    ...
    @Test(
        ...
        let response = Operations.listGenerations.Output.Ok.Body.jsonPayload(runs: [run])

        given(listGenerationsService).listGenerations(
        ...
        let generationsJSON = String(data: try jsonEncoder.encode(response), encoding: .utf8)!
        #expect(ui().contains(generationsJSON))
    }
}
