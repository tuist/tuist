...
final class DependenciesContentHasherTests: TuistUnitTestCase {
    ...
    func test_hash_whenPackageTraitsChange_returnsDifferentHash() async throws {
        // Given
        let package = Package.local(path: filePath1)
        let dependency = TargetDependency.package(product: "foo", type: .runtime)
    ...
    func test_hash_whenPackageTraitsAreReordered_returnsTheSameHash() async throws {
        // Given
        let firstPackage = Package.local(path: filePath1)
        let secondPackage = Package.local(path: filePath2)
        let dependency = TargetDependency.package(product: "foo", type: .runtime)
        let firstGraphTarget = GraphTarget.test(
            target: Target.test(dependencies: [dependency]),
            project: Project.test(
