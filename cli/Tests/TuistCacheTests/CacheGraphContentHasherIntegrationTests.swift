...
struct ContentHashingIntegrationTests {
    ...
    @Test(
        ...
        .withMockedSwiftVersionProvider,
        .withMockedXcodeController
    ) func contentHashes_when_framework_depends_on_target_with_testable_import_excludes_both_targets() async throws {
        // Given
        let temporaryPath = try #require(FileSystem.temporaryTestDirectory)
        let supportSource = try createTemporarySourceFile(
            on: temporaryPath,
            name: "Support.swift",
            content: """
        ...
        let consumerSource = try createTemporarySourceFile(
            on: temporaryPath,
            name: "Consumer.swift",
            content: """
            import Foundation
        ...
        let supportTarget = makeFramework(
            name: "Support",
            productName: "SupportFramework",
            sources: [supportSource]
        )
        let consumerTarget = makeFramework(
            name: "Consumer",
            productName: "ConsumerFramework",
        ...
        let project = Project.test(
            ...
            targets: [
                supportTarget,
                Target.test(
                    name: consumerTarget.name,
                    platform: .iOS,
                    product: .framework,
                    productName: consumerTarget.productName,
                    settings: consumerTarget.settings,
                    sources: consumerTarget.sources,
                    dependencies: [.target(name: supportTarget.name)]
                ),
            ]
        )
        let supportGraphTarget = GraphTarget(path: project.path, target: supportTarget, project: project)
        let consumerGraphTarget = GraphTarget(
            path: project.path,
            target: project.targets[consumerTarget.name]!,
            project: project
        )
        let graph = Graph.test(
            path: project.path,
            ...
            dependencies: [
                .target(name: consumerGraphTarget.target.name, path: project.path): [
                    .target(name: supportTarget.name, path: project.path),
                ],
            ]
        )
        ...
        // Then
        #expect(contentHash[supportGraphTarget] == nil)
        #expect(contentHash[consumerGraphTarget] == nil)
    }
