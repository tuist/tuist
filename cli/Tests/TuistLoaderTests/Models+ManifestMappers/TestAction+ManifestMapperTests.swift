...
struct TestActionManifestMapperTests {
    ...
    @Test func test_testAction_with_literal_test_plans() async throws {
    ...
    @Test func test_testAction_with_glob_pattern_single_match() async throws {
        try await fileSystem.runInTemporaryDirectory(prefix: UUID().uuidString) { temporaryDirectory in
            // Given
            let testPlanPath = temporaryDirectory.appending(component: "TestPlan.xctestplan")
            try await fileSystem.writeText(testPlanContent, at: testPlanPath)
    ...
    @Test func test_testAction_with_glob_pattern_multiple_matches() async throws {
        try await fileSystem.runInTemporaryDirectory(prefix: UUID().uuidString) { temporaryDirectory in
            // Given
    ...
    @Test func test_testAction_with_recursive_glob_pattern() async throws {
        try await fileSystem.runInTemporaryDirectory(prefix: UUID().uuidString) { temporaryDirectory in
            // Given
    ...
    @Test func test_testAction_with_mixed_literal_and_glob_patterns() async throws {
        try await fileSystem.runInTemporaryDirectory(prefix: UUID().uuidString) { temporaryDirectory in
            // Given
    ...
    @Test func test_testAction_with_glob_pattern_no_matches() async throws {
        try await fileSystem.runInTemporaryDirectory(prefix: UUID().uuidString) { temporaryDirectory in
            // Given
            let manifest = ProjectDescription.TestAction.testPlans([
                .path("*.xctestplan")
    ...
    @Test func test_testAction_with_non_xctestplan_files_filtered_out() async throws {
        try await fileSystem.runInTemporaryDirectory(prefix: UUID().uuidString) { temporaryDirectory in
            // Given
            let testPlanPath = temporaryDirectory.appending(component: "TestPlan.xctestplan")
