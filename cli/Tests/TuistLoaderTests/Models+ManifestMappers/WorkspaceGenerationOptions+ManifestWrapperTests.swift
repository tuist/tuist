...
final class WorkspaceGenerationOptionsManifestMapperTests: TuistTestCase {
    ...
    func test_from_mapsSwiftTestingOnlyParallelization() throws {
        ...
        let manifest = ProjectDescription.Workspace.GenerationOptions.options(
            autogeneratedWorkspaceSchemes: .enabled(parallelization: .swiftTestingOnly)
        )

        // When
        let model = try XcodeGraph.Workspace.GenerationOptions.from(manifest: manifest, generatorPaths: generatorPaths)

        // Then
        XCTAssertEqual(model.autogeneratedWorkspaceSchemes.parallelization, .swiftTestingOnly)
        XCTAssertFalse(model.autogeneratedWorkspaceSchemes.randomExecutionOrdering)
    }
    ...
    func test_from_mapsParallelizationEnabled() throws {
        ...
        let generatorPaths = GeneratorPaths(manifestDirectory: temporaryPath, rootDirectory: temporaryPath)
        let manifest = ProjectDescription.Workspace.GenerationOptions.options(
            autogeneratedWorkspaceSchemes: .enabled(parallelization: .enabled, randomExecutionOrdering: true)
        )
        ...
        // Then
        XCTAssertEqual(model.autogeneratedWorkspaceSchemes.parallelization, .all)
        XCTAssertTrue(model.autogeneratedWorkspaceSchemes.randomExecutionOrdering)
    }
    ...
    func test_from_mapsDeprecatedTestingOptionsParallelizable() throws {
        ...
        let generatorPaths = GeneratorPaths(manifestDirectory: temporaryPath, rootDirectory: temporaryPath)
        let manifest = ProjectDescription.Workspace.GenerationOptions.options(
            autogeneratedWorkspaceSchemes: .enabled(testingOptions: [.parallelizable, .randomExecutionOrdering])
        )
