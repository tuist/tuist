...
final class TargetScriptManifestMapperTests: TuistUnitTestCase {
    ...
    func test_outputPaths_arePreserved_whenFileDoesNotExistAtGenerateTime() async throws {
        // Given
        let temporaryPath = try temporaryPath()
        let rootDirectory = temporaryPath
        let generatorPaths = GeneratorPaths(
        ...
        // Intentionally do not create `Generated/AutoGenerated.swift` to ensure
        // non-glob output paths are still preserved during mapping.
        let manifest = ProjectDescription.TargetScript.test(
            name: "GenerateFile",
            tool: "my_tool",
            order: .pre,
            arguments: [],
            outputPaths: ["Generated/AutoGenerated.swift"]
        )
        ...
        // Then
        XCTAssertEqual(model.outputPaths, ["Generated/AutoGenerated.swift"])
    }
