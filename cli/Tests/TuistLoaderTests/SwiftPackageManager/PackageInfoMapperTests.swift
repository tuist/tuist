...
struct PackageInfoMapperTests {
    ...
    @Test(
        ...
        if case let .xcframework(path, _, _, _) = dep {
            // Should not contain __MACOSX in the path
            #expect(!path.pathString.contains("__MACOSX"))
            #expect(path.pathString.contains("ValidFramework.xcframework"))
        }
    }
    ...
    @Test(.withMockedSwiftVersionProvider)
    func enabledTraits_whenPackageHasDefaultTrait_enablesDefaultAndResolvedTraits() {
        let rootPackageInfo = PackageInfo.test(
            name: "RootPackage",
            products: [],
            targets: [],
        ...
        let dependencyAInfo = PackageInfo.test(
            ...
            targets: [],
            traits: [
                PackageTrait(enabledTraits: ["FeatureX"], name: "default", description: nil),
        ...
        #expect(result["dependency-a"] == Set(["default", "FeatureX"]))
    }

    @Test(.withMockedSwiftVersionProvider)
    func enabledTraits_whenReachablePackageEnablesDependencyTraitsWithoutOwnTraits_propagatesTransitively() {
        let rootPackageInfo = PackageInfo.test(
            name: "RootPackage",
            products: [],
            targets: [],
        ...
        #expect(result["dependency-b"] == Set(["FeatureX"]))
    }

    @Test(.withMockedSwiftVersionProvider)
    func enabledTraits_whenDefaultTraitMatchesConditionalDependency_enablesTransitiveTrait() {
        let rootPackageInfo = PackageInfo.test(
            name: "RootPackage",
            products: [],
            targets: [],
