...
struct XcodeCacheSettingsProjectMapperTests {
    ...
    func map_whenRunnerCASDirSet_pointsCASAtTheVolumeMount() async throws {
        // Given: the macOS runner host exports TUIST_COMPILATION_CACHE_DIR (the
        // mount of the per-account CAS disk image the volume attaches).
        Environment.mocked?.variables["TUIST_COMPILATION_CACHE_DIR"] = "/Users/runner/xcode-cas"
        let fullHandle = "test-org/test-project"
        let tuist = Tuist(
            project: .generated(
                .test(
        ...
        // Then: the CAS is routed onto the volume mount, per project handle, kept
        // across builds, and bounded to a fraction of the dedicated image volume.
        let baseSettings = mappedProject.settings.base
        #expect(baseSettings["COMPILATION_CACHE_CAS_PATH"] == .string("/Users/runner/xcode-cas/test-org_test-project"))
        #expect(baseSettings["COMPILATION_CACHE_KEEP_CAS_DIRECTORY"] == .string("YES"))
        #expect(baseSettings["COMPILATION_CACHE_LIMIT_PERCENT"] == .string("80"))
    }

    @Test(.inTemporaryDirectory, .withMockedEnvironment())
    func map_whenRunnerCASDirUnset_leavesCASPathDefault() async throws {
        // Given: off runners the env is unset — the CAS must stay Xcode's default.
        let tuist = Tuist(
            project: .generated(
                .test(
                    generationOptions: .test(enableCaching: true)
        ...
        // Then
        let baseSettings = mappedProject.settings.base
        #expect(baseSettings["COMPILATION_CACHE_CAS_PATH"] == nil)
        #expect(baseSettings["COMPILATION_CACHE_KEEP_CAS_DIRECTORY"] == nil)
        #expect(baseSettings["COMPILATION_CACHE_LIMIT_PERCENT"] == nil)
    }
}
