...
final class AutogeneratedWorkspaceSchemeWorkspaceMapperTests: TuistUnitTestCase {
    ...
    func test_map_replaces_autogenerated_schemes_when_target_becomes_binary_source() async throws {
        // Given
        let subject = AutogeneratedWorkspaceSchemeGraphMapper(
            forceWorkspaceSchemes: false,
            buildInsightsDisabled: true,
        ...
        var (graphWithWorkspaceScheme, _, _) = try await subject.map(
            graph: graph,
            environment: MapperEnvironment()
        )
        let binarySourceFramework = Target.test(
            name: "Framework",
            product: .framework,
            metadata: TargetMetadata.metadata(tags: Set(["tuist:binary-sources"]))
        )
        graphWithWorkspaceScheme.projects[projectPath] = Project.test(
            path: projectPath,
            targets: [binarySourceFramework]
        )

        // When
        let (got, sideEffects, _) = try await subject.map(
            graph: graphWithWorkspaceScheme,
            environment: MapperEnvironment()
        )

        // Then
        XCTAssertEmpty(sideEffects)
        XCTAssertEqual(got.workspace.schemes.count, 2)
        let workspaceScheme = try XCTUnwrap(got.workspace.schemes.first(where: { $0.name == "A-Workspace" }))
        let cachedScheme = try XCTUnwrap(got.workspace.schemes.first(where: { $0.name == "A-Cached" }))
        XCTAssertEmpty(try XCTUnwrap(workspaceScheme.buildAction?.targets))
        XCTAssertEqual(cachedScheme.buildAction?.targets.map(\.name), ["Framework"])
    }
    ...
    func test_map_testing_options_swift_testing_only() async throws {
        // Given
        let subject = AutogeneratedWorkspaceSchemeGraphMapper(
            forceWorkspaceSchemes: false,
            buildInsightsDisabled: true,
