...
final class AutogeneratedWorkspaceSchemeWorkspaceMapperTests: TuistUnitTestCase {
    ...
    func test_map() async throws {
        ...
        let graph = Graph.test(
            ...
            dependencies: [
                .target(name: "ATests", path: projectPath): Set([.target(name: "A", path: projectPath)]),
                .target(name: "A", path: projectPath): Set(),
                .target(name: "BTests", path: projectBPath): Set([.target(name: "B", path: projectBPath)]),
                .target(name: "B", path: projectBPath): Set(),
            ]
        )
    ...
    func test_map_codeCoverageMode_targets() async throws {
        ...
        let graph = Graph.test(
            ...
            dependencies: [
                .target(name: "ATests", path: projectPath): Set([.target(name: "A", path: projectPath)]),
                .target(name: "A", path: projectPath): Set(),
                .target(name: "B", path: projectPath): Set(),
            ]
        )
    ...
    func test_map_codeCoverageMode_relevant_complex() async throws {
        ...
        let graph = Graph.test(
            ...
            dependencies: [
                .target(name: "ATests", path: projectAPath): Set([.target(name: "A", path: projectAPath)]),
                .target(name: "A", path: projectAPath): Set(),
                .target(name: "B", path: projectAPath): Set(),
                .target(name: "C", path: projectBPath): Set(),
                .target(name: "D", path: projectBPath): Set(),
            ]
        )
    ...
    func test_map_preferredScreenCaptureFormat() async throws {
        ...
        let graph = Graph.test(
            ...
            dependencies: [
                .target(name: "AUITests", path: projectPath): Set([.target(name: "A", path: projectPath)]),
                .target(name: "A", path: projectPath): Set(),
            ]
        )
    ...
    func test_map_excludes_non_test_dependency_targets_from_build_action() async throws {
        // Given
        let subject = AutogeneratedWorkspaceSchemeGraphMapper(
            forceWorkspaceSchemes: false,
            buildInsightsDisabled: true,
        ...
        let graph = Graph.test(
            ...
            dependencies: [
                .target(name: "FrameworkTests", path: projectPath): Set([
                    .target(name: "Framework", path: projectPath),
                ]),
                .target(name: "FrameworkExample", path: projectPath): Set([
                    .target(name: "Framework", path: projectPath),
                ]),
                .target(name: "Framework", path: projectPath): Set(),
            ]
        )
        ...
        // Then
        let scheme = try XCTUnwrap(got.workspace.schemes.first(where: { $0.name == "A-Workspace" }))
        let buildTargetNames = Set(scheme.buildAction?.targets.map(\.name) ?? [])
        XCTAssertEqual(buildTargetNames, Set(["Framework", "FrameworkTests"]))
        XCTAssertFalse(buildTargetNames.contains("FrameworkExample"))

        let testTargetNames = Set(scheme.testAction?.targets.map(\.target.name) ?? [])
        XCTAssertEqual(testTargetNames, Set(["FrameworkTests"]))
    }
}
