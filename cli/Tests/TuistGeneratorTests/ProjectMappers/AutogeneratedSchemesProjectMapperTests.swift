...
final class AutogeneratedSchemesProjectMapperTests: TuistUnitTestCase {
    ...
    func test_map_swiftTestingOnlyParallelization() async throws {
        let project = Project.test(
            options: .test(
                automaticSchemesOptions: .enabled(
                    targetSchemesGrouping: .singleScheme,
            ...
            targets: [.test(name: "AppTests", product: .unitTests)]
        )

        let (mappedProject, _) = try await subject.map(project: project)

        let testableTarget = try XCTUnwrap(mappedProject.schemes.first?.testAction?.targets.first)
