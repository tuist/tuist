...
final class AutogeneratedSchemesProjectMapperTests: TuistUnitTestCase {
    private var subject: AutogeneratedSchemesProjectMapper!
    ...
    func test_grouping_singleScheme() async throws {
        // Given
        ...
        // When
        let (got, sideEffects) = try await subject.map(project: project)
    ...
    func test_binarySources() async throws {
        // Given
        ...
        let scheme = try XCTUnwrap(got.schemes.first)
        XCTAssertEqual(scheme.name, "Project")
        XCTAssertEqual(scheme.buildAction?.targets.isEmpty, true)
    }

    func test_grouping_byNameSuffix() async throws {
        // Given
        let targetB = Target.test(name: "B", product: .app)
        let targetBTests = Target.test(
            name: "BTests",
        ...
        verify(buildInsightsActionMapper)
            .map(.any, target: .value(TargetReference(projectPath: projectPath, name: "A")), buildInsightsDisabled: .any)
            .called(1)
    ...
    func test_grouping_byNameSuffix_with_suffix_the_name_of_target() async throws {
        // Given
        let targetA = Target.test(
            name: "A",
            product: .framework,
    ...
    func test_grouping_notGrouped() async throws {
        // Given
        let targetB = Target.test(name: "B", product: .app)
        let targetBTests = Target.test(
            name: "BTests",
    ...
    func test_map_doesnt_override_user_schemes() async throws {
        // Given
        let targetB = Target.test(name: "B", product: .app)
        let targetBTests = Target.test(
            name: "BTests",
    ...
    func test_when_build_insights_are_enabled() async throws {
        // Given
        let targetA = Target.test(
            name: "A",
            product: .framework
    ...
    func test_coverage_enabled() async throws {
        // Given
        let targetB = Target.test(name: "B", product: .app)
        let targetBTests = Target.test(
            name: "BTests",
    ...
    func test_testing_options() async throws {
        // Given
        let targetB = Target.test(name: "B", product: .app)
        let targetBTests = Target.test(
            name: "BTests",
    ...
    func test_map_whenSwiftTestingOnlyParallelization_setsSwiftTestingOnlyOnTestableTargets() async throws {
        // Given
        ...
        // When
        let (got, _) = try await subject.map(project: project)

        // Then
        let testableTargets = got.schemes.flatMap { $0.testAction?.targets ?? [] }
        XCTAssertFalse(testableTargets.isEmpty)
        for testableTarget in testableTargets {
            XCTAssertEqual(testableTarget.parallelization, .swiftTestingOnly)
        }
    }

    func test_map_whenParallelizableAndSwiftTestingOnlyBothSet_swiftTestingOnlyWins() async throws {
        // Given: both options set together. TestingOptions.parallelization documents that
        // swiftTestingOnlyParallelizable takes precedence over parallelizable, so the resolved
        // parallelization must be .swiftTestingOnly rather than .all.
        let app = Target.test(name: "App", product: .app)
        let appTests = Target.test(
            name: "AppTests",
            product: .unitTests,
        ...
        let project = Project.test(
            ...
            options: .test(
                automaticSchemesOptions: .enabled(
                    targetSchemesGrouping: .byNameSuffix(build: [], test: ["Tests"], run: ["Demo"]),
                    codeCoverageEnabled: false,
                    testingOptions: [.parallelizable, .swiftTestingOnlyParallelizable]
                )
            ),
            targets: [app, appTests]
        )
    ...
    func test_run_arguments() async throws {
        // Given
    ...
    func test_test_arguments() async throws {
        // Given
        let targetB = Target.test(
            name: "B",
            product: .app
    ...
    func test_screenCaptureFormat() async throws {
        // Given
        let targetA = Target.test(
            name: "A",
            product: .framework
    ...
    func test_disabled() async throws {
        // Given
        let targetB = Target.test(name: "B", product: .app)
        let targetBTests = Target.test(
            name: "BTests",
    ...
    func test_app_extension() async throws {
        // Given
        let targetB = Target.test(name: "B", product: .app)
        let targetBTests = Target.test(
            name: "BTests",
    ...
    func test_watch_extension() async throws {
        // Given
        let targetB = Target.test(name: "B", product: .app)
        let targetBTests = Target.test(
            name: "BTests",
