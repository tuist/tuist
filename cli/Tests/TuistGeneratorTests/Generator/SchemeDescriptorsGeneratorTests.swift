...
final class SchemeDescriptorsGeneratorTests: XCTestCase {
    ...
    /// An autogenerated execution action with `target: nil` (e.g. the autogenerated insight
    /// post-actions) must resolve `environmentBuildable` from the parent action's first
    /// buildable target at scheme-generation time so the inspect script still receives
    /// BUILD_DIR/CONFIGURATION.
    func test_schemeBuildAction_autogenerated_execution_action_resolves_environmentBuildable_from_parent_scheme() throws {
        // Given
        let projectPath = try AbsolutePath(validating: "/somepath/Project")
        let xcodeProjPath = projectPath.appending(component: "Project.xcodeproj")
        let target = Target.test(name: "App", product: .app)
