...
final class CacheGraphMutatorTests: TuistUnitTestCase {
    ...
    /// Regression test for the sharded-test bug where the alphabetically first test
    /// target leaks into the Pinterest-Workspace scheme of every shard.
    ///
    /// Setup (mirrors `tuist test --test-targets BTests --build-only`):
    ///  - Project has frameworks A, B and test targets ATests (-> A), BTests (-> B).
    ///  - Workspace scheme "X-Workspace" lists [ATests, BTests] in testAction.targets,
    ///    sorted alphabetically (matches AutogeneratedWorkspaceSchemeGraphMapper's output).
    ///  - ATests is tagged `tuist:prunable` (matches FocusTargetsGraphMapper's output for a
    ///    shard that only owns BTests).
    ///  - Framework A is cached as a binary (matches a real CI cache hit).
    ///
    /// Expectation: after CacheGraphMutator runs, the workspace scheme's testAction.targets
    /// must NOT contain ATests. The CacheGraphMutator must not re-introduce a tree-shaken
    /// test target into the workspace scheme.
    func test_map_does_not_re_introduce_pruned_test_targets_into_workspace_scheme() async throws {
        let path = try temporaryPath()

        let aFramework = Target.test(name: "A", platform: .iOS, product: .framework)
        let bFramework = Target.test(name: "B", platform: .iOS, product: .framework)
        var aTests = Target.test(
            name: "ATests",
            platform: .iOS,
            product: .unitTests,
            dependencies: [.target(name: "A")]
        )
        // Simulate FocusTargetsGraphMapper having tagged the off-shard test target prunable.
        aTests.metadata.tags.insert("tuist:prunable")
        let bTests = Target.test(
            name: "BTests",
            platform: .iOS,
            product: .unitTests,
        ...
        let project = Project.test(
            path: path,
            name: "X",
            targets: [aFramework, bFramework, aTests, bTests]
        )
        let aFrameworkGraphTarget = GraphTarget.test(path: project.path, target: aFramework, project: project)
        let bFrameworkGraphTarget = GraphTarget.test(path: project.path, target: bFramework, project: project)

        // Simulate AutogeneratedWorkspaceSchemeGraphMapper having generated an X-Workspace scheme
        // with the test targets alphabetically sorted. After tree-shaking, only BTests survives.
        let workspaceScheme = Scheme(
            name: "X-Workspace",
            shared: true,
            buildAction: BuildAction(targets: [
                TargetReference(projectPath: project.path, name: bFramework.name),
                TargetReference(projectPath: project.path, name: bTests.name),
            ]),
            testAction: TestAction.test(
                targets: [
                    TestableTarget(target: TargetReference(projectPath: project.path, name: bTests.name)),
                ]
            )
        )
        ...
        let graph = Graph.test(
            ...
            projects: [project.path: project],
            dependencies: [
                .target(name: aFramework.name, path: project.path): [],
                .target(name: bFramework.name, path: project.path): [],
                .target(name: aTests.name, path: project.path): [
                    .target(name: aFramework.name, path: project.path),
                ],
                .target(name: bTests.name, path: project.path): [
                    .target(name: bFramework.name, path: project.path),
                ],
            ]
        )
        ...
        let bXCFrameworkPath = path.appending(component: "B.xcframework")
        let xcframeworks = [
            aFrameworkGraphTarget: aXCFrameworkPath,
            bFrameworkGraphTarget: bXCFrameworkPath,
        ]

        artifactLoader.loadStub = { artifactPath in
            if artifactPath == aXCFrameworkPath {
                return GraphDependency.testXCFramework(path: aXCFrameworkPath)
            } else if artifactPath == bXCFrameworkPath {
                return GraphDependency.testXCFramework(path: bXCFrameworkPath)
            } else {
                fatalError("Unexpected load call for \(artifactPath)")
            }
        }
        ...
        let got = try await subject.map(
            graph: graph,
            precompiledArtifacts: xcframeworks,
            sources: Set(["ATests", "BTests"]),
            keepSourceTargets: false
        )

        // Then
        let surviving = got.workspace.schemes
            .first(where: { $0.name == "X-Workspace" })?
            .testAction?
            .targets
            .map(\.target.name)
            .sorted()
        XCTAssertEqual(surviving, ["BTests"])
    }
}
