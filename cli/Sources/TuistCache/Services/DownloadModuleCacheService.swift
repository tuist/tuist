...
public struct DownloadModuleCacheService: DownloadModuleCacheServicing {
    ...
    private func fetchArtifact(
        ...
        // Nothing on this path sends a `Range`, so a ranged answer is not a
        // reply to the request that was made. Its body is a fragment, and
        // returning it as the artifact would store a truncated one under a key
        // that claims to be whole, so it is refused. Declared on the operation
        // because kura honours ranges on this route.
        case .partialContent:
