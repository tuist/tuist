...
extension XcodeGraph.Package {
    ...
    static func from(manifest: ProjectDescription.Package, generatorPaths: GeneratorPaths) throws -> XcodeGraph.Package {
        switch manifest {
        case let .local(local, groupPath, excludingPath, keepStructure):
            return .local(
                config: .init(
                    path: try generatorPaths.resolve(path: local),
                    groupPath: groupPath?.pathString,
                    excludingPath: excludingPath?.pathString,
