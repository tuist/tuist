...
public class CachedManifestLoader: ManifestLoading {
    ...
    private func calculateHashes(
        ...
        let environmentHash = try calculateEnvironmentHash(for: manifest, manifestPath: manifestPath)
    ...
    private func calculateEnvironmentHash(for manifest: Manifest, manifestPath: AbsolutePath) throws -> String? {
        let environmentVariables: [String: String]
        if case .packageSettings = manifest {
            environmentVariables = Environment.current.variables
            return try packageDescriptionContextProvider.cacheHash(
