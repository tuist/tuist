...
- A transfer that delivers nothing for `defaultTransferIdleTimeout` is cut, which is what lets a stalled read resume quickly. Call deadlines are sized from the bytes a call carries so a slow link meets them; they are a backstop against a trickle, not the stall guard.
