...
public protocol Environmenting: Sendable {
    ...
    /// Where `tuist setup cache` installs the CAS plugin that projects not generated by
    /// Tuist point `COMPILATION_CACHE_PLUGIN_PATH` at. Fixed, unlike the plugin that
    /// ships next to `tuist`, whose path changes from one Tuist install to the next.
    func casPluginInstallPath() -> AbsolutePath
...
public struct Environment: Environmenting {
    ...
    /// Anchored to `HOME`, like `casProxySocketPath()`, so the `$HOME`-relative build
    /// setting that points at it is the same on every machine.
