...
struct SchemeDescriptorsGenerator: SchemeDescriptorsGenerating {
    ...
    func schemeExecutionAction(
        ...
        // The action's `target` only exists to expose build settings via
        // `<EnvironmentBuildable>`. When the caller leaves it nil (autogenerated insight
        // post-actions do this so they never anchor a target through focus) or when the
        // named target has since been pruned, resolve against the parent action's first
        // surviving buildable so the script still gets BUILD_DIR / CONFIGURATION.
        let resolvedTargetReference: TargetReference? = {
            if let ref = action.target,
               graphTraverser.target(path: ref.projectPath, name: ref.name) != nil
            {
                return ref
            }
