...
struct SchemeDescriptorsGenerator: SchemeDescriptorsGenerating {
    ...
    func schemeBuildAction(
        ...
        preActions = try buildAction.preActions.map {
            try schemeExecutionAction(
                ...
                generatedProjects: generatedProjects,
                rootPath: rootPath,
                autogeneratedFallbackTarget: buildActionFallback
            )
        }
    ...
    func schemeTestAction(
        ...
        preActions = try testAction.preActions.map {
            try schemeExecutionAction(
                ...
                generatedProjects: generatedProjects,
                rootPath: rootPath,
                autogeneratedFallbackTarget: testActionFallback
            )
        }
    ...
    func schemeExecutionAction(
        ...
        generatedProjects: [AbsolutePath: GeneratedProject],
        rootPath: AbsolutePath,
        autogeneratedFallbackTarget: TargetReference? = nil
    ) throws -> XCScheme.ExecutionAction {
        // Autogenerated actions' `target` is advisory — it exists only to expose build settings.
        // When such an action carries no specific target (autogen leaves it nil so the field
        // never anchors a target through focus), fall back to the parent scheme's first
        // buildable so the script still gets BUILD_DIR / CONFIGURATION.
        let resolvedTargetReference: TargetReference? = {
            if action.isAutogenerated, action.target == nil {
                return autogeneratedFallbackTarget
            }
