...
struct SchemeDescriptorsGenerator: SchemeDescriptorsGenerating {
    ...
    func schemeBuildAction(
        ...
        preActions = try buildAction.preActions.map {
            try schemeExecutionAction(
                ...
                generatedProjects: generatedProjects,
                rootPath: rootPath,
                anyTargetFallback: buildActionFallback
            )
        }
    ...
    func schemeTestAction(
        ...
        preActions = try testAction.preActions.map {
            try schemeExecutionAction(
                ...
                generatedProjects: generatedProjects,
                rootPath: rootPath,
                anyTargetFallback: testActionFallback
            )
        }
    ...
    func schemeExecutionAction(
        ...
        generatedProjects: [AbsolutePath: GeneratedProject],
        rootPath: AbsolutePath,
        anyTargetFallback: TargetReference? = nil
    ) throws -> XCScheme.ExecutionAction {
        // Actions with `target: .any` (e.g. autogenerated insight post-actions) want any of the
        // parent scheme's surviving targets' build settings. Resolve to the caller-provided
        // fallback so the script still gets BUILD_DIR / CONFIGURATION.
        let resolvedTargetReference: TargetReference? = {
            switch action.target {
            case let .target(ref): return ref
            case .any: return anyTargetFallback
            case nil: return nil
            }
        }()
