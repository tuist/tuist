...
public struct AutogeneratedSchemesProjectMapper: ProjectMapping { // swiftlint:disable:this type_body_length
    ...
    public func map(project: Project) async throws -> (Project, [SideEffectDescriptor]) {
        ...
        for target in project.targets.filter({ !$0.1.metadata.tags.contains("tuist:binary-sources") }).values.sorted() {
            ...
            case .framework, .staticLibrary, .dynamicLibrary, .staticFramework, .bundle, .appExtension, .watch2AppContainer,
                 .watch2Extension, .tvTopShelfExtension, .messagesExtension, .stickerPackExtension, .systemExtension,
                 .extensionKitExtension, .macro:
    ...
    private func createDefaultScheme(
        ...
        let testableTargets: [TestableTarget] = testTargets
            .map {
                TestableTarget(
                    ...
                    parallelization: project.options.parallelization,
                    randomExecutionOrdering: project.options.randomExecutionOrdering
                )
            }
        ...
        return Scheme(
            ...
            buildAction: try await buildInsightsActionMapper.map(
                buildAction,
                target: buildTargetReferences.first(where: { $0.name == name }),
                buildInsightsDisabled: buildInsightsDisabled
            ),
            testAction: try await testInsightsActionMapper.map(
                testableTargets.isEmpty ? nil : TestAction(
...
extension Product {
    var isExtension: Bool {
        ...
        case .staticFramework, .app, .appClip, .staticLibrary, .dynamicLibrary, .framework, .unitTests, .uiTests, .bundle,
             .commandLineTool, .watch2App, .watch2AppContainer, .xpc, .macro:
            return false
        }
    }
}
