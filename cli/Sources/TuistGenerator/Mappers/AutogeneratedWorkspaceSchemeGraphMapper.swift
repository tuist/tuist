...
public struct AutogeneratedWorkspaceSchemeGraphMapper: GraphMapping { // swiftlint:disable:this type_name
    ...
    private func workspaceScheme(
        ...
        var allEligibleTargets: [TargetReference] = []
        var testableTargets: [TestableTarget] = []
        var testGraphTargets: [GraphTarget] = []

        for projectPath in workspace.projects {
            guard let project = graph.projects[projectPath] else { continue }
            for target in project.targets.values {
                guard !target.supportedPlatforms.isDisjoint(with: platforms),
                      !target.metadata.tags.contains("tuist:binary-sources")
                else { continue }

                allEligibleTargets.append(TargetReference(projectPath: project.path, name: target.name))

                guard target.product.testsBundle else { continue }

                testGraphTargets.append(GraphTarget(path: project.path, target: target, project: project))
                testableTargets.append(
                    TestableTarget(
                        target: TargetReference(projectPath: project.path, name: target.name),
        ...
        if testGraphTargets.isEmpty {
            targets = allEligibleTargets.sorted(by: { $0.name < $1.name })
        } else {
            let testDependencies = graphTraverser.allTargetDependencies(traversingFromTargets: testGraphTargets)
            let buildGraphTargets = Set(testGraphTargets).union(testDependencies)
            targets = buildGraphTargets
                .map { TargetReference(projectPath: $0.path, name: $0.target.name) }
