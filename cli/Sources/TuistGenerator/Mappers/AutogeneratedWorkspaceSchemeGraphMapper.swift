...
public final class AutogeneratedWorkspaceSchemeGraphMapper: GraphMapping { // swiftlint:disable:this type_name
    ...
    public func map(
        ...
        schemes = [
            ...
            try await cachedScheme(name: "\(graph.workspace.name)-Cached",
                                         platforms: platforms,
                                         project: project,
                                         workspace: graph.workspace,
                                         graph: graph)
        ...
        var (targets, testableTargets): ([TargetReference], [TestableTarget]) = workspace.projects
            ...
            .reduce(([], [])) { result, project in
                let targets = project.targets
                    .values
                    .filter { !$0.supportedPlatforms.isDisjoint(with: platforms) && !$0.metadata.tags.contains("tuist:binary-sources") }
