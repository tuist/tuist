...
public struct TestAction: Equatable, Codable, Sendable {
    /// List of test plans referenced by path. The first in the list will be the default plan.
    public var testPlans: [Path]?

    /// List of test plans whose `.xctestplan` files will be generated by Tuist from their Swift declaration.
    /// The first in the list (or the one marked with `isDefault = true`) will be the default plan.
    public var generatedTestPlans: [TestPlan]?
    ...
    private init(
        ...
        self.generatedTestPlans = generatedTestPlans
    ...
    /// Returns a test action from a list of test plans whose `.xctestplan` files are generated by Tuist.
    ///
    /// Use this overload to declare test plans in Swift so the list of test targets stays in sync with the
    /// project manifest. If no plan is marked as default, the first one is used as the default.
    ///
    /// - Parameters:
    ///   - testPlans: List of test plans whose `.xctestplan` files Tuist will generate.
    ///   - configuration: Configuration to be used.
    ///   - attachDebugger: A boolean controlling whether a debugger is attached to the process running the tests.
    ///   - preActions: Actions to execute before running the tests.
    ///   - postActions: Actions to execute after running the tests.
