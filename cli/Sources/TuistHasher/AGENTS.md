...
- `SettingsContentHasher` drops `COMPILATION_CACHE_*` keys and `-cas-plugin-option` pairs, and `TUIST_PREFIX_MAPPING_ROOT_DIR` (the checkout path the generated prefix mappings reference), so checkouts at different paths share binary-cache hashes. `SWIFT_OTHER_PREFIX_MAPPINGS`/`CLANG_OTHER_PREFIX_MAPPINGS` are hashed verbatim: both sides of a mapping reach the product. `*_ENABLE_*PREFIX_MAPPING` keys stay hashed: they change `#filePath` in the product.
- `SettingsContentHasher` drops warning-only settings: `SWIFT_TREAT_WARNINGS_AS_ERRORS`, `GCC_TREAT_WARNINGS_AS_ERRORS` (including conditioned variants), and warning flags such as `-warnings-as-errors`, `-Werror`, `-Wno-*` and `-Xfrontend -warn-*`. In `OTHER_SWIFT_FLAGS`, `-Werror` consumes its diagnostic-group argument. Settings inside xcconfig files are hashed as file content and are not filtered.
