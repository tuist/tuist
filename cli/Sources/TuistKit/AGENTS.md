...
- Cache setup does not resolve or register remote demand. Hosted cache requests activate capacity through their derived URL. A self-hosted server missing `TUIST_CACHE_ENDPOINT` must still let ordinary builds and the Xcode proxy use local storage with a warning; malformed endpoints and authentication errors remain failures.
