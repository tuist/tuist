...
protocol GenerationsListCommandServicing {
    func run(
        project: String?,
        path: String?,
        gitBranch: String?,
...
enum GenerationsListCommandServiceError: Equatable, LocalizedError {
...
final class GenerationsListCommandService: GenerationsListCommandServicing {
    ...
    func run(
        ...
        guard let resolvedFullHandle = project != nil ? project! : config.fullHandle else {
            throw GenerationsListCommandServiceError.missingFullHandle
        }
        ...
        if response.runs.isEmpty {
            Noora.current.passthrough("No generations found for project \(resolvedFullHandle).")
            return
        }
