...
struct SetupCacheCommandService { // swiftlint:disable:this type_body_length
    ...
    func run(
        ...
        if try await manifestLoader.hasRootManifest(at: path) {
            ...
            let pluginPath = try await pluginPathInstructions()
            Logger.current.info(
                """
                Xcode Cache setup is almost complete!
                ...
                COMPILATION_CACHE_PLUGIN_PATH=\(pluginPath.value)
    ...
    /// instructions, `$HOME`-relative as `tuist generate` writes it, and a note on
    /// where that path comes from.
    private func pluginPathInstructions() async throws -> (value: String, note: String) {
        guard let pluginPath = try await resourceLocator.casPlugin() else {
            return (
                "<path to libtuist_cas_plugin.dylib>",
                "The CAS plugin (libtuist_cas_plugin.dylib) was not found next to `tuist`, so the path above is a placeholder. Reinstall Tuist, or set TUIST_CAS_PLUGIN_PATH to the dylib, and run `tuist setup cache` again to print its path."
            )
        }
        return (
            Environment.current.homeRelativePathString(pluginPath),
            "`COMPILATION_CACHE_PLUGIN_PATH` points into this Tuist installation, so it can change when you update Tuist. Run `tuist setup cache` again after updating and update the setting if the path changed."
        )
    }
