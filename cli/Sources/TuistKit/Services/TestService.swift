...
public enum TestServiceError: FatalError, Equatable {
    ...
    case hostlessTestTargetWithoutCompatibleScheme(target: String)
    ...
    public var description: String {
        ...
        case let .hostlessTestTargetWithoutCompatibleScheme(target):
            return
                "The host-less unit test target \(target) cannot run from the workspace scheme because no compatible single-target scheme is available. Enable autogenerated target schemes, or run the target with a dedicated scheme and derived data path."
        }
    }
    ...
    public var type: ErrorType {
        ...
        case .schemeNotFound, .schemeWithoutTestableTargets, .testPlanNotFound,
             ...
             .shardingRequiresFullHandle, .hostlessTestTargetWithoutCompatibleScheme:
            return .abort
        }
    }
}
...
public struct TestService { // swiftlint:disable:this type_body_length
    ...
    public func run(
        ...
        if let schemeName {
            ...
            testInvocations = try defaultTestInvocations(
    ...
    private func defaultTestInvocations(
        ...
        return try workspaceSchemes.indices.flatMap { index -> [TestInvocation] in
            try testInvocations(
    ...
    private func testInvocations(
        ...
        let workspaceTargetNames = Set(targets.subtracting(hostlessTargets).map(\.name))

        guard !hostlessTargets.isEmpty, !workspaceTargetNames.isEmpty else {
            return [workspaceInvocation]
        }

        let hostlessInvocations = try hostlessTargets
            .sorted {
                ($0.name, $0.projectPath.pathString) < ($1.name, $1.projectPath.pathString)
            }
            .map { target -> TestInvocation in
                guard let scheme = compatibleSchemesByTarget[target] else {
                    throw TestServiceError.hostlessTestTargetWithoutCompatibleScheme(target: target.name)
                }
