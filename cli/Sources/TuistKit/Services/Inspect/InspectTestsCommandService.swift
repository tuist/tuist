...
enum InspectTestsCommandServiceError: Equatable, LocalizedError {
...
struct InspectTestsCommandService {
    ...
    func run(
        ...
        guard let executablePath = Environment.current.currentExecutablePath() else {
            throw InspectTestsCommandServiceError.executablePathMissing
        }

        if Environment.current.variables["TUIST_INSPECT_TESTS_WAIT"] != "YES",
           Environment.current.workspacePath != nil
        {
            var environment = Environment.current.variables
            environment["TUIST_INSPECT_TESTS_WAIT"] = "YES"
    ...
    private func mostRecentActivityLogPath(
        ...
        try await withTimeout(
            .seconds(1),
            onTimeout: {
                throw InspectTestsCommandServiceError.mostRecentActivityLogNotFound(projectPath)
            }
        ) {
            while true {
    ...
    private func createBuild(
        ...
        guard let fullHandle = config.fullHandle else {
            throw InspectTestsCommandServiceError.missingFullHandle
        }
    ...
    private func projectPath(_ path: String?) async throws -> AbsolutePath {
        if let workspacePath = Environment.current.workspacePath {
            ...
            .first {
                ...
                throw InspectTestsCommandServiceError.projectNotFound(basePath)
            }
        }
    }
}
