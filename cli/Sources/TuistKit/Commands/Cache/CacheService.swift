...
struct CacheService: CacheServicing {
    ...
    func run(
        ...
        let targetsToBinaryCache = cacheableTargets.reduce(into: [Platform: Set<TargetQuery>]()) { acc, next in
            next.0.target.supportedPlatforms.forEach { platform in
    ...
    /// xcodebuild invocations might delete old activity logs, so to prevent that from happening, we copy them into a temporary
    /// directory.
...
//        }
        ...
        // If there's a bundle target for multiple platforms the bundle that we'll end persisting is the one for the last platform
        // that we build.
