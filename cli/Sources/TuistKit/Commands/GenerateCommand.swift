...
public struct GenerateCommand: ParsableCommand {
    ...
    public static var configuration: CommandConfiguration {
        CommandConfiguration(
            ...
            subcommands: [GenerateRunCommand.self, GenerationListCommand.self, GenerationShowCommand.self],
...
public struct GenerateRunCommand: AsyncParsableCommand, RecentPathRememberableCommand {
    ...
    @Option(
        name: .long,
        help:
        "Binary cache profile to use: \(BaseCacheProfile.allCases.map(\.rawValue).joined(separator: ", ")), or a custom profile name. Defaults to the profile configured in Tuist.swift, or 'only-external' if not configured.",
