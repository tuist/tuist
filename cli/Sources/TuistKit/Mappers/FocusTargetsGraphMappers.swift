...
public struct FocusTargetsGraphMappers: GraphMapping {
    ...
    public func map(graph: Graph, environment: MapperEnvironment) throws -> (Graph, [SideEffectDescriptor], MapperEnvironment) {
        ...
        // Rewrite autogenerated insight post-action targets that point at a non-filtered target
        // (e.g. the alphabetically first test target across the whole workspace) to a surviving
        // target, so the action's environmentBuildable still exposes BUILD_DIR / DERIVED_DATA_DIR
        // / CONFIGURATION to `tuist inspect test|build` without dragging the off-shard target
        // into the kept set via the execution-action preservation pass below.
        graph.workspace.schemes = graph.workspace.schemes.map { scheme in
            Self.rewriteAutogeneratedInsightActionTargets(
                scheme: scheme,
    ...
    private static let autogeneratedInsightActionTitles: Set<String> = [
        "Push test insights",
        "Push build insights",
    ]

    private static func rewriteAutogeneratedInsightActionTargets(
        scheme: Scheme,
        ...
        if var buildAction = scheme.buildAction {
            buildAction.preActions = buildAction.preActions.map {
                rewriteAutogeneratedInsightActionTarget($0, filteredTargetRefs: filteredTargetRefs, fallback: buildFallback)
            }
        ...
        if var testAction = scheme.testAction {
            testAction.preActions = testAction.preActions.map {
                rewriteAutogeneratedInsightActionTarget($0, filteredTargetRefs: filteredTargetRefs, fallback: testFallback)
            }
    ...
    private static func rewriteAutogeneratedInsightActionTarget(
        _ action: ExecutionAction,
        filteredTargetRefs: Set<TargetReference>,
        fallback: TargetReference?
    ) -> ExecutionAction {
        guard autogeneratedInsightActionTitles.contains(action.title),
