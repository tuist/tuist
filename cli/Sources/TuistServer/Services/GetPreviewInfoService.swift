...
@Mockable
public protocol GetPreviewInfoServicing: Sendable {
    func getPreviewInfo(
        _ previewId: String,
        fullHandle: String,
        serverURL: URL
...
public struct GetPreviewInfoService: GetPreviewInfoServicing {
    ...
    public func getPreviewInfo(
        ...
        case let .forbidden(forbiddenResponse):
            ...
            case let .json(error):
                throw GetPreviewInfoServiceError.forbidden(error.message)
            }
        case let .notFound(notFoundResponse):
            switch notFoundResponse.body {
            case let .json(error):
                throw GetPreviewInfoServiceError.notFound(error.message)
            }
        case let .unauthorized(unauthorized):
            switch unauthorized.body {
            case let .json(error):
                throw GetPreviewInfoServiceError.unauthorized(error.message)
            }
        case let .badRequest(badRequest):
            switch badRequest.body {
            case let .json(error):
                throw GetPreviewInfoServiceError.badRequest(error.message)
            }
        }
    }
}
