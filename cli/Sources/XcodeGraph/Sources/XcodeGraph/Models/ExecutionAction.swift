...
public struct ExecutionAction: Equatable, Codable, Sendable {
    ...
    /// True when the action was added automatically by Tuist (e.g. the autogenerated insight
    /// post-actions). Such an action's `target` is purely advisory — it exists only to surface
    /// `environmentBuildable` build settings to the script, and the action does not depend on
    /// that specific target. Focus is therefore free to leave the field nil and let the scheme
    /// generator pick any surviving target's build settings at generation time.
