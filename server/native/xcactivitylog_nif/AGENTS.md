...
- The CAS database is an immutable, checkpointed snapshot from the build archive, not the proxy's live database. Open it with SQLite's `immutable` URI option: a read-only WAL-mode database without its `-wal`/`-shm` sidecars can open successfully but silently fail every metadata query. Keep the sidecar-free WAL regression and the compiler analytics ShellSpec in `kura/spec/e2e/xcode_analytics_spec.sh`.
