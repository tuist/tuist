...
- Account billing information and subscriptions, including the daily public cache egress byte totals reported to Stripe for metered billing. Each Stripe meter event is associated with the account's Stripe customer identifier and the calendar date represented by the aggregate.
...
- **Kura usage events** (`kura_usage_events` table): Hourly account and project cache-traffic rollups reported by Kura nodes. Columns include the event, account, project, node, and region identifiers; traffic plane, network path, direction, operation, protocol, and artifact kind; transferred bytes and request count; window start and duration; and insertion timestamp. These events power the operational cache-usage page. Egress delivered over the public Internet is reported to Stripe for billing, while private or unknown network paths, peer traffic, and customer-operated traffic are excluded from that meter. The PostgreSQL `kura_billing_events` ledger records the account, Kura event identifier, and reporting timestamp after Stripe accepts an event so repeated delivery is not billed twice. Neither table currently has an automatic expiration policy.
