...
- Tailscale JIT elevation requests (`tailscale_jit_requests` table): Slack-driven just-in-time elevation requests for the Tuist team's internal Kubernetes admin access. Columns: `id` (PK), `requester_email` (resolved from Slack via `users.info`), `requester_slack_id`, `target_group` (the Tailscale break-glass group, e.g. `group:tuist-prod-write`), `intent` (free-text rationale supplied by the requester), `ttl_seconds`, `status` (`pending` / `approved` / `denied` / `expired` / `failed` / `cancelled`), `slack_channel_id`, `slack_message_ts`, `approver_email`, `approver_slack_id`, `approved_at`, `denied_at`, `expires_at` (Slack-approval deadline), `failure_reason`, `inserted_at`, `updated_at`. This is operator-side data about Tuist staff, not customer data; included for completeness of the per-table inventory. The Tailscale OAuth client credentials and Slack signing secret backing this flow are stored in 1Password and are not in the database.
- Tailscale JIT elevations (`tailscale_jit_elevations` table): runtime grants spawned from approved requests above. Columns: `id` (PK), `request_id` (FK to `tailscale_jit_requests`), `requester_email`, `target_group`, `expires_at` (when the grant lapses), `status` (`active` / `reverting` / `reverted` / `revert_failed`), `reverted_at`, `revert_failure_reason`, `inserted_at`, `updated_at`. Same scope as the requests table: operator audit trail for break-glass access into our own clusters.
