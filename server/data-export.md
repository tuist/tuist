...
- Kura usage rollups (`kura_usage_events` ClickHouse table): per-minute usage records pushed by managed Kura nodes, including account and project handles, mapped account/project ids when available, node id, region, traffic plane, direction, operation, protocol, artifact kind, transferred bytes, request count, and window timestamps. Used for usage reporting and future egress billing.
