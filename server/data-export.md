...
- **Recent test case runs per case** (`test_case_runs_recent_per_case` materialized view): AggregatingMergeTree keyed on (`project_id`, `test_case_id`) with bounded `groupArrayLast` aggregate states for recent `(ran_at, is_flaky)` and `(ran_at, status = 'success')` tuples. Powers rolling-window test automations; contains no data not already covered by the source `test_case_runs` table.
