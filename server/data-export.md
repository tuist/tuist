...
- Legacy test attachment retention cursor (`legacy_test_attachment_retention_cursors` table): global cleanup progress for test attachments created before attachment metadata included a test-run identifier. The cursor stores the completed and active insertion-time boundaries plus the last processed test-case-run and attachment identifiers, so each retention-time band is scanned once rather than once per account. It contains operational progress only and does not change the exported attachment metadata or object keys.
...
- **Test case run project lookup** (`test_case_run_projects` table): A slim copy of each test case run's identifier, project identifier, and insertion timestamp, ordered by run identifier for bounded ownership lookups. It is maintained from `test_case_runs`, contains no data beyond those source rows, and is exported through the source records rather than as a separate dataset.
