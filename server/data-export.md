...
- Kura geographic attribution telemetry: region-level location derived from a request's client IP and from the serving node's own egress IP, used for capacity and node-placement decisions. Two granularities are emitted. Country (ISO 3166-1): the `client_country` Prometheus label on `kura_http_requests_total`, the `geo.country.iso_code` OTel span attribute, and the same `geo.country.iso_code` OTel resource attribute for the node. Subdivision (ISO 3166-2, e.g. `US-CA`, roughly state or province level): the `geo.region.iso_code` OTel span attribute and the same `geo.region.iso_code` OTel resource attribute for the node. These follow the OpenTelemetry `geo.*` semantic conventions. Subdivision is deliberately kept off Prometheus metrics to bound label cardinality, so it exists only on sampled traces with the trace store's normal short retention. Both granularities are coarse region-level data, never address-level. Raw client IP addresses are never persisted: the IP is resolved to a country and subdivision in-process and immediately discarded, so there is no per-account or per-user record to export.
