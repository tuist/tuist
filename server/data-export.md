...
- **Per-account availability and concurrency configuration**: whether runners are enabled for an account is gated by the `:runners` feature flag (`Tuist.FeatureFlags.runners_enabled?/1`). Whether an account receives co-located private Kura cache infrastructure is controlled independently by the account-actor gates on the `:runner_cache` feature flag. Account-actor gates for both flags are stored in PostgreSQL `feature_flags`; the actor target contains the account identifier, and the operator-managed gate remains until it is cleared. PostgreSQL `runner_concurrency_limits` stores one row per account and runner platform (`linux | macos`) with the aggregate `vcpus` and `memory_gb` that active runners may consume concurrently. Runner concurrency rows do not contain observed usage history and are deleted with their account.
