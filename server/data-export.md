...
- **Workflow_job lifecycle** (ClickHouse `runner_jobs`, ReplacingMergeTree on `workflow_job_id`): one logical row per workflow_job carrying the full lifecycle from `queued` → `claimed` → `running` → `completed`. Columns include the GitHub correlation fields (`workflow_job_id`, `workflow_run_id`, `run_attempt`, `workflow_name`, `job_name`, `head_branch`, `head_sha`, `repository`), lifecycle state (`status`, `conclusion`), timestamps (`enqueued_at`, `claimed_at`, `started_at`, `completed_at`, `updated_at`), binding (`pod_name`, `runner_name`), the per-step breakdown (`steps`, a JSON array captured from the `workflow_job.completed` webhook carrying each step's name, status, conclusion, number, and start/finish timestamps), and the captured-log mirror (`log_state`, `log_line_count`, `log_archive_key` — see the runner job logs entry below). Powers the customer-facing "queued / running / recent runs" surfaces and the job detail page's step list.
...
- **Runner job log archives** (S3 `runners/{account_id}/{workflow_job_id}/runner.log.gz`): once a job finishes, `Tuist.Runners.Workers.ArchiveLogsWorker` gzips the full captured log into a single object so the "Download logs" action serves a presigned URL instead of re-streaming ClickHouse. Same content as `runner_job_logs`, rendered to plain text (`<ISO timestamp> <message>` per line).
