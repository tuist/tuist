...
- **Kura usage events** (`kura_usage_events` table): Hourly account and project cache-traffic rollups reported by Kura nodes. Columns include the event, account, project, node, and region identifiers; traffic plane, direction, operation, protocol, and artifact kind; transferred bytes and request count; window start and duration; and insertion timestamp. These events power the operational cache-usage page. Public egress from Tuist-managed regions is also aggregated by day and reported to Stripe for billing, while private runner-cache, peer, and customer-operated traffic is excluded from that meter. The table currently has no automatic expiration policy.
