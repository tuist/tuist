...
- **Per-account availability and concurrency configuration**: whether runners are enabled for an account is gated by the `:runners` feature flag (`Tuist.FeatureFlags.runners_enabled?/1`). PostgreSQL `accounts` also stores independent aggregate resource limits for each runner platform: `runner_linux_vcpus_limit`, `runner_linux_memory_gb_limit`, `runner_macos_vcpus_limit`, and `runner_macos_memory_gb_limit`. These limits control how much active runner CPU and memory the account may consume concurrently; they do not contain observed usage history.
