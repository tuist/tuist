...
- Kura server records (`kura_servers` table): per-account Kura server configuration including region, image tag, public URL, status, and the observed-state projection (`observed_image_tag`, `observed_ready_at`, `last_observed_at`) recording which image the backing cluster reports running and when it was last confirmed serving
