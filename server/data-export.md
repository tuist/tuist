...
- Pending automation alert test-case evaluations (`automation_alert_pending_test_case_evaluations` table): transient per-alert sets of affected test case IDs waiting for the next flaky-test evaluation drain. Rows include the automation alert id, test case id, insertion timestamp, and an internal generation counter used to preserve IDs that are updated while evaluation is in flight; they are deleted after successful evaluation or when the parent automation alert is deleted.
