...
- **Workflow_job lifecycle** (ClickHouse `runner_jobs`, ReplacingMergeTree on `workflow_job_id`): one logical row per workflow_job carrying the full lifecycle from `queued` → `claimed` → `running` → `completed`. Columns include the GitHub correlation fields (`workflow_job_id`, `workflow_run_id`, `run_attempt`, `workflow_name`, `job_name`, `head_branch`, `head_sha`, `repository`), lifecycle state (`status`, `conclusion`), timestamps (`enqueued_at`, `claimed_at`, `started_at`, `completed_at`, `updated_at`), binding (`pod_name`, `runner_name`), and the per-step breakdown (`steps`, a JSON array captured from the `workflow_job.completed` webhook carrying each step's name, status, conclusion, number, and start/finish timestamps). Powers the customer-facing "queued / running / recent runs" surfaces and the job detail page's step list.
