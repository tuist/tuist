defmodule Tuist.IngestRepo.Migrations.RenameAutomationStatesToAutomationTriggers do
  ...
  def up do
    ...
    execute(
      "ALTER TABLE automation_triggers ADD INDEX idx_automation_id (automation_id) TYPE bloom_filter GRANULARITY 4"
    )

    execute(
      "ALTER TABLE automation_triggers ADD INDEX idx_test_case_id (test_case_id) TYPE bloom_filter GRANULARITY 4"
    )
  end
