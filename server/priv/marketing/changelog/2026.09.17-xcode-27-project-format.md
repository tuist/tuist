...
Because the format is still experimental in Xcode itself, you should treat this as an opt in for teams that are already on Xcode 27 across local development and CI. Older Xcode versions can't read `project.xcproj`, so make sure every machine that opens the workspace, including runners, is on 27 or later before you turn it on.
