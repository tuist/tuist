...
On the first successful run, we store the hashes we just computed – locally and remotely (if you have a [Tuist project](https://tuist.dev/en/docs/server/introduction/accounts-and-projects#projects) set up). On a subsequent run, Tuist can skip all tests if nothing changes. But what if something _did_ change?
