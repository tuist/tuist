...
<script>
  // Intersection Observer to start/pause/resume iframe animations based on visibility
  ...
  // Track which iframes have started
    ...
    entries.forEach(entry => {
      ...
      if (entry.isIntersecting) {
        if (!startedIframes.has(iframe)) {
          // Start animation when iframe becomes visible for the first time
          startedIframes.add(iframe);
          iframe.contentWindow?.postMessage({ action: 'start' }, '*');
        } else {
          // Resume animation when iframe is visible again
