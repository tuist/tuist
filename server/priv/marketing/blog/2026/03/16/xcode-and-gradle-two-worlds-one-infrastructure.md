...
Xcode gives you no structured way to observe a build as it happens. Instead, after a build finishes, Xcode writes an `.xcactivitylog` file to your derived data directory. For Gradle developers: derived data is where Xcode stores all build outputs, indexes, and logs, similar to Gradle's `.gradle` and `build` directories. The activity log is a binary format that contains timing information, errors, warnings, and file-level compilation data. To extract [build insights](https://docs.tuist.dev/en/guides/features/build-insights/xcode), we parse this file after the build using `tuist inspect build`, which runs as a scheme post-action (a script Xcode executes after a build or test scheme completes). You can see how this looks on [our own project's build dashboard](https://tuist.dev/tuist/tuist/builds).
