...
Worth being honest about the constraints first. Unlike NPM, which has a central registry and an API you can hit to get fast answers like "which versions of this package exist", the Swift package ecosystem is decentralized by design. That decentralization is a real strength, but it comes with this trade-off: the traversal of the graph is not something we can fundamentally speed up. A registry-first approach would change a lot of things, but it is what it is, and we worked within those constraints rather than wishing them away.
