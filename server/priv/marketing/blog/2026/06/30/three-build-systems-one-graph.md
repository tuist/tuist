...
Coding harnesses are what made us want to open it. They broke a model almost every build system was designed around, one project, one cache. Spin up a few worktrees so a couple of agents can work in parallel and, as far as the build system is concerned, you have several unrelated projects that share nothing, each compiling the same code from scratch on the same machine. We gave that missing thing a name in [another post](https://pepicrft.me/blog/distributed-incrementality/), distributed incrementality, but the part that stuck with us was bigger. If an assumption that basic had quietly stopped holding, how many others had too? So we did the obvious thing and started tinkering with an automation layer of our own, one a harness could drive directly. And almost immediately we hit a wall: what should it even look like? You can't design that without answering something more basic first. What is a build system, really, once you take everything else away? This is the first post in a series where we work through that in the open.
...
To make this concrete, we built a small graph with a premise behind it. A lot of the appeal of [React Native](https://reactnative.dev) comes down to two things. You write the app once and run it everywhere, and it feels fast to work on, because a dynamic JS runtime keeps the build graph small and gets you close to hot reloading. You pay for it with the abstraction sitting between your code and the platform. So what happens if you drop the abstraction? Lean on Rust as the shared layer instead, native on both sides, and put the speed back with a build system that keeps the graph fast rather than a runtime that skips it. That's the bet the example is built on.
...
Come back to the thing that set this off: the coding harness. We went into build systems because a harness needs a way to close its own loop, and the test for any interface we landed on was whether an agent could live in it without much ceremony. Looking at the finished thing, a Rust core and two native apps built out of plain scripts, we think it holds. The only vocabulary that isn't already bash is the `# once` comments, and an agent reaches for them only when it writes or changes a script.

That's most of what we wanted. Writing these scripts is squarely what the models are good at. Keeping the comments honest is a local edit, because they live next to the command they describe instead of in a separate build model that drifts out of sync. And running the graph is just running scripts, so the caching and the remote execution never come up as something the agent has to reason about. The layer we wanted a harness to drive ended up looking a lot like the scripts already in the repo, with a few comments on top.

We should be honest about where this stops. Plain scripts are coarse. If you tried to model a real build graph this way, the kind with thousands of fine-grained nodes and dependencies discovered mid-build, you'd be fighting the grain the whole time, and you'd be better off with an engine actually built for it, whether that's rustc's own incremental compilation or Bazel. You can stretch scripts in that direction, but that's not where they're good. Where they're good is one level up, at the coarse work that's already written as scripts: build this, test that, package the thing, pull the dependencies. That's the altitude a harness tends to work at when it's closing a loop, so the entry point and the need line up.

## Where this goes

None of this is tied to compilers. A script that installs dependencies or runs a test suite in a JavaScript workspace is a node like any other, and skipping it when nothing changed is [Nx](https://nx.dev) territory. [Once](https://github.com/tuist/once) doesn't care whether the command is `rustc` or `npm test`. And where the work runs stays a separate concern: Once talks to infrastructure through a [provider interface](https://docs.buildonce.dev/guide/infrastructure/), so the graph doesn't care who's underneath it. Tuist is one provider, the one we're building, so you point Once at it and get low-latency remote caching without standing anything up. The graph stays yours, the infrastructure is swappable.

That's the shape of it. The parts we don't have answers for yet are where the series goes from here:

- **The toolchain.** The graph knows what a node reads and writes, but not that it needs `rustc` to exist at all. On your machine that's invisible. The second the work runs on a fleet, the tools have to be there too, at the same versions. Bazel and Buck2 solved this by making the toolchain a declared, hermetic dependency. The open question for us is how to stand up those environments fast enough that running a graph remotely actually pays off.
- **Observability.** A build graph is a great thing for a harness to see into: what ran, what ran last time, and whether what the scripts declare still matches what they actually do.
- **Tests.** An interface that hides the differences between test runners without flattening what makes each one useful.
- **Reusability.** Bazel and Buck built communities around sharing logic as rules and walking the graph with [aspects](https://bazel.build/extending/aspects) and [BXL](https://buck2.build/docs/bxl/). We're not sure that trade holds when a harness can reproduce just the pieces you need, letting you own your graph and fix your own problems instead of waiting for the next version of Xcode.
- **The client.** Once you assume it's an agent acting on someone's behalf, not a person at a terminal, the plumbing changes shape, from how it authenticates to how you attribute a cached result back to who, and what, produced it.
