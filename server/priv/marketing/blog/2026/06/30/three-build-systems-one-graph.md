...
That's the shape of it. The parts I don't have answers for yet are where the series goes from here:
...
- **Reusability.** Bazel and Buck built communities around sharing logic as rules and walking the graph with [aspects](https://bazel.build/extending/aspects) and [BXL](https://buck2.build/docs/bxl/). I'm not sure that trade holds when a harness can reproduce just the pieces you need, letting you own your graph and fix your own problems instead of waiting for the next version of Xcode.
