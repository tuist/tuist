...
One cache specific surprise did show up, but it was on us. Cache warming attempted a Mac Catalyst build even though the app does not support Catalyst. That should not happen. Codex traced it to a gap in `ExternalProjectsPlatformNarrowerGraphMapper`. When dependencies are declared as package products, we were not mapping those products to their generated external targets during platform narrowing, so those external targets kept their default destinations and still advertised Mac Catalyst. Codex wired the external dependency mapping into the mapper so package product edges are treated like target dependencies when narrowing. That aligns cache warming with normal builds and removes Catalyst from external packages unless a local target actually supports it.
