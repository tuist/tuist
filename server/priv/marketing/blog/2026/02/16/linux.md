...
Before touching the Tuist CLI itself, we had already invested in Linux support across our open-source libraries. [Noora](https://github.com/tuist/Noora), our terminal UI framework, and [FileSystem](https://github.com/tuist/FileSystem), our file system abstraction, already compiled on Linux. This meant the foundational layers were ready.

While Tuist was already heavily modularized, we still had a couple of monolith modules that mixed platform-specific and platform-independent code. To enable a gradual migration, we split those first. `TuistSupport`, our shared utility layer, was broken into focused modules like `TuistConstants`, `TuistEnvironment`, and `TuistLogging`. `TuistKit`, which contained all the command logic, was split into per-command modules: `TuistAuthCommand`, `TuistCacheCommand`, `TuistBuildCommand`, and so on. Once separated, each module could independently target Linux compilation without pulling in macOS-only dependencies. That not only allowed for a gradual migration, but it allowed us to use compiler directives like `#if os(macOS)` and `#if canImport(...)` sparingly, keeping the code easier to read and maintain.
