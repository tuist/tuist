...
Many native toolchains were designed around a developer building on one machine and still have work to do before they can use the amount of parallel build work a modern organization produces. Local incrementality and remote caching are valuable steps, but they do not by themselves make a build ready to run its tasks on a fleet. People often ask us whether Xcode's compilation cache will grow into that kind of system. We do not know. It is a toolchain decision, and we do not know when, or whether, Apple will prioritize it.
