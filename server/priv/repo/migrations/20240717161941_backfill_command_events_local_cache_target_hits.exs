defmodule Tuist.Repo.Migrations.BackfillCommandEventsLocalCacheTargetHits do
  use Ecto.Migration
  import Ecto.Query
  ...
  def page_query(last_id) do
    from(
      e in "command_events",
      select: e.id,
      where: is_nil(e.local_cache_target_hits) and e.id > ^last_id,
      order_by: [asc: e.id],
      limit: @batch_size
    )
  end
