defmodule Tuist.Repo.Migrations.CreateLegacyBaseline do
  ...
  def up do
    unless baseline_already_loaded?() do
      ...
      sql()
      |> String.replace("public.", schema_prefix)
      |> execute_sql_dump()
    end
  end
  ...
  defp split_sql_statements(sql) do
    {statements, current, has_sql} = split_sql_statements(sql, [], [], false, :normal)

    statements
    |> maybe_add_statement(current, has_sql)
    |> Enum.reverse()
  end

  defp split_sql_statements(<<>>, statements, current, has_sql, _state) do
    {statements, current, has_sql}
  end

  defp split_sql_statements(<<";", rest::binary>>, statements, current, true, :normal) do
    statement = current |> Enum.reverse() |> IO.iodata_to_binary() |> String.trim()
    split_sql_statements(rest, [statement | statements], [], false, :normal)
  end

  defp split_sql_statements(<<";", rest::binary>>, statements, _current, false, :normal) do
    split_sql_statements(rest, statements, [], false, :normal)
  end

  defp split_sql_statements(<<"--", rest::binary>>, statements, current, has_sql, :normal) do
    split_sql_statements(rest, statements, ["--" | current], has_sql, :line_comment)
  end

  defp split_sql_statements(<<"/*", rest::binary>>, statements, current, has_sql, :normal) do
    split_sql_statements(rest, statements, ["/*" | current], has_sql, :block_comment)
  end

  defp split_sql_statements(<<"'", rest::binary>>, statements, current, _has_sql, :normal) do
    split_sql_statements(rest, statements, ["'" | current], true, :single_quote)
  end

  defp split_sql_statements(<<"\"", rest::binary>>, statements, current, _has_sql, :normal) do
    split_sql_statements(rest, statements, ["\"" | current], true, :double_quote)
  end

  defp split_sql_statements(<<"$", _rest::binary>> = sql, statements, current, _has_sql, :normal) do
    case dollar_quote_delimiter(sql) do
      nil ->
        <<character, rest::binary>> = sql
        split_sql_statements(rest, statements, [<<character>> | current], true, :normal)

      delimiter ->
        <<_delimiter::binary-size(byte_size(delimiter)), rest::binary>> = sql
  ...
  defp split_sql_statements(<<"\n", rest::binary>>, statements, current, has_sql, :line_comment) do
    split_sql_statements(rest, statements, ["\n" | current], has_sql, :normal)
  end

  defp split_sql_statements(<<"*/", rest::binary>>, statements, current, has_sql, :block_comment) do
    split_sql_statements(rest, statements, ["*/" | current], has_sql, :normal)
  end

  defp split_sql_statements(<<"''", rest::binary>>, statements, current, has_sql, :single_quote) do
    split_sql_statements(rest, statements, ["''" | current], has_sql, :single_quote)
  end
  ...
  defp split_sql_statements(<<"\"\"", rest::binary>>, statements, current, has_sql, :double_quote) do
    split_sql_statements(rest, statements, ["\"\"" | current], has_sql, :double_quote)
  end
  ...
  defp split_sql_statements(sql, statements, current, has_sql, {:dollar_quote, delimiter}) do
    if String.starts_with?(sql, delimiter) do
      <<_delimiter::binary-size(byte_size(delimiter)), rest::binary>> = sql
      split_sql_statements(rest, statements, [delimiter | current], has_sql, :normal)
    else
      <<character, rest::binary>> = sql
  ...
  defp split_sql_statements(<<character, rest::binary>>, statements, current, has_sql, state) do
    has_sql = has_sql or (state == :normal and character not in ~c" \t\n\r")
    split_sql_statements(rest, statements, [<<character>> | current], has_sql, state)
  end

  defp maybe_add_statement(statements, current, true) do
    statement = current |> Enum.reverse() |> IO.iodata_to_binary() |> String.trim()
    [statement | statements]
  end

  defp maybe_add_statement(statements, _current, false), do: statements

  defp dollar_quote_delimiter(<<"$", rest::binary>>) do
    with {tag_size, 1} <- :binary.match(rest, "$"),
         tag <- binary_part(rest, 0, tag_size),
         true <- Regex.match?(~r/^$|^[a-zA-Z_][a-zA-Z0-9_]*$/, tag) do
      "$" <> tag <> "$"
    else
      _ -> nil
    end
