...
Module cache binaries do not go through this HTTP client. They transfer over the cache's Remote Execution API endpoint, so the `TUIST_HTTP_*` variables above do not apply to them. A module cache download that stops partway resumes from the byte it reached instead of starting over, and keeps resuming for as long as each attempt gets further into the artifact; one that delivers nothing for 30 seconds is cut and resumed. A module cache upload that breaks starts over. Downloaded content is verified against its SHA-256 digest before it is used.
