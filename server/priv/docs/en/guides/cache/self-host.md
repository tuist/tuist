...
>
> You can connect self-hosted cache nodes to either the hosted Tuist server (`https://tuist.dev`) or a self-hosted Tuist server. Self-hosting the Tuist server itself requires a separate server license. See the <TuistWeb.Docs.MarkdownComponents.localized_link href="/guides/server/self-host/install">server self-hosting guide</TuistWeb.Docs.MarkdownComponents.localized_link>.
...
For a detailed explanation of how each process works internally, see the <TuistWeb.Docs.MarkdownComponents.localized_link href="/guides/cache/architecture">architecture guide</TuistWeb.Docs.MarkdownComponents.localized_link>.
