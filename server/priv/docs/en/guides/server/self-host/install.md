...
> [!NOTE]
> `.xcresult` parsing leans on `xcresulttool` from Xcode, so the worker that consumes `:process_xcresult` only runs on macOS. On a self-hosted Linux-only deployment nothing claims the queue and `tuist test` results aren't ingested. To enable test-result ingestion, run a macOS host that boots a Tart VM image of the Tuist release with `TUIST_XCRESULT_PROCESSOR_MODE=1`, points at the same Postgres, and consumes the `:process_xcresult` queue. The managed cloud schedules these as standard k8s `Deployment` Pods onto a virtual node fronted by [vk-orchard](https://github.com/tuist/tuist/tree/main/infra/vk-orchard) (Virtual Kubelet provider) which translates them to Tart VMs on a Scaleway Mac mini fleet via Cirrus Labs' Orchard. Self-hosters who want the same shape can deploy `vk-orchard` against their own Orchard cluster; setting `xcresultProcessor.enabled: true` in the Helm chart then schedules the workload onto it.
