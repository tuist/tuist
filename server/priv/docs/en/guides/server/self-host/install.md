...
> [!NOTE]
> `.xcresult` parsing leans on `xcresulttool` from Xcode, so the worker that consumes `:process_xcresult` only runs on macOS. The CLI handles this transparently for self-hosted deployments: `tuist test` defaults to **local** processing mode against any non-`tuist.dev` server, parsing the xcresult on the developer machine (which already has Xcode for the build itself) and uploading the structured results. Your Linux-only server doesn't need a Mac fleet — test results are ingested without any additional infrastructure.
>
> If you specifically want server-side xcresult parsing (for example, running `tuist test` from a runner that doesn't keep the xcresult around), pass `--inspect-mode remote` at invocation time and stand up a macOS Pod fleet. See the [cluster-api-provider-scaleway-applesilicon](https://github.com/tuist/tuist/tree/main/infra/cluster-api-provider-scaleway-applesilicon) operator if you're on Scaleway.
