...
> [!NOTE]
> `.xcresult` parsing leans on `xcresulttool` from Xcode, so the worker that consumes `:process_xcresult` only runs on macOS. On a self-hosted Linux-only deployment nothing claims the queue and `tuist test` results aren't ingested. To enable test-result ingestion, set `macosFleet.enabled: true` (plus `macosFleet.controlPlane.host` + `macosFleet.machine.podCIDR`) and `xcresultProcessor.enabled: true` in the Helm chart. The [cluster-api-provider-scaleway-applesilicon](https://github.com/tuist/tuist/tree/main/infra/cluster-api-provider-scaleway-applesilicon) operator orders Scaleway Mac minis declaratively from your cluster's CRs and joins them as `kubernetes.io/os=darwin` nodes via [tart-cri](https://github.com/tuist/tuist/tree/main/infra/tart-cri); the xcresult-processor Deployment schedules onto them automatically. Scaling is `kubectl scale machinedeployment <name> --replicas=N`.
