...
> [!NOTE]
> `.xcresult` parsing leans on `xcresulttool` from Xcode, so the worker that consumes `:process_xcresult` only runs on macOS. On a self-hosted Linux-only deployment nothing claims the queue and `tuist test` results aren't ingested. To enable test-result ingestion, join a Mac mini to your cluster as a real Kubernetes node using [tart-cri](https://github.com/tuist/tuist/tree/main/infra/tart-cri) (CRI implementation that drives Tart on macOS). Once the Mac mini shows up in `kubectl get nodes` with `kubernetes.io/os=darwin`, set `xcresultProcessor.enabled: true` in the Helm chart and the xcresult worker will schedule onto it as a standard `Deployment` Pod that boots a Tart VM running the Tuist server release with `TUIST_XCRESULT_PROCESSOR_MODE=1`. The bootstrap script at [`infra/tart-cri/platform/provision.sh`](https://github.com/tuist/tuist/blob/main/infra/tart-cri/platform/provision.sh) handles the join.
