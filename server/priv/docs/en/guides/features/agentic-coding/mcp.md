...
| `get_gradle_build` | Get build metadata, execution options, and the duration of the longest dependency chain. Set `include_execution_graph` to inspect task/transform dependencies and ordering; missing timings remain null. | `build_run_id` |
| `list_gradle_build_tasks` | List task outcomes, composite build identity, actual cacheability and execution reasons, incremental status, and observed remote operation timings. | `build_run_id` |
