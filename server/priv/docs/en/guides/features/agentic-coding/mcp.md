...
| Tool | Description | Required parameters |
|------|-------------|---------------------|
| `list_bazel_invocations` | List completed [Bazel Build Event Protocol](https://bazel.build/remote/bep) invocations and their correlated remote-cache totals for a project. Build pages use the `build` and `run` command records. | `account_handle`, `project_handle` |
...
| `list_bazel_test_results` | List final Bazel test-target summaries, including status, duration, retry count, and invocation identifier. | `account_handle`, `project_handle` |
| `get_bazel_test_result` | Get one final Bazel test-target summary. | `account_handle`, `project_handle`, `test_result_id` |
