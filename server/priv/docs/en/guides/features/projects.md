...
- **<.localized_link href="/references/cli/build">Build</.localized_link>:** This workflow not only generates the Xcode project but also employs `xcodebuild` to compile it.
- **<.localized_link href="/references/cli/test">Test</.localized_link>:** Operating much like the build workflow, this not only generates the Xcode project but utilizes `xcodebuild` to test it.
