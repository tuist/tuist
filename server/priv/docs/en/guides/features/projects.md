...
- **<TuistWeb.Docs.MarkdownComponents.localized_link href="/cli/build">Build</TuistWeb.Docs.MarkdownComponents.localized_link>:** This workflow not only generates the Xcode project but also employs `xcodebuild` to compile it.
- **<TuistWeb.Docs.MarkdownComponents.localized_link href="/cli/test">Test</TuistWeb.Docs.MarkdownComponents.localized_link>:** Operating much like the build workflow, this not only generates the Xcode project but utilizes `xcodebuild` to test it.
