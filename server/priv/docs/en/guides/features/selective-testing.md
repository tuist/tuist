...
- **`-testProductsPath` (`.xctestproducts`) — recommended for new pipelines.** It's a self-contained directory: the built `.xctest` binaries and their `.xctestrun` files live inside it with paths made relative to the bundle root. Zip it, ship it to any machine, unzip it, run. One artifact, no path-rewriting, no derived data to carry around. It's also the format that has had the selective-testing fast path the longest, so it's the most-exercised path.
- **`-xctestrun` (raw `.xctestrun`) — use it if you already have a pipeline built around derived data.** `xcodebuild build-for-testing` writes an `.xctestrun` at the top of `<derivedData>/Build/Products/` by default and points it at test bundles that live elsewhere in that tree. Portability means shipping the whole `Build/Products/` tree with paths intact, so it's typically used inside a single machine, or with a CI job that caches derived data as-is (which is why teams that grew up on this format tend to stay on it).
