...
A task's detail page shows analytics and a **Task executions** table with Project, Outcome, Branch, Ran by, Duration, and Ran at columns. **Ran by** shows CI for CI builds, the account name for local builds, or Unknown when the account is unavailable. Select a row to open its build. Composite builds remain separate using their build paths and root project names. Historical reports can show task durations and inferred project paths. Open an individual build to explore its dependencies when execution telemetry is available.
