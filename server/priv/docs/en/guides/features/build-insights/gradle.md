...
A task's detail page shows analytics and a **Task executions** table with Project, Outcome, Branch, Ran by, Duration, and Ran at columns. **Ran by** shows CI for CI builds, the account name for local builds, or Unknown when the account is unavailable. Select a row to inspect that task execution, including its cacheability, execution reasons, and available remote-operation timings. Execution details link back to the build and to the task overview. The Tasks table on an individual build opens these same execution details. Composite builds remain separate using their build paths and root project names. Historical reports can show task durations and inferred project paths. Open an individual build to explore its dependencies when execution telemetry is available.
...
## Explore one build's execution {#execution-graph}

Open the **Dependencies** tab on a Gradle build. Select an operation to explore its dependencies, consumers and ordering constraints. The timeline shows observed start and end times. Task details include cacheability, Gradle's execution reasons, incremental status, and observed remote operation durations.

The longest dependency chain includes tasks, artifact transforms, dependencies, `mustRunAfter` constraints and finalizers. Soft `shouldRunAfter` ordering is displayed separately. This is a duration-weighted dependency model, **not the build's actual critical path**: worker scheduling, resource contention and other unobserved waits can add time. Missing timings, incomplete graphs and cycles make the duration model unavailable instead of treating missing operations as zero.

Execution collection uses Gradle's internal operation APIs and is compatible with configuration-cache reuse. Reports identify the telemetry version. Graph capture is bounded to 20,000 nodes and 100,000 edges; a truncated or unsupported capture is marked incomplete. Cache hits with an unobserved origin are reported as `cache_hit` rather than attributed to the local or remote cache.
