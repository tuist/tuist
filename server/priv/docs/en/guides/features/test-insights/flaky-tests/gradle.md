...
In the dashboard, the test run carries the gate's verdict beside its branch and commit, and every stressed test case is badged in the run's test case list. A test case that disagreed with itself is flaky, so it appears with the run's flaky tests under a Stress gate label, expandable to each repetition and the failure it produced.
