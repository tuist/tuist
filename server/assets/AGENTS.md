...
## Responsibilities
- Timeline metrics use a responsive 2×2 grid with independent 120px plots and square card corners; the build-step viewport stays 600px tall. Search, target selection and the step legend sit above the step lanes below the metric grid, outside chart gesture handling, with a time ruler for each section.
- The inspector and its resize divider align with the step lanes below the metrics and controls, and retain the step viewport's height so details and logs stay beside the selected work.
