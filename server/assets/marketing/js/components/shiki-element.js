...
stylesheet.replaceSync(`
  ...
  pre.shiki {
    margin: 0px;
    padding: 0 var(--shiki-padding, 1em) var(--shiki-padding, 1.5em) var(--shiki-padding, 1em);
...
export class ShikiHighlightElement extends HTMLElement {
  ...
  constructor() {
    ...
    this._observer = new MutationObserver((mutations) => this._onDomChanged(mutations));
  }
  ...
  async _triggerUpdate() {
    this._highlightedContent = await shiki.codeToHtml(this.textContent ?? "", this.options);
