defmodule Tuist.Accounts do
  ...
  On the Tuist-hosted server the endpoints are the account's Kura instances.
  While the account has none serving, the answer holds only the account's own
  custom endpoints, and only for an account that has never routed through Kura:
  there is no Tuist-hosted lane to stand in, so the client builds without a
  remote cache until its instance serves. A self-hosted server answers with the
  endpoints it is configured with.
  ...
  usual interval leaves a build without a remote cache long after its instance
  is back.
  ...
  a poller.
  """
  def get_cache_resolution_for_handle(account_handle, origin \\ nil) do
    if Environment.tuist_hosted?() do
      hosted_cache_resolution(account_handle, origin)
    else
      %{endpoints: Environment.cache_endpoints() || [], provisioning: false}
    end
  end
  ...
  defp hosted_cache_resolution(account_handle, origin) when is_binary(account_handle) do
    case get_account_by_handle(account_handle) do
      %Account{} = account ->
        # A client asking where to send cache traffic is the request boundary
        # the demand-driven lifecycle measures: it covers the Xcode, Module, and
        # Gradle lanes uniformly, and it is the same call whether the client is
        # a developer machine or a runner. The write is buffered in memory and
        # flushed periodically, so this stays one ETS insert.
  ...
  defp hosted_cache_resolution(_account_handle, _origin), do: %{endpoints: [], provisioning: false}

  # What to answer while the account has no Kura instance serving: archived,
  # provisioning, draining, or refused for capacity. A lifecycle-managed account
  # gets nothing rather than its own custom endpoints, since routing archived
  # accounts down the custom-endpoint path would make archival the thing that
  # keeps that path alive. Accounts that have never routed through Kura keep the
  ...
  heartbeats. Provisioning is the only gate on whether these are handed to the
  CLI.
