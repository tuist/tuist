defmodule Tuist.Storage.Workers.DeleteExpiredGitLabCacheArtifactsWorker do
  @moduledoc false
  use Oban.Worker,
  ...
  def perform(%Oban.Job{args: args} = job) do
    with {:enabled, retention_days} <- effective_retention_days(args, :cache_artifacts),
         {:ok, next_continuation_token} <-
           CacheArtifactRetention.delete_expired(:gitlab_cache, options_from_args(args, retention_days)) do
