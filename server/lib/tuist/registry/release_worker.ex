defmodule Tuist.Registry.ReleaseWorker do
  @moduledoc """
  Downloads and uploads missing registry artifacts, and updates metadata in S3.

  Runs on the registry-population pod. The pod is single-replica so no
  distributed locking is needed; Oban's `unique` constraint prevents concurrent
  jobs for the same release within the population pod itself.
  """

  use Oban.Worker,
    queue: :registry_release,
    max_attempts: 3,
    unique: [
      keys: [:scope, :name, :tag],
  ...
  defp do_sync_release(scope, name, full_handle, tag) do
    case Environment.registry_github_token() do
      nil ->
  ...
  # packages that contain them. Repack the archive with symlinks resolved before upload.
  ...
  defp upload_source_archive(scope, name, version, archive_path) do
    ...
    Storage.upload_file(key, archive_path, content_type: "application/zip")
  end
  ...
  defp upload_manifest(scope, name, version, filename, content) do
    ...
    Storage.upload_content(key, content, content_type: "text/x-swift")
  end
