...
- `Kura.Lifecycle` archives by suspending, not deleting: `Provisioner.suspend/2` applies the manifest with `spec.suspended`, and the row becomes `:archived` only once `Provisioner.suspension_state/1` reports `:suspended` (workload stopped, retained volumes emptied) or the resource is gone. The daily sweep deletes suspensions archived for `Lifecycle.suspension_days/0` and stamps `suspension_released_at`. A return re-applies the normal manifest, which carries no suspension, so it only starts pods. Cache-endpoint resolution with no Kura endpoint enqueues `Workers.ProvisionOnDemandWorker` (one per account at a time), which runs `Lifecycle.provision_account/2` with the tick's eligibility rules, applies each instance coming up via `Reconciler.reconcile_server/1`, and hands it to `Workers.AwaitActivationWorker`, which checks `Reconciler.activate_when_ready/1` every second and never applies. The minute tick stays the authority for everything these paths miss.
