...
- `Kura.StableEndpoint` derives managed stable-host intent independently of image rollouts, including drain-pending rows. Rendering and hand-out flags default off and share an optional account allowlist. Placement demotion does not alter advertising. Retirement waits for a stable-ready survivor; the controller owns provider-confirmed withdrawal and the post-withdrawal drain. Endpoint requests read only expiring controller observations from KeyValueStore, preserving their original timestamp and checking generation; absent/stale evidence falls back to regional URLs. See `../../../infra/cache-dns/README.md`.
