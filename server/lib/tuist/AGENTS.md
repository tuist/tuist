...
- `Kura.StableEndpoint` derives managed stable-host intent independently of image rollouts, including drain-pending rows. Rendering and hand-out environment switches default off and share an optional account allowlist. Managed overlays enable those switches for canary and production; `FeatureFlags.kura_stable_hostname_enabled?/1` additionally requires the `kura_stable_hostname` account/global FunWithFlags opt-in in production, absent by default. Canary requires no account opt-in. Gate both advertising intent and hand-out so unselected production accounts publish no stable records. Placement demotion does not alter advertising. Retirement waits for a stable-ready survivor; the controller owns provider-confirmed withdrawal and the post-withdrawal drain. Persist readiness in `kura_servers.stable_endpoint` so the reconciler and every web replica share it even without Redis. Endpoint requests use this projection, preserving the original controller timestamp and checking generation at observation; absent/stale evidence falls back to regional URLs after three minutes. Archival and cold return clear the projection. See `../../../infra/cache-dns/README.md`.
