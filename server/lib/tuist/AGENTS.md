...
- Module-cache daily hits, misses and distinct module counts can be derived from the full invalidation breakdown with `module_timeseries_from_breakdown/2`. Derive these before filtering out hit-only modules or applying a list limit; count a module name once across products.
