...
- `Release` grants the processor read/write access to `bazel_profile_uploads` for profile processing and terminal-state updates. Keep the CNPG fallback SQL synchronized and verify processor paths under the restricted role, not only the schema owner.
