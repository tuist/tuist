...
- Runner Kura participates in account disk sizing and plan memory/CPU profiles. New and returning instances pin the account claim; the enrollment migration preserves existing runner claims at 50Gi. The runner pool does not advertise `tuist.dev/memory-ceiling-mib`, so keep `memory_ceiling_bin_packed` disabled there. Disk sizing remains account-scoped across public and runner regions, including their telemetry and resize history.
