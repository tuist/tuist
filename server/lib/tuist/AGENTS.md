...
- `Runners.Catalog.xcode_versions/0` is the set of Xcode versions a macOS pool can actually run, not the full Helm `runnersFleet.xcodeVersions`: the deploy workflow probes GHCR for each pool image and the chart leaves versions without one out of `TUIST_RUNNER_MACOS_XCODE_VERSIONS` while still rendering their pools. Everything that offers or validates a version reads this list (profile form, `Profile` validation, docs default table). A profile saved on a version outside it is not dispatched (`:xcode_version_unavailable`, since a queued job would count as demand for a pool that never starts), GitLab rejects its jobs with that reason, and the Profiles page warns about it. `config/config.exs` mirrors the Helm catalog for dev and tests; docs must not enumerate versions from it, because it cannot know which images are released.
