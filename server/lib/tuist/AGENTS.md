...
- Xcode code coverage (`Tuist.Tests.XcodeCoverage`) is tool-specific: the client reads `xccov`'s report off the result bundle and sends it as `xcode_coverage` with the test run, rows land in `xcode_coverage_files` as reported (one row per target and file), and `test_runs` carries two totals that count each path once. The totals ride the xcresult processing job's arguments like the stress verdict, because the worker rewrites the row from them. Gradle coverage gets its own model rather than a generic one.
