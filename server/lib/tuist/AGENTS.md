...
- Xcode code coverage (`Tuist.Tests.XcodeCoverage`) is tool-specific. Coverage is read from the result bundle by the shared Swift parser (`xccov` report + archive) wherever the bundle is processed: the macOS xcresult processor for uploaded bundles (the CLI writes a `tuist_coverage_manifest.json` with root spellings, Git blob ids and a partial flag into the bundle), or the client in local mode (sent as `xcode_coverage`). `xcode_coverage_files` holds one row per file with per-line execution counts and function arrays; partial runs carry forward the latest observed row for the same path and blob (30 days). Readers merge shard rows per path (union of lines, observed rows win over carried-forward ones); `test_runs` keeps reported and observed totals. Gradle coverage gets its own model rather than a generic one.
