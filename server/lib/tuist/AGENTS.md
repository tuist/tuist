...
## Responsibilities
- Metric automations accept a one-time `trigger_config.apply_actions_to_existing_matches` request on create/update. A fresh request starts a baseline generation even when the condition is unchanged. Baselines recheck matches in bounded batches and serialize publication per alert with a session advisory lock. Each test case's actions run outside a row-lock transaction after a short preflight check; edits may cancel the remaining work while an authorized action finishes. Slack notifications, trigger events, and the durable checkpoint follow once per group of up to 50 matches, so an interrupted group reruns its test case actions. Clear the request on completion. Silent publication deduplication tokens include the sorted test ID set so changed payloads on retry are not dropped. Read-only match counts enumerate bounded pages and share the baseline metric, trusted-branch validation, and current-state eligibility logic. Condition edits require a fresh opt-in. Omitting the request preserves pending work; explicit false cancels remaining actions. Baseline attempt generations invalidate stale workers; nullable `event_generation` separately scopes recovery history (falling back to `baseline_generation` for existing rows), so opt-ins and cancellations retain active recovery events. Returned action errors are logged and checkpointed without publishing a successful trigger, allowing the baseline and subsequent evaluation to progress.
- Scheduled metric evaluations collect triggers and recoveries from every evaluated range before running actions. `ActionExecutor.execute_grouped_actions/4` applies test case changes one at a time and posts each Slack action once per group of up to 50 test cases; a group of one keeps the message template. A failed delivery fails its whole group, so none of those test cases records an event.
