...
- Usage-based pricing (`Tuist.Billing.UsagePricing`, measured by `Tuist.Billing.UsageMeters`) is enabled per account by the `:usage_based_pricing` flag. For those accounts the nightly sync reports `cache_download_megabytes`, `cache_requests`, and `passing_test_cases` instead of `remote_cache_hit`, and the usage and billing pages show the new charges. Cache downloads and requests come from Kura (`kura_usage_events`) plus the legacy compilation cache (`cas_events`); traffic from private runner-cache regions is halved before reporting. Passing test cases exclude runs linked to Tuist Runners jobs (`test_runs.ci_run_id` = `runner_jobs.workflow_run_id`). Everything is attributed to the project owner and read over half-open windows.
