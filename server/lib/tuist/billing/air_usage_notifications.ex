defmodule Tuist.Billing.AirUsageNotifications do
  ...
  def enqueue(account_id, updated_at) do
    Repo.transaction(fn ->
      ...
      usage = account.current_month_remote_cache_hits_count
      threshold = threshold(usage, limit)

      if threshold && Billing.effective_plan(account) == :air && current_month?(updated_at) do
        enqueue_recipients(account, updated_at, threshold, usage, limit)
      end
    end)
  end

  def period_start(account, date) do
  ...
  defp enqueue_recipients(account, updated_at, threshold, usage, limit) do
    now = DateTime.truncate(DateTime.utc_now(), :second)
    period_start = period_start(account, updated_at)
    ...
    {_count, inserted} =
      Repo.insert_all(AirUsageNotification, notifications,
        on_conflict: :nothing,
        conflict_target: [:account_id, :user_id, :period_start, :threshold],
        returning: [:id]
      )
