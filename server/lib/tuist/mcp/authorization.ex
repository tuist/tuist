defmodule Tuist.MCP.Authorization do
  ...
  def authorize_project_id(action, project_id, subject) do
    project = Projects.get_project_by_id(project_id)

    if is_nil(project) do
      Errors.invalid_params("Project not found.")
    else
      case Authorization.authorize(action, subject, project) do
        :ok -> :ok
  ...
  def get_authorized_project(action, account_handle, project_handle, subject) do
