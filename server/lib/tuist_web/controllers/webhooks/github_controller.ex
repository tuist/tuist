defmodule TuistWeb.Webhooks.GitHubController do
  ...
  alias Tuist.GitHub.WebhookValidator
  ...
  defp validate_webhook_signature(conn, _opts) do
    signature_header = conn |> get_req_header("x-hub-signature-256") |> List.first()
    webhook_secret = Environment.github_app_webhook_secret()
    
    # Read the raw body for signature validation
    {:ok, body, conn} = Plug.Conn.read_body(conn)
    
    case WebhookValidator.validate_signature(body, signature_header, webhook_secret) do
      {:ok, :valid} ->
        # Parse the body and put it back as params for the controller
        case Jason.decode(body) do
          {:ok, decoded_params} ->
            conn
            |> Map.put(:params, decoded_params)
            |> Map.put(:body_params, decoded_params)
