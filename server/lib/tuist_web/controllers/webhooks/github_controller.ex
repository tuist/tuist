defmodule TuistWeb.Webhooks.GitHubController do
  ...
  defp start_or_enqueue_qa_run(project, prompt, git_ref) do
    ...
    # Always create the QA run first so we have an ID for the dashboard link
    {:ok, qa_run} =
      QA.create_qa_run(%{
        ...
        git_ref: git_ref,
        # Will be updated after comment creation
    ...
    # Update the QA run with the comment ID
    QA.update_qa_run(qa_run, %{issue_comment_id: comment_id})
  ...
  defp post_initial_comment(simulator_app_build, repository_full_handle, git_ref, project, qa_run) do
    ...
    body =
      if simulator_app_build do
        ...
        "Running QA for [#{preview.display_name}](#{preview_url}). [View QA run →](#{qa_run_url})"
      else
        "No preview found for your PR. Tuist QA will be triggered as soon as a Tuist Preview is available. [View QA run →](#{qa_run_url})\n\nMake sure to include `tuist share` as part of your CI pipeline. For more details on how to set up Previews, head over to our [documentation](https://docs.tuist.dev/en/guides/features/previews)."
      end
