defmodule TuistWeb.RegistryController do
  use TuistWeb, :controller

  alias Tuist.Environment
  alias Tuist.Registry
  alias Tuist.Registry.AlternateManifests
  alias Tuist.Registry.Disk
  alias Tuist.Registry.KeyNormalizer
  alias Tuist.Registry.Metadata
  alias Tuist.Registry.PrefetchWorker
  alias Tuist.Registry.RepositoryURL
  alias Tuist.Registry.Storage
  alias TuistWeb.API.Schemas.SafePathComponent
    ...
    case normalize_registry_scope_name_version(scope, name, version) do
      {:ok, {scope, name, normalized_version}} ->
        ...
        |> Enum.reduce_while(:not_found, fn filename, _acc ->
          ...
          cond do
            Disk.exists?(scope, name, normalized_version, filename) ->
              {:halt,
               {:served,
  ...
  defp do_download_archive(conn, scope, name, normalized_version) do
    ...
    if Disk.exists?(scope, name, normalized_version, "source_archive.zip") do
      maybe_track_registry_download(conn, scope, name, normalized_version)
  ...
  defp render_local_archive(conn, scope, name, normalized_version) do
    local_path =
      Disk.local_accel_path(scope, name, normalized_version, "source_archive.zip")
  ...
  defp render_remote_archive(conn, scope, name, normalized_version, key) do
    if Storage.exists?(key) do
      maybe_enqueue_prefetch(conn, key)
      maybe_track_registry_download(conn, scope, name, normalized_version)

      case Storage.presign_download_url(key) do
        {:ok, url} ->
          conn
          |> put_resp_header("content-version", "1")
          |> put_resp_header("x-accel-redirect", Storage.remote_accel_path(url))
          |> put_resp_content_type("application/zip")
          |> put_resp_header(
            "content-disposition",
            "attachment; filename=\"#{name}-#{normalized_version}.zip\""
  ...
  defp serve_manifest_from_disk(conn, scope, name, version, filename, swift_version, _key) do
    local_path = Disk.local_accel_path(scope, name, version, filename)
  ...
  defp maybe_enqueue_prefetch(conn, key) do
    if not head_request?(conn) and not is_nil(Disk.storage_dir()) do
      PrefetchWorker.enqueue(key)
    end

    :ok
  end

  defp maybe_track_registry_download(conn, scope, name, normalized_version) do
    if not head_request?(conn) do
      Registry.track_download(%{scope: scope, name: name, version: normalized_version})
    end
