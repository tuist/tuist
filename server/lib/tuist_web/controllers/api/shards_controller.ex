defmodule TuistWeb.API.ShardsController do
      ...
      {"Shard plan params", "application/json",
       %Schema{
         ...
         properties: %{
           ...
           start_upload: %Schema{
             type: :boolean,
             description: "Whether to start the shard bundle multipart upload while creating the plan."
           }
         },
         required: [:reference]
       }},
  ...
  def create(%{assigns: %{selected_project: selected_project}, body_params: body_params} = conn, _params) do
    ...
    response =
      if Map.get(body_params, :start_upload, false) do
        {:ok, upload_id} = Shards.start_upload_for_plan(selected_project, selected_project.account, result.plan)
        Map.put(response, :upload_id, upload_id)
      else
        response
      end
