defmodule TuistWeb.Router do
  ...
  scope "/" do
    ...
    for locale <- ["en"] ++ Localization.additional_locales() do
      ...
      live_session String.to_atom("marketing_#{locale}"),
        ...
        live(
          Path.join(locale_path_prefix, "/changelog"),
        ...
        live(
          Path.join(locale_path_prefix, "/qa"),
          TuistWeb.Marketing.MarketingQALive,
          metadata: %{type: :marketing},
          private: private
        )

        for %{slug: blog_post_slug} <- Blog.get_posts() do
          locale_path_prefix
          |> Path.join(blog_post_slug)
          |> Path.join("/iframe.html")
          |> live(
            TuistWeb.Marketing.MarketingBlogPostIframeLive,
            metadata: %{type: :marketing},
            private: private
          )
        end
      ...
      get(
        Path.join(locale_path_prefix, "/pricing"),
        MarketingController,
        :pricing,
        metadata: %{type: :marketing},
        private: private
      ...
      for %{slug: blog_post_slug} <- Blog.get_posts() do
        get(
          Path.join(locale_path_prefix, blog_post_slug),
          MarketingController,
          :blog_post,
          metadata: %{type: :marketing},
          private: private
      ...
      for %{slug: page_slug} <- Tuist.Marketing.Pages.get_pages() do
        get(
          Path.join(locale_path_prefix, page_slug),
          MarketingController,
          :page,
          metadata: %{type: :marketing},
          private: private
      ...
      get(
        Path.join(locale_path_prefix, "/newsletter"),
        MarketingController,
        :newsletter,
        metadata: %{type: :marketing},
        private: private
      ...
      get(
        Path.join(locale_path_prefix, "/newsletter/verify"),
        MarketingController,
        :newsletter_verify,
        metadata: %{type: :marketing},
        private: private
      ...
      get(
        Path.join(locale_path_prefix, "/newsletter/issues/:issue_number"),
        MarketingController,
        :newsletter_issue,
        metadata: %{type: :marketing},
        private: private
  ...
  scope path: "/api",
    ...
    put(
      "/runs/:run_id/complete_artifacts_uploads",
      AnalyticsController,
    ...
    scope "/projects" do
      ...
      scope "/:account_handle/:project_handle" do
        ...
        scope "/runs" do
          ...
          put(
            "/:run_id/complete_artifacts_uploads",
            AnalyticsController,
        ...
        scope "/qa" do
          ...
          post(
            "/runs/:qa_run_id/recordings/upload/generate-url",
            QAController,
            :generate_recording_upload_url
          )

          post(
            "/runs/:qa_run_id/recordings/upload/complete",
            QAController,
    ...
    delete(
      "/organizations/:organization_name/members/:user_name",
