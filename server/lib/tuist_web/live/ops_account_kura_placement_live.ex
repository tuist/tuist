defmodule TuistWeb.OpsAccountKuraPlacementLive do
  @moduledoc """
  Every placement decision recorded for an account, newest first.
  """
  ...
  def mount(%{"id" => account_id}, _session, socket) do
    case Accounts.get_account_by_id(parse_id(account_id)) do
      ...
      {:ok, account} ->
        {:ok,
         socket
         |> assign(:head_title, "#{account.name} · #{dgettext("dashboard", "Placement")} · Tuist Ops")
  ...
  def handle_params(_params, uri, socket) do
    ...
    {decisions, meta} =
      Kura.paginate_placement_history(socket.assigns.account, %{page: page, page_size: @page_size})
