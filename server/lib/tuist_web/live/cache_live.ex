defmodule TuistWeb.CacheLive do
  ...
  def node_credentials_section(assigns) do
    ~H"""
    <.card_section data-part="credentials-card">
      <div data-part="header">
        <div data-part="title-group">
          <span data-part="title">{dgettext("dashboard_account", "Node credentials")}</span>
          <span data-part="subtitle">
            {dgettext(
              "dashboard_account",
              "Tenant-scoped credentials your self-hosted cache nodes present to Tuist. A credential only authorizes this account's traffic."
            )}
          </span>
        </div>
      ...
      <div :if={@new_self_hosted_client_secret} data-part="credential-reveal">
        <.alert
          status="warning"
          type="secondary"
          size="small"
  ...
  attr(:self_hosted_endpoints, :list, required: true)
  ...
  def cache_endpoints_section(assigns) do
    ...
    <.card_section data-part="endpoints-card">
      <div data-part="header">
        <div data-part="title-group">
          <span data-part="title">{dgettext("dashboard_account", "Cache endpoints")}</span>
          <span data-part="subtitle">
            {dgettext(
              "dashboard_account",
              "The URLs the CLI uses to reach your self-hosted cache (for example, the load balancer in front of your nodes). Tuist never connects to them directly."
            )}
          </span>
        </div>
        <div data-part="actions">
      ...
      <.table id="self-hosted-endpoints-table" rows={@self_hosted_endpoints}>
        ...
        <:empty_state>
          <.table_empty_state
            title={dgettext("dashboard_account", "No cache endpoints yet")}
            subtitle={
              dgettext(
                "dashboard_account",
                "Add the URL the CLI should use to reach your self-hosted cache."
              )
            }
          />
        </:empty_state>
