...
- GitLab CI connections in Integrations take only an instance URL and runner token. Disconnect replaces Connect in the card header after connecting; show the instance URL only in its editable field. Each job selects an account-owned profile through its pipeline tags. Never repopulate token inputs; job detail views load account-scoped metadata without execution payloads.
