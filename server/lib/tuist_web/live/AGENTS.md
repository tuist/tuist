...
- `RecordedBuildTimeline` lazily loads Gradle/Bazel metadata when Timeline opens, delivers it through the versioned hook reply, then retains only the summary. Reopening reloads metadata. These sources use local keyboard navigation and have no per-step log requests. Bazel renders bounded coverage; legacy Gradle reports explain their timestamp origin.
