...
- Code coverage has three pages. `CoverageLive` (`/tests/coverage`) is a glance at the **default branch** over the chosen period — analytics (coverage, covered and executable lines, one colour each, switching the chart; View more opens the default branch's page) and the branches and pull requests measured in the period (searchable, paged under `branches-search`/`branches-page`). `CoverageDetailLive` reads one subject as its head commit: a commit (`/tests/coverage/commits/:sha`: Overview, Targets, Files, Test Runs), a branch (`/tests/coverage/branches/*branch`: its analytics over a period of its own, plus a Commits tab paged by cursor) or a pull request (`/tests/coverage/pull-requests/:number`: its newest commit or the one `?commit=` picks, its commits and all their runs). `CoverageFileLive` (`/tests/coverage/files/*path?commit=`) leads back to the tab it came from. Every figure is a commit's (`Tuist.Tests.Coverage.Commits`), pooled over the schemes that measured it, and the pages read on the connected render only, the static one showing a skeleton.
