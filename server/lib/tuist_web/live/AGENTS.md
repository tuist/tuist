...
- Gradle execution graph components cache decoding, indexing and chain analysis per build ID. Neighbor lists, ordering constraints, chain links and the timeline are paginated; selection queries run only when the selected node changes.
