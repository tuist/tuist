defmodule Tuist.Tests.AnalyticsTest do
  ...
  describe "test_case_ids_with_successful_default_branch_run/3" do
    test "returns only test cases with a successful, non-flaky run on the default branch" do
      ...
      # When
      got = Analytics.test_case_ids_with_successful_default_branch_run(project.id, ids, "main")
    ...
    test "returns an empty list when given no test case ids" do
      ...
      assert Analytics.test_case_ids_with_successful_default_branch_run(project.id, [], "main") == []
    end
  end
