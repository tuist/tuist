defmodule Tuist.Automations.Actions.ChangeStateActionTest do
  # Integration test: exercise the legacy shim through the ActionExecutor and
  # the real Tests.update_test_case down to a ClickHouse write so we catch
  # drift in the context contract.
  ...
  describe "execute/2" do
    ...
    test "records no hold claim since it has no owning automation" do
      project = ProjectsFixtures.project_fixture()
      test_case = RunsFixtures.test_case_fixture(project_id: project.id, state: "enabled")
      IngestRepo.insert_all(TestCase, [test_case |> Map.from_struct() |> Map.delete(:__meta__)])
      ...
      assert {:ok, %{state: "muted"}} = Tests.get_test_case_by_id(test_case.id)
      assert Holds.current_claims(project.id, [test_case.id]) == %{}
    end
