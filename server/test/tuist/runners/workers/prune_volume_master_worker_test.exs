defmodule Tuist.Runners.Workers.PruneVolumeMasterWorkerTest do
  ...
  describe "perform/1" do
    test "prunes the master object named by its id" do
      expect(Runners, :prune_superseded_volume_master, fn 42, "tuist-cache", "tree-content" -> :ok end)

      assert :ok = perform_job(PruneVolumeMasterWorker, %{account_id: 42, master_id: "tree-content"})
    end

    test "prunes in the volume the job names" do
      expect(Runners, :prune_superseded_volume_master, fn 42, "repo-eae044a4c27633ea", "tree" -> :ok end)

      assert :ok =
               perform_job(PruneVolumeMasterWorker, %{
                 account_id: 42,
    ...
    test "prunes by the inventory digest a job enqueued before master ids carries" do
      expect(Runners, :prune_superseded_volume_master, fn 42, "tuist-cache", "tree" -> :ok end)
