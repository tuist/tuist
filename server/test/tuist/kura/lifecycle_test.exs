defmodule Tuist.Kura.LifecycleTest do
  ...
  describe "operator archive and unarchive" do
    setup do
      stub(Provisioner, :destroy, fn _server -> :ok end)
      user = AccountsFixtures.user_fixture()
      %{operator: user}
    ...
    test "archives an instance the sweep would have left alone", %{operator: operator} do
      # Demand is fresh, so the automatic rule would never touch this.
      account = account()
      server = active_instance(account)
      with_demand(account, 1)

      assert {:ok, _action} = Lifecycle.archive_now(reload(server), operator)

      assert reload(server).status == :drain_pending
      assert reload_lifecycle(account).drain_started_at
    end

    test "archives an Enterprise instance, which the sweep never would", %{operator: operator} do
      account = account(plan: :enterprise, region: :usa)
      server = active_instance(account)
      with_demand(account, 1)
    ...
    test "archives even when the archival flag is off", %{operator: operator} do
      stub(FunWithFlags, :enabled?, fn :kura_archival, [for: _account] -> false end)
    ...
    test "still drains rather than tearing down immediately", %{operator: operator} do
      account = account()
      server = active_instance(account)
      with_demand(account, 1)

      Lifecycle.archive_now(reload(server), operator)
      Lifecycle.reconcile()

      # In-flight work gets the same window it would on an automatic archive.
      assert reload(server).status == :drain_pending
      assert reload_lifecycle(account).teardown_started_at == nil
    end

    test "records who archived and who returned", %{operator: operator} do
      stub(Provisioner, :current_image_tag, fn _server -> {:error, :not_found} end)
      ...
      assert {:ok, _action} = Lifecycle.unarchive_now(reload(server), operator)
      assert reload(server).status == :provisioning

      actions = Lifecycle.operator_actions(account.id)
      assert [%{action: :unarchive}, %{action: :archive}] = actions
      assert Enum.all?(actions, &(&1.performed_by_user.id == operator.id))
      assert Enum.all?(actions, &(&1.service_region == @region))
    end

    test "is one-shot: an account still being built against comes back on its own", %{operator: operator} do
      stub(Provisioner, :current_image_tag, fn _server -> {:error, :not_found} end)

      account = account()
      server = active_instance(account)
      # Fresh demand: this account is in active use.
      with_demand(account, 1)

      Lifecycle.archive_now(reload(server), operator)
      elapse_drain(account)

      # Teardown, archival, and the cold return all happen in one pass, so the
      # cache is genuinely reclaimed and the account is handed a fresh empty
      # instance rather than being left without one.
      Lifecycle.reconcile()

      assert reload(server).status == :provisioning
      assert reload(server).current_image_tag == nil
      assert [%{action: :archive}] = Lifecycle.operator_actions(account.id)
    end

    test "leaves an idle account reclaimed", %{operator: operator} do
      stub(Provisioner, :current_image_tag, fn _server -> {:error, :not_found} end)
    ...
    test "refuses to archive an instance that is not serving", %{operator: operator} do
      account = account()
      server = active_instance(account)
      with_demand(account, 200)
      start_drain(account, server)

      assert {:error, :not_drainable} = Lifecycle.archive_now(reload(server), operator)
    end

    test "refuses to unarchive an instance that is not archived", %{operator: operator} do
      account = account()
      server = active_instance(account)

      assert {:error, :not_archived} = Lifecycle.unarchive_now(reload(server), operator)
    end
  end
