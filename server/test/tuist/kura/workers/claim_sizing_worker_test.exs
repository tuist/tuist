defmodule Tuist.Kura.Workers.ClaimSizingWorkerTest do
  ...
  test "reports a refused apply instead of dropping it", %{account: account} do
    ...
    expect(Kura, :apply_claim_proposal, fn _proposal, "automatic" -> {:error, :capacity_exhausted} end)

    ref = TelemetryCapture.attach_event_handlers([Telemetry.event_name_claim_apply_refused()])

    assert :ok = perform_job(ClaimSizingWorker, %{})

    assert_receive {_event, ^ref, %{count: 1}, %{region: "us-east", reason: "capacity_exhausted"}}
