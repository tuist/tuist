defmodule Tuist.TestsTest do
  ...
  describe "list_flagged_flaky_test_case_ids/1" do
    test "returns ids of test cases whose latest row has is_flaky = true" do
      project = ProjectsFixtures.project_fixture()
        ...
        RunsFixtures.test_fixture(
          ...
          test_modules: [
            %{
              ...
              duration: 1000,
              test_cases: [
                %{name: "flaky", status: "success", duration: 100},
      ...
      flaky = Enum.find(test_cases, &(&1.name == "flaky"))
      stable = Enum.find(test_cases, &(&1.name == "stable"))

      {:ok, _} = Tests.update_test_case(flaky.id, %{is_flaky: true})

      assert Tests.list_flagged_flaky_test_case_ids(project.id) == [flaky.id]
      refute stable.id in Tests.list_flagged_flaky_test_case_ids(project.id)
    end

    test "excludes test cases that were unflagged after being flagged" do
      project = ProjectsFixtures.project_fixture()
        ...
        RunsFixtures.test_fixture(
          ...
          test_modules: [
            %{
              ...
              duration: 1000,
              test_cases: [
                %{name: "recovered", status: "success", duration: 100}
              ]
            }
          ]
        )
      ...
      assert Tests.list_flagged_flaky_test_case_ids(project.id) == []
    end

    test "scopes results to the given project" do
      project = ProjectsFixtures.project_fixture()
      ...
      {[mine], _meta} = Tests.list_test_cases(project.id, %{})
      {[theirs], _meta} = Tests.list_test_cases(other_project.id, %{})

      {:ok, _} = Tests.update_test_case(mine.id, %{is_flaky: true})
      {:ok, _} = Tests.update_test_case(theirs.id, %{is_flaky: true})

      assert Tests.list_flagged_flaky_test_case_ids(project.id) == [mine.id]
      assert Tests.list_flagged_flaky_test_case_ids(other_project.id) == [theirs.id]
    end
  end
