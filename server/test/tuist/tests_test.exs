defmodule Tuist.TestsTest do
  ...
  describe "create_test/1" do
    ...
    test "marks the run as success when the only failures are quarantined" do
      project = ProjectsFixtures.project_fixture()
      account = AccountsFixtures.user_fixture(preload: [:account]).account
      ...
      test_attrs = %{
        ...
        test_modules: [
          %{
            ...
            duration: 1500,
            test_suites: [
              %{name: "FlakySuite", status: "failure", duration: 1000}
            ],
            test_cases: [
              %{
                name: "testStable",
                test_suite_name: "FlakySuite",
                status: "success",
                duration: 500
              },
              %{
      ...
      {modules, _meta} =
        Tests.list_test_module_runs(%{filters: [%{field: :test_run_id, op: :==, value: test.id}]})

      assert hd(modules).status == "success"

      {suites, _meta} =
        Tests.list_test_suite_runs(%{filters: [%{field: :test_run_id, op: :==, value: test.id}]})

      assert hd(suites).status == "success"
    end

    test "keeps the run as failure when at least one failing test is not quarantined" do
      project = ProjectsFixtures.project_fixture()
      account = AccountsFixtures.user_fixture(preload: [:account]).account
      ...
      test_attrs = %{
        ...
        test_modules: [
          %{
            ...
            duration: 1500,
            test_cases: [
              %{name: "testFlaky", status: "failure", duration: 500, is_quarantined: true},
              %{name: "testReal", status: "failure", duration: 500}
            ]
          }
        ]
      }
    ...
    test "keeps the run as failure when no test cases are reported" do
      project = ProjectsFixtures.project_fixture()
      account = AccountsFixtures.user_fixture(preload: [:account]).account
  ...
  describe "create_test/1 with sharding" do
    ...
    test "shards reporting only quarantined failures yield a successful run" do
      project = ProjectsFixtures.project_fixture()
      account = AccountsFixtures.user_fixture(preload: [:account]).account
      plan = ShardsFixtures.shard_plan_fixture(project_id: project.id, shard_count: 2)
        ...
        Tests.create_test(%{
          ...
          test_modules: [
            %{
              ...
              duration: 500,
              test_cases: [
                %{name: "testA", status: "failure", duration: 500, is_quarantined: true}
              ]
            }
          ]
        })
