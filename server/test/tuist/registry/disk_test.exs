defmodule Tuist.Registry.DiskTest do
  ...
  setup do
    storage_dir =
      Path.join(
        System.tmp_dir!(),
        "tuist_registry_disk_test_#{:erlang.unique_integer([:positive, :monotonic])}"
      )

    File.mkdir_p!(storage_dir)
    on_exit(fn -> File.rm_rf!(storage_dir) end)

    stub(Environment, :registry_storage_dir, fn -> storage_dir end)
  ...
  describe "exists?/4" do
    test "returns true when file exists" do
      ...
      assert Disk.exists?(scope, name, version, filename) == true
    end

    test "returns false when file doesn't exist" do
      assert Disk.exists?("nonexistent", "package", "1.0.0", "file.zip") == false
    end

    test "returns false when storage dir is not configured" do
      stub(Environment, :registry_storage_dir, fn -> nil end)

      assert Disk.exists?("apple", "parser", "1.0.0", "file.zip") == false
    end
  end
  ...
  describe "put/5" do
    test "writes binary data to disk" do
      ...
      assert Disk.put(scope, name, version, filename, data) == :ok
    ...
    test "handles file tuple input" do
      ...
      assert Disk.put(scope, name, version, filename, {:file, tmp_path}) == :ok
  ...
  describe "stat/4" do
    test "returns file stat for existing artifact" do
      ...
      assert {:ok, stat} = Disk.stat(scope, name, version, filename)
    ...
    test "returns error for non-existent artifact" do
      assert {:error, :enoent} = Disk.stat("nonexistent", "package", "1.0.0", "file.zip")
    end
  end

  describe "local_accel_path/4" do
    test "builds internal X-Accel-Redirect path" do
      path = Disk.local_accel_path(@test_scope, @test_name, @test_version, @test_filename)
      assert path == "/internal/local/registry/swift/apple/parser/1.0.0/source_archive.zip"
    end

    test "normalizes components in path" do
      path = Disk.local_accel_path("Apple", "Parser", "v1.2", "Package.swift")
