defmodule TuistWeb.TestCaseRunLiveTest do
  ...
  describe "tabs" do
    test "defaults to overview tab", %{
      ...
      assert has_element?(lv, "[data-part='overview']")
    end

    test "switches to failures tab", %{
      conn: conn,
      account: account,
      project: project
    } do
      ...
      {:ok, _lv, html} =
        live(
          conn,
          ~p"/#{account.name}/#{project.name}/tests/test-cases/runs/#{test_case_run.id}?tab=failures"
        )

      assert html =~ "Expected true but got false"
    end
  end
end
