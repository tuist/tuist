defmodule TuistWeb.OpsAccountLiveTest do
  ...
  describe "Kura placement" do
    test "shows the resolved region and offers every assignable one", %{conn: conn, user: user} do
      ...
      # is the only route to a region no storage preference derives to — both of
      # them, without this page naming either.
      assert html =~ "US West (us-west)"
      assert html =~ "EU Central (eu-central)"
      assert html =~ "Asia Pacific Southeast (ap-southeast)"
    end

    test "says an assigned region is unserved rather than blaming the storage region", %{conn: conn, user: user} do
      BillingFixtures.subscription_fixture(account_id: user.account.id, plan: :pro)

      # Only the local controller region is served in this environment, so an
      # assignment to a real one is exactly the case `AccountPolicies` refuses
      # rather than falling back to the default.
      {:ok, _assignment} =
        AccountPolicies.assign_service_region(user.account, "eu-central", user, "Measured demand is European")

      {:ok, _lv, html} = live(conn, ~p"/ops/accounts/#{user.account.id}")

      assert html =~ "EU Central (eu-central) is assigned but this deployment does not serve it yet"
      refute html =~ "no Kura region serves this account"
    end
