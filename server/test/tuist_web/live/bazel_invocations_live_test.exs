defmodule TuistWeb.BazelInvocationsLiveTest do
  ...
  test "renders completed invocations and correlated cache totals", %{
    ...
    assert has_element?(live_view, "#bazel-builds-environment-dropdown")
    ...
    assert has_element?(
             live_view,
             "#bazel-invocation-duration [data-part='legend'][data-color='secondary']"
           )
  ...
  test "filters Bazel build analytics and the build list by environment", %{
    conn: conn,
    organization: organization,
    project: project
  } do
    ...
    local_build =
      project
      |> invocation_attributes("local-build", "build", finished_at)
      |> Map.put(:target_patterns, ["//App:Local"])

    ci_build =
      project
      |> invocation_attributes("ci-build", "build", finished_at)
      |> Map.merge(%{is_ci: true, target_patterns: ["//App:CI"]})

    Bazel.create_invocations([local_build, ci_build])

    path =
      ~p"/#{organization.account.name}/#{project.name}/builds?analytics-environment=ci&analytics-date-range=custom&analytics-start-date=2000-01-01T00%3A00%3A00Z&analytics-end-date=2100-01-01T00%3A00%3A00Z"

    {:ok, live_view, _html} = live(conn, path)
    render_async(live_view, @render_async_timeout)

    assert has_element?(live_view, "#bazel-total-invocations", "1")
    assert has_element?(live_view, "#bazel-invocations-table", "//App:CI")
    refute has_element?(live_view, "#bazel-invocations-table", "//App:Local")
  end

  test "renders Bazel build runs at the shared build runs path", %{
    conn: conn,
    organization: organization,
    project: project
  } do
  ...
  test "renders a build invocation detail from the builds page", %{
    ...
    assert has_element?(live_view, "[data-part='build-details']", "Build Details")
    assert has_element?(live_view, "[data-part='header'] [data-part='label']", "//App:App")
    assert has_element?(live_view, "[data-part='build-details-section']", "Passed")
    assert has_element?(live_view, "[data-part='build-details-section']", "Built by")
    assert has_element?(live_view, "[data-part='build-details-section']", "Build duration")
    assert has_element?(live_view, "[data-part='build-details-section']", "Built at")
    assert has_element?(live_view, "[data-part='custom-metadata-section']", "Custom metadata")
    assert has_element?(live_view, "#custom-metadata-table", "environment")
    assert has_element?(live_view, "#custom-metadata-table", "local")
    assert has_element?(live_view, "#custom-metadata-table", "runner")
    assert has_element?(live_view, "#custom-metadata-table", "linux-arm64")
    ...
    assert has_element?(live_view, "#bazel-invocation", "Cache Summary")
    ...
    assert has_element?(live_view, "#bazel-invocation-cache-actions-breakdown")
    assert has_element?(live_view, "#bazel-invocation-cache-read-latency", "10ms")
    assert has_element?(live_view, "#bazel-invocation-cache-write-latency", "No data")
    ...
    assert has_element?(live_view, "#bazel-invocation-cache-events-table", "Remote")
    refute has_element?(live_view, "#bazel-invocation-cache-events-table", "cas-digest-1…")
    ...
    render_patch(
      live_view,
      ~p"/#{organization.account.name}/#{project.name}/builds/invocations/BUILD-INVOCATION?tab=cache&cache-view=content-objects"
    )

    assert has_element?(live_view, "#bazel-invocation-cache-content-breakdown")
    assert has_element?(live_view, "#bazel-invocation-cache-download-throughput", "2.3 Mbps")
    assert has_element?(live_view, "#bazel-invocation-cache-upload-throughput", "No data")
    assert has_element?(live_view, "#bazel-invocation-cache-events-table thead th:first-child", "Key")
    assert has_element?(live_view, "#bazel-invocation-cache-events-table", "Download")
    ...
    assert has_element?(live_view, "#outcome", "Status")
  ...
  test "paginates invocation cache requests", %{
    ...
    assert has_element?(live_view, "#bazel-invocation-cache-events-table")
    assert has_element?(live_view, ".noora-pagination-group")
  end
  ...
  test "renders filtered build empty states without onboarding copy", %{
    conn: conn,
    organization: organization,
    project: project
  } do
  ...
  test "falls back to average for an invalid cache percentile", %{
    ...
    assert has_element?(live_view, "#bazel-cache-hit-rate", "Avg. cache hit rate")
    assert has_element?(live_view, "#bazel-cache-hit-rate", "100.0%")
  end
