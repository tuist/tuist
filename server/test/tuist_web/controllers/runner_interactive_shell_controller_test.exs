defmodule TuistWeb.RunnerInteractiveShellControllerTest do
  ...
  defp shell_session(account, user) do
    {:ok, session} =
  ...
  test "404s when another account member reuses a token they did not request", %{conn: conn} do
    ...
    organization =
      organization_fixture(
        name: "runner-shell-controller-#{System.unique_integer([:positive])}",
        creator: creator,
        preload: [:account]
      )
    ...
    session = shell_session(account, creator)
