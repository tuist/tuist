...
class TuistTestInsightsTest {
    ...
    @Test
    fun `buildReport groups parameterized test invocations as arguments`() {
        val collector = TestReportCollector()
        collector.collectTestResult(
            ":app", "testLogin[1]", "com.example.LoginTest",
            TestResult.ResultType.SUCCESS, 0, 100, null
        )
        collector.collectTestResult(
            ":app", "testLogin[2]", "com.example.LoginTest",
            TestResult.ResultType.FAILURE, 0, 200,
            AssertionError("Expected true but was false")
        )
        collector.collectTestResult(
            ":app", "testLogin[3]", "com.example.LoginTest",
            TestResult.ResultType.SUCCESS, 0, 150, null
        )

        val report = collector.buildReport(
            totalDurationMs = 450,
            isCi = false, scheme = null, gitBranch = null,
            gitCommitSha = null, gitRef = null, gitRemoteUrlOrigin = null,
            gradleBuildId = null
        )
        ...
        val testCase = module.testCases[0]
        assertEquals("testLogin", testCase.name)
        assertEquals("com.example.LoginTest", testCase.testSuiteName)
        assertEquals("failure", testCase.status)
        assertEquals(450, testCase.duration)

        val arguments = testCase.arguments!!
        assertEquals(3, arguments.size)
        assertEquals("[1]", arguments[0].name)
        assertEquals("success", arguments[0].status)
        assertEquals("[2]", arguments[1].name)
        assertEquals("failure", arguments[1].status)
        assertEquals(1, arguments[1].failures.size)
        assertEquals("[3]", arguments[2].name)
        assertEquals("success", arguments[2].status)
    }

    @Test
    fun `buildReport does not group single parameterized invocation as argument`() {
        val collector = TestReportCollector()
        collector.collectTestResult(
            ":app", "testLogin[1]", "com.example.LoginTest",
            TestResult.ResultType.SUCCESS, 0, 100, null
        ...
        val testCase = report.testModules[0].testCases[0]
        assertEquals("testLogin[1]", testCase.name)
        assertNull(testCase.arguments)
    }

    @Test
    fun `buildReport handles parameterized tests with retries`() {
        val collector = TestReportCollector()
        // First invocation: passes on retry
        collector.collectTestResult(
            ":app", "testLogin[1]", "com.example.LoginTest",
            TestResult.ResultType.FAILURE, 0, 100, AssertionError("flaky")
        )
        collector.collectTestResult(
            ":app", "testLogin[1]", "com.example.LoginTest",
            TestResult.ResultType.SUCCESS, 100, 200, null
        )
        // Second invocation: passes first time
        collector.collectTestResult(
            ":app", "testLogin[2]", "com.example.LoginTest",
            TestResult.ResultType.SUCCESS, 0, 150, null
        )
        ...
        val testCase = report.testModules[0].testCases[0]
        assertEquals("testLogin", testCase.name)
        assertEquals("success", testCase.status)

        val arguments = testCase.arguments!!
        assertEquals(2, arguments.size)

        // First argument had retries
        val arg1 = arguments[0]
        assertEquals("[1]", arg1.name)
        assertEquals("success", arg1.status)
        assertEquals(2, arg1.repetitions!!.size)
        assertEquals("failure", arg1.repetitions!![0].status)
        assertEquals("success", arg1.repetitions!![1].status)

        // Second argument passed first time
        val arg2 = arguments[1]
        assertEquals("[2]", arg2.name)
        assertEquals("success", arg2.status)
        assertNull(arg2.repetitions)
    }
}
