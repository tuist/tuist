...
- Cache endpoint resolution uses a sole URL without a latency probe. Stable managed hostnames delegate regional selection to DNS; multiple URLs remain unranked and can include custom/self-hosted caches or regional fallback, so retain their latency/reachability selection. Do not infer preference from hostname suffix or API order. See `../infra/cache-dns/README.md` for the client contract and rollout hold.
