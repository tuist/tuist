...
- **Breaking** Remove `Config.GenerationOptions.disableAutogeneratedSchemes` helper and `Config.GenerationOptions.codeCoverageMode` case
  - **Migration**: Use the `Workspace.GenerationOption.autogeneratedWorkspaceSchemes(.disabled)` instead
...
- **Breaking** rename `*-Project*` autogenerated schemes to `*-Workspace*`
...
- **Breaking** change `Workspace.generationOptions` from `enum` to `struct`
  - **Motivation**: Consistency with other generation options types
  - **Migration**: Use the enum instead
