...
This integration targets native Buildkite commands. Docker-plugin and other
`docker run` containers started after the hook see volumes inside the checkout
through the checkout mount; no extra volume mapping is needed. Paths outside the
work directory, such as `~/.gradle`, are visible to native commands only.
macOS support is separate.
