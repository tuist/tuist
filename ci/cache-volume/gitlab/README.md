...
A job's `image:` does not select a Docker executor. Volumes are mounted only in
the job's own environment, so `docker run` child containers don't see them: a
child that binds the checkout gets the empty directory underneath the mount,
and its writes don't reach the volume. macOS support is not included. Caches
must not contain secrets or irreplaceable data.
