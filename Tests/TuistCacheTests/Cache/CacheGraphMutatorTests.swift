...
//        XCTAssertTrue(c.dependencies.contains(where: { $0.path == eXCFrameworkPath }))
    ...
    // 10th scenario
    // App ---> B (Cached Framework) ---> C (Cached Static Framework)
    // The static frameworks should not be added to the graph as it is statically embedded inside the dynamic one
    func test_map_when_tenth_scenario() throws {
        ...
        let cGraphTarget = GraphTarget.test(path: cProject.path, target: cFramework, project: cProject)
        let cCachedFrameworkPath = path.appending(component: "C.framework")
        let cCachedFramework = GraphDependency.testFramework(path: cCachedFrameworkPath, linking: .static)
        ...
        let bGraphTarget = GraphTarget.test(path: bProject.path, target: bFramework, project: bProject)
        let bCachedFrameworkPath = path.appending(component: "B.framework")
        let bCachedFramework = GraphDependency.testFramework(path: bCachedFrameworkPath, linking: .dynamic)
    ...
    // 11th scenario
    // App ---> B (Framework) ---> C (Cached Static Framework)
    // The static frameworks should be added to the graph as the dynamic framework is not cached
    func test_map_when_eleventh_scenario() throws {
        ...
        let expectedGraph = Graph.test(
            ...
            dependencies: [
                .target(name: bFramework.name, path: bGraphTarget.path): [
                    .testFramework(path: cCachedFrameworkPath, linking: .static),
                ],
                .target(name: appGraphTarget.target.name, path: appGraphTarget.path): [
                    .target(name: bFramework.name, path: bGraphTarget.path),
                ],
