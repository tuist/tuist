...
final class VersionResolverErrorTests: XCTestCase {
    func test_errorDescription() {
        ...
        XCTAssertEqual(VersionResolverError.readError(path: path).errorDescription, "Cannot read the version file at path /test.")
        XCTAssertEqual(VersionResolverError.invalidFormat("3.2", path: path).errorDescription, "The version 3.2 at path /test doesn't have a valid semver format: x.y.z.")
    }
