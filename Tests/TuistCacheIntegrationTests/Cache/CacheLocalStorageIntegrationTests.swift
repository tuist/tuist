...
final class CacheLocalStorageIntegrationTests: TuistTestCase {
    ...
    func test_fetch_when_a_cached_xcframework_exists() throws {
        ...
        // When
        let got = try subject.fetch(hash: hash, userConfig: config).toBlocking().first()
    ...
    func test_fetch_when_a_cached_xcframework_does_not_exist() throws {
        ...
        XCTAssertThrowsSpecific(try subject.fetch(hash: hash, userConfig: config).toBlocking().first(),
    ...
    func test_store() throws {
        ...
        // When
        _ = try subject.store(hash: hash, userConfig: config, xcframeworkPath: xcframeworkPath).toBlocking().first()
