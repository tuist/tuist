...
final class CodesignControllerTests: TuistUnitTestCase {
    ...
    func test_codesignSignature_returnsSignature() async throws {
        ...
        given(commandRunner)
            .run(arguments: .value(["/usr/bin/codesign", "-dvv", signedPath.pathString]), environment: .any, workingDirectory: .any)
            .willReturn(
    ...
    func test_codesignSignature_returnsNilIfUnsigned() async throws {
        ...
        given(commandRunner)
            .run(arguments: .value(["/usr/bin/codesign", "-dvv", unsignedPath.pathString]), environment: .any, workingDirectory: .any)
            .willReturn(
                AsyncThrowingStream { continuation in
...
func XCTAssertThrowsCommandErrorTerminated<T>(
