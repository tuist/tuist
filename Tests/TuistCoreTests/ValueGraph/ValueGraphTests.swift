...
final class ValueGraphTests: TuistUnitTestCase {
    func test_init() {
        ...
        let aFrameworkNode = FrameworkNode.test(
            path: aFrameworkPath,
            linking: .dynamic,
            architectures: [.armv7],
            dependencies: []
        ...
        let bFrameworkNode = FrameworkNode.test(
            path: bFrameworkPath,
            linking: .dynamic,
            architectures: [.armv7],
        ...
        let libraryNode = LibraryNode.test(
            path: libraryPath,
        ...
        let xcframeworkNode = XCFrameworkNode.test(
        ...
        // Then: App -> A
        XCTAssertEqual(
            valueGraph.dependencies[.target(name: appTarget.name, path: appNode.path)]?
                .contains(.target(name: aTarget.name, path: aNode.path)),
            true
        )
        // Then: App -> B
        XCTAssertEqual(
            valueGraph.dependencies[.target(name: appTarget.name, path: appNode.path)]?
                .contains(.target(name: bTarget.name, path: bNode.path)),
            true
        )
        // Then: App -> XCFramework
        XCTAssertEqual(
            valueGraph.dependencies[.target(name: appTarget.name, path: appNode.path)]?
                .contains(.xcframework(path: xcframeworkNode.path, infoPlist: xcframeworkNode.infoPlist, primaryBinaryPath: xcframeworkNode.primaryBinaryPath, linking: xcframeworkNode.linking)),
            true
        )
        // Then: B -> Library
        XCTAssertEqual(
            valueGraph.dependencies[.target(name: bTarget.name, path: bNode.path)]?
                .contains(.library(path: libraryNode.path, publicHeaders: libraryNode.publicHeaders, linking: libraryNode.linking, architectures: libraryNode.architectures, swiftModuleMap: libraryNode.swiftModuleMap)),
            true
        )
        // Then: A -> XCTest
        XCTAssertEqual(
            valueGraph.dependencies[.target(name: aTarget.name, path: aNode.path)]?
                .contains(.sdk(name: xctestNode.name, path: xctestNode.path, status: xctestNode.status, source: xctestNode.source)),
            true
        )
        // Then: A -> BFramework
        XCTAssertEqual(
            valueGraph.dependencies[.target(name: aTarget.name, path: aNode.path), default: []]
                .contains(.framework(
                    path: bFrameworkPath,
                    ...
                    isCarthage: bFrameworkNode.isCarthage
                )),
            true
        )
        // Then: A -> Package
        XCTAssertEqual(
            valueGraph.dependencies[.target(name: aTarget.name, path: aNode.path)]?
                .contains(.packageProduct(path: packageProduct.path, product: packageProduct.product)),
            true
        )
        // Then: BFramework -> AFramework
        XCTAssertEqual(
            valueGraph.dependencies[.framework(
                path: bFrameworkNode.path,
                binaryPath: bFrameworkNode.binaryPath,
                dsymPath: bFrameworkNode.dsymPath,
                bcsymbolmapPaths: bFrameworkNode.bcsymbolmapPaths,
                linking: bFrameworkNode.linking,
        ...
        // then: AFramework
        XCTAssertNotNil(valueGraph.dependencies[.framework(
            path: aFrameworkNode.path,
            binaryPath: aFrameworkNode.binaryPath,
            dsymPath: aFrameworkNode.dsymPath,
            bcsymbolmapPaths: aFrameworkNode.bcsymbolmapPaths,
        ...
        // Then: Library
        XCTAssertNotNil(valueGraph.dependencies[.library(
            path: libraryNode.path,
