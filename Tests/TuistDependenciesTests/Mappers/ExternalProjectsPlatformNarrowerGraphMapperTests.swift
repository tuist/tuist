...
final class ExternalProjectsPlatformNarrowerGraphMapperTests: TuistUnitTestCase {
    ...
    func test_map_when_external_dependency_without_platform_filter() async throws {
        ...
        XCTAssertEqual(
            try XCTUnwrap(mappedGraph.projects[project.path]?.target(named: appTarget.name))?.supportedPlatforms,
            Set([.iOS])
        )
        XCTAssertEqual(
            try XCTUnwrap(mappedGraph.projects[externalProject.path]?.target(named: externalPackage.name)?.supportedPlatforms),
            Set([.iOS])
        )
    }
    ...
    func test_map_when_external_with_platform_filter() async throws {
        ...
        XCTAssertEqual(
            try XCTUnwrap(mappedGraph.projects[externalProject.path]?.target(named: externalPackage.name)?.deploymentTargets),
            .iOS("16.0")
        )
    }
    ...
    func test_map_when_external_transitive_dependency_without_platform_filter() async throws {
        ...
        XCTAssertEqual(
            try XCTUnwrap(
                mappedGraph.projects[externalProject.path]?.target(named: transitiveExternalPackage.name)?
                    .supportedPlatforms
            ),
            Set([.iOS])
        )
