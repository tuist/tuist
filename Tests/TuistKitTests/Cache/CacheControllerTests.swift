...
final class CacheControllerTests: TuistUnitTestCase {
    ...
    func test_cache_builds_and_caches_the_frameworks() throws {
        ...
        let targetReferences = [aTarget, bTarget, cTarget].map {
            TargetReference(projectPath: xcworkspacePath, name: $0.name)
        }
        let scheme = Scheme(
        ...
        let graph = Graph.test(
            ...
            targets: nodeWithHashes.keys.reduce(into: [project.path: [String: Target]()]) {
                $0[project.path]?[$1.target.name] = $1.target
            },
            dependencies: [
    ...
    func test_filtered_cache_builds_with_dependencies_only_and_caches_the_frameworks() throws {
        ...
        let graph = Graph.test(
            projects: [project.path: project],
            targets: graphTargets.reduce(into: [project.path: [String: Target]()]) {
                $0[project.path]?[$1.target.name] = $1.target
            },
            dependencies: [
    ...
    func test_given_target_to_filter_is_not_cacheable_should_cache_its_depedendencies() throws {
        ...
        let graph = Graph.test(
            projects: [project.path: project],
            targets: [aGraphTarget, bGraphTarget].reduce(into: [project.path: [String: Target]()]) {
                $0[project.path]?[$1.target.name] = $1.target
            },
            dependencies: [
                // `bTarget` is a dependency of `aTarget`.
