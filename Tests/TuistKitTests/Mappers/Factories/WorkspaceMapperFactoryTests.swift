...
final class WorkspaceMapperFactoryTests: TuistUnitTestCase {
    ...
    func test_default_contains_the_project_workspace_mapper() {
        ...
        subject = WorkspaceMapperFactory(
            projectMapper: SequentialProjectMapper(
                mappers: projectMapperFactory.default(config: config)
            )
        )
    ...
    func
        test_default_contains_the_autogenerated_project_scheme_mapper_when_autogenerated_schemes_are_enabled()
    {
        // Given
        let config = Config.test(generationOptions: [])
        subject = WorkspaceMapperFactory(
    ...
    func
        test_automation_contains_the_autogenerated_project_scheme_workspace_mapper_even_if_autogenerated_schemes_are_disabled()
        throws
    {
        // Given
        let workspaceDirectory = try temporaryPath()
        ...
        let mapper = XCTAssertContainsElementOfType(
            got,
            AutogeneratedProjectSchemeWorkspaceMapper.self
        )
    ...
    func test_cache_contains_the_generate_cacheable_schemes_workspace_mapper() throws {
        ...
        let mapper = XCTAssertContainsElementOfType(
            got,
            GenerateCacheableSchemesWorkspaceMapper.self
        )
