...
final class CloudSessionControllerErrorTests: TuistUnitTestCase {
    func test_description_when_missingParameters() {
        // Given
        let subject = ScaleSessionControllerError.missingParameters
    ...
    func test_description_when_authenticationError() {
        // Given
        let subject = ScaleSessionControllerError.authenticationError("error")
    ...
    func test_description_when_invalidParameters() {
        // Given
        let subject = ScaleSessionControllerError.invalidParameters(["invalid"])
...
final class CloudSessionControllerTests: TuistUnitTestCase {
    ...
    var serverURL: URL!
    var subject: ScaleSessionController!
    ...
    override func setUp() {
        ...
        opener = MockOpener()
        serverURL = URL.test()
        subject = ScaleSessionController(credentialsStore: credentialsStore,
    ...
    func test_authenticate_when_parametersAreMissing() throws {
        ...
        httpRedirectListener.listenStub = { (port, path, message, _) -> (Swift.Result<[String: String]?, HTTPRedirectListenerError>) in
            XCTAssertEqual(port, ScaleSessionController.port)
        ...
        // Then
        XCTAssertThrowsSpecific(try subject.authenticate(serverURL: serverURL), ScaleSessionControllerError.missingParameters)
    ...
    func test_authenticate_when_parametersIncludeError() throws {
        ...
        // Then
        XCTAssertThrowsSpecific(try subject.authenticate(serverURL: serverURL), ScaleSessionControllerError.authenticationError("value"))
        XCTAssertPrinterOutputContains("""
        Opening \(authURL().absoluteString) to start the authentication flow
        """)
    }
    ...
    func test_authenticate_when_parametersContainInvalidKeys() throws {
        ...
        // Then
        XCTAssertThrowsSpecific(try subject.authenticate(serverURL: serverURL), ScaleSessionControllerError.invalidParameters(["invalid"]))
        XCTAssertPrinterOutputContains("""
        Opening \(authURL().absoluteString) to start the authentication flow
        """)
    }
