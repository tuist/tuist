...
final class CloudClientOutputWarningsMiddlewareTests: TuistUnitTestCase {
    fileprivate var warningController: MockWarningController!
    var subject: CloudClientOutputWarningsMiddleware!

    override func setUp() {
        super.setUp()
        warningController = MockWarningController()
        subject = CloudClientOutputWarningsMiddleware(warningController: warningController)
    }
