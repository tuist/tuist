...
final class FileHandlerTests: TuistUnitTestCase {
    ...
    func test_replace() throws {
        ...
        try "content".write(to: URL(string: tempFile.pathString)!, atomically: true, encoding: .utf8)

        // When
        try subject.replace(destFile, with: tempFile)

        // Then
        let content = try String(contentsOf: URL(string: destFile.pathString)!)
    ...
    private func countItemsInRootTempDirectory(appropriateFor url: URL) throws -> Int {
        ...
        try fileManager.removeItem(at: URL(string: tempPath.pathString)!)
