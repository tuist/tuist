...
final class ProjectDescriptionHelpersBuilderIntegrationTests: TuistTestCase {
    ...
    func test_build_when_the_helpers_is_a_dylib() throws {
        ...
        let helpersPath = path.appending(
            RelativePath("\(Constants.tuistDirectoryName)/\(Constants.helpersDirectoryName)")
        )
        ...
        XCTAssertNotNil(
            FileHandler.shared.glob(path, glob: "*/*/ProjectDescriptionHelpers.swiftmodule").first
        )
        XCTAssertNotNil(
            FileHandler.shared.glob(path, glob: "*/*/libProjectDescriptionHelpers.dylib").first
        )
        XCTAssertNotNil(
            FileHandler.shared.glob(path, glob: "*/*/ProjectDescriptionHelpers.swiftdoc").first
        )
