...
final class SwiftPackageManagerInteractorTests: TuistUnitTestCase {
    ...
    func test_generate_usesSystemGitCredentials() throws {
        // Given
        let temporaryPath = try self.temporaryPath()
        let config = Config(compatibleXcodeVersions: .all, cloud: nil, cache: nil, swiftToolsVersion: nil, plugins: [], generationOptions: [.resolveDependenciesWithSystemScm], path: nil)
