...
final class AutogeneratedSchemesProjectMapperTests: TuistUnitTestCase {
    ...
    func test_map_onlySetsArgumentsWhenAvailableInTarget() throws {
        ...
        let targetWithEnvironment = Target.test(
            ...
            product: .framework,
            environmentVariables: [.init(key: "A", value: "B", isEnabled: true)],
            launchArguments: []
        )
        let projectPath = try temporaryPath()
        let project = Project.test(
        ...
        XCTAssertEqual(arguments, [
            nil,
            Arguments(environmentVariables: [], launchArguments: [.init(name: "--run-argument", isEnabled: true)]),
            Arguments(environmentVariables: [.init(key: "A", value: "B", isEnabled: true)], launchArguments: []),
        ])
    }
