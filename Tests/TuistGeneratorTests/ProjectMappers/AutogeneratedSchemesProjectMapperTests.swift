...
final class AutogeneratedSchemesProjectMapperTests: TuistUnitTestCase {
    ...
    func test_map_appExtensions() throws {
        ...
        XCTAssertEqual(
            ...
            [
                ...
                [
                    TargetReference(projectPath: path, name: "AppExtension"),
                    TargetReference(projectPath: path, name: "App"),
                ],
                [
                    TargetReference(projectPath: path, name: "MessageExtension"),
                    TargetReference(projectPath: path, name: "App"),
                ],
            ]
        )
    ...
    func test_map_watch2() throws {
        ...
        let watchApp = Target.test(
            name: "WatchApp",
            product: .watch2App,
            dependencies: [.target(name: "WatchExtension")]
        )
        ...
        Scheme(
            ...
            testAction: TestAction.test(
                targets: testTargetNames.map {
                    TestableTarget(target: TargetReference(projectPath: projectPath, name: $0))
                },
                arguments: nil
            ),
