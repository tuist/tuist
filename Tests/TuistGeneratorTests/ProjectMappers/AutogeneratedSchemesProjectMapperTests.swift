...
final class AutogeneratedSchemesProjectMapperTests: TuistUnitTestCase {
    ...
    func test_map_enables_parallelized_tests_on_generated_schemes() throws {
        // Given
        let targetA = Target.test(
            name: "A",
            dependencies: [
        ...
        // Then: A Tests
        let gotAScheme = try XCTUnwrap(got.schemes.first { $0.name == "ATests" })
        let action = try XCTUnwrap(gotAScheme.testAction)
        let target = try XCTUnwrap(action.targets.first)
        
        XCTAssertTrue(target.isParallelizable)
    }
