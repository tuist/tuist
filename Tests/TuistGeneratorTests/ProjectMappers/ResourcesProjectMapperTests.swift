...
final class ResourcesProjectMapperTests: TuistUnitTestCase {
    ...
    func test_map_when_a_target_that_has_resources_and_doesnt_supports_them() throws {
        ...
        let expectedTuistBundlePath = expectedTuistBundleSwiftFilePath(for: target)

        // Got
        let (gotProject, gotSideEffects) = try subject.map(project: project)

        // Then: Side effects
        try verify(gotSideEffects, for: target, by: "\(project.name)_\(target.name)")
        ...
        XCTAssertEqual(gotTarget.sources.last?.path, expectedTuistBundlePath)
    ...
    func test_map_when_a_target_that_has_resources_and_supports_them() throws {
        ...
        // Then: Side effects
        try verify(gotSideEffects, for: target, by: irrelevantBundleName)
    ...
    func test_map_when_a_target_that_has_core_data_models_and_supports_them() throws {
        ...
        // Then: Side effects
        try verify(gotSideEffects, for: target, by: "\(project.name)_test_tuist")
    ...
    func test_map_simple_target_no_bundle_accessors() throws {
        ...
        project = Project.test(
            options: .test(bundleAccessorsOptions: .disabled),
            targets: [target]
        )
    ...
    func test_map_target_internal_only_bundle_accessors() throws {
        // Given
        let bundleAccessorsOptions = Project.Options.BundleAccessorOptions(internalAccessors: true, publicAccessors: false)
        let resources: [ResourceFileElement] = [.file(path: "/image.png")]
        let target = Target.test(
            name: "test-tuist",
            product: .staticLibrary,
        ...
        project = Project.test(
            options: .test(bundleAccessorsOptions: bundleAccessorsOptions),
            targets: [target]
        )
        ...
        // Then: Side effects
        try verify(gotSideEffects, for: target, by: "\(project.name)_test_tuist", with: bundleAccessorsOptions)
    }

    func test_map_target_public_only_bundle_accessors() throws {
        // Given
        let bundleAccessorsOptions = Project.Options.BundleAccessorOptions(internalAccessors: false, publicAccessors: true)
        let resources: [ResourceFileElement] = [.file(path: "/image.png")]
        let target = Target.test(
            name: "test-tuist",
            product: .staticLibrary,
    ...
    func test_map_target_with_objc_bundle_accessors_and_with_class_with_the_same_symbols() throws {
        // Given
        let resources: [ResourceFileElement] = [.file(path: "/image.png")]
    ...
    private func verify(
        ...
        with bundleAccessorsOptions: Project.Options.BundleAccessorOptions = .default
    ) throws {
