...
final class AutogeneratedWorkspaceSchemeWorkspaceMapperTests: TuistUnitTestCase {
    func test_map() throws {
        ...
        XCTAssertEqual(
            Set(scheme.buildAction.map(\.targets)?.map(\.reference) ?? []),
            Set([
                TargetReference(
    ...
    func test_multiple_project_sorting() throws {
        ...
        let targetsNames = scheme.buildAction?.targets.map(\.reference.name)
    ...
    func test_map_when_multiple_platforms() throws {
        ...
        XCTAssertEqual(
            scheme.buildAction.map(\.targets)?.map(\.reference) ?? [],
            [
                TargetReference(
                    projectPath: projectPath,
                    name: targetA.name
    ...
    func test_map_codeCoverageMode_relevant_complex() throws {
        ...
        let projectAllCoverage = Project.test(
            ...
            schemes: [
                Scheme.test(
                    buildAction: BuildAction.test(targets: [BuildAction.Target(targetCRef), BuildAction.Target(targetDRef)]),
