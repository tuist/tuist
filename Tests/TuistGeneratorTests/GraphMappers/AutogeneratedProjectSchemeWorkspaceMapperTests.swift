...
final class AutogeneratedProjectSchemeWorkspaceMapperTests: TuistUnitTestCase {
    ...
    func test_map_codeCoverageMode_targets() throws {
        ...
        let subject = AutogeneratedProjectSchemeWorkspaceMapper(
            codeCoverageMode: .targets([targetBRef])
        )
    ...
    func test_map_codeCoverageMode_relevant_complex() throws {
        ...
        let (got, _) = try subject.map(
            workspace: WorkspaceWithProjects(
                workspace: workspace,
                projects: [projectSingleCoverage, projectAllCoverage]
            )
        )
        ...
        XCTAssertEqual(
            Set(testAction.codeCoverageTargets),
            Set([targetBRef, targetCRef, targetDRef])
        )
    }
