...
final class TargetLinterTests: TuistUnitTestCase {
        ...
        let target = Target.test(
            ...
            scripts: [
                TargetScript(
                    name: "Test script",
                    order: .post,
                    script: .embedded("echo 'This is a test'")
                )
            ]
        )
        let got = subject.lint(target: target)
    ...
    func test_lint_when_a_infoplist_file_is_being_copied() {
        ...
        XCTContainsLintingIssue(
            ...
            LintingIssue(
                reason:
                    "Info.plist at path \(infoPlistPath.pathString) being copied into the target \(target.name) product.",
                severity: .warning
            )
        )
        XCTDoesNotContainLintingIssue(
            ...
            LintingIssue(
                reason:
                    "Info.plist at path \(googeServiceInfoPlistPath.pathString) being copied into the target \(target.name) product.",
                severity: .warning
            )
        )
    }
    ...
    func test_lint_when_a_entitlements_file_is_being_copied() {
        ...
        XCTContainsLintingIssue(
            ...
            LintingIssue(
                reason:
                    "Entitlements file at path \(path.pathString) being copied into the target \(target.name) product.",
                severity: .warning
            )
        )
    }
    ...
    func test_lint_when_library_has_resources() throws {
        ...
        XCTContainsLintingIssue(
            ...
            LintingIssue(
                reason:
                    "Target \(staticLibrary.name) cannot contain resources. static library targets do not support resources",
                severity: .error
            )
        )
        ...
        XCTContainsLintingIssue(
            ...
            LintingIssue(
                reason:
                    "Target \(dynamicLibrary.name) cannot contain resources. dynamic library targets do not support resources",
                severity: .error
            )
        )
    }
    ...
    func test_lint_when_ios_bundle_has_sources() {
        ...
        XCTContainsLintingIssue(
            ...
            LintingIssue(
                reason:
                    "Target \(bundle.name) cannot contain sources. iOS bundle targets don't support source files",
                severity: .error
            )
        )
    }
    ...
    func test_lint_when_target_platform_and_deployment_target_property_mismatch() throws {
        ...
        for combinations in invalidCombinations {
            ...
            XCTContainsLintingIssue(
                ...
                LintingIssue(
                    reason:
                        "Found an inconsistency between a platform `\(combinations.0.caseValue)` and deployment target `\(combinations.1.platform)`",
                    severity: .error
                )
            )
        }
    ...
    func test_lint_invalidProductPlatformCombinations() throws {
        ...
        let expectedIssues: [LintingIssue] = [
            LintingIssue(
                reason:
                    "'WatchApp_for_iOS' for platform 'iOS' can't have a product type 'watch 2 application'",
                severity: .error
            ),
            LintingIssue(
                reason:
                    "'Watch2Extension_for_iOS' for platform 'iOS' can't have a product type 'watch 2 extension'",
                severity: .error
            ),
        ]
    ...
    func test_lint_when_target_has_duplicate_dependencies_specified() {
        ...
        XCTContainsLintingIssue(
            ...
            .init(
                reason:
                    "Target '\(target.name)' has duplicate sdk dependency specified: 'libc++.tbd'",
                severity: .warning
            )
        )
    }
    ...
    func test_lint_when_target_has_core_data_models_with_default_versions_that_dont_exist() throws {
        ...
        XCTContainsLintingIssue(
            ...
            .init(
                reason:
                    "The default version of the Core Data model at path \(dataModelPath.pathString), 1.0.0, does not exist. There should be a file at \(dataModelPath.appending(component: "1.0.0.xcdatamodel").pathString)",
                severity: .error
            )
        )
    }
    ...
    func test_lint_when_target_has_valid_codegen_sources() throws {
        ...
        XCTContainsLintingIssue(
            ...
            .init(
                reason:
                    "Target '\(target.name)' has a source file at path \(target.sources[1].path) with unsupported `codeGen` attributes. Only intentdefinition and mlmodel are known to support this.",
                severity: .warning
            )
        )
    }
