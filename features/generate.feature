Feature: Generate a new project using Tuist
  ...
  Scenario: The project is an iOS application with custom storyboard names (ios_app_with_custom_storyboard_names)
    Given that tuist is available
    And I have a working directory
    Then I copy the fixture ios_app_with_custom_storyboard_names into the working directory
    Then tuist generates the project
    Then I should be able to build the scheme App
    Then the product 'App.app' with destination 'Debug-iphoneos' has an entry of 'UIMainStoryboardFile' in the info.plist with the value of 'Custom Main'
    Then the product 'App.app' with destination 'Debug-iphoneos' has an entry of 'UILaunchStoryboardName' in the info.plist with the value of 'Custom Launch Screen'
