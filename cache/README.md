...
- **Two S3 buckets**: Xcode cache artifacts (`S3_XCODE_CACHE_BUCKET`) and module/Gradle cache (`S3_BUCKET`). When `S3_XCODE_CACHE_BUCKET` is unset, Xcode cache artifacts continue using `S3_BUCKET`. When `S3_XCODE_CACHE_BUCKET` points to a different bucket, Xcode cache reads and writes use this bucket directly. Project cleanup still runs both Xcode cache and general cache deletion passes, so duplicate cleanup work in logs is expected when both artifact types resolve to the same bucket.
