defmodule Cache.Registry.GitHub do
  ...
  def list_tags(repository_full_handle, token) do
    ...
    with {:ok, tags} <- paginated_get(url, token, fn body ->
  ...
  def list_repository_contents(repository_full_handle, token, ref) do
    ...
    request(:get, url, token, params: %{ref: ref})
    |> case do
  ...
  def download_zipball(repository_full_handle, token, tag, destination_path) do
    ...
    request(:get, url, token, decode_body: false, into: File.stream!(destination_path, [:write]))
    |> case do
      {:ok, %{status: 200}} -> :ok
      {:ok, %{status: status}} -> {:error, {:http_error, status}}
      {:error, reason} -> {:error, reason}
  ...
  defp paginated_get(url, token, mapper) do
    with {:ok, %{status: 200, body: body, headers: headers}} <- request(:get, url, token) do
  ...
  defp request(method, url, token, opts \\ []) do
    ...
    req_opts =
      [method: method, url: url, headers: headers, finch: Cache.Finch, retry: false]
      |> Keyword.merge(opts)
