defmodule Cache.XcodeModule.PromExPlugin do
  @moduledoc """
  Custom Prometheus metrics for Xcode Module Cache operations.
  ...
  executed by `CacheWeb.XcodeModuleController` and `Cache.S3TransferWorker`.
  """
  use PromEx.Plugin

  @impl true
  def event_metrics(_opts) do
    Event.build(:cache_xcode_module_event_metrics, [
      # Downloads
      counter(
        [:tuist_cache, :xcode_module, :download, :hits, :total],
        event_name: [:cache, :xcode_module, :download, :hit],
        description: "Total module cache download requests received."
      ),
      counter(
        [:tuist_cache, :xcode_module, :download, :disk_hits, :total],
        event_name: [:cache, :xcode_module, :download, :disk_hit],
        description: "Module cache downloads served from local disk."
      ),
      sum(
        [:tuist_cache, :xcode_module, :download, :disk_bytes],
        event_name: [:cache, :xcode_module, :download, :disk_hit],
        measurement: :size,
        description: "Total bytes served from local disk for module cache."
      ),
      distribution(
        [:tuist_cache, :xcode_module, :download, :artifact_size, :bytes],
        event_name: [:cache, :xcode_module, :download, :disk_hit],
        measurement: :size,
        unit: :byte,
      ...
      distribution(
        [:tuist_cache, :xcode_module, :multipart, :parts_count, :distribution],
        event_name: [:cache, :xcode_module, :multipart, :complete],
