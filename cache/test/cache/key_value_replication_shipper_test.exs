defmodule Cache.KeyValueReplicationShipperTest do
  ...
  test "shared entry no-op upserts do not refresh updated_at" do
    source_updated_at = ~U[2026-03-12 12:00:00.123456Z]
    ...
    assert :ok = KeyValueReplicationShipper.upsert_shared_entries([row])
    ...
    unchanged = Repo.get!(Entry, "keyvalue:acme:ios:cas")
    assert unchanged.updated_at == original.updated_at
  end

  test "access-only no-op flush does not refresh updated_at" do
    parent = self()
    ...
    pending_entry = %KeyValuePendingReplicationEntry{
      key: "keyvalue:acme:ios:cas",
      json_payload: Jason.encode!(%{entries: [%{"value" => "artifact"}]}),
      source_node: "node-a",
      last_accessed_at: replication_enqueued_at,
      source_updated_at: source_updated_at,
      replication_enqueued_at: DateTime.add(replication_enqueued_at, 1, :second)
    }
    ...
    unchanged = Repo.get!(Entry, "keyvalue:acme:ios:cas")
    assert unchanged.last_accessed_at == replication_enqueued_at
    assert unchanged.updated_at == original.updated_at
  end
