defmodule Cache.Gradle.DiskTest do
  ...
  describe "key/3" do
    test "constructs sharded Gradle key" do
      key = Disk.key(@test_account, @test_project, @test_cache_key)
      assert key == @test_key
    end

    test "uses sharding for nested cache key" do
      nested_key = "deeply/nested/artifact"
      key = Disk.key(@test_account, @test_project, nested_key)
  ...
  describe "exists?/3" do
    test "returns true when file exists" do
      path = Cache.Disk.artifact_path(@test_key)
      File.mkdir_p!(Path.dirname(path))
      File.write!(path, "test content")

      assert Disk.exists?(@test_account, @test_project, @test_cache_key) == true
    end

    test "returns false when file doesn't exist" do
      assert Disk.exists?("nonexistent", "project", "abcd1234") == false
    end
  end
  ...
  describe "put/4" do
    test "writes binary data to disk successfully" do
      ...
      assert Disk.put(@test_account, @test_project, @test_cache_key, data) == :ok
    ...
    test "creates parent directories if they don't exist" do
      ...
      path = Cache.Disk.artifact_path("account/project/gradle/de/ep/deeply/nested/artifact")
      assert File.exists?(path)
      assert File.read!(path) == data
    end
    ...
    test "overwrites existing file" do
      ...
      assert Disk.put(@test_account, @test_project, @test_cache_key, new_data) == :ok
    ...
    test "handles binary data" do
      ...
      assert Disk.put(@test_account, @test_project, @test_cache_key, binary_data) == :ok
    ...
    test "handles file tuple input" do
      ...
      assert Disk.put(@test_account, @test_project, @test_cache_key, {:file, tmp_path}) == :ok
  ...
  describe "stat/3" do
    test "returns file stat for existing artifact" do
      ...
      assert {:ok, stat} = Disk.stat(@test_account, @test_project, @test_cache_key)
    ...
    test "returns error for non-existent artifact" do
      assert {:error, :enoent} = Disk.stat("nonexistent", "project", "abcd1234")
    end
  end

  describe "local_accel_path/3" do
    test "builds internal X-Accel-Redirect path with sharded structure" do
      path = Disk.local_accel_path(@test_account, @test_project, @test_cache_key)
    ...
    test "builds internal path for nested cache key with sharded structure" do
      nested_key = "deeply/nested/artifact"
      path = Disk.local_accel_path(@test_account, @test_project, nested_key)
  ...
  describe "ensure_artifact_directory/3" do
    test "creates parent directory and returns its path", %{test_storage_dir: test_storage_dir} do
      assert {:ok, dir} = Disk.ensure_artifact_directory(@test_account, @test_project, @test_cache_key)
      assert File.dir?(dir)
    ...
    test "returns directory containing the artifact path" do
      assert {:ok, dir} = Disk.ensure_artifact_directory(@test_account, @test_project, @test_cache_key)
      expected_artifact_path = Cache.Disk.artifact_path(@test_key)
      assert dir == Path.dirname(expected_artifact_path)
    end

    test "is idempotent" do
      assert {:ok, dir1} = Disk.ensure_artifact_directory(@test_account, @test_project, @test_cache_key)
      assert {:ok, dir2} = Disk.ensure_artifact_directory(@test_account, @test_project, @test_cache_key)
      assert dir1 == dir2
    end
  end
  ...
  describe "integration test" do
    test "put and exists? roundtrip" do
      ...
      assert Disk.put(@test_account, @test_project, @test_cache_key, original_data) == :ok
      assert Disk.exists?(@test_account, @test_project, @test_cache_key) == true
    end
  end
end
