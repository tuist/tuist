defmodule CacheWeb.RegistryControllerTest do
  ...
  describe "GET /api/registry/swift/:scope/:name/:version.zip (download_archive)" do
    test "serves from disk when file exists locally", %{conn: conn} do
      ...
      expect(RegistryDisk, :exists?, fn ^scope, ^name, ^version, "source_archive.zip" -> true end)

      expect(RegistryDisk, :local_accel_path, fn ^scope, ^name, ^version, "source_archive.zip" ->
    ...
    test "falls back to S3 when file not on disk", %{conn: conn} do
      ...
      expect(RegistryDisk, :exists?, fn ^scope, ^name, ^version, "source_archive.zip" -> false end)
  ...
  describe "GET /api/registry/swift/:scope/:name/:version/Package.swift (show_manifest)" do
    test "serves manifest from disk when file exists", %{conn: conn} do
      ...
      expect(RegistryDisk, :exists?, fn ^scope, ^name, ^version, "Package.swift" -> true end)

      expect(RegistryDisk, :local_accel_path, fn ^scope, ^name, ^version, "Package.swift" ->
    ...
    test "redirects to default manifest when swift-version specified but not found", %{conn: conn} do
      ...
      expect(RegistryDisk, :exists?, fn ^scope, ^name, ^version, "Package@swift-5.8.swift" -> false end)
    ...
    test "falls back to S3 when manifest not on disk", %{conn: conn} do
      ...
      expect(RegistryDisk, :exists?, fn ^scope, ^name, ^version, "Package.swift" -> false end)
