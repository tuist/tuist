defmodule Cache.Config.DevInstance do
  @moduledoc false
  ...
  def server_url(base_port \\ 8080) do
    "http://localhost:#{port(base_port)}"
  end

  def sqlite_database_name(base_name) when is_binary(base_name) do
    extension = Path.extname(base_name)
    basename = Path.rootname(base_name, extension)
