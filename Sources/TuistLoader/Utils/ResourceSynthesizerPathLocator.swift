...
enum ResourceSynthesizerPathLocatorError: FatalError, Equatable {
    ...
    var description: String {
        ...
        case let .pluginNotFound(name, availablePlugins):
            return
                "Plugin \(name) was not found. Available plugins: \(availablePlugins.joined(separator: ", "))"
...
final class ResourceSynthesizerPathLocator: ResourceSynthesizerPathLocating {
    ...
    func templatePath(
        ...
        guard
            ...
            throw ResourceSynthesizerPathLocatorError.pluginNotFound(
                pluginName,
                resourceSynthesizerPlugins.map(\.name)
            )
        }
        ...
        guard
            ...
            throw ResourceSynthesizerPathLocatorError.resourceTemplateNotFound(
                name: "\(resourceName).stencil",
                plugin: plugin.name
            )
        }
