...
extension XCTestCase {
    ...
    public func XCTAssertEmpty<T: Collection>(
        _ collection: T,
    ...
    public func XCTAssertNotEmpty<T: Collection>(
        _ collection: T,
        file: StaticString = #file,
        line: UInt = #line
    ) {
        XCTAssertNotEqual(
            collection.count,
            0,
            "Expected to not be empty but it has 0 elements",
            file: file,
            line: line
        )
    }
    ...
    public func XCTAssertEqualPairs<T: Equatable>(
        _ subjects: [(T, T, Bool)],
        file: StaticString = #file,
        line: UInt = #line
    ) {
        subjects.forEach {
            if $0.2 {
                XCTAssertEqual(
                    $0.0,
                    $0.1,
                    "Expected \($0.0) to be equal to \($0.1) but they are not.",
                    file: file,
                    line: line
                )
            } else {
                XCTAssertNotEqual(
                    $0.0,
                    $0.1,
                    "Expected \($0.0) to not be equal to \($0.1) but they are.",
                    file: file,
                    line: line
                )
            }
    ...
    public func XCTAssertStandardOutput(
        pattern: String,
        file: StaticString = #file,
        line: UInt = #line
    ) {
    ...
    public func XCTTry<T>(
        _ closure: @autoclosure @escaping () throws -> T,
        file: StaticString = #file,
        line: UInt = #line
    ) -> T {
        var value: T!
    ...
    public func XCTAssertCodableEqualToJson<C: Codable>(
        ...
        let decoded = XCTTry(
            try decoder.decode(C.self, from: json.data(using: .utf8)!),
            file: file,
            line: line
        )
    ...
    public func XCTAssertCodable<C: Codable & Equatable>(
        _ subject: C,
        ...
        XCTAssertEqual(
            subject,
            decoded,
            "The subject is not equal to it's encoded & decoded version"
        )
    }
    ...
    public func XCTAssertEncodableEqualToJson<C: Encodable>(
        ...
        let jsonObject = XCTTry(
            try JSONSerialization.jsonObject(with: json.data(using: .utf8)!, options: [])
        )
        ...
        if let subjectArray = subjectObject as? [Any], let jsonArray = jsonObject as? [Any] {
            ...
            XCTAssertTrue(
                subjectNSArray.isEqual(to: jsonNSArray),
                errorString,
                file: file,
                line: line
            )

        } else if let subjectDictionary = subjectObject as? [String: Any],
            let jsonDictionary = jsonObject as? [String: Any]
        {
            ...
            XCTAssertTrue(
                subjectNSDictionary.isEqual(to: jsonNSDictionary),
                errorString,
                file: file,
                line: line
            )
            ...
            XCTFail(
                "Failed comparing the subject to the given JSON. Has the JSON the right format?"
            )
        }
    }
    ...
    public func XCTAssertHTTPResourcePath<T, E: Error>(
        ...
        XCTAssertEqual(
            requestPath,
            path,
            "Expected the path \(path) but got \(requestPath)",
            file: file,
            line: line
        )
    }
    ...
    @discardableResult public func XCTAssertContainsElementOfType<T>(
        ...
        guard let element = collection.first(where: { $0 is T }) else {
            XCTFail(
                "Didn't found an element of type \(String(describing: element))",
                file: file,
                line: line
            )
            return nil
    ...
    @discardableResult public func XCTAssertDoesntContainElementOfType<T>(
        ...
        if let element = collection.first(where: { $0 is T }) {
            XCTFail(
                "Found an element of type \(String(describing: element))",
                file: file,
                line: line
            )
            return nil
    ...
    @discardableResult public func XCTAssertContainsElementOfType<T, U>(
        ...
        guard let previousElementIndex = collection.firstIndex(where: { $0 is U }) else {
            XCTFail(
                "Didn't found an element of type \(String(describing: after))",
                file: file,
                line: line
            )
            return nil
    ...
    @discardableResult public func XCTAssertContainsElementOfType<T, U>(
        ...
        guard let afterElementIndex = collection.lastIndex(where: { $0 is U }) else {
            XCTFail(
                "Didn't found an element of type \(String(describing: before))",
                file: file,
                line: line
            )
            return nil
