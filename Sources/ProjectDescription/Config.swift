...
extension Config.GenerationOptions {
    ...
    public init(
        ...
        if container.allKeys.contains(.xcodeProjectName),
            try container.decodeNil(forKey: .xcodeProjectName) == false
        {
        ...
        } else if container.allKeys.contains(.organizationName),
            try container.decodeNil(forKey: .organizationName) == false
        {
        ...
        } else if container.allKeys.contains(.developmentRegion),
            try container.decodeNil(forKey: .developmentRegion) == false
        {
        ...
        {
            var associatedValues = try container.nestedUnkeyedContainer(
                forKey: .lastXcodeUpgradeCheck
            )
...
extension Config.GenerationOptions.CodeCoverageMode {
    ...
    public init(
        ...
        if container.allKeys.contains(.all), try container.decode(Bool.self, forKey: .all) {
            self = .all
        } else if container.allKeys.contains(.relevant),
            try container.decode(Bool.self, forKey: .relevant)
        {
            self = .relevant
