...
public struct BuildAction: Equatable, Codable, Sendable {
    ...
    public struct Target: Equatable, Codable, Sendable, ExpressibleByStringInterpolation {
        ...
        ///   - buildFor: The list of actions the target reference is associated to. They represent the action when a given target
        /// is built.
    ...
    @available(
        *,
        deprecated,
        message: "Use the initializer that accepts targets as [BuildAction.Target], which supports passing the actions when targets build for each target in the list."
    )
