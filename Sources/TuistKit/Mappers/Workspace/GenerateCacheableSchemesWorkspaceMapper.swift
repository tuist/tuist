...
final class GenerateCacheableSchemesWorkspaceMapper: WorkspaceMapping {
    ...
    private func scheme(
        ...
        return [
            Scheme(
                name:
                    "\(Constants.AutogeneratedScheme.bundlesSchemeNamePrefix)-\(platform.caseValue)",
                shared: true,
                buildAction: BuildAction(targets: bundleTargetReferences)
            ),
            Scheme(
                name:
                    "\(Constants.AutogeneratedScheme.binariesSchemeNamePrefix)-\(platform.caseValue)",
                shared: true,
