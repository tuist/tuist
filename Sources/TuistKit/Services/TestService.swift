...
final class TestService {
    ...
    func run(schemeName: String?, generate: Bool, clean: Bool, configuration: String?, path: AbsolutePath) throws {
        ...
        if let schemeName = schemeName {
            ...
            try testScheme(scheme: scheme, graph: graph, path: path, clean: clean, configuration: configuration)
        } else {
            var cleaned: Bool = false
            try testableSchemes.forEach {
                try testScheme(scheme: $0, graph: graph, path: path, clean: !cleaned && clean, configuration: configuration)
                cleaned = true
            }
        }
    ...
    private func testScheme(scheme: Scheme, graph: Graph, path: AbsolutePath, clean: Bool, configuration: String?) throws {
