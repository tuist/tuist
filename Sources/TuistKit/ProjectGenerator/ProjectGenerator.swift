...
class ProjectGenerator: ProjectGenerating {
    ...
    private func generateProject(path: AbsolutePath) throws -> (AbsolutePath, Graph) {
        // Load
        let (project, graph, _, sideEffects) = try loadProject(path: path)
    ...
    internal func generateProjectWorkspace(path: AbsolutePath) throws -> (AbsolutePath, Graph) {
        // Load
        let (project, graph, config, sideEffects) = try loadProjectWorkspace(path: path)
        ...
        // Apply workspace mappers
        let (updatedWorkspace, workspaceMapperSideEffects) = try workspaceMapperProvider
            .mapper(config: config)
            .map(workspace: workspace, graph: graph)
        
        let workspaceDescriptor = try generator.generateWorkspace(workspace: updatedWorkspace, graph: graph)

        // Write
        try writer.write(workspace: workspaceDescriptor)

        // Mapper side effects
        try sideEffectDescriptorExecutor.execute(sideEffects: sideEffects + workspaceMapperSideEffects)
    ...
    private func loadProject(path: AbsolutePath) throws -> (Project, Graph, Config, [SideEffectDescriptor]) {
        // Load all manifests
        ...
        return (project, updatedGraph, config, modelMapperSideEffects + graphMapperSideEffects)
    }

    // swiftlint:disable:next large_tuple
    private func loadProjectWorkspace(path: AbsolutePath) throws -> (Project, Graph, Config, [SideEffectDescriptor]) {
        // Load project
        let (project, graph, config, sideEffects) = try loadProject(path: path)
        ...
        return (project, graph, config, sideEffects + additionalSideEffects)
    }
    ...
    private func loadWorkspace(path: AbsolutePath) throws -> (Workspace, Graph, [SideEffectDescriptor]) {
        ...
        /// Used to disambiguate which workspaces are generated by tuist and which are not
        let tuistGeneratedFileDescriptor = FileDescriptor(
            path: updatedWorkspace.path.appending(components: "\(workspace.name).xcworkspace", Constants.tuistGeneratedFileName)
        )
        ...
        return (updatedWorkspace, updatedGraph, modelMapperSideEffects + graphMapperSideEffects + workspaceMapperSideEffects + additionalSideEffects)
    }
