    ...
    func store(
        ...
        try await withThrowingTaskGroup(of: Void.self) { group in
            for (target, hash) in hashesByCacheableTarget {
                ...
                let suffix =
                    "\(isBinary ? Constants.AutogeneratedScheme.binariesSchemeNamePrefix : Constants.AutogeneratedScheme.bundlesSchemeNamePrefix)-\(target.target.legacyPlatform.caseValue)"
