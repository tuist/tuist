    ...
    func store(
        ...
        try await withThrowingTaskGroup(of: Void.self) { group in
            for (target, hash) in hashesByCacheableTarget {
                ...
                let platformsIdentifier = target.target.supportedPlatforms.map(\.caseValue).sorted().joined()
                let suffix =
                    "\(isBinary ? Constants.AutogeneratedScheme.binariesSchemeNamePrefix : Constants.AutogeneratedScheme.bundlesSchemeNamePrefix)-\(platformsIdentifier)"
