...
final class CacheController: CacheControlling {
    ...
    func cache(
        ...
        logger.notice(
            "Targets to be cached: \(hashesByTargetToBeCached.map(\.0.target.name).sorted().joined(separator: ", "))"
        )

        logger.notice("Filtering cacheable targets")

        let targetsToBeCached = Set(hashesByTargetToBeCached.map(\.0.target.name))
        let (projectPath, updatedGraph) =
            try generatorFactory
            .cache(
                config: config,
    ...
    private func archive(
        ...
        try FileHandler.shared.inTemporaryDirectory { outputDirectory in
            ...
            let targetsToStore = hashesByCacheableTarget.map(\.0.target.name).sorted().joined(
                separator: ", "
            )
            logger.notice(
                "Storing \(hashesByCacheableTarget.count) cacheable targets: \(targetsToStore)"
            )
    ...
    func makeHashesByTargetToBeCached(
        ...
        return try graph.compactMap(context: .concurrent) {
            target throws -> (GraphTarget, String)? in
            guard
