...
protocol CacheControlling {
    ...
    func cache(path: AbsolutePath, userConfig: Config) throws
}
...
final class CacheController: CacheControlling {
    ...
    func cache(path: AbsolutePath, userConfig: Config) throws {
        ...
        let cacheableTargets = try self.cacheableTargets(graph: graph, userConfig: userConfig)

        let completables = try cacheableTargets.map { try buildAndCacheXCFramework(path: path, target: $0.key, hash: $0.value, userConfig: userConfig) }
    ...
    fileprivate func cacheableTargets(graph: Graph, userConfig: Config) throws -> [TargetNode: String] {
        try graphContentHasher.contentHashes(for: graph)
            .filter { target, hash in
                if let exists = try self.cache.exists(hash: hash, userConfig: userConfig).toBlocking().first(), exists {
    ...
    fileprivate func buildAndCacheXCFramework(path: AbsolutePath, target: TargetNode, hash: String, userConfig: Config) throws -> Completable {
        ...
        return cache
            .store(hash: hash, userConfig: userConfig, xcframeworkPath: xcframeworkPath)
