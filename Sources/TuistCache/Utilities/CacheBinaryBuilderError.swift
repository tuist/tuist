...
enum CacheBinaryBuilderError: FatalError {
    ...
    var description: String {
        ...
        case let .nonFrameworkTargetForXCFramework(name):
            return
                "Can't generate an .xcframework from the target '\(name)' because it's not a framework target"
        case let .nonFrameworkTargetForFramework(name):
            return
                "Can't generate a .framework from the target '\(name)' because it's not a framework target"
        case let .nonBundleTarget(name):
            return
                "Can't generate a .bundle from the target '\(name)' because it's not a bundle target"
        }
    }
}
