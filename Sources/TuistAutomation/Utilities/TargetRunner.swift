...
public enum TargetRunnerError: Equatable, FatalError {
    ...
    public var description: String {
        ...
        case let .runningNotSupported(target):
            return "Cannot run \(target.name) - the platform \(target.mainPlatform.caseValue) and product type \(target.product.caseValue) are not currently supported."
...
public final class TargetRunner: TargetRunning {
    ...
    public func runTarget(
        ...
        let xcodeBuildDirectory = try xcodeProjectBuildDirectoryLocator.locate(
            platform: target.target.deploymentTargets.first!.platform,
        ...
        switch (target.target.deploymentTargets.first!.platform, target.target.product) {
