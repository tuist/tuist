...
public protocol HTTPRequestDispatching {
    func dispatch<T, E: Error>(
        resource: HTTPResource<T, E>
    ) -> Single<(object: T, response: HTTPURLResponse)>
    func dispatch<T, E: Error>(
        resource: HTTPResource<T, E>
    ) -> AnyPublisher<(object: T, response: HTTPURLResponse), Error>
}
...
public final class HTTPRequestDispatcher: HTTPRequestDispatching {
    ...
    public func dispatch<T, E: Error>(
        resource: HTTPResource<T, E>
    ) -> Single<(object: T, response: HTTPURLResponse)> {
                    ...
                    if let error = error {
                        ...
                        default:  // Error
                            do {
                                ...
                                observer(
                                    .error(
                                        HTTPRequestDispatcherError.serverSideError(error, response)
                                    )
                                )
                            } catch {
                                observer(.error(HTTPRequestDispatcherError.parseError(error)))
