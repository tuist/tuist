...
public class MachineEnvironment {
    ...
    public let clientId = Host.current().name?.checksum(algorithm: .md5)
    ...
    public let swiftVersion = try? System.shared.capture("/usr/bin/xcrun", "swift", "-version").components(separatedBy: "Swift version ").last?.components(separatedBy: " ").first
