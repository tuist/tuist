...
public final class AutogeneratedProjectSchemeWorkspaceMapper: WorkspaceMapping { // swiftlint:disable:this type_name
    ...
    let testingOptions: TestingOptions?

    // MARK: - Init

    public init(codeCoverageMode: CodeCoverageMode?, testingOptions: TestingOptions?) {
            ...
            .reduce(([], [])) { result, project in
                ...
                let testableTargets = project.targets
                    ...
                    .map {
                        TestableTarget(
                            target: $0,
                            parallelizable: testingOptions?.contains(.parallelizable) ?? true,
                            randomExecutionOrdering: testingOptions?.contains(.randomExecutionOrdering) ?? false
                        )
                    }
