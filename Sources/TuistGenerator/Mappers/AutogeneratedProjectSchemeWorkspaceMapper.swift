...
public final class AutogeneratedProjectSchemeWorkspaceMapper: WorkspaceMapping { // swiftlint:disable:this type_name
    ...
    let autogenerationOptions: AutogenerationOptions?

    // MARK: - Init

    public init(codeCoverageMode: CodeCoverageMode?, autogenerationOptions: AutogenerationOptions?) {
        self.codeCoverageMode = codeCoverageMode
        self.autogenerationOptions = autogenerationOptions
    }
            ...
            .reduce(([], [])) { result, project in
                ...
                let testableTargets = project.targets
                    ...
                    .map {
                        TestableTarget(
                            target: $0,
                            parallelizable: autogenerationOptions?.parallelizable ?? false,
                            randomExecutionOrdering: autogenerationOptions?.randomExecutionOrdering ?? false
                        )
                    }
