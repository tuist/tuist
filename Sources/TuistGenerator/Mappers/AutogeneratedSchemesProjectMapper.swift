...
public final class AutogeneratedSchemesProjectMapper: ProjectMapping {
    ...
    private func isAssociatedTestTarget(for target: Target, potentialTestTarget: Target) -> Bool {
        ...
        let isNameMatching = potentialTestTarget.name == "\(target.name)Tests" ||
            potentialTestTarget.name == "\(target.name)UITests" ||
            potentialTestTarget.name == "\(target.name)IntegrationTests"
