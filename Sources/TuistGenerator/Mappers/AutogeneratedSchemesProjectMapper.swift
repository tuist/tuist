...
public final class AutogeneratedSchemesProjectMapper: ProjectMapping {
    ...
    private func createDefaultScheme(
        ...
        var buildTargetReferences = (
                buildTargets.sorted { $0.name < $1.name } +
                testTargets.sorted { $0.name < $1.name } +
                runTargets.sorted { $0.name < $1.name }
            )
            .map {
        ...
        return Scheme(
            name: name,
            shared: true,
            buildAction: BuildAction(targets: buildTargetReferences),
    ...
    ) -> [String: (build: Set<Target>, test: Set<Target>, run: Set<Target>)] {
