...
public final class AutogeneratedSchemesProjectMapper: ProjectMapping { // swiftlint:disable:this type_body_length
    ...
    private func createDefaultScheme(
        ...
        if buildTargetReferences.isEmpty {
            buildAction = XcodeGraph.BuildAction(targets: testableTargets.map(\.target).map { XcodeGraph.BuildAction.Target(
                $0,
                buildFor: nil
            ) })
        } else {
            buildAction = XcodeGraph.BuildAction(targets: buildTargetReferences.map { XcodeGraph.BuildAction.Target(
                $0,
                buildFor: nil
            ) })
        }
