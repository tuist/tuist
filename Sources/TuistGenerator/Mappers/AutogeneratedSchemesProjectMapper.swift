...
public final class AutogeneratedSchemesProjectMapper: ProjectMapping {
    ...
    private func createDefaultScheme(
        ...
        let buildTargetReferences = buildTargets
            .union(testTargets)
            .map {
                .init(projectPath: project.path, name: $0.name)
            } + (runAction?.executable.map { [$0] } ?? [])

        let testableTargets: [TestableTarget] = testTargets.map {
        ...
        return Scheme(
            name: name,
            shared: true,
            buildAction: BuildAction(targets: buildTargetReferences.sorted { $0.name < $1.name }),
            testAction: testableTargets.isEmpty ? nil : TestAction(
                targets: testableTargets.sorted { $0.target.name < $1.target.name },
                arguments: nil,
                configurationName: buildConfiguration,
                attachDebugger: true,
    ...
    private func groupByName(
        ...
        return Dictionary(uniqueKeysWithValues: allGroupNames.map { name in
            ...
            let runTarget = groupToRunTarget[name]
            let schemeName: String

            let targetsCount = buildTargets.count + testTargets.count + (runTarget != nil ? 1 : 0)
            if targetsCount == 1 {
                // use target name as scheme name
                schemeName = (buildTargets.first ?? testTargets.first ?? runTarget)!.name // swiftlint:disable:this force_unwrapping
            } else {
                schemeName = name
            }
            return (
