...
public struct AutogeneratedSchemesProjectMapper: ProjectMapping { // swiftlint:disable:this type_body_length
    ...
    public func map(project: Project) async throws -> (Project, [SideEffectDescriptor]) {
        ServiceContext.current?.logger?.debug("Transforming project \(project.name): Auto-generating project schemes")
        ...
        for target in project.targets.values.sorted() {
            ...
            case .app, .appClip, .commandLineTool, .watch2App, .xpc, .loginItem:
        ...
        if !targetGroups.isEmpty {
            autogeneratedSchemes = try await targetGroups.concurrentMap { name, targets in
                try await createDefaultScheme(
                    name: name,
                    project: project,
            ...
            let remainingBuildTargetsSchemes = try await buildTargets.concurrentMap {
                try await createDefaultScheme(
                    name: $0.name,
                    project: project,
                    buildTargets: [$0],
            ...
            let remainingTestTargetsSchemes = try await testTargets.concurrentMap {
                try await createDefaultScheme(
                    name: $0.name,
                    project: project,
                    buildTargets: [],
            ...
            let remainingRunTargetsSchemes = try await runTargets.concurrentMap {
                try await createDefaultScheme(
                    name: $0.name,
                    project: project,
                    buildTargets: [],
    ...
    private func createDefaultScheme(
        ...
        let testableTargets: [TestableTarget] = testTargets
            .map {
                ...
                return TestableTarget(
                    ...
                    parallelization: parallel ? .all : .none,
        ...
        if buildTargetReferences.isEmpty {
            buildAction = BuildAction(
                targets: testableTargets.map(\.target)
            )
        } else {
            buildAction = BuildAction(
                targets: buildTargetReferences
            )
        }
        return Scheme(
            name: name,
            shared: true,
            buildAction: try await buildInsightsActionMapper.map(
                buildAction,
                buildInsightsDisabled: buildInsightsDisabled
            ),
        ...
        let groupToBuildTargets: [String: Set<Target>] = buildTargets
            .map { target -> (name: String, target: Target) in
                for buildSuffix in sortedBuildSuffixes where target.name.hasSuffix(buildSuffix) {
                    ...
                    if target.name == buildSuffix {
                        groupName = target.name
                    } else {
                        groupName = String(target.name.dropSuffix(buildSuffix))
                    }
