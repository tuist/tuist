...
public final class AutogeneratedSchemesProjectMapper: ProjectMapping {
    ...
    private func createDefaultScheme(
        ...
        let buildTargets = buildableTargets(
            targetReference: targetReference,
            target: target,
            project: project
        )
        let testTargets = testableTargets(
            targetReference: targetReference,
            target: target,
            project: project
        )
        let executable = runnableExecutable(
            targetReference: targetReference,
            target: target,
            project: project
        )
    ...
    private func runnableExecutable(
        targetReference: TargetReference,
        target: Target,
        project: Project
    ) -> TargetReference? {
    ...
    private func hostAppTargetReferences(
        for target: Target,
        project: Project
    ) -> [TargetReference] {
        project.targets
            .filter {
                $0.product.canHostTests() && $0.dependencies.contains(.target(name: target.name))
            }
            .sorted(by: { $0.name < $1.name })
            .map { TargetReference(projectPath: project.path, name: $0.name) }
    }

    private func hostWatchAppTargetReferences(
        for target: Target,
        project: Project
    ) -> [TargetReference] {
        project.targets
            .filter {
                $0.product == .watch2App && $0.dependencies.contains(.target(name: target.name))
            }
            .sorted(by: { $0.name < $1.name })
            .map { TargetReference(projectPath: project.path, name: $0.name) }
    }
    ...
    private func isAssociatedTestTarget(for target: Target, potentialTestTarget: Target) -> Bool {
        ...
        let isNameMatching =
            potentialTestTarget.name.hasPrefix(target.name)
            && potentialTestTarget.name.hasSuffix("Tests")
