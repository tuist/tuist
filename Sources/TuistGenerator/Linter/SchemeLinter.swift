...
class SchemeLinter: SchemeLinting {
    func lint(project: Project) -> [LintingIssue] {
        ...
        issues.append(
            contentsOf: lintReferencedBuildConfigurations(
                schemes: project.schemes,
                settings: project.settings
            )
        )
        issues.append(
            contentsOf: lintCodeCoverageTargets(schemes: project.schemes, targets: project.targets)
        )
        issues.append(
            contentsOf: lintExpandVariableTarget(schemes: project.schemes, targets: project.targets)
        )
        issues.append(
            contentsOf: projectSchemeCantReferenceRemoteTargets(
                schemes: project.schemes,
                project: project
            )
        )
...
extension SchemeLinter {
    fileprivate func lintReferencedBuildConfigurations(
        schemes: [Scheme],
        settings: Settings
    ) -> [LintingIssue] {
        ...
        if let runAction = scheme.runAction {
                ...
                !FileHandler.shared.exists(storeKitPath)
            {
                issues.append(
                    LintingIssue(
                        reason:
                            "StoreKit configuration file not found at path \(storeKitPath.pathString)",
                        severity: .error
                    )
                )
            }
    ...
    fileprivate func missingBuildConfigurationIssue(
    ...
    fileprivate func lintExpandVariableTarget(
        schemes: [Scheme],
        targets: [Target]
    ) -> [LintingIssue] {
        ...
        for scheme in schemes {
                ...
                let target = testAction.expandVariableFromTarget
            {
                if !targetNames.contains(target.name) {
                    issues.append(
                        missingExpandVariablesTargetIssue(
    ...
    fileprivate func lintCodeCoverageTargets(schemes: [Scheme], targets: [Target]) -> [LintingIssue]
    {
        ...
        for scheme in schemes {
            for target in scheme.testAction?.codeCoverageTargets ?? [] {
                if !targetNames.contains(target.name) {
                    issues.append(
                        missingCodeCoverageTargetIssue(
                            missingTargetName: target.name,
                            schemaName: scheme.name
                        )
                    )
    ...
    fileprivate func missingCodeCoverageTargetIssue(
    ...
    fileprivate func missingExpandVariablesTargetIssue(
        missingTargetName: String,
        schemaName: String
    ) -> LintingIssue {
        let reason =
    ...
    fileprivate func projectSchemeCantReferenceRemoteTargets(
        schemes: [Scheme],
        project: Project
    ) -> [LintingIssue] {
        ...
        scheme.targetDependencies().forEach {
            if $0.projectPath != project.path {
                issues.append(
                    .init(
                        reason:
                            "The target '\($0.name)' specified in scheme '\(scheme.name)' is not defined in the project named '\(project.name)'. Consider using a workspace scheme instead to reference a target in another project.",
                        severity: .error
                    )
                )
            }
