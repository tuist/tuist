...
public protocol XCActivityLogControlling {
    ...
    func buildTimesByTarget(projectDerivedDataDirectory: AbsolutePath) async throws -> [String:
        Double]
...
public struct XCActivityLogController: XCActivityLogControlling {
        ...
        async throws -> AbsolutePath?
    {
        ...
        guard
            let latestLog = plist.logs.values.sorted(by: {
            ...
            environment
                ...
                || (after.timeIntervalSinceReferenceDate - 10..<after.timeIntervalSinceReferenceDate
                    + 10) ~= latestLog.timeStoppedRecording
        else {
            return nil
        }
    ...
    public func parse(_ path: AbsolutePath) async throws -> XCActivityLog {
        ...
        return XCActivityLog(
            ...
            category: category,
            issues: try await issues(steps: steps, targets: targets)
        )
    }

    private func issues(steps: [BuildStep], targets: [XCActivityTarget]) async throws -> [XCActivityIssue] {
        return try await
            steps
            .compactMap { $0 }
    ...
    private func xcactivityIssues(_ step: BuildStep) async throws -> [XCActivityIssue] {
        let targetName =
            (try? RegEx(pattern: "in target '([^']+)'").matchGroups(in: step.signature)
            .first?.first)?
            .trimmingCharacters(in: .whitespaces) ?? ""
        let projectName =
            (try? RegEx(pattern: "from project '([^']+)'").matchGroups(in: step.signature)
            .first?.first)?
            .trimmingCharacters(in: .whitespaces) ?? ""
        ...
        if let file = try? AbsolutePath(
            ...
            path = file.relative(to: rootDirectory ?? AbsolutePath.root)
        } else {
            path = nil
        }
        ...
        return (errors + warnings).map { notice in
            let type: XCActivityIssueType = switch notice.type {
            case .analyzerWarning, .clangWarning, .deprecatedWarning, .interfaceBuilderWarning, .swiftWarning, .projectWarning, .note:
                    .warning
            case .error, .clangError, .swiftError, .linkerError, .scriptPhaseError, .failedCommandError, .packageLoadingError:
                    .error
            }
    ...
    private func buildCategory(steps: [BuildStep], targets: [XCActivityTarget])
...
public enum XCActivityStepType {
