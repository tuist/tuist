defmodule TuistRegistry.Finch.Pools do
  @moduledoc false

  def config do
    server_url = TuistRegistry.Config.server_url()
    s3_protocols = TuistRegistry.Config.s3_protocols()
      ...
      server_url => [
        conn_opts: [
          ...
          transport_opts:
            [
              verify: :verify_peer
            ] ++ TuistCommon.FinchPools.ca_cert_opts(TuistRegistry.Config.server_ca_cert_pem())
        ],
        size: 32,
        count: 8,
        protocols: [:http2, :http1],
    ...
    case TuistRegistry.Config.s3_config() do
      {:ok, s3_config} ->
        s3_url =
        ...
        {_s3_url, s3_pool_opts} =
          TuistCommon.FinchPools.s3_pool(
            ...
            ca_cert_pem: TuistRegistry.Config.s3_ca_cert_pem()
          )
  ...
  defp virtual_host_s3_endpoints(s3_url) do
    if TuistRegistry.Config.s3_virtual_host() do
      [TuistRegistry.Config.registry_bucket()]
