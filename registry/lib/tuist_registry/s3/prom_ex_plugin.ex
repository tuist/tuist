defmodule TuistRegistry.S3.PromExPlugin do
  @moduledoc """
  ...
  defp head_metrics do
    Event.build(:tuist_registry_s3_head_event_metrics, [
      counter(
        [:tuist_registry, :s3, :head, :requests, :total],
        event_name: [:tuist_registry, :s3, :head],
        description: "S3 HEAD request count by result.",
        tags: [:result],
        tag_values: fn metadata -> %{result: to_string(Map.get(metadata, :result, :unknown))} end
      ),
      distribution(
        [:tuist_registry, :s3, :head, :duration, :milliseconds],
        event_name: [:tuist_registry, :s3, :head],
  ...
  defp upload_metrics do
    Event.build(:tuist_registry_s3_upload_event_metrics, [
      counter(
        [:tuist_registry, :s3, :upload, :requests, :total],
        event_name: [:tuist_registry, :s3, :upload],
        description: "S3 upload request count by result.",
        tags: [:result],
        tag_values: fn metadata -> %{result: to_string(Map.get(metadata, :result, :unknown))} end
      ),
      distribution(
        [:tuist_registry, :s3, :upload, :duration, :milliseconds],
        event_name: [:tuist_registry, :s3, :upload],
        measurement: :duration,
        unit: {:microsecond, :millisecond},
  ...
  defp download_metrics do
    Event.build(:tuist_registry_s3_download_event_metrics, [
      counter(
        [:tuist_registry, :s3, :download, :requests, :total],
        event_name: [:tuist_registry, :s3, :download],
        description: "S3 download request count by result.",
        tags: [:result],
        tag_values: fn metadata -> %{result: to_string(Map.get(metadata, :result, :unknown))} end
      ),
      distribution(
        [:tuist_registry, :s3, :download, :duration, :milliseconds],
        event_name: [:tuist_registry, :s3, :download],
        measurement: :duration,
        unit: {:microsecond, :millisecond},
  ...
  defp delete_metrics do
    Event.build(:tuist_registry_s3_delete_event_metrics, [
      counter(
        [:tuist_registry, :s3, :delete, :requests, :total],
        event_name: [:tuist_registry, :s3, :delete],
        description: "S3 delete request count by result.",
        tags: [:result],
        tag_values: fn metadata -> %{result: to_string(Map.get(metadata, :result, :unknown))} end
      ),
      distribution(
        [:tuist_registry, :s3, :delete, :duration, :milliseconds],
        event_name: [:tuist_registry, :s3, :delete],
        measurement: :duration,
        unit: {:microsecond, :millisecond},
