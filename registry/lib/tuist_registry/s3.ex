defmodule TuistRegistry.S3 do
  ...
  """
  def upload(key) do
    local_path = TuistRegistry.Disk.artifact_path(key)

    Logger.info("Starting S3 upload for artifact: #{key}")

    if File.exists?(local_path) do
      case upload_file(key, local_path, type: :registry) do
        :ok ->
  ...
  defp handle_download_result(key, local_path, tmp_path, dl_duration, {:ok, :done}) do
    case publish_download(tmp_path, local_path) do
      :ok ->
        :telemetry.execute([:tuist_registry, :s3, :download], %{duration: dl_duration}, %{result: :ok})
        {:ok, :hit}

      {:error, reason} ->
        :telemetry.execute([:tuist_registry, :s3, :download], %{duration: dl_duration}, %{result: :error})
  ...
  defp handle_download_result(key, _local_path, tmp_path, dl_duration, {:error, {:http_error, 429, _}}) do
    ...
    :telemetry.execute([:tuist_registry, :s3, :download], %{duration: dl_duration}, %{result: :rate_limited})
  ...
  defp publish_download(tmp_path, local_path) do
    case TuistRegistry.Disk.move_file(tmp_path, local_path) do
      :ok ->
        :ok
