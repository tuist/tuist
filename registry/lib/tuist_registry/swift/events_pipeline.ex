defmodule TuistRegistry.Swift.EventsPipeline do
  @moduledoc """
  ...
  def start_link(_opts) do
    batch_size = Application.get_env(:tuist_registry, :events_batch_size, 100)
    batch_timeout = Application.get_env(:tuist_registry, :events_batch_timeout, 5_000)
  ...
  def async_push(event) do
    if TuistRegistry.AnalyticsCircuitBreaker.accept_event?(webhook_url()) do
  ...
  defp send_batch(events) do
    ...
    TuistRegistry.WebhookClient.signed_post(webhook_url(), body, "registry download events")
  end

  defp webhook_url do
    "#{TuistRegistry.Config.server_url()}/webhooks/registry"
  end
end
