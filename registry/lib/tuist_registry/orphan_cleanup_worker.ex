defmodule TuistRegistry.OrphanCleanupWorker do
  @moduledoc """
  ...
  In the registry path, Phoenix writes the file first and
  `CacheArtifacts.track_artifact_access/1` enqueues a metadata record to an
  ETS buffer that periodically flushes to SQLite.
  ...
  identified as an orphan and removed. The next registry request downloads
  the artifact from S3 again, so the system is self-healing.
  """

  use Oban.Worker, queue: :maintenance, max_attempts: 1

  alias TuistRegistry.CacheArtifacts
  alias TuistRegistry.Config
  alias TuistRegistry.Disk
  alias TuistRegistry.OrphanScanCursors
