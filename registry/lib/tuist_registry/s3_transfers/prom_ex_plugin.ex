defmodule TuistRegistry.S3Transfers.PromExPlugin do
  @moduledoc """
  ...
  def polling_metrics(opts) do
    ...
    Polling.build(
      :tuist_registry_s3_transfers_polling_metrics,
      poll_rate,
      {__MODULE__, :execute_queue_metrics, []},
      [
        last_value(
          [:tuist_cache, :s3_transfers, :pending, :uploads],
          event_name: [:tuist_registry, :prom_ex, :s3_transfers, :queue],
  ...
  def execute_queue_metrics do
    ...
    :telemetry.execute(
      [:tuist_registry, :prom_ex, :s3_transfers, :queue],
      %{
