defmodule TuistRegistryWeb.Swift.RegistryController do
  ...
  alias TuistRegistry.Swift.EventsPipeline
    ...
    case normalize_registry_scope_name_version(scope, name, version) do
      {:ok, {scope, name, normalized_version}} ->
        ...
        |> Enum.reduce_while(:not_found, fn filename, _acc ->
          ...
          cond do
            Swift.Disk.exists?(scope, name, normalized_version, filename) ->
              {:halt,
               {:served,
  ...
  defp do_download_archive(conn, scope, name, normalized_version) do
    ...
    if Swift.Disk.exists?(scope, name, normalized_version, "source_archive.zip") do
  ...
  defp render_local_archive(conn, scope, name, normalized_version) do
    local_path =
      Swift.Disk.local_accel_path(scope, name, normalized_version, "source_archive.zip")
  ...
  defp serve_manifest_from_disk(conn, scope, name, version, filename, swift_version, key) do
    ...
    local_path = Swift.Disk.local_accel_path(scope, name, version, filename)
  ...
  defp registry_path(_conn, suffix), do: @registry_base_path <> suffix
