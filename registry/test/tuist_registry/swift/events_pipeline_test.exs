defmodule TuistRegistry.Swift.EventsPipelineTest do
  use ExUnit.Case, async: true
  use Mimic

  import TuistRegistry.AnalyticsCircuitBreakerTestHelpers, only: [setup_analytics_circuit_breaker: 1]
  ...
  setup do
    stub(TuistRegistry.Config, :server_url, fn -> @server_url end)
    :ok
  end
  ...
  describe "handle_batch/4" do
    test "skips sending events when API key is not configured" do
      stub(TuistRegistry.Config, :api_key, fn -> nil end)
    ...
    test "computes correct HMAC-SHA256 signature" do
      ...
      stub(TuistRegistry.Config, :api_key, fn -> secret end)
