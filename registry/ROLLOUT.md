...
This document covers how the new `registry` Phoenix service is brought
...
`.github/workflows/registry-deploy.yml` pick up changes under
`registry/**` and `tuist_common/**`. The cascade is canary → production
(gated on canary success). The chart deploys behind ingress on
`registry-canary.tuist.dev` and `registry.tuist.dev`, neither of
which any client currently resolves through.

Prereqs (cluster-side, not in this PR):

- `REGISTRY` 1Password item exists in `tuist-k8s-canary` and
  ...
  GitHub token:
      ...
      S3_REGION, TUIST_REGISTRY_SWIFT_API_KEY (= TUIST_CACHE_API_KEY),
...
- DNS A/AAAA records for `registry-canary.tuist.dev` and
  `registry.tuist.dev` point at the canary and production cluster
...
Smoke-test against `registry-canary.tuist.dev` directly:

- `curl -fsS https://registry-canary.tuist.dev/up` → 200
- `curl -fsS -H 'Accept: application/vnd.swift.registry.v1+json' \
    https://registry-canary.tuist.dev/swift/availability` → 200
- Metadata fetch:
  `https://registry-canary.tuist.dev/swift/apple/swift-argument-parser` →
  200 with `Content-Version: 1` and a JSON list of releases.
- Manifest fetch:
  `https://registry-canary.tuist.dev/swift/apple/swift-argument-parser/1.0.0/Package.swift`
...
- Source archive download:
  `https://registry-canary.tuist.dev/swift/apple/swift-argument-parser/1.0.0.zip`
  → 200 with archive bytes (the X-Accel-Redirect handoff to nginx
  `/internal/local` or `/internal/remote`).
- Legacy URL through the Cloudflare Worker rewrite (cutover stage):
  `https://tuist.dev/api/registry/swift/apple/swift-argument-parser/1.0.0/Package.swift`
  proxies to `https://registry.tuist.dev/swift/apple/swift-argument-parser/1.0.0/Package.swift`
  with the same response. Same for `registry.tuist.dev/api/registry/swift/*`.
...
- Operator scripts:
  - `mise run registry:sync apple/swift-argument-parser 1.4.0 \
      --namespace registry --context tuist-canary` → enqueues an Oban
    job; verify completion via Oban logs or `Oban.Job` query.
  - `mise run registry:purge apple/swift-argument-parser 1.4.0 \
      --namespace registry --context tuist-canary` → removes the
...
- Update the Cloudflare Worker in `infra/registry-router/` to proxy
  ...
  `registry-canary.tuist.dev` (or remove the canary host from the
...
- Watch error rates and download latency on
  `registry-canary.tuist.dev` for ≥24h.
...
- Update the Worker to proxy `registry.tuist.dev/*` and
  `tuist.dev/api/registry/*` to `registry.tuist.dev`.
- `wrangler deploy`.
- Watch ≥48h.
...
Independent of stages 3–5, once `registry.tuist.dev` is stable:

- Re-introduce the `cli/Sources/TuistConstants/Constants.swift` helper that
  returns `registry.tuist.dev` and the associated
...
Staging (`registry-staging.tuist.dev`,
`.github/workflows/registry-staging-deploy.yml`) is wired in the chart
but its `REGISTRY` 1Password item is not provisioned yet — cache itself
runs staging without a real registry bucket
(`cache/.kamal/secrets.staging` leaves `S3_REGISTRY_BUCKET` blank). To bring
staging online, create the staging `REGISTRY` item either pointing at
...
PR after weeks of stable registry traffic.
