...
## 13. `import` / `alias` / `require` belong at module level, not in function bodies

Module-level declarations are how this codebase signals dependencies at a
glance — every `lib/` and `test/` module opens with its `alias` block
(and the rare `import`) at the top, sorted alphabetically. Inlining
`import Ecto.Query` (or any `alias`, `require`, `use`) into a function
body hides that dependency from anyone scanning the file, prevents the
compiler from catching unused aliases, and quietly produces a different
behaviour each call (the import only applies inside that one clause).

### Flag (Severity: low)

- A new `import Foo` / `alias Foo` / `require Foo` / `use Foo` line that
  appears inside a `def`, `defp`, `test`, `setup`, `describe`, or any
  block other than the top of the module — including LiveView mount /
  handle_event bodies and Ecto migration `up/0` / `down/0`.

When suggesting a fix, move the declaration to the module's top alias
block (alphabetical with the others) and remove the inline copy.

### Do not flag

- Inline `alias __MODULE__` or `alias __MODULE__.Foo` inside a body
  where the explicit alias only makes sense in that local scope (rare
  but legitimate).
- `import` lines inside Phoenix.Component or Phoenix.LiveView macros
  generated by `use TuistWeb, :live_view` (those are macro expansions,
  not author-written).
- Aliases declared inside `quote do ... end` blocks of a macro
  definition — those are intentionally scoped to the generated code.
...
2. The category above is one of 1–13; if it isn't, downgrade to a
