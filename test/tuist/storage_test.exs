defmodule Tuist.StorageTest do
  ...
  describe "multipart_start/1" do
    ...
    test "sends the right request and parses the body when the response is successful and the object key has spaces" do
      # Given
      Environment |> stub(:on_premise?, fn -> false end)
      Environment |> stub(:s3_bucket_name, fn -> "bucket" end)
      base_url = Environment.s3_endpoint()

      Req
      |> stub(:post!, fn %{
                           options: %{base_url: ^base_url},
                           url: %{query: "uploads", path: "/bucket/object%20key"}
                         } ->
        %{
          status: 200,
          body: ~s"""
      ...
      # When
      assert Storage.multipart_start("object key") == {:ok, "upload-id"}
    end

    test "sends the right request and parses the body when the response is successful and the object key has no spaces" do
      # Given
      Environment |> stub(:on_premise?, fn -> false end)
      Environment |> stub(:s3_bucket_name, fn -> "bucket" end)
      base_url = Environment.s3_endpoint()
      ...
      |> stub(:post!, fn %{
                           options: %{base_url: ^base_url},
                           url: %{query: "uploads", path: "/bucket/object-key"}
                         } ->
        %{
          status: 200,
          body: ~s"""
    ...
    test "sends the right req request and parses the body when the response is a failure and the object key has spaces" do
      # Given
      Environment |> stub(:on_premise?, fn -> false end)
      Environment |> stub(:s3_bucket_name, fn -> "bucket" end)
      base_url = Environment.s3_endpoint()
      ...
      # When
      assert Storage.multipart_start("object key") ==
               {:error, "The resource you requested does not exist"}
    end

    test "sends the right req request and parses the body when the response is a failure and the object key has no spaces" do
      # Given
      Environment |> stub(:on_premise?, fn -> false end)
      Environment |> stub(:s3_bucket_name, fn -> "bucket" end)
      base_url = Environment.s3_endpoint()
