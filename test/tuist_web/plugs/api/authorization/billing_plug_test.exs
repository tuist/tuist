defmodule TuistWeb.API.Authorization.BillingPlugTest do
  ...
  test "returns the same connection when it's the new billing, the plan is air, and the usage is below the threshold",
  ...
  test "returns an error when it's the new billing, the plan is air, and the usage is above the threshold",
       %{conn: conn, project: project} do
    # Given
    account = project.account
  ...
  test "returns an error when it's the new billing, the plan is pro, and the subscription is not active",
       %{conn: conn, project: project} do
    # Given
    account = project.account
  ...
  @tag current_month_remote_cache_hits_count: 1000
  test "returns the same connection it's the new billing, the plan is pro, and the subscription is active",
       %{conn: conn, project: project} do
    # Given
    account = project.account
  ...
  @tag current_month_remote_cache_hits_count: 1000
  test "returns the same connection it's the new billing, the plan is open_source, and the subscription is active",
       %{conn: conn, project: project} do
    # Given
    account = project.account
  ...
  @tag current_month_remote_cache_hits_count: 1000
  test "returns the an error if it's the new billing, the plan is open_source, and the subscription is canceled",
       %{conn: conn, project: project} do
    # Given
    account = project.account
