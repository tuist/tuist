defmodule TuistCloud.CacheTest do
  alias TuistCloud.CommandEventsFixtures
  alias TuistCloud.ProjectsFixtures
  alias TuistCloud.Cache
  use TuistCloud.DataCase
  use Mimic

  describe "current_month_remote_binary_cache_hits_count" do
    test "returns only the events from the current month when it's a project" do
      ...
      # When
      got = Cache.current_month_remote_binary_cache_hits_count(project)
    ...
    test "returns only the events from the current month when it's an account" do
      ...
      # When
      got = Cache.current_month_remote_binary_cache_hits_count(account)
