...
and just yesterday,
we shipped a the version 1.13.0,
which we called **Bella Vita**.

In this blog post I'll tell you about major features that we shipped in these 2 releases,
...
The problem though is that developers are used to use **CMD+K** when the compilation on Xcode yields errors.
That invalidates Xcode's cache and results in a clean build.

Some projects are minimizing the build time of clean builds by having a more horizontal project architecture _(e.g. [µFeatures architecture](/docs/architectures/microfeatures/))_. Large companies prefer to replace Xcode's build system with other build systems like [Bazel](https://bazel.build) or [Buck](https://buck.build/) that have support for caching remotely the output of individual build steps. Unfortunately, the latter is a huge undertaking that is not feasible for small and medium companies.

In Tuist we aim to help teams with the challenges that they face when scaling up projects so this was an obvious one to tackle.
...
<Message
  ...
  description="The biggest challenge for this feature is to be able to handle all project scenarios gracefully. For that reason any feedback you can provide from your projects is very valuable. It's possible that the first time you try to warm the cache it fails to compile xcframeworks. If that's the case, we'd love to know why and help you get it working."
/>
