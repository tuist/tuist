...
Some projects minimize the build time of clean builds by having a more horizontal project architecture _(e.g., [µFeatures architecture](/docs/usage/microfeatures/))_. Large companies prefer to replace Xcode's build system with other build systems like [Bazel](https://bazel.build) or [Buck](https://buck.build/) that the output of individual build steps remotely. Unfortunately, the latter is a huge undertaking that is not feasible for small and medium companies.
