...
Since the commands are standard, developers can familiarize themselves with them, and use them in any project defined with Tuist. The commands are designed optimizing for zero-arguments. That means that users will be able to run the command without passing any arguments.
