...
- **Explicit definition:** Dependencies can be defined implicitly through build settings. Xcode is smart enough to detect them and determine the order in which targets should be built. Although implicitness might work fine in small projects, as they get larger, it might turn into a source of issues and slowness. Default to explicit definition of dependencies using the [dependencies API](/docs/usage/dependencies/) that Tuist provides. Tuist has handy built-in features such as [tuist graph](/docs/usage/commands/) or detection of circular dependencies that rely on dependencies being explicitly defined.

## File structure

- **Reflect the targets in the folders structure:** That makes it easy to know which files belong to which target.
- **Don't include a file in more than one target:** If you need to share a file among multiple targets, extract it into a shared library or framework that the others can import.
...
- **All the scheme's targets have the same platform:** Although it's possible to add targets to a scheme whose product is for different platforms, avoid it because commands like `tuist build` are designed around this assumption.
