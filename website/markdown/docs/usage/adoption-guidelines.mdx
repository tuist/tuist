...
How tedious this process is depends on how complex your projects are.

As you probably know,
Xcode doesn't prevent developers from adding accidental complexity.
That leads to complexity in the shape of _groups that don't match the directory structure_,
_files that are shared across targets_, or
_file references that point to nonexisting files_ _(to mention some)._
All that accumulated complexity makes it hard for us to provide a comman that reliably ports migrates project.

Moreover, a manual migration is a **good exercise to clean up and simplify** your projects.
...
Tuist will make sure that proejcts are consistently defined and that they remain simple.

To ease that work,
we include some guidelines in this document that are based on the feedback that we have received from the users of the project.

<Message
  info
  title="Migration commands"
  description="We are currently working on providing commands for to assist you in the process. Until those commands are available, you might consider building simple CLI tools that levarage [XcodeProj](https://github.com/tuist/xcodeproj)"
/>
...
It's important that you prevent developers from defining settings using build settings.
You can do that by implementing a simple scripts that reads the build settings using [XcodeProj](https://github.com/tuist/xcodeproj),
and fails if the settings of projects and targets are not empty.
...
Otherwise you might run into compilation issues because your glob patterns _(e.g. `**/*.swift`)_ ended up matching them,
...
and lets developers know that they should modify the manifest files instead.
...
go through the `Project.swift` files to identify common patterns,
and extract them into helpers.
