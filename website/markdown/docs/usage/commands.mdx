...
With `scaffold` you can define your own templates and soon also templates that Tuist provides to generate a basic structure of your module!

If you want to define your own template, we recommend you to run `tuist edit` - after that you can create a new directory at `Tuist/Templates/name_of_your_template` where `Tuist/Templates` is a directory where all your custom templates will live. Additionaly you have to create a `Template.swift` in that directory:
...
You could also omit `--platform` argument since it is optional and the template provides a default value.

If you do not need any difficult logic, feel free to use `.string` for `files` parameter.
Otherwise, you can leverage a powerful templating tool [Stencil](https://github.com/stencilproject/Stencil) via `.file` case.
