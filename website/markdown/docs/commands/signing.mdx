...
Export the public (as .cer file) and private key (as .p12 file with an empty string as password) from your keychain. Use the same basename for these two file: `SomeName.cer` and `SomeName.p12` - this name doesn't need to match any target or configuration.
If multiple provisioning profiles use the same certificate it's fine to have that .p12 and .cer file just once in the folder - Tuist will find the matching one based on information embedded into the provisioning profile.
