---
name: Signing
excerpt: 'Tuist offers a deterministic, pain-free solution to signing. Read one if you'd like to learn more about how it works and how you can set it up.'
---
...
when working with Xcode. While Xcode does an option of automatic signing,
most teams choose not to use it since it creates an unmanagable mess in their accounts.
But manually working with signing artifacts is tedious, error-prone and tools
automating this process are always non-deterministic and you still have to manually change
...
As a first step, download all your provisioning profiles and certficates that you'll need.
...
To reencrypt them run `tuist encrypt`.
