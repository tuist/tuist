...
With plugins you can extend Tuist to add support for custom project description helpers.
Plugins are meant to be separate isolated directories that contain code used to extend existing Tuist functionality. 
You may use plugins that exist locally on your machine or keep them under source control and pull them from a remote repository.
...
You can import a project description helper plugin with the name defined in the [`Plugin.swift`](/docs/plugins/creating-plugins/) manifest, which can then be used in a project manfiest:
