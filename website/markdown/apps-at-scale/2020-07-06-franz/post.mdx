...
As said before we use RIBs and FRP with Combine. Additionally, we are trying to modularise our application heavily. Some other interesting things we are doing: Design System, UI in code, Single responsibility, generate code when possible.
...
When scaling, the performance of tools is very important. Especially, when these tools need to run during build time. Lastly, tools should be used when there is a big potential for critical bugs such as API contracts. API clients & server stubs should be generated by tools to avoid any misinterpretations.
