---
layout: post
title: 'With Bazel we were able to reduce build times by 95% on clean builds - Kamil Pyć'
categories:
  [
    Tuist,
    kamil,
...
There is nothing revolutionary about our code architecture - we mostly use MVVM, but if there is part of app where other paradigms would fit better we can use them. For example reactive programming architecture is applied just in the module of user registration which has a lot of input and validation.
